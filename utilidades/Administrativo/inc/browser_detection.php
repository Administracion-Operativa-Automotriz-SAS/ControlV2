...
function client_data(info)
{
	if (info == 'width')
	{
		width_height_html = '<h4  class="right-bar">Resolución de la pantalla:</h4>';
...
<h3 class="h-right-bar">Su computador</h3>
<?php
$os = '<h4 class="right-bar">Sistema Operativo:</h4><p class="right-bar">';
$full = '';
...
$full .= $os . '</p><h4 class="right-bar">Navegador:</h4><p class="right-bar">';
if ($browser_info[0] == 'moz' )
{
	...
	$full .= $a_temp[1] . '<br />';
	$full .= 'ProductSub: ';
	$full .= ( $a_temp[4] != '' ) ? $a_temp[4] . '<br />' : 'No Disponible<br />';
