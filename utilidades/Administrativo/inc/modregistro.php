...
class campomod
{
	...
	function campomod($C,$Tabla,$NCapa)
	{
		$this->Tabla=$Tabla;
		$this->Ncapa=$NCapa;
		$this->campo=$C->campo;
		...
		$this->Estilo='color:'.$this->Config->primer_campo.';background-color:'.$this->Config->fondo_campo.';';
	...
	function pinta_imagen_old(&$Info,&$LINK,&$R)
	{
		echo "<table border cellspacing=0 cellpadding=0><tr><td>";
		if($Info)
		{
			if(preg_match("/\.jpg|.gif|.png$/i", $Info))
				echo "<img src='$Info' border=0 valign='top' height='".$this->Config->cols_text."' name='img_$this->campo' id='img_$this->campo'
						onclick=\"modal2('$Info',0,0,300,300,'Imagen');\">";
			else
				echo "<img src='gifs/foto.gif' border=0 valign='top' name='img_$this->campo' id='img_$this->campo'
						onclick=\"modal2('$Info',0,0,300,300,'Imagen');\">";
		}
		else
			echo "<img src='gifs/foto_gris.gif' border=0 valign='top' name='img_$this->campo' id='img_$this->campo'>";
		if(!$this->Sololectura)
			echo "<img src='gifs/buscar.gif' border=0 valign='top' width=40
				onclick=\"V_ancho=screen.availWidth-10;V_alto=screen.availHeight-20;
				modal('marcoindex.php?Acc=reg_cambia_imagen&Forma=mod&Campo=$this->campo&Dato=$Info&Ruta=".$this->Config->rutaimg."&id=$R->id',0,0,V_alto,V_ancho,'Imagen');\">";
		echo "<br /><input type='text' name='$this->campo' value='$Info' ".($this->Sololectura?"readonly":"")." size='40'></td></tr></table>";
	}
	...
	function pinta_imagen(&$Info,&$LINK,&$R)
	{
		...
		echo "<iframe id='simg_$this->campo' name='simg_$this->campo' src='".($Info?$Info:"gifs/standar/img_neutra.png")."' height='$Alto' width='$Ancho' frameborder='0' ></iframe>";
	...
	function pinta_fecha(&$Info,&$LINK,&$R)
	{
		if($this->Sololectura)
			...
			echo pinta_FC('mod',$this->campo,$Info,'f',$this->Estilo);
	}
	...
	function pinta_fechahora(&$Info,&$LINK,&$R)
	{
		if($this->Sololectura)
			...
			echo pinta_FC('mod',$this->campo,$Info,'t',$this->Estilo);
	}
	...
	function pinta_relacionado(&$Info,&$LINK,&$R)
	{
		...
		else
		{
			...
			elseif($this->Config->busca_ciudad)
			{
				...
				echo "\" size='$this->Tamano' onclick=\"busqueda_ciudad('$this->campo','$Info');\" readonly>
					<input type='hidden' name=".$this->campo." id=".$this->campo." value='".$Info."'>";
			}
			else
			{
	...
	function pinta_descripcion()
	{
		...
		if($_SESSION['Disenador']) echo "<a class='info'><font style='font-family:Arial;' color='".$this->Config->primer_desc."' >".
			$this->Config->descripcion."</font><span style='width:200px;'>".
...
class capamod
{
	...
	function capamod($N /*nombre de la capa */,$Tabla_Control,$Tabla,$Add /*adicionando*/,$LINK)
	{
		...
		$Campos=mysql_query("select * from $Tabla_Control where capa='$N' and modificar=1 ".(($Add && $Llave)?"and llave=1":"")." order by orden,suborden",$LINK);
		while($C=mysql_fetch_object($Campos))
		{
			$this->Campo[]= new campomod($C,$Tabla,$N);
		}
...
class modregistro
{
	...
	{
		$this->Idtabla=$nt;
		$this->Config=qo("select * from usuario_tab where id=$this->Idtabla and usuario=".$_SESSION['User']);
		$this->Nombretabla=$this->Config->tabla;
		$this->Ntut=qo1("select id from usuario_tab where tabla='usuario_tab' and usuario=".$_SESSION['User']);
	...
	function capas()
	{
		...
		$Capas=mysql_query("select distinct capa from $Tabla_Control where modificar=1 ".($this->Adicionando?" and llave=1":"")." order by capa",$LINK);
		if(!mysql_num_rows($Capas))
		{
			$Capas=mysql_query("select distinct capa from $Tabla_Control where modificar=1 order by capa",$LINK);
		}
