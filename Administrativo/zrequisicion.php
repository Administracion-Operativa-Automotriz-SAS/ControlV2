...
/********************$Hoy  =  $_POST["Hoy"];
		$Nombre_aprobador  =  $_POST["Nombre_aprobador"];
		$Email_aprobador  =  $_POST["Email_aprobador"];
		$Email_usuario =  $_POST["Email_usuario"];
		$user  =  $_POST["user"];
		$Fecha_control  =  $_POST["Fecha_control"];
		$Nick =  $_POST["Nick"]; 
		
	   $Ruta_correo="http://app.aoacolombia.com/utilidades/Operativo/operativo.php?id=$id&user=$Nick'&email=$Email_usuario&Fecha=$Hoy&Usuario=$Nombre_aprobador&eUsuario=$Email_aprobador&Solicitado_por=$user&eSolicitado_por=$Email_usuario";
        ...
        if(!$Cotizaciones) $Cotizaciones="No hay imagenes cargadas";
   	    ...
   	    $Ruta_aprobacion = base64_encode("$Ruta_correo&Acc=aprobar_requisicion");
   	    $Ruta_daprobacion = base64_encode("$Ruta_correo&Acc=daprobar_requisicion");
...
$apro_compra=$_SESSION['apro_compra'];
...
function ver_balance()
{
		...
		if($USER == 15){
			if(!$FI) {
				$FI=date('Y-m-d', strtotime('-1 month'));
			$FF=date('Y-m-d');}
		}else{
			if(!$FI) {
		...
		echo "<form action='zrequisicion.php' target='_self' method='POST' name='forma' id='forma'>
                Requisici&oacute;nes de fecha: ".pinta_FC('forma','FI',$FI)." hasta fecha  ".pinta_FC('forma','FF',$FF)." <input type='submit' name='btncontinuar' id='btncontinuar' value=' CONTINUAR ' >
...
function filtro_causacion()
{
	header('Content-Type: text/html; charset=utf-8');
	include("views/subviews/requisiciones/filtrar_causacion.html");
}
...
function ver_requisiciones()
{
        ...
        if($Requisici�nes=q("select * from $BDA.requisicion where ubicacion=0 and solicitado_por='$NUSUARIO' and 
												fecha between '$FI 00:00:00' and '$FF 23:59:59' order by fecha desc "))
        {
                ...
                while($Rq=mysql_fetch_object($Requisici�nes))
                {
                        ...
                        if($Rq->estado==2) echo "<a style='cursor:pointer' onclick='evaluar_requisicion($Rq->id);' class='rinfo'><img src='img/diploma.png' height='22px'><span style='width:100px'>Evaluar Requisici&oacute;n</span></a>";
                        echo "</td></tr></table>";
                        ...
                        if(!$Rq->cerrada){
							...
							echo "<a class='info' href='javascript:adicionar_detaller($Rq->id);'><img src='gifs/standar/nuevo_registro.png' border='0'>Adicionar item<span style='width:200px'>Adicionar un item al detalle de la requisicion</span></a>
                                &nbsp;&nbsp;&nbsp;&nbsp;<a class='info' href='javascript:cerrar_requisicion($Rq->id);'><img src='gifs/standar/stop_16.png' border='0'> Cerrar Requisici&oacute;n - Enviar correo de solicitud de aprobaci&oacute;n<span style='width:200px'>Cerrar Requisici&oacute;n - envieacute;a solicitud de aprobaci&oacute;n autom&aacute;tica.</span></a>
                                &nbsp;&nbsp;&nbsp;&nbsp;<a class='info' href='javascript:borrar_requisicion($Rq->id);'><img src='gifs/standar/borra_registro.png' border='0'> Borrar Requisici&oacute;n<span style='width:200px'>Borrar Requisici&oacute;n</span></a>";
							echo "</td></tr>";
						}
        ...
        echo "<a class='info' href='javascript:adicionar_requisicion();'><img src='gifs/standar/nuevo_registro.png' border='0'>  Adicionar Requisici&oacute;n<span>Adicionar Requisici&oacute;n</span></a>";
}
...
function ver_requisiciones2()
{
		...
		if($USER!=8) $FILTRO2=" and re.solicitado_por='$NUSUARIO' "; else $FILTRO2='';
		if($USER!=8) $FILTRO3=" and requ.solicitado_por='$NUSUARIO' "; else $FILTRO3='';
        $Estados=q("select * from estado_requisicion order by id");
        $A_estados=array();
		$A_estados[0]=array("color"=>'dddddd',"nombre"=>'Nueva');
        ...
        echo "<table cellspacing='2' bgcolor='eeeeee'><tr><th colspan='2'>Control de Requisici&oacute;nes</th></tr>";
		...
		if(mysql_num_rows($Periodos))
        {
                ...
                while($Pe=mysql_fetch_object($Periodos))
                {
                        ...
                        foreach($A_estados as $idestado => $aestado)
                        {
								...
								if($USER == 15){
									$Requisici�nes_por_estado=mysql_query("select *,$BDA.t_proveedor(proveedor) as nprov from $BDA.requisicion where estado=$idestado 
												  and date_format(fecha,'%Y-%m')='$Pe->periodo'  order by fecha desc",$LINK);
								}else{
									$Requisici�nes_por_estado=mysql_query("select *,$BDA.t_proveedor(proveedor) as nprov 
									            from $BDA.requisicion
												where estado=$idestado 
												$FILTRO  and date_format(fecha,'%Y-%m')='$Pe->periodo' and perfil in ($Perfil_requisicion)
                                                order by fecha desc",$LINK);
								}
								
								$Cantidad=mysql_num_rows($Requisici�nes_por_estado);
                                if($Cantidad>0)
                                {
                                        ...
                                        $Contador_req=0;
                                        while($Req=mysql_fetch_object($Requisici�nes_por_estado))
                                        {
                                                $Contador_req++;
												...
												echo "</td>
                                                                ...
                                                                <td bgcolor='ffffff' nowrap='yes'><a class='info' onclick='ver_requisicion($Req->id);'><img src='gifs/standar/Preview.png'><span style='width:200px'>Ver Requisici&oacute;n</span></a></td>
                                                                </tr>";
                                        }
                                        echo "</table></td></tr>";
                                }
		...
		if($Periodos=q($sqlPeriodos)){
                include('inc/link.php');
                echo "<tr><td valign='top' bgcolor='ffffff' width='80px'>ADMINISTRATIVAS DE LA OPERACION</td><td valign='top'>
                                                        <table cellspacing='2' bgcolor='eeeeee'>";
                while($Pe=mysql_fetch_object($Periodos))
                {
                        ...
                        foreach($A_estados as $idestado => $aestado)
                        {
                                $llave_estado=$llave_periodo.'-a-'.$idestado;
                                $estado_bgcolor=$aestado['color'];$estado_nombre=$aestado['nombre'];
                                
								if($USER == 15){
									$sqlRequistate = "select re.*,$BDA.t_proveedor(proveedor) as nprov 
									            from $BDA.requisicion re
												where re.estado=$idestado 
												and date_format(re.fecha,'%Y-%m')='$Pe->periodo' and re.clase = 5
                                                order by re.fecha desc";
								}else{
									$sqlRequistate = "select re.*,$BDA.t_proveedor(proveedor) as nprov 
									            from $BDA.requisicion re 
												where re.estado=$idestado 
												$FILTRO2 and date_format(fecha,'%Y-%m')='$Pe->periodo' and re.perfil=$Perfil_requisicion and re.clase = 5
                                                order by re.fecha desc";
								...
								$Requisici�nes_por_estado=mysql_query($sqlRequistate,$LINK);
                                ...
                                if($Cantidad>0)
                                {
                                        echo "<tr><td bgcolor='$estado_bgcolor' valign='top' width='110px' nowrap='yes'>
                                                                        ...
                                                                        <table cellspacing='1' bgcolor='eeeeee' id='$llave_estado' style='visibility:hidden;position:absolute;'>
                                                                        <tr><th>#</th><th>Numero</th><th>Fecha</th><th>Proveedor</th><th>Valor Total</th></tr>";
                                        $Contador_req=0;
                                        while($Req1=mysql_fetch_object($Requisici�nes_por_estado))
                                        {
										    $Contador_req++;
											//echo $sqlRequistate."<br><br><br><br>";
											
											$consultaPrecio = "select SUM(rd.valor_total) VALOR_TOTAL
									            from $BDA.requisicion requ
									            LEFT JOIN aoacol_administra.requisiciond rd ON rd.requisicion = requ.id
												where requ.id = $Req1->id AND estado=$idestado 
												$FILTRO3 and date_format(fecha,'%Y-%m')='$Pe->periodo' and perfil=$Perfil_requisicion
                                                order by fecha desc";
												$consultaPrecio2=mysql_query($consultaPrecio,$LINK);
												while($Pre=mysql_fetch_object($consultaPrecio2)){
													$valorTotal = $Pre->VALOR_TOTAL;
												}
												
												
												//$precio = q($consultaPrecio);
												
												//var_dump($precio->VALOR_TOTAL;
                                                echo "<tr><td>$Contador_req</td><td bgcolor='ffffff' align='center'>$Req1->id</td>
                                                                <td bgcolor='ffffff' nowrap='yes'>$Req1->fecha</td>
                                                                <td bgcolor='ffffff' nowrap='yes' width='300px'>$Req1->nprov</td>
																<td bgcolor='ffffff' nowrap='yes' width='100px'>$".number_format($valorTotal)."</td>
																<td bgcolor='ffffff' nowrap='yes'><a class='info' onclick='ver_requisicion($Req1->id);'><img src='gifs/standar/Preview.png'><span style='width:200px'>Ver Requisici&oacute;n</span></a></td>
                                                                </tr>";
                                        }
                                        echo "</table></td></tr>";
                                }
		...
		if($Periodos=q($sqlPeriodos)){
                ...
                while($Pe=mysql_fetch_object($Periodos))
                {
                        ...
                        foreach($A_estados as $idestado => $aestado)
                        {
								...
								if($USER == 15){
									$sqlRequistate = "select *,$BDA.t_proveedor(proveedor) as nprov 
									            from $BDA.requisicion re
												where re.estado=$idestado 
												and date_format(re.fecha,'%Y-%m')='$Pe->periodo' and re.clase !=5
                                                order by re.fecha desc";
									...
									$sqlRequistate = "select re.*,$BDA.t_proveedor(proveedor) as nprov 
									            from $BDA.requisicion re
												where re.estado=$idestado 
												$FILTRO2 and date_format(re.fecha,'%Y-%m')='$Pe->periodo' 
												and re.perfil=$Perfil_requisicion and re.clase !=5
                                                order by re.fecha desc";
                                ...
                                if($Cantidad>0)
                                {
                                        ...
                                        while($Req=mysql_fetch_object($Requisici�nes_por_estado))
                                        {
											...
											$consultaPrecio = "select SUM(rd.valor_total) VALOR_TOTAL
									            from $BDA.requisicion requ
									            LEFT JOIN aoacol_administra.requisiciond rd ON rd.requisicion = requ.id
												where requ.id = $Req->id AND estado=$idestado 
												$FILTRO3 and date_format(fecha,'%Y-%m')='$Pe->periodo' and perfil=$Perfil_requisicion
                                                order by fecha desc";
												$consultaPrecio2=mysql_query($consultaPrecio,$LINK);
												while($Pre=mysql_fetch_object($consultaPrecio2)){
...
function adicionar_requisicion()
{
        ...
        echo "<script language='javascript'>
                function grabar_n_req()
                {
                        ...
                        if(!document.forma.proveedor) {alert('Debe seleccionar un proveedor confiable o medianamente confiable. Cree nuevo proveedor o solicite calificaci&oacute;n de uno existente.');return false;}
						...
						if(!document.forma.perfil.value) {alert('Debe seleccionar a quien va dirigida la requisici&oacute;n');document.forma.perfil.style.backgroundColor='ffffaa';return false;}
  ...
  crossorigin='anonymous'></script>
		...
		<body><h3>Adici&oacute;n de Requisici&oacute;n</h3>
                ...
                <table cellspacing='5' bgcolor='dddddd'>"; 
                    if($USER == 15 ){
					 ...
					 $sqlProveedores = "select id,nombre,calificacion_actual,$BDA.t_ciudad(ciudad) as nciudad from $BDA.proveedor where calificacion_actual in ('M','C') and activo = 1 or tipo='E' order by nombre";
                        ...
                        La lista que aparece a continuaci&oacute;n corresponde a los proveedores totalmente confiables y medianamente confiables, de acuerdo a la selecci&oacute;n realizada siguiendo los lineamientos del proceso de Calidad. <br>
                        Tambi&eacute;n aparecen los empleados de AOA a los que se les hace anticipos.<br>
                        ";
		if($Proveedores=q($sqlProveedores))
        {
			echo "<select name='proveedor' id='proveedorSelect' style='width:300px' onchange='valida_cambio_proveedor();'><option value=''></option>";
			...
			<br><li> Que no est&aacute; creado en la base de datos. Por lo tanto puede tramitar con adquisiciones: <a style='display:none' onclick='crear_nuevo_proveedor();' class='info'><img src='img/adicionar_proveedor.png' height='30' border='0'><span style='width:100px'>Crear Proveedor</span></a>
			<li> Que si est&aacute; creado pero no haya sido seleccionado. Puede realizar la <b><i>Selecci&oacute;n</i></b> para que aparezca en la lista de proveedores dando click aqu&iacute;: <a onclick='realizar_calificacion()' class='info'><img src='img/diploma.png' height='30' border='0'><span style='width:100px'>Realizar Calificaci&oacute;n</span></a>
			";
        }
        else echo "<b style='color:red'>No hay proveedores confiables</b><br>
                Si desea puede <b><i>Seleccionar</i></b> un proveedor espec&eacute;fico dando click aqu&iacute;: <a onclick='realizar_calificacion();' class='info'><img src='img/diploma.png' height='30' border='0'><span style='width:100px'>Realizar Calificaci&oacute;n</span></a><br>
                Si desea crear un nuevo proveedor puede dar click aqu&iacute;: <a  onclick='crear_nuevo_proveedor();' class='info'><img src='img/adicionar_proveedor.png' height='30' border='0'><span style='width:100px'>Crear Proveedor</span></a><br> ";
        echo "</td></tr>
...
function ver_imagen_requisicion()
{
        ...
        if($D->cotizacion_f) echo "<iframe name='cot1' id='cot1' style='visibility:visible' width='100%' height='400px' src='$D->cotizacion_f'></iframe>";
                ...
                echo "Por favor cargue la im&aacute;gen de la cotizaci&oacute;n de la requisici&oacute;n y tan pronto la cargue cierre esta ventana.<br>
        ...
        if($D->cotizacion3_f) echo "<iframe name='cot3' id='cot3' style='visibility:visible' width='100%' height='400px' src='$D->cotizacion3_f'></iframe>";
                ...
                echo "Por favor cargue la im&aacute;gen de la cotizaci&aacute;n de la requisici&oacute;n y tan pronto la cargue cierre esta ventana.<br>
	...
	';
        ...
        function validar_nuevo_item(continua)
        {
                with(document.forma)
                {
						...
						if(!alltrim(tipo1.value)) {alert('Debera seleccionar un &iacute;tem');tipo1.style.backgroundColor='ffffaa';tipo1.focus();return false;}
        ...
        <form action='zrequisicion.php' target='Oculto_item' method='POST' name='forma' id='forma'>
                <table>
				<tr><td>Seleccion&oacute; el tipo</td><td>".menu1("tipo2","SELECT id,nombre FROM $BDA.tipo;",0,1,"width:300px")."<br>
                        </td></tr>
						
                        <tr><td>Tipo de Item</td><td>".menu1("tipo1","$varSqlItem",0,1,"width:300px")."<br>
                        Se&ntilde;or Usuario, si el tipo de requisici&oacute;n no aparece en la lista, puede solicitar su creaci&oacute;n<br>
                        mediante este link:
                        <a class='info' onclick='solicitar_nuevo_tipo();'><img src='img/nuevotipo.png' height='20px'> Solicitar creaci&oacute;n de nuevo tipo.<span style='width:100px'>Solicitar creaci&oacute;n de nuevo tipo de requisici&oacute;n.</span></a>
                        </td></tr>
						
                        <tr><td>Clase de Requisici&oacute;n</td><td>".menu1("clase","select * from $BDA.requisicionc where id = 2 or id = 1 or id = 5 order by nombre",1)."</td></tr>
						<tr><td>Cobro:</td><td><select name='tipo_cobro'>
						
					                <option value='S'>SIN RECOBRO GASTO COMPA&Ntilde;IA</option>
						...
						<tr>
							<td colspan='2'>
								...
								<strong><p>Nota:<br>
									...
									Proveedores de R&eacute;gimen Simplificado: Persona Natural no factura IVA.<br>
									Proveedores de R&eacute;gimen Com&uacute;n: Persona Natural � Jur&uacute;dica que factura IVA.<br>
									El valor que presenta la cotizaci&oacute;n puede estar incluido el IVA. Ejemplo: Valor+ IVA o Valor IVA incluido. En caso tal usted deber&aacute; realizar la operaci&oacute;n para hallar la base antes de IVA.</p>
								</strong>
							
							</td>
						</tr>
                        <tr>
							<td>Centro de operaci&oacute;n</td>
        ...
        <!--<button onclick='open_cot_window()'>Agregar cotizaciones a la requisici&oacute;n</button>-->		
		</body>
		<script>
...
function enviar_mail_solicitud_aprobacion_administrativa()
{
	 ...
	 $Ruta_correo="utilidades/Operativo/operativo.php?id=$id&user=".$_SESSION['Ususario']."&email=".$_SESSION['email']."&Fecha=$Hoy&Usuario=$Nombre_aprobador&eUsuario=$Email_aprobador&Solicitado_por=".$_SESSION['Nombre']."&eSolicitado_por=$Email_usuario";
       ...
       $Ruta_aprobacion=base64_encode("\$Programa='$Ruta_correo&Acc=aprobar_requisicion&observaciones='.\$observaciones.'&cotapr='.\$cotapr;\$Fecha_control=date('Y-m-d');");
		...
		$Det="<table id='table'  border cellspacing='0'><tr><th>Tipo de Requisici&oacute;n</th><th>Item</th><th>Unidad de medida</th><th>Descripcion</th><th>Cantidad</th><th>Valor unitario</th><th>Valor</th><th>Proyecto placa</th><th>Centro de operacion</th><th>Centro de costo</th><th>Factor</th>";
		...
		$Dm=qo("select * from aoacol_administra.requisicion where id=$id");
					...
					"id" => $id,
				     "contenido" => $contenido ,
					"para" => "jennybuitrago@aoacolombia.com",
					"copia" => "",
					"fecha" => $D->fecha,
        ...
        echo "<body><script language='javascript'>alert('Email satisfactoriamente 1 a $Email_aprobador');</script></body>";
}
...
function enviar_mail_solicitud_aprobacion_operativa() // envia un correo para aprobacion de la requisicion control operativo
{
	...
	$Det="<table class='table' border cellspacing='0'><tr><th>Tipo de Requisici&oacute;n</th><th>Item</th><th>Unidad de medida</th><th>Descripcion</th><th>Centro de operacion</th><th>Cantidad</th><th>Valor unitario</th><th>Valor</th>";
	...
	$Res="<table  class='table' border cellspacing='4'><tr><th>Total </th>";
        //echo "select *,t_requisiciont(tipo) as ntipo, t_requisicionc(clase) as nclase from requisiciond where requisicion=$id";
		$retorno=q("select requisiciond.requisicion,requisiciond.valor_total,
		            sum(requisiciond.valor_total) as resultado 
					from aoacol_administra.requisiciond
...
function solicitar_nuevo_bien_servicio_ok()
{
        ...
        nl2br("Estimada Se&ntilde;ora Claudia Castro,
...
function evaluar_requisicion()
{
        ...
        <form action='zrequisicion.php' target='_self' method='POST' name='forma' id='forma'>
                <table><tr><th>Criterio</th><th>Opci&oacute;n</th><th>Comentario</th><th>Resultado</th></tr>";
                include('inc/link.php');
                $Js='';
...
function evaluar_requisicion_ok()
{
		...
		graba_bitacora('requisicion','M',$id,'Evalua requisici&oacute;n');
...
{
	header('Content-Type: text/html; charset=utf-8');
        global $id,$BDA,$USER,$NUSUARIO;
		
		$CamEstado=qo("select apro_compra from aoacol_administra.usuario_compras where nombre ='$NUSUARIO'");
		                 $compras = $CamEstado;
        echo $CamEstado->apro_compra;
		echo $NUSUARIO,$BDA;
		$verificar=qo("select * from aoacol_administra.requisicion where id = $id");

								if($verificar->ubicacion == 0){
									 if($verificar->estado==5 ) {
											if($verificar->perfil == 3){
													include('anularPdfAdmOriente.php'); 
												}else{	
		...
		if($CamEstado->apro_compra == '1' ) {
			
	
		$pintar ="<div class='embed-responsive embed-responsive-16by9'>
	    <iframe class='embed-responsive-item' 
		  src='https://app.aoacolombia.com/Administrativo/zrequisicionDev.php?Acc=ver_requisicion&id=$id'></iframe> </div>";
		
		}else{
			$pintar ="<div class='embed-responsive embed-responsive-16by9'> No tiene Permiso   </div>";
		...
		if($Req == null)
		{
			echo "No existe la requisici&oacute;n";
			exit;
		}
	   ...
	   echo "<script language='javascript'>
                ...
                {if(document.getElementById('proveedor').value)  {if(confirm('Desea asignar este proveedor a la requisici&oacute;n?'))
					...
					<tr>
						<td style='width:50%;'>
							<table border bgcolor='ffffff' cellspacing='0' width='100%'>
								<tr><td align='right'>Fecha de ".utf8_encode("Requisici&oacute;n:")."</td><td><b>$Req->fecha</b></td></tr>
        ...
        echo "<tr><td align='right'>Ciudad:</td><td><b>$Ciudad</b></td></tr>
					...
					if($Proveedor->tipo=='P') // si es proveedor no empleado
					{
						...
						if(inlist($Req->estado,'2,4'))
						{
							...
							if($Req->evaluada_por && $Req->fecha_evaluacion!='0000-00-00 00:00:00')
							{
								...
								echo "<tr><td align='right'>".utf8_encode("Evaluaci&oacute;n")."</td><td>Fecha: $Req->fecha_evaluacion<br>
											Por: $Req->evaluada_por<br>Calificacion: $Req->calificacion</td></tr>";
							}
							else
							{
								
								echo "<tr><td align='right'>".utf8_encode("Evaluaci&oacute;n")."</td>
                ...
                else
                {
                        if($Proveedores=q("select id,nombre,calificacion_actual from $BDA.proveedor where calificacion_actual in ('M','C') or tipo='E' order by nombre"))
                        {
                                ...
                                echo "</select><br><br>
                                Si el proveedor que busca no aparece en la lista, existen dos posibilidades: <br>
                                <li> Que no est&aacute; creado en la base de datos. Por lo tanto puede crearlo dando click aqu&iacute;: <a onclick='crear_nuevo_proveedor();' class='info'><img src='img/adicionar_proveedor.png' height='30' border='0'><span style='width:100px'>Crear Proveedor</span></a>
                                <li> Que si est&aacute; creado pero no haya sido calificado. Puede calificar un proveedor dando click aqu&iacute;: <a onclick='realizar_calificacion()' class='info'><img src='img/diploma.png' height='30' border='0'><span style='width:100px'>Realizar Calificaci&oacute;n</span></a>
                                </td></tr>";
                        }
                }
        echo "
		...
		if($Proveedor)
		{
			if($SEDES=q("select *,t_ciudad(ciudad) as nciudad from prov_sede where proveedor=$Proveedor->id"))
			{
				echo "
					<form name='sedes'>
						<b style='color:#ffffff;'>Debe seleccionar una sede para enviar el correo de solicitud de aprobaci&oacute;n en el momento del cierre de la Requisici&oacute;n</b><br>
		...
		if($Detalle=q($sql)) 
        {
                echo "<table border cellspacing='0' bgcolor='ffffff'><tr>
					<th>Item</th>
					<th>Tipo de Requisici&oacute;n</th>
					<th>Unidad de medida</th>
					<th>Clase</th>
					<th>Tipo Cobro</th>
					<th>".utf8_encode("Descripci&oacute;n")."</th>
					...
					<th>Factor</th>
					<th>".utf8_encode("Asociaci&oacute;n")."</th>
						...
						if($Req->estado==2 && $D->tipo_cobro)
						{
							if($Req->proveedor) 
							{
								if($Proveedor->tipo=='P')
									...
									echo "<b>El proveedor es empleado de AOA.<br>No requiere selecci&oacute;n ni ".utf8_encode("evaluaci&oacute;n").".</b><br>
											<a class='rinfo' onclick='asociar($D->id);'><img src='gifs/standar/Next.png'><span>Asociar a una factura de proveedores</span></a>";
							}
							else echo "<b style='color:red'>Debe seleccionar un proveedor para poder evaluarlo y luego asociar una factura a esta requisici&oacute;n</b>";
						}
		...
		$cotizaciones_table .=	"<tr>";
			$cotizaciones_table .=	"<td width='33%' height='165px'>";
				...
				if($cotizacion1!= null)
				{
					$cotizaciones_table .= "<a href='/Administrativo/$cotizacion1' target='_blank'><b>".utf8_encode("Ver cotizaci&oacute;n")."</b></a>";
				...
				{
					$cotizaciones_table .= "<br><b>".utf8_encode("Valor cotizaci&oacute;n")."</b> : ".$Req->cotizacion_f_valor."<br>";			
				...
				if(($_SESSION['User']==1 ||  $Req->cotizacion_f != null) and $Req->cotizacion_seleccionada == null ){
					...
					<form onsubmit='asignar_cotizacion(this)'>
						...
						<button style='width:100%;'>".utf8_encode("Asignar como cotizaci&oacute;n seleccionada para compra")."</button>
					</form>";	
				}
				...
				{
					$cotizaciones_table .= "<br> 
					<span><strong>".utf8_encode("COTIZACI&Oacute;N SELECCIONADA")."</strong></span>";
				}					
			...
			if($cotizacion2 != null)
			{
				$cotizaciones_table .= "<a href='/Administrativo/$cotizacion2' target='_blank'>".utf8_encode("Ver cotizaci&oacute;n")."</a>";
				$cotizaciones_table .= "<br>";
			...
			{
				$cotizaciones_table .= "<br><b>".utf8_encode("Valor cotizaci&oacute;n")."</b> : ".$Req->cotizacion2_f_valor."<br>";			
			...
			if($cotizacion3 != null)
			{
				$cotizaciones_table .= "<a href='/Administrativo/$cotizacion3' target='_blank'>".utf8_encode("Ver cotizaci&oacute;n")."</a>";
				$cotizaciones_table .= "<br>";
			...
			{
				$cotizaciones_table .= "<br><b>".utf8_encode("Valor cotizaci&oacute;n")."</b> : ".$Req->cotizacion3_f_valor."<br>";			
		...
		echo "<br><a style='cursor:pointer;' onclick='adicionar_detalle();'><img src='gifs/standar/nuevo_registro.png' border='0'> ".utf8_encode("Adicionar detalle a esta requisici&oacute;n")."</a>&nbsp;&nbsp;&nbsp;&nbsp;";
		...
		if($USER == 15 ){
			...
			echo "&nbsp;&nbsp;&nbsp;&nbsp;<a class='info' href='javascript:borrar_requisicion($id);'><img src='gifs/standar/borra_registro.png' border='0'>".utf8_encode("Borrar Requisici&oacute;n")."<span style='width:200px'>Borrar Requisici&oacute;n</span></a>";
		}else{
			echo " <a class='info' href='javascript:cerrar_requisicion($id);'><img src='gifs/standar/stop_16.png' border='0'>".utf8_encode("Cerrar Requisici&oacute;n - Enviar correo de solicitud de aprobaci&oacute;n")."<span style='width:200px'>Cerrar Requisici&oacute;n - env&iacute;a solicitud de aprobaci&oacute;n autom&aacute;tica.</span></a>";
		}						
		...
		if($Req->estado==2 or $Req->estado==4) echo "<a class='info' href='javascript:re_enviar_aprobacion($id);'><img src='gifs/standar/derecha.png' border='0'>".utf8_encode("Re-enviar la aprobaci&oacute;n una vez mas al proveedor")."</a>";
...
function asignar_cotizacion()
{
	...
	q($sql);
	echo "cotizaci&oacute;n seleccionada";
}
...
function incluir_factura_requisiciond()
{
		...
		else
		{
			...
			{
				$sql = "Insert into aoacol_aoacars.requisiciond_facturas  (requisiciond,consecutivo_suno,consecutivo_provee,facprov_f,valor_factura,fecha_creacion) values ('$requisiciond_id','$consecutivo_uno','$consecutivo_provee','$sqlpath','$valor_factura','$fecha_creacion') ";
				q($sql);
...
function asociar_factura()
{
	...
	<script language='javascript'>
		function asociar(id)
		{if(confirm('Desea asociar esta factura a la requisici&oacute;n?')) window.open('zrequisicion.php?Acc=asociar_factura_requisicion&idf='+id+'&iddr=$iddr','Oculto_asociar');}
        ...
        function valida_cambio_proveedor() 
		{
			...
			if(confirm('Desea asignar al proveedor: '+Np+' a esta requisici&oacute;n?')) 
	...
	<table border cellspacing='0'>
		...
		<tr><td align='right'>Fecha de Requisici&oacute;n</td><td>$Req->fecha</td></tr>
	...
	if($Proveedor->tipo=='P')
	{
		...
		else
		{
			...
			<a style='cursor:pointer;' onclick='cerrar();'><u>Click aqu&iacute; para regresar a la pantalla anterior y evaluar el proveedor.</u></a> ";
		}
	}
	else
	{
		echo "<br><b>El proveedor es un empleado de AOA</b><br>
		Si se requiere puede cambiar el proveedor de esta requisici&oacute;n por un proveedor confiable o medianamente confiable. 
		O si se requiere puede crear un nuevo proveedor cumpliendo todos los lineamientos de calidad.<br><br>
		Para asignar un proveedor calificado, seleccionelo del siguiente men&uacute;:<br>
		";
	if($Proveedores=q("select id,nombre,calificacion_actual from proveedor where calificacion_actual in ('M','C') and tipo='P' order by nombre"))
    {
		...
		echo "</select><br><br>
			Si el proveedor que busca no aparece en la lista, existen dos posibilidades: <br>
			<li> Que no est&aacute; creado en la base de datos. Por lo tanto puede crearlo dando click aqu&iacute;: <a onclick='crear_nuevo_proveedor();' class='info'><img src='img/adicionar_proveedor.png' height='30' border='0'><span style='width:100px'>Crear Proveedor</span></a>
			<li> Que si est&aacute; creado pero no haya sido seleccionado. Puede realizar la <i><b>Selecci&oacute;n</b></i> para que aparezca en la lista de proveedores dando click aqu&iacute;: <a onclick='realizar_calificacion()' class='info'><img src='img/diploma.png' height='30' border='0'><span style='width:100px'>Realizar Selecci&oacute;n</span></a>
			";
    }
    else echo "<b style='color:red'>No hay proveedores confiables</b><br>
				Si desea puede <b><i>Seleccionar</i></b> un proveedor espec&eacute;fico dando click aqu&iacute;: <a onclick='realizar_calificacion();' class='info'><img src='img/diploma.png' height='30' border='0'><span style='width:100px'>Realizar Calificaci&oacute;n</span></a><br>
...
b) Que al momento de aprobar una factura se vaya un correo electr�nico autom�tico al proveedor anunciando la aprobaci�n de la requisici�n a modo de pedido. 

Estas dos solicitudes son de tipo Desarrollo.
*/
function re_envio_requisicion_aprobada()
{
	global $id,$email_user;
	...
	$Usuario=$Nombre_aprobador;
	$eUsuario=$Email_aprobador;
	/*if($D->estado==4) 
	{
		echo "<body>alert('El estado de esta requisici�n ya fue procesado y es: Calificado');</body>";
		die();
	}*/
	if($D->estado==3) 
	{
		echo "<body>alert('El estado de esta requisici&oacute;n ya fue procesado y es: Rechazado');</body>";
		die();
	}
	if($D->estado==2 or $D->estado==4)
	{
		...
		if($EmailDestino)
		{
			...
			$Dealle=q("select provee_produc_serv.nombre as item,tipo.nombre as tipo,unidad_de_medida.nombre as unidad_medida,requisiciond.observaciones,requisiciond.cantidad,
			...
			while($Dt =mysql_fetch_object($Dealle ))
	        {
     	...
     	concat( oficina.centro_operacion,' ',oficina.nombre) as centrodeoperacion,aseguradora.ccostos_uno as centrocosto,aseguradora.nombre as ASEGURADORA, ubicacion.flota 
             ...
             $verificar=qo("select ubicacion from aoacol_administra.requisicion where id = $id");
				...
				if($verificar->ubicacion == 0){
					...
					include('pdf/pdfrequisicionAdm.php');
				
				}else{
					
					include('pdf/pdfrequisicionOpe.php');
					
				}
				
			
				  $pdf->Output('/var/www/html/public_html/Administrativo/inc/requicion'.$id.'.pdf','F');
               $archivo = "https://app.aoacolombia.com/Administrativo/inc/requicion$id.pdf";
				$nameArchivo = "Requisiciondecompra.pdf";
				    $Email_usuario=usuario('email');
				$copia = $Email_usuario;
				...
				$mensajeError  = "Mensaje enviado satisfactoriamente $EmailDestino " 	;
				$mensaje  = "Mensaje enviado satisfactoriamente $EmailDestino ";
				$nameArchivo = "Requisiciondecompra.pdf";
				$para = $EmailDestino;
		...
		else echo "<body><script language='javascript'>alert('El proveedor no tiene correo electr&oacute;nico definido.');</script></body>";
	}
	else echo "<body><script language='javascript'>alert('La requisici&oacute;n no est&aacute; en estado APROBADA.');</script></body>";
}
