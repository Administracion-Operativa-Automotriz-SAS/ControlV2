...
<hr>
     <form method='post' style='  margin-bottom:1%; text-align : center;  '  action=''>Nombre
        <input type="text" name="name"  value="<?php echo"RequisiciónAnulacion$id "?>"/>
        <input type="submit" value="Descargar TrasOriente " name="submit" />
...
$ER=qo("select requisicion.placa, 
	...
	$pdf->setXY(10,15);
        ...
        $pdf->Image('https://pbs.twimg.com/profile_images/1305916683178123273/1BtxwW1A_400x400.jpg',17,20,180); 
	...
	$pdf->setXY(46,85);
		...
		$pdf->Cell(40,5,utf8_decode($D->fecha),1,1,'C',1);
		...
		$pdf->setXY(141,85);
			$pdf->SetFont('Arial','B',7);
			$pdf->SetFillColor(	255, 255, 255);	
			$pdf->Cell(59,5,utf8_decode($id),1,1,'C',1);
			$pdf->SetFont('Arial','B',7);
	...
	$pdf->SetFillColor(	215, 240, 127);	
		...
		$pdf->SetFont('Arial','B',7);
		$pdf->SetFillColor(255, 255, 255);	
		$pdf->Cell(88,5,$D->solicitado_por,1,1,'C',1);
		...
		$pdf->setXY(141,90);
			$pdf->SetFont('Arial','B',7);
				$pdf->SetFillColor(	255, 255, 255);	
			$pdf->Cell(59,5,utf8_decode($Ciu),1,1,'C',1);
			$pdf->SetFont('Arial','B',7);
	...
	$pdf->setXY(40,110);
		...
		$pdf->SetTextColor(0,0,0);
			$pdf->SetFillColor(	255, 255, 255);	
		$pdf->Cell(30,5,utf8_decode($D->placa),1,1,'C',1);
		...
		$pdf->setXY(100,110);
			$pdf->SetFont('Arial','B',7);
				$pdf->SetFillColor(	255, 255, 255);	
			$pdf->Cell(33,5,utf8_decode($DV->modelo),1,1,'C',1);
			$pdf->SetFont('Arial','B',7);
		...
		$pdf->setXY(161,110);
			$pdf->SetFont('Arial','B',7);
				$pdf->SetFillColor(	255, 255, 255);	
			$pdf->Cell(39,5,utf8_decode($Mr->nombre),1,1,'C',1);
			$pdf->SetFont('Arial','B',7);
	...
	$pdf->setXY(10,115);
	 $pdf->SetFillColor(215, 240, 127);
		...
		$pdf->SetFont('Arial','B',7);
			$pdf->SetFillColor(	255, 255, 255);	
		$pdf->Cell(40,5,utf8_decode($li->nombre),1,1,'C',1);
		...
		$pdf->setXY(140,115);
			...
			$pdf->SetFillColor(255, 255, 255);
				$pdf->SetFillColor(	255, 255, 255);	
			$pdf->Cell(60,5,utf8_decode($HT->fecha_inicial),1,1,'C',1);
			$pdf->SetFont('Arial','B',7);
		...
		$pdf->SetFillColor(255, 255, 255);
			$pdf->SetFillColor(	255, 255, 255);	
		$pdf->Cell(40,5,number_format($HT->odometro_inicial),1,1,'C',1);
		...
		$pdf->setXY(140,120);
			$pdf->SetFont('Arial','B',7);
				$pdf->SetFillColor(255, 255, 255);
					$pdf->SetFillColor(	255, 255, 255);	
			$pdf->Cell(60,5,utf8_decode($HT->fecha_final),1,1,'C',1);
			$pdf->SetFont('Arial','B',7);	
		...
		$pdf->SetFont('Arial','B',7);
		  $pdf->SetFillColor(255, 255, 255);
		$pdf->Cell(51,5,utf8_decode($Prov->nombre),1,1,'C',1);
				$pdf->SetFillColor(	255, 255, 255);	
		$pdf->SetFont('Arial','B',7);
		$pdf->setXY(100,140);
		$pdf->SetFillColor(	215, 240, 127);
		...
		$pdf->setXY(141,140);
        $pdf->SetFillColor(255, 255, 255);
	    $pdf->Cell(59,5,utf8_decode($Prov->identificacion),1,1,'C',1);
			$pdf->SetFillColor(	255, 255, 255);	
        $pdf->setXY(10,145);
		$pdf->SetFillColor(	215, 240, 127);
		...
		$pdf->SetFont('Arial','B',7);
		$pdf->SetFillColor(255, 255, 255);
		$pdf->Cell(51,5,utf8_decode($Prov->ciudad),1,1,'C',1);
				$pdf->SetFillColor(	255, 255, 255);	
		$pdf->SetFont('Arial','B',7);
		$pdf->setXY(100,145);
		$pdf->SetFillColor(	215, 240, 127);
		...
		$pdf->SetFont('Arial','B',7);
			$pdf->SetFillColor(255, 255, 255);
		$pdf->Cell(51,5,utf8_decode($Prov->contacto),1,1,'C',1);
		$pdf->SetFont('Arial','B',7);
		$pdf->setXY(100,150);
     	$pdf->SetFillColor(	215, 240, 127);
		...
		$pdf->setXY(141,150);
		$pdf->SetFillColor(255, 255, 255);
		$pdf->Cell(59,5,utf8_decode($Prov->email),1,1,'C',1);
    ...
    $pdf->SetFillColor(215, 240, 127);
		...
		$Det.="<tr><td>$Dt->tipo</td><td>$Dt->item</td><td>$Dt->unidad_medida</td><td>$Dt->observaciones</td><td>$Dt->centrodeoperacion</td><td>$Dt->cantidad</td><td align='right'>$".coma_format($Dt->valor_unitario)."</td><td align='right'>$".coma_format($Dt->valor_total)."</td></tr>";
		    ...
		    $yPos=$pdf->GetY();
				$pdf->SetFillColor(255, 255, 255);
		    $pdf->MultiCell($cellWidth,$cellHeight,$observacions,1,1,1);
