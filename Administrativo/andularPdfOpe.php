    ...
    $pdf->SetFillColor(215, 240, 127);
		...
		$Det.="<tr><td>$Dt->tipo</td><td>$Dt->item</td><td>$Dt->unidad_medida</td><td>$Dt->observaciones</td><td>$Dt->centrodeoperacion</td><td>$Dt->cantidad</td><td align='right'>$".coma_format($Dt->valor_unitario)."</td><td align='right'>$".coma_format($Dt->valor_total)."</td></tr>";
			...
			$cellHeight=5;//normal one-line cell height
			 ...
			 if($pdf->GetStringWidth($observaciones) < $cellWidth){
			  ...
			  $textLength=strlen($observaciones); //total text length
			  ...
			  while($startChar < $textLength){ //loop until end of text
			   ...
			   ($startChar+$maxChar) < $textLength ) {
				$maxChar++;
				$tmpString=substr($observaciones,$startChar,$maxChar);
			   }
		    ...
		    $yPos=$pdf->GetY();
			$pdf->SetFillColor(255, 255, 255);
		    $pdf->MultiCell($cellWidth,$cellHeight,$observaciones,1);
	  ...
	  $Res.="</table>";
      $Det.="</table>"; 
	  $pdf->Output('/var/www/html/public_html/Administrativo/inc/requicionAnulada'.$id.'.pdf','F');
