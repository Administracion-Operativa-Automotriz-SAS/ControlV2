...
<hr>
    <form method='post' style='  margin-bottom:1%; text-align : center;  ' action=''>
       <img src='https://pbs.twimg.com/profile_images/1305916683178123273/1BtxwW1A_400x400.jpg'  width='40' height='40' ></img>
	  	<br>
	Nombre  <input type="text" name="name"  value="<?php echo"RequisiciónTransOriente$id "?>"/>
        <input type="submit" value="Descargar TransOriente " name="submit" />
...
$ER=qo("select  
    ...
    $pdf->SetFillColor(50, 214, 79);
		...
		$pdf->Cell(18,8,utf8_decode('VALOR TOTAL '),1,0,'C',1);
				$pdf->Cell(43.5,8,utf8_decode('ITEM'),1,0,'C',1);

		$pdf->Cell(43.5,8,utf8_decode('DESCRIPCIÓN'),1,0,'C',1);
		...
		while($row = mysql_fetch_object($Detalle))
		{
	     ...
	     $pdf->SetFont('Arial','',7);
			...
			$pdf->MultiCell($cellWidth,$cellHeight,$item, 1,  'C', true);
			$pdf->SetXY($x +26+24+13+ 21+18+ $cellWidth , $y);
		    $pdf->MultiCell($cellWidth , $cellHeight,$observacions, 1, 'C', true);
			//Go to next row
			$y_axis = $y_axis + $row_height;
			$i = $i + 1;
		}
	  ...
	  $pdf->SetFillColor(255, 255, 255);
		$pdf->Cell(97,5,"$".number_format($Total->resultado),1,1,'C',1);
