  ...
  aoa_modulo.tipoNovedad.idtipoNovedad as id_nombre_tipoN, 
   ...
   LEFT OUTER JOIN aoacol_aoacars.siniestro on novedad.id_siniestro = siniestro.numero 
	...
	$pdf->setXY(10,15);
		...
		$pdf->Text(30,69,utf8_decode("ACTA DE NOVEDAD #$novedad->id_novedad DEL VEHÍCULO  #$novedad->id_placa "));
	...
	$imagen_size = 290;
		...
		$pdf->SetFont('Arial','B',8);
	    $pdf->SetFillColor(187,224,67);
		$pdf->Cell(190,7,utf8_decode('INFORMACIÓN DE LA NOVEDAD PENDIENTE '),1,1,'C',1);
    ...
    $pdf->SetFillColor(215, 240, 127);
		...
		$pdf->Cell(16,5,utf8_decode('ID'),1,0,'C',1);
				...
				$pdf->Cell(26,5,utf8_decode('DESCRIPCIÓN'),1,0,'C',1);
				 ...
				 while($row = mysql_fetch_object($Detalle))
		{
			...
			if ($i == $max)
			{
				...
				$pdf->Cell(26,8,utf8_decode('DESCRIPCIÓN'),1,0,'C',1);
