...
?>
     ...
     <form method='post' style='  margin-bottom:1%; text-align : center;  ' action=''>Nombre
        <input type="text" name="nameA"  value="<?php echo"Requisición$id "?>"/>
        <input type="submit" value="Descargar Tras Oriente " name="submit" />
...
$ER=qo("select requisicion.placa, 
	...
	$Fecha_control=date('Y-m-d',strtotime(aumentadias(date('Y-m-d'),30)));
	// incluye el detalle de la requisicion
	$Det="<table class='table' border cellspacing='0' style='margin-bottom: 2%;  border: 1px solid rgba(118,136,29,1);'> <caption> <h3 style='margin-bottom: 5%;  color: 1px solid rgba(118,136,29,1);' class='titulo'><br  class='titulo' >Detalle de la requisición:<br></h3> </caption><tr><th>Tipo de Requisicion</th><th>Item</th><th>Unidad de medida</th><th>Descripcion</th><th>Centro de operacion</th><th>Cantidad</th><th>Valor unitario</th><th>Valor</th>";
	...
	$pdf->setXY(10,115);
		...
		$pdf->MultiCell(59,5,utf8_decode($Prov->email),1,1,'C');
