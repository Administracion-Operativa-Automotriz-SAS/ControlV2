...
class clsMsDocGenerator{
	...
	function addParagraph($content, $inlineStyle = NULL, $className = 'normalText')
	{
		...
		$this->documentBuffer .= "<p class=\"$className\"" . ($style != '' ? " style=\"$style\"" : '') . ">".($content == '' ? '<br><br>' : $content)."</p>\n";
	}//end addParagraph()
	 ...
	 */
	function bufferImage_old($imagePath, $width, $height, $title = ''){
		$this->numImages++;
		$buffer = "<!--[if gte vml 1]>";
		if($this->numImages == 1){
	...
	function bufferImage($imagePath, $width, $height, $title = ''){
		$this->numImages++;
		$buffer = "<img width=\"$width\" height=\"$height\" src=\"$imagePath\" v:shapes=\"_x0000_i102{$this->numImages}\">";
		return $buffer;
	}//end buffe
