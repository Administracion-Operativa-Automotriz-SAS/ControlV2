...
class brow_columna
{
	...
	var $tabla_relacion=''; // si este campo est� relacionado con una tabla aqui queda el nombre fisico de la tabla
	var $campo_relacion=''; // si estecampo est� relacionado con una tabla aqui queda el campo conel cual est� relacionado
	var $contenido_relacion=''; // si el campo est� relacionado conuna tabla, aqui queda lo que se desea que aparezca en el brow
	var $menudirecto=false; // Este campo corresponde a un menu directo donde las opciones est�n separadas por ; y el value y la descripcion por ,
	...
	function brow_columna($C,$LINK,$Tabla)
	{
		...
		if(!$C->traet && $C->traex && strpos($C->traex,';'))
		{
			...
			$sql = "show function status like '$Nombre_funcion' ";
			
			
			//echo $sql;
			
			
			if($Funcion=mysql_query($sql,$LINK))
			{
				if($Tabla != "provee_produc_serv"){
	...
	function pinta_titulo($Movil=0)
	{
		echo "<th nowrap='yes' id='th_$this->campo' name='th_$this->campo'  bgcolor='ffffff' onclick=\"m_c('$this->campo',$this->idcampo);return false;\" oncontextmenu=\"m_c('$this->campo',$this->idcampo);return false;\" >$this->titulo</th>";
	}
}
...
class brow_tabla
{
	...
	var $TextoTabla=''; // Texto adicional en la presentaci�n de la tabla.
	var $Condicion_sql=''; // Condici�n en formato sql para la seleccion de los registros
	...
	var $Pagina=1; // P�gin de registros
	var $Cantidad_paginas=1; // cantidad de paginas
	var $Lineas_por_pagina=50; // lineas por p�gina
	var $Vinculo_externo_campo=''; // si la tabla debe tener un filtro autom�tico por un campo externo
	var $Vinculo_externo_contenido=''; // si la tabla debe tener un filtro autom�tico  por un contenido externo
	...
	var $Vinculo_primario=''; // En caso de que la informaci�n dependa de un v�nculo primario, es un campo donde se incluye la informaci�n del Id Alterno del usurario y se utiliza para filtrar las tablas
	var $Info_cabecera=''; // Se usa para la informaci�n adicional que se debe mostrar en la cabecera.
	var $Buscador_relacionado='';
	var $Dispositivo_movil=0; // si es dispositivo movil es 1
	...
	function brow_tabla($Id=0)  // recibe el id del numero de la tabla
	{
		...
		$ExB /*busqueda exacta */,
		$TI  /* Trae Informaci�n para mostrar en la cabecera */;
		if(browser_movil()) $this->Dispositivo_movil=1;
	...
	function aparece()
	{
		...
		echo "
			...
			echo "Contenido+=\"</table>\";
				...
				function m_c(Campo,Idcampo)
				{
					...
					Contenido+=\"<IMG SRC='gifs/standar/Down.png' border='0' height='12' vspace='0' hspace='0' align='bottom'> Ordenar Descendentemente</td></tr>\";
		...
		if($_SESSION['Disenador'])
		{
			echo "Contenido+=\"<tr><td bgcolor='#ffffdd' align='center'><b>".$this->nombre_tabla.".\"+Campo+\"</b></td></tr>\";
					...
					Contenido+=\"<tr><td style='cursor:pointer' onclick=\\\"oculta_mc();modal('marcoindex.php?Acc=generar_t_mysql&Tabla=".$this->nombre_tabla."&Campo=\"+Idcampo+\"',0,0,10,10,'GenT');\\\" nowrap='yes'>\";
					Contenido+=\"<img src='gifs/standar/dsn_estructura.png' border='0' align='bottom'> Redefinir Funcion T</td></tr>\";
		...
		else
		{
         ...
         <table width='100%' border=0 cellspacing=1 oncontextmenu=\"m_ct();return false;\" onclick=\"m_ct();return false;\">
			...
			($this->Icono?"<img src='$this->Icono' height=20 border=0 align='top'> ":"").
   			...
   			[<span id='tCantidad_paginas'>$this->Cantidad_paginas</span> p�ginas]".
	...
	function busca_columnas()
	{
		...
		$sql = "select * from $Tabla_control where find_in_set(campo,'$this->Vercampos') order by capa,orden,suborden";
		//echo $sql;
		//exit;
	...
	function pinta_titulos()
	{
		...
		for($i=0;$i<count($this->Cols);$i++) $this->Cols[$i]->pinta_titulo($this->Dispositivo_movil);
		echo "</tr></table>
		";
	}
		...
		echo "<table><tr><td valign='top' nowrap='yes'>
            <img src='gifs/standar/refrescar.png' align='top' border=0 height=20
               ...
               <img src='gifs/standar/pagina_inicial.png' border=0 align='top' onclick='ir_pagina(0);'>
				...
				Lineas por p�gina: <select onchange='cambia_lineas_por_pagina(this.value);' style='font-size:9px;padding:0;'>
		...
		echo "</td>
         <td nowrap='yes'><A class='sinfo' style='cursor:pointer;' onclick=\"window.open('marcoindex.php?Acc=ver_mas_campos&Num_Tabla=$this->id_tabla','Detalle_tabla$this->id_tabla');\" >
	        ...
	        <a class='sinfo' style='cursor:pointer;' onclick='cambio_tamano_marco(1);'>
		        <img src='gifs/standar/marcoup.png' hspace=3 border=0'><span style='width:300px'>Reducir el tama�o del marco</span></a>
		    <a class='sinfo' style='cursor:pointer;' onclick='cambio_tamano_marco(0);'>
			    <img src='gifs/standar/marco.png' hspace=3 border='0'><span style='width:300px'>Normalizar el tama�o del marco</span></a>
	        <a class='sinfo' style='cursor:pointer;' onclick='cambio_tamano_marco(2);'>
		        <img src='gifs/standar/marcodown.png' hspace=3 border=0'><span style='width:300px'>Ampliar el tama�o del marco</span></a>
...
class brow_campo
{
	...
	var $Imagen=0; // si la presentaci�n es imagen
	var $Pdf=0; // si la presentaci�n es un archivo pdf
	var $Checkbox=0; // si la presentaci�n es check
	var $Url=0; // si es una direcci�n url de internet
	var $Email=0; // si es una direcci�n de correo electr�nico
	...
	function brow_campo($Campo,$LINK)
	{
	  ...
	  */
		...
		$this->Url=$Campo[6]; // direcci�n Url
		$this->Email=$Campo[7]; // direcci�n de correo electr�nico
		...
		$this->tdp=$Campo[10]; // propiedades td del brow
	}

	function pinta($R,$BMM,$Movil)
	{
		global $Nombre_tabla;
		$Capa='cd_'.$R->id.'_'.$this->nombre_campo;
		eval("\$Contenido=\$R->$this->nombre_campo;");
		echo "<td align='$this->alineacion' valign='top'".
			($this->MoDirecta?" onmouseover=\"muestra('$Capa');\" onclick=\"oculta_ultima_captura();Ultima_captura='$Capa';\" onmouseout=\"oculta('$Capa');\" ":"");
	//	if($this->nombre_campo=='id') echo " onclick='m_c($R->id);' ";
		if($BMM && $this->nombre_campo=='id') 
		{
			echo " nowrap='yes' ><input type='checkbox' name='Bmm_$R->id' id='Bmm_$R->id'";
		}
		echo ">";
	//	if($Movil && $this->nombre_campo=='id') echo "<a style='cursor:pointer' onclick='if(parent.Modifica==1) mod_reg($R->id); else no_modreg();'><img src='gifs/lapiz.gif' border=0></a>"; 
		if($Movil && $this->nombre_campo=='id') echo "<a style='cursor:pointer' onclick='m_c($R->id);'><img src='gifs/mas_opciones.png' border=0></a>"; 
		if($this->MoDirecta)
		{
		...
		elseif($this->Imagen)
		{
			...
			else
			{
				if($Contenido)
				{
					...
					{
						$Sub_Contenido=substr($Contenido,strrpos($Contenido,'/')+1);$Sub_Tumb='tumb_'.$Sub_Contenido;$Tumb=str_replace($Sub_Contenido,$Sub_Tumb,$Contenido);
						if(!file_exists($Tumb) && file_exists($Contenido))
						{
							if(strpos(strtolower($Sub_Contenido),'.jpg')) picresize($Contenido,TUMB_SIZE,'jpg',$Tumb);
							if(strpos(strtolower($Sub_Contenido),'.gif')) picresize($Contenido,TUMB_SIZE,'gif',$Tumb);
							if(strpos(strtolower($Sub_Contenido),'.png')) picresize($Contenido,TUMB_SIZE,'png',$Tumb);
						}
						echo "<img src='$Tumb' border='0' height=30 onclick=\"modal('$Contenido',0,0,600,800);\">";
					}
					else
					{
						echo "<img src='gifs/standar/img_neutra.png' border='0' height=30>";
					}
				}
				else
				{
...
class brow_tabla_detalle
{
	...
	var $Lineas_por_pagina=50; // lineas por pagina
	...
	function brow_tabla_detalle($Query,$CQ,$Pagina,$Lineas_por_pagina,$OQ,$OC='',$OT='asc',$Id_tabla=0)
	{
		...
		if($LCampos=mysql_query("select campo,alinea,coma,tipo,caja,td_propiedades from ".$this->nombre_tabla."_t where find_in_set(campo,'$Vercampos') order by capa,orden,suborden",$LINK))
		{
			while($Campo=mysql_fetch_object($LCampos))
			{
				...
				$Campito[10]=$Campo->td_propiedades;
	...
	function aparece()
	{
		...
		function no_modreg()
		{
			alert('No tiene acceso a la modificaci�n de este registro');
		}
       ...
       function valida_scroll()
       {
        ...
        <table border=0 cellspacing=1 id='Contenido_tabla_$this->id_tabla' style='empty-cells:show;top:-20' bgcolor='#bbddcc' width='100%'>";
			//echo "sql ".$this->Query.$this->Condiciones_query.$this->Orden_query.$this->Limit_query;
			echo $this->Titulos;
			
			$sql = $this->Query.$this->Condiciones_query.$this->Orden_query.$this->Limit_query;
			//echo $this->Query;
			//exit;
      ...
      <br><br><span name='TextoTabla' id='TextoTabla'></span><br><br>";
			...
			echo "<iframe id='context_registro' name='context_registro' width='100' height='100'  frameborder='yes' src='gifs/standar/loading.gif'
				style='visibility:hidden;position:absolute;border-style:solid;border-width:2px;background-color:#fdfdfd;' scrolling='yes'></iframe>";
	...
	function pinta_campos($R)
	{
		...
		echo "
		<tr id='Reg_$R->id' oncontextmenu=\"m_c($R->id);return false;\" onclick=\"m_c($R->id);return false;\" ondblclick='if(parent.Modifica==1) mod_reg($R->id);' bgcolor='$Fondo' >";
		for($i=0;$i<count($this->Campos);$i++)
		{
			$this->Campos[$i]->pinta($R,$this->Borrado_masivo,$this->Dispositivo_movil);
		}
		echo "</tr>";
	}
