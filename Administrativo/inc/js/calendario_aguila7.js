...
var Asignando_calendario=false;
...
function Calendario_clase(Objeto,Formato_fecha)
{
	...
	this.asigna=function(m,d)
	{
		...
		this.cerrar();
		eval('var Cm=document.'+this.Forma+'.'+this.Campo+';');
		Cm.value=this.Ano+'-'+M+'-'+D+(this.Formato=='t'?' '+this.Hora+':'+this.Minuto+':00':'');
		Asignando_calendario=true;
		Cm.focus();
		return true;
	}
...
{
	var Forma=Objeto.form.name;
	var Campo=Objeto.name;
	var Nombre=Forma+'_'+Campo;
...
function diamas(Campo,Formulario)
{
	eval('var Cm=document.'+Formulario+'.'+Campo+';');
	Asignando_calendario=true;
	Cm.value=aumentadia(Cm.value,1);
	Cm.focus();
}

function diamenos(Campo,Formulario)
{
	eval('var Cm=document.'+Formulario+'.'+Campo+';');
	Cm.value=aumentadia(Cm.value,-1);
	Asignando_calendario=true;
