...
function ejecutar()
{
	...
	IF($ID == 36)
	{
		...
		include("/var/www/html/public_html/Administrativo/views/reports/requisiciones_por_fecha.html");
		exit;
	}
	###############################  SOLICITUDES DE VARIABLES DE ACUERDO A LA FORMULA INICIAL ###################
	$Reporte=qo("select * from aqr_reporte where id=$ID");
	//print_r($Reporte);
	if($Reporte->extension_url != null)
	{
		header("location:$Reporte->extension_url");
		exit;	
	}	
...
function ejec_pinta()
{
	...
	{
		//echo "Select * from $_Tabla_temporal $Filtros_adicionales";
