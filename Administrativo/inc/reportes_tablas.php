...
function ad_tabla()
{
	...
	$Tablascars=q("SHOW FULL TABLES FROM aoacol_aoacars");
	echo "<table border cellspacing=0><tr><td colspan=2><b>Tablas Disponibles aoacol_aoacars</b></td></tr>";
	while ($R=mysql_fetch_row($Tablascars))
	{
		$L=strlen($R[0]);
		$ul=substr($R[0],$L-2,2);
		if($ul != '_t' && $ul != '_s' && substr($R[0],0,15) != 'control_reporte' && substr($R[0],0,11) != 'aqr_reporte' && !strpos($Ts,','.$R[0].',') && $R[0] != 'usuario' && $R[0] !='usuario_custom' && $R[0] != 'usuario_tab')
