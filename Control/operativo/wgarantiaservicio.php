...
Clave secreta
Úsala para las comunicaciones entre tu sitio y Google. Ten la precaución de no revelársela a nadie.
...
function inicio_pantalla()
{
	...
	<form action='wgarantiaservicio.php' target='_self' method='POST' name='forma' id='forma'>
		<p style='font-size:20px'>
			Por favor digite la placa del vehículo Siniestrado o su Código de Servicio de Renta (sin espacios ni separadores):
		...
		<p style='font-size:20px'>
			Digite el número de teléfono registrado: <input type='password' style='font-size:20px;' size=10 name='telefono' id='telefono'>
		</p>
...
function consultar_placa()
{
	...
	echo "
		...
		function validacion_general_campos()
		{
			with(document.forma)
			{
				if(Number(identificacion.value)<1000 || !identificacion.value) {alert('Debe escribir un número de identificación válido.');identificacion.style.backgroundColor='#ffffaa';identificacion.focus();return false;}
				lugar_expdoc.value=alltrim(lugar_expdoc.value);
				if(!lugar_expdoc.value) {alert('Debe escribir un lugar de expedición de la identificación válido.');lugar_expdoc.style.backgroundColor='#ffffaa';lugar_expdoc.focus();return false;}
				nombre.value=alltrim(nombre.value);
				if(!nombre.value) {alert('Debe escribir un nombre válido.');nombre.style.backgroundColor='#ffffaa';nombre.focus();return false;}
				apellido.value=alltrim(apellido.value);
				if(!apellido.value) {alert('Debe escribir un apellido válido.');apellido.style.backgroundColor='#ffffaa';apellido.focus();return false;}
				if(!tipo_id.value) {alert('Debe seleccionar un tipo de identificación válido.');tipo_id.style.backgroundColor='#ffffaa';tipo_id.focus();return false;}
				if(!sexo.value) {alert('Debe seleccionar un sexo válido.');sexo.style.backgroundColor='#ffffaa';sexo.focus();return false;}
				if(!ciudad.value) {alert('Debe seleccionar una ciudad válida.');_ciudad.style.backgroundColor='#ffffaa';_ciudad.focus();return false;}
				direccion.value=alltrim(direccion.value);
				if(!direccion.value) {alert('Debe escribir una dirección válida.');direccion.style.backgroundColor='#ffffaa';direccion.focus();return false;}
				telefono_oficina.value=alltrim(telefono_oficina.value);
				if(!telefono_oficina.value) {alert('Debe escribir un teléfono de oficina válido o en su defecto el número de celular actual.');telefono_oficina.style.backgroundColor='#ffffaa';telefono_oficina.focus();return false;}
				telefono_casa.value=alltrim(telefono_casa.value);
				if(!telefono_casa.value) {alert('Debe escribir un teléfono de casa válido o en su defecto el número de celular actual.');telefono_casa.style.backgroundColor='#ffffaa';telefono_casa.focus();return false;}
				celular.value=alltrim(celular.value);
				if(!celular.value) {alert('Debe escribir un número de celular válido.');celular.style.backgroundColor='#ffffaa';celular.focus();return false;}
				...
				if(ConGarantia_tarjeta)
				{
					...
					if(!numero_tarjeta.value) {alert('Debe escribir un número de tarjeta válido.');numero_tarjeta.style.backgroundColor='#ffffaa';numero_tarjeta.focus();return false;}
					...
					if(!vencimiento_ano.value) {alert('Debe seleccionar un año de vencimiento.');vencimiento_ano.style.backgroundColor='#ffffaa';vencimiento_ano.focus();return false;}
					codigo_seguridad.value=alltrim(codigo_seguridad.value);
					if(!codigo_seguridad.value) {alert('Debe escribir un código de seguridad válido.');codigo_seguridad.style.backgroundColor='#ffffaa';codigo_seguridad.focus();return false;}
					devol_cuenta_banco.value=alltrim(devol_cuenta_banco.value);
					if(!devol_cuenta_banco.value) {alert('Debe escribir un número de cuenta bancaria válido.');devol_cuenta_banco.style.backgroundColor='#ffffaa';devol_cuenta_banco.focus();return false;}
					...
					if(!devol_ncuenta.value) {alert('Debe escribir un nombre de titular de cuenta bancaria válido.');devol_ncuenta.style.backgroundColor='#ffffaa';devol_ncuenta.focus();return false;}
					if(Number(identificacion_devol.value)<1000 || !identificacion_devol.value) {alert('Debe escribir un número de identificación válido.');identificacion_devol.style.backgroundColor='#ffffaa';identificacion_devol.focus();return false;}
					if(!img_tarjeta_credito.value) {alert('Debe subir la imagen de la tarjeta de crédito');img_tarjeta_credito.style.backgroundColor='#ffffaa';img_tarjeta_credito.focus();return false;}
				}
				if(ConGarantia_noreembolsable)
				{
					if(!numero_comprobante.value) {alert('Debe escribir el número de comprobante de consignación.');numero_comprobante.style.backgroundColor='#ffffaa';numero_comprobante.focus();return false;}
					if(!comprobante_consignacion.value) {alert('Debe cargar la imagen del comprobante de consitnación. ');comprobante_consignacion.style.backgroundColor='#ffffaa';numero_comprobante.focus();return false;}
					if(!fecha_consignacion.value) {alert('Debe seleccionar la fecha de consignación. ');fecha_consignacion.style.backgroundColor='#ffffaa';fecha_consignacion.focus();return false;}
				}
				if(ConGarantia_efectivo)
				{
					if(!numero_comprobante1.value) {alert('Debe escribir el número de comprobante de consignación.');numero_comprobante.style.backgroundColor='#ffffaa';numero_comprobante.focus();return false;}
					if(!comprobante_consignacion1.value) {alert('Debe cargar la imagen del comprobante de consitnación. ');comprobante_consignacion.style.backgroundColor='#ffffaa';numero_comprobante.focus();return false;}
					if(!fecha_consignacion1.value) {alert('Debe seleccionar la fecha de consignación. ');fecha_consignacion.style.backgroundColor='#ffffaa';fecha_consignacion.focus();return false;}
				}
				submit();
			}
		}
	...
	if($P=qo("select id,estado from siniestro where placa='$placa' and declarante_celular='$telefono' order by id desc"))
	{
		...
		elseif($P->estado==5)
		{echo "<h4>La placa $placa se encuentra en estado PENDIENTE aún no ha sido adjudicado el servicio.</h4><input type='button' class='button green large' name='volver' id='volver' value='REGRESAR' onclick='regresar();'>";}
		elseif($P->estado==7)
		{echo "<h4>La placa $placa se encuentra en SERVICIO no requiere de datos de garantía.</h4><input type='button' class='button green large' name='volver' id='volver' value='REGRESAR' onclick='regresar();'>";}
		elseif($P->estado==8)
		{echo "<h4>La placa $placa se encuentra en estado SERVICIO CONCLUIDO no requiere de datos de garantía.</h4><input type='button' class='button green large' name='volver' id='volver' value='REGRESAR' onclick='regresar();'>";}
		elseif($P->estado==3) /*ADJUDICADO*/
		{
			...
			if($S->renta)
			{
				echo "<h3>INFORMACION DEL SERVICIO DE RENTA</H3><table>
						<tr><td>Código de Servicio de Renta:</td><td>$S->placa</td></tr>
						...
						<tr><td>Ciudad de Atención</td><td>$C->nciu</td></tr>
						<tr><td>Fecha de Autorización</td><td>".fecha_completa($S->fec_autorizacion)."</td></tr>
					</table>";
			}
			else
			{
				echo "<h3>INFORMACION DEL SINIESTRO:</h3>
					<table>
						<tr><td>Placa del vehículo Siniestrado</td><td>$S->placa</td></tr>
			...
			{
				echo "<h3>GARANTIA EN PROCESO</h3>Estimado Usuario: Ya se encuentra una o más garantías en proceso. Debe esperar a que sea aprobada o rechazada para continuar con el proceso o volver a incluir datos de una nueva garantía.";
				echo "<br><br>Garantía(s) en proceso: <br><br><table border cellspacing='0'>
			...
			{
				echo "<h3>GARANTIA PROCESADA</h3>Estimado Usuario: Ya se encuentra una o más garantías procesadas. ";
				echo "<br><br>Garantía(s) procesadas: <br><br><table border cellspacing='0'>
			...
			echo "<p style='font-size:20px'>Estimado Usuario: La garantía que se constituye para acceder al servicio se puede hacer usando la documentación propia o de cualquier persona de su confianza.
				Por favor diligencie la información de la persona que va a prestar la garantía en el siguiente formulario:</p>
				...
				<form action='wgarantiaservicio.php' enctype='multipart/form-data' target='_self' method='POST' name='forma' id='forma'>
					...
					<table border cellspacing='0'>
						<tr><td>Número de Identificación</td><td><input type='text' name='identificacion' id='identificacion' value='' size='30' maxlength='15' placeholder='Número sin comas ni puntos'></td></tr>
						<tr><td>Lugar de Expedición de la Identificación</td><td><input type='text' name='lugar_expdoc' id='lugar_expdoc' value='' size='50' maxlength='50' onkeyUp='this.value=this.value.toUpperCase();'></td></tr>
						...
						<tr ><td>Tipo de identificación:</td><td >";
			...
			echo "</td></tr>
						<tr><td>Ciudad de Domicilio </td><td ><input type='text' style='color:#000099;background-color:#FFFFFF;' name='_ciudad' id='_ciudad' size='30' onclick=\"busqueda_ciudad2('ciudad','05001000');\" readonly placeholder='click aquí'><input type='hidden' name=ciudad id=ciudad value=''><span id='bc_ciudad'></span> <img src='gifs/standar/izquierda.png'> Click en la casilla para seleccionar la ciudad.</td></tr>
						<tr ><td>Dirección Domicilio</td><td ><input type='text' name='direccion' id='direccion' size='50' maxlength='50' onkeyUp='this.value=this.value.toUpperCase();' placeholder='Dirección de donde vive'></td></tr>
						<tr><td>Teléfono Oficina</td><td ><input type='text' name='telefono_oficina' id='telefono_oficina' size='50' maxlength='50'></td></tr>
						<tr ><td>Teléfono Vivienda</td><td ><input type='text' name='telefono_casa' id='telefono_casa' size='50' maxlength='50'></td></tr>
						<tr><td>Celular</td><td ><input type='text' name='celular' id='celular' size='50' maxlength='50'></td></tr>
						<tr ><td>Cotreo Electrónico</td><td><input type='text' name='email_e' id='email_e' size='50' maxlength='70' onkeyUp='this.value=this.value.toLowerCase();' placeholder='correo electrónico'></td></tr>
						<tr><td>Observaciones</td><td><textarea name='observaciones' cols=80 rows=2></textarea></td></tr>
					</table>
					<br>";
			// A PARTIR DE LA CITA 130508 empezo a funcionar la variable siniestro.tipogarantia que se asigna al momento de la adjudicación
			...
			if($S->tipogarantia!=1 && $S->no_garantia==0)
			{
				...
				// Se valida si la aseguradora tiene la opción de Garantia no reembolsable dentro de sus tarifas
				...
				echo "<h3 style='color:blue;'>Seleccione un tipo de garantía:</h3>
							<br>TIPO DE GARANTIA <input type='button' class='button small blue' value='Tarjeta de Crédito' onclick='cambio_tipo_garantia(1);'> <input type='button' class='button small blue' value='Consignación Efectivo' onclick='cambio_tipo_garantia(3);'> ";
				...
				echo "<div id='dtc' style='display:none;'>
					...
					<table border cellspacing='0'>
						...
						<tr><td>NUMERO DE TARJETA</td><td><input type='text' name='numero_tarjeta' id='numero_tarjeta' value='' size='50' maxlength='16' placeholder='Número de tarjeta sin guiones ni separadores'></td></tr>
						...
						<tr><td>CODIGO DE SEGURIDAD</td><td valign='top'><input type='password' id='codigo_seguridad' name='codigo_seguridad' class='numero' size='4' maxlength='4'> El código de seguridad aparece en el reverso de su tarjeta <br />
						como aparece en las imágenes: <img src='img/codigo_seguridad_tc.jpg' height='150' align='top'></td></tr>
						<tr><td height='100' width='200'>Tarjeta de Crédito (solo el frente, sin código de seguridad) </td><td><input type='file' name='img_tarjeta_credito'></td></tr>
					</table>
					</div>
					<div id='dtco' style='display:none;'>
						...
						<table border cellspacing='0'>
							...
							<tr><td>NUMERO DE COMPROBANTE DE CONSIGNACION</td><td><input type='text' name='numero_comprobante1' id='numero_comprobante1' value='' size='30' maxlength='30' placeholder='No. Comprobante Consignación'></td></tr>
					...
					<table border cellspacing='0'>
						...
						<tr><td>NUMERO DE COMPROBANTE DE CONSIGNACION</td><td><input type='text' name='numero_comprobante' id='numero_comprobante' value='' size='30' maxlength='30' placeholder='No. Comprobante Consignación'></td></tr>
						<tr><td colspan='2'>COMPROBANTE DE CONSIGNACION</td></tr>
				...
				echo "
					<div id='resto_datos' style='display:none;'>
						...
						En algunos casos, si aplica, la garantía puede usarse para cancelar facturas asociadas al servicio. Cuando esto sucede, se debe hacer devolución del dinero sobrante mediante transferencia electrónica.
						La información financiera para devoluciones es utilizada para agilizar el traslado electrónico de dichos montos. Por favor llene completamente los siguientes campos:
						<table border cellspacing='0'>
						<tr><td>Numero de cuenta Bancaria</td><td><input type='text' name='devol_cuenta_banco' id='cuenta' size=50 maxlength='20' placeholder='Número de cuenta sin separadores'></td></tr>
				...
				echo "</td></tr>
						...
						<tr><td>Identificación</td><td><input type='text' name='identificacion_devol' id='identificacion_devol' value='' size='30' maxlength=15 placeholder='sin comas ni puntos'></td></tr>
						</table>";
	...
	{
		echo "<h4>No se encuentra la placa $placa con el teléfono $telefono en la base de datos.</h4><input type='button' class='button green large' name='volver' id='volver' value='REGRESAR' onclick='regresar();'>";
	}
	echo "</body>";
}
...
function grabar_informacion_garantia()
{
	...
	graba_bitacora_garantiaservicio('cliente','M',$idcli,'Actualización de datos del cliente via formulario web publico');
	...
	if($TIPO_GARANTIA=='noreembolsable')
	{
		...
		if($_FILES['img_licencia_conduccion_frente'])
		{
			...
			if(!in_array($type, $mimetypes))
			{
				echo "<script language='javascript'>function carga(){history.back();alert('no seleccionó una imagen válida ".$type." ');}</script><body onload='carga()'></body>";die();
			}
			$Caracteristicas_imagen = getimagesize($tmp_name);
			$File_destino=$directorio.'/'.$Subdirectorio.'/'.$Id.'/'.$C.'_'.strtolower(str_replace(' ','_',$name));
			if(is_file($File_destino)) @unlink($File_destino);
	...
	<body><img src='img/LOGO_AOA_200.png'><h3>CAPTURA INFORMACION DE GARANTIA</h3>
		Su información ha quedado registrada en nuestra base de datos. <br>
		Gracias por cumplir con este paso, su servicio tendrá un ahorro de tiempo de aproximadamente 30 minutos en la cita de entrega del vehículo. <br>
		<br>
