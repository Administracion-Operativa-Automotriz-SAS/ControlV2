 ...
 * Cambios:
 *Autor: Sergio Urbina
 * 1. Se agrego la funcionalida de multiplicacion en Control zbalance_estadoDev
 *
 * 
 * Fecha:27/02/2019
 *********************/
...
function java($id) // pinta herramientas javascript para la pantalla general
{
	echo "<script language='javascript'>
			function adicionar_requisicion() { window.open('zbalance_estadoDev.php?Acc=adicionar_requisicion&id=$id','_self'); }
			function ver_imagen_requisicion(id) { modal('zbalance_estadoDev.php?Acc=ver_imagen_requisicion&id='+id,0,0,600,600,'imgrq'); }
			function adicionar_detaller(id) { modal('zbalance_estadoDev.php?Acc=adicionar_detaller&idr='+id+'&id=$id',0,0,300,300,'add_dreq'); }
			function recargar() { window.open('zbalance_estadoDev.php?Acc=ver_balance&id=$id','_self'); }
			function cerrar_requisicion(id) { if(confirm('Desea cerrar la requisicion?')) { window.open('zbalance_estadoDev.php?Acc=cerrar_requisicion&id='+id,'Oculto_req'); } }
			function re_enviar_solicitud_aprobacion(id) { window.open('zbalance_estadoDev.php?Acc=enviar_mail_solicitud_aprobacion&id='+id+'&Aviso=1','Oculto_req'); }
			function asociar_facprov() { modal('zbalance_estadoDev.php?Acc=asociar_facprov&id=$id',50,100,600,800,'asociar_facprov'); }
			function asociar_cajamenor() { modal('zbalance_estadoDev.php?Acc=asociar_cajamenor&id=$id',50,100,600,800,'asociar_cajamenor'); }
			function add_control() { modal('zbalance_estadoDev.php?Acc=adicionar_control_operaciones&id=$id',50,100,500,800,'adicionar_ctrol'); }
			function desasociar_cajamenor(id) { if(confirm('Desea des-asociar esta caja menor?')) { window.open('zbalance_estadoDev.php?Acc=desasociar_cajamenor&id='+id,'Oculto_cajamenor'); } }
			function desasociar_facprov() { if(confirm('Desea des-asociar esta factura de proveedor?')) { window.open('zbalance_estadoDev.php?Acc=desasociar_facprov&id='+id,'Oculto_proveedores'); } }
			function borrar_item_requisicion(id) {if(confirm('Desea borrar este item?')) {window.open('zbalance_estadoDev.php?Acc=borrar_item_requisicion&id='+id,'Oculto_req');} }
			function actualizar_factura_referencia(factura,id) {if(confirm('Desea dejar '+factura+' como factura referencia?')) {window.open('zbalance_estadoDev.php?Acc=actualizar_factura_referencia&factura='+factura+'&id='+id,'Oculto_req');return true;}}
			...
			function cambia_tipo1(id_detalle)
                {var T1=document.getElementById('tipo1_'+id_detalle);
                        if(T1.value)    {
                                ...
                                window.open('zbalance_estadoDev.php?Acc=ajusta_bs&id='+id_detalle+'&dato='+T1.value,'Oculto_detallerq');
                                else T1.value='';}
                        return true;}
			function crear_nuevo_proveedor() {modal('../../Administrativo/zproveedor.php?Acc=adicion_de_proveedor','crear_nuevo_proveedor');}
			function realizar_calificacion() {window.open('../../Administrativo/zproveedor.php?Acc=realizar_calificacion&refrescar_opener=1','_self');}
			function valida_cambio_proveedor(idp,idr) {if(confirm('Desea asignar ese proveedor a la requisición?')) window.open('zbalance_estadoDev.php?Acc=asigna_proveedor&idp='+idp+'&idr='+idr,'Oculto_detallerq');}
			function evaluar_requisicion(id){modal('zbalance_estadoDev.php?Acc=evaluar_requisicion&id='+id,0,0,500,500,'evrq');}
			function asigna_sede(sede,req) {window.open('zbalance_estadoDev.php?Acc=asignar_sede&sede='+sede+'&requisicion='+req,'Oculto_detallerq');}
			function adicionar_observaciones(idreq){modal('zbalance_estadoDev.php?Acc=adicionar_observaciones&id='+idreq,0,0,100,100,'adobs');}
		</script>";
}
...
function actualizar_factura_referencia() // actualiza la factura de referencia en una requisición
{
...
function ver_requisiciones($id) // muestra todas las requisiciones de un balance de estado
{
	...
	if($Requisiciones=q("select * from $BDA.requisicion where ubicacion=$id")) // busca las requisiciones
	{
		...
		while($Rq=mysql_fetch_object($Requisiciones))
		{
			...
			<td><table border cellspacing='0' width='100%'>
				...
				if($proov){			
				   ...
				   if($Sedes=q($sql)){
					...
					echo " La lista que aparece a continuación corresponde a los proveedores totalmente confiables y medianamente confiables, de acuerdo a la selección realizada siguiendo los lineamientos del proceso de Calidad. <br>
			...
			else echo "bgcolor='dddddd'><b>Requisición sin cerrar</b>";
			echo "</td>";
			...
			if($Proveedor)
			{
				if($Proveedor->tipo=='P')
				{
					...
					elseif($Rq->estado==4)
					{ // si ya esta evaluada muestra información de la evaluación
						echo "<td bgcolor='eeeeee' align='center'><b>Evaluación</b><br>Fecha: $Rq->fecha_evaluacion<br>Por: $Rq->evaluada_por<br>Calificacion: $Rq->calificacion</td>";
						$Calificados=true;
					}
				}
				else
				...
				elseif($Proveedor)
				{
					...
					if($Proveedor->tipo=='P')
					{
						...
						{// verificacion de confiabilidad del proveedor
							...
							echo "Realizar la <b><i>Selección</i></b> aquí: <a onclick='realizar_calificacion()' class='info'><img src='img/diploma.png' height='30' border='0'><span style='width:100px'>Realizar Selección</span></a>";
						}
					}
					else
					{
			...
			echo "<a class='info' href='javascript:adicionar_detaller($Rq->id);'><img src='gifs/standar/nuevo_registro.png' border='0' height='12'>Adicionar item<span style='width:200px'>Adicionar un item al detalle de la requisicion</span></a>
				&nbsp;&nbsp;&nbsp;&nbsp;<a class='info' href='javascript:cerrar_requisicion($Rq->id);'><img src='gifs/standar/stop_16.png' border='0' height='12'> Cerrar Requisición<span style='width:200px'>Cerrar Requisición</span></a>";
			echo "</td>
				</tr>";
		}
		mysql_close($LINK);
...
{ // muestra las asociaciones de caja menor , los busca en la base de datos administrativa
	...
	if($Calificados || $Empleados) // permite asociar mas gastos de caja menor al estado
		...
		echo "<b style='color:red'>Debe Evaluar bienes y/o servicios de requisición para asociar gastos de Caja Menor</b>";
}
...
function adicionar_requisicion() // formulario para adicionar requisicion
{
	...
	echo "<script language='javascript'>
		...
		function volver() {window.open('zbalance_estadoDev.php?Acc=ver_balance&id=$id','_self');}
		function crear_nuevo_proveedor() {modal('../../Administrativo/zproveedor.php?Acc=adicion_de_proveedor','crear_nuevo_proveedor');}
        function realizar_calificacion() {window.open('../../Administrativo/zproveedor.php?Acc=realizar_calificacion&refrescar_opener=1','_self');}
	...
	<form action='zbalance_estadoDev.php' target='_self' method='POST' name='forma' id='forma'>
		<table>
			<tr><td>Fecha:</td><td><input type='text' name='fecha' value='$Hoy' readonly></td></tr>
			<tr><td>Solicitado Por:</td><td><input type='text' name='solicitado_por' value='$Nusuario' size='80' readonly></td></tr>
		...
		<!--<tr><td>Documento Referencia</td><td><input type='text' name='referencia' id='referencia' value='' size='20' maxlength='20'></td></tr>
			 ...
			 <tr><td style='font-size:18px'>Proveedor:</td><td bgcolor='ffffff'><br>
                        ...
                        También aparecen los empleados de AOA a los que se les hace anticipos.<br>
                        ";
		// busca los proveedores que sean mediana y totalmente confiables. también muestra los tipo empleados de AOA
        if($Proveedores=q("select id,nombre,calificacion_actual,$BDA.t_ciudad(ciudad) as nciudad from $BDA.proveedor where calificacion_actual in ('M','C') or tipo='E' order by nombre"))
        {
	...
	<br><br>NOTA: El documento referencia se utilizará únicamente para hacer cruce de información con requisiciones previas de grandes volúmenes de insumos o partes.
	</body>";
}

function adicionar_requisicion_ok() // guarda la información de una nueva requisición
{
	...
	$varTest = "insert into $BDA.requisicion (fecha,solicitado_por,placa,ciudad,ubicacion,estado,perfil,referencia,factura_referencia,proveedor) 
		values ('$fecha','$solicitado_por','$placa','$ciudad','$id','','1','$referencia','$factura_referencia','$proveedor')";
	echo $varTest;	
...
function ver_imagen_requisicion() // muestra la imagen de la requisicion 
{
	...
	echo ' <div class="panel panel-primary">
      <div class="panel-heading">Cotización 1</div>
      <div class="panel-body">
		<div class="col-lg-6  col-md-6">';
			if($D->cotizacion_f_valor != null  and $D->cotizacion_f_proveedor != null)
			{
				...
				echo "<br><b><span>Valor Cotización:</span><b>
				<br><br>
				...
				if($D->cotizacion_seleccionada == null)
				{
					...
					<form onsubmit='asignar_cotizacion(this)'>
						...
						<button class='btn btn-primary form-control'>Asignar como cotización seleccionada para compra</button>
					</form>";
				}
				else
				{
	...
	echo ' <div class="panel panel-primary">
      <div class="panel-heading">Cotización 2</div>
      <div class="panel-body">
		<div class="col-lg-6  col-md-6">';
	...
	echo ' <div class="panel panel-primary">
      <div class="panel-heading">Cotización 3</div>
      <div class="panel-body">
		<div class="col-lg-6  col-md-6">';
	...
	echo "<script>
		...
		$('select[name='prov']').on('change', function(ev) {
		  ...
		  $.post( 'zbalance_estadoDev.php', { Acc:'get_data_proveedor',id:this.value }).done(function( data ) {
			...
			if(s_proveedor.activo == 0)
			{
				...
				{
					 $.post( 'zbalance_estadoDev.php', { Acc:'check_eval_proveedor',id:self.value }).done(function( data ) {
						response = JSON.parse(data);
						...
						{
							 $.post( 'zbalance_estadoDev.php', { Acc:'activate_proveedor',id:self.value }).done(function( data ) {
...
function adicionar_detaller() // adiciona un nuevo item al detalle de la requisicion
{
	...
	<script>	
		$(document).ready(function() {
			...
			$("#tipo2").change(function(){
				...
				$.post( "zbalance_estadoDev.php",{Acc:"get_prod_type",type:valor}, function( data ) {
						//console.log( data );
						let html = "";
	...
	function creartipos()
	{
		modal('zbalance_estadoDev.php?Acc=tipos_requisicion',0,0,600,600,'tr');
	}
	...
	function continuar() {window.open('zbalance_estadoDev.php?Acc=adicionar_detaller&idr=$idr&id=$id','_self');}
	function solicitar_nuevo_tipo() { window.open('zbalance_estadoDev.php?Acc=solicitar_nuevo_bien_servicio','_self');}
	...
	<form action='zbalance_estadoDev.php' target='Oculto_item' method='POST' name='forma' id='forma'>
		<table>

<tr><td>Seleccioné el tipo</td><td>".menu1("tipo2","SELECT id,nombre FROM $BDA.tipo;",0,1,"width:300px;")."  </td><br></tr>	
			
			<tr><td>Seleccione item</td><td>".menu1("tipo1","SELECT provee_produc_serv.id,concat(provee_produc_serv.nombre , ' = ' ,sistema.nombre, ' = ', unidad_de_medida.nombre)  as nombre
													...
													order by provee_produc_serv.nombre;",0,1,"width:400px;")."<br>
			...
			<tr><td>Clase de Requisición</td><td>".menu1("clase","select * from $BDA.requisicionc where id = 2 or id = 1 order by nombre",1)." Tipo de Cobro: <select name='tipo_cobro'><option value='S'>Sin Recobro</option><option value='C'>Con Recobro</option></select></td></tr>
...
function crear_nuevo_tipo_requisicion() // funcion que crea nuevo tipo de requisicion (dejada de usar)
{
	...
	header("location:zbalance_estadoDev.php?Acc=tipos_requisicion");
}
...
function cerrar_requisicion() // cierra una requisicion 
{
	...
	echo "<body><script language='javascript'>parent.recargar();</script></body>";
	enviar_mail_solicitud_aprobacion(); // llama la función de envio del correo para aprobacion
}
...
function enviar_mail_solicitud_aprobacion() // envia un correo para aprobacion de la requisicion control operativo
{
	...
	$ER=qo("select requisicion.placa,
     	concat( oficina.centro_operacion,' ',oficina.nombre) as centrodeoperacion,aseguradora.ccostos_uno as centrocosto,aseguradora.nombre as ASEGURADORA 
				 from aoacol_administra.requisiciond 
				 LEFT OUTER JOIN aoacol_administra.ccostos_uno on requisiciond.centro_costo = ccostos_uno.codigo 
	...
	$Hoy=date('Y-m-d H:i:s');
	if($Pr->contingencia) {$Email_aprobador=$Pr->email_aprobacion2;$Nombre_aprobador=$Pr->aprobado_por2;} // obtiene perfil de contingencia para la aprobación 
	else {$Email_aprobador=$Pr->email_aprobacion;$Nombre_aprobador=$Pr->aprobado_por;} // perfil estandar de aprobación
	...
	$Ruta_alterna=base64_encode("header('location:../Control/operativo/zbalance_estadoDev.php?Acc=aprobacion_requisicion&id=$id');");
	...
	$Envio1=enviar_gmail($Email_usuario /*de */,
				...
				<br><a href='http://app.aoacolombia.com/Control/operativo/zbalance_estadoDev.php?Acc=ver_balance&id=$D->ubicacion' target='_blank'>Click aqui para ver el Balance de Estado</a>
				<br>
	...
	if($Aviso) echo "<body><script language='javascript'>alert('Email enviado satisfactoriamente');</script></body>";
}
...
function asociar_facprov() // formulario que asocia una factura de proveedor al balance de estado
{
	...
	function buscar_asociaciones()
	{
		...
		<form action='zbalance_estadoDev.php' target='tablero_asociacion' method='POST' name='forma' id='forma'>
...
function asociar_facprov_buscar() // busca las facturas de proveedores que coincidan con unos criterios dados.
{
	...
	html();
	$U=qo("select * from ubicacion where id=$ubicacion"); // dato de la ubicación del estado
	...
	echo "<script language='javascript'>
		...
		{if(confirm('Desea asociar este gasto al balance de estado del vehiculo?'))
			{window.open('zbalance_estadoDev.php?Acc=asociar_facprov_ok&id_detalle='+id+'&ubicacion=$ubicacion','Oculto_asocia');}}
		</script><body>
...
function solicitar_nuevo_bien_servicio()
{
        ...
        echo "<script language='javascript'>
                ...
                <form action='' target='_self' method='POST' name='forma' id='forma'>
                        ...
                        Nombre: <input type='text' name='nombre' id='nombre' value='' size='80' maxlength='200' onkeyup='this.value=this.value.toUpperCase();'><br><br>
...
function solicitar_nuevo_bien_servicio_ok()
{
        ...
        'sergiocastillo@aoacolombia.com;ernestogonzalez@aoacolombia.com;sergiourbina@aoacolombia.com;
		rociocruz@aoacolombia.com','SOLICITUD DE CREACION DE BIEN O SERVICIO',
        nl2br("Estimados Señor@s Claudia Castro y/o Ernesto Gonzales,
...
function desasociar_facprov() // quita la asociación de una factura al balance de estado.
{
	global $id,$BDA;
...
function modificar_seguro(){
	include("views/subviews/editar_seguros.php");
}		
function asociar_cajamenor() // formulario para asociar una caja menor al balance de estado
{
	...
	function buscar_asociaciones2()
	{
		...
		<form action='zbalance_estadoDev.php' target='tablero_asociacion2' method='POST' name='forma' id='forma'>
			<B>CRITERIOS DE BUSQUEDA</B><BR>
			Busqueda por proveedor: <input type='text' name='proveedor' id='proveedor' value='' size='80' maxlength='100' onblur='this.value=this.value.toUpperCase();' ><br>
			Fecha inicial: ".pinta_FC('forma','FI')." Fecha final: ".pinta_FC('forma','FF')." <br>
...
function asociar_cajamenor_buscar() // buysca los detalles de la caja menor para asociarlos al balance de estado de acuerdo a un criterio
{
	...
	echo "<script language='javascript'>
		function asociar_cajamenor(id)
		{
			if(confirm('Desea asociar este gasto al balance de estado del vehiculo?'))
			{
				window.open('zbalance_estadoDev.php?Acc=asociar_cajamenor_ok&id_detalle='+id+'&ubicacion=$ubicacion','Oculto_asocia2');
			}
		}
		</script><body>
	...
	if($proveedor || $FI || $FF || $concepto) // si el usuario dio criterios de asociacion construye la consulta de lo contrario no la hace
	{
		...
		if($Facturas=q($Consulta)) // si encuentra registros de caja menor
		{
			...
			while($F =mysql_fetch_object($Facturas)) // muestra registro por registro de caja menor
			{
				...
				// si el registro de caja menor ya está asociado lo muestra 
				if($F->ubicacion) echo "<td align='center' bgcolor='aaffbb'><img src='gifs/standar/si.png' border='0'> Asociada</td>";
...
function adicionar_control_operaciones() // formulario para adicionar control de operaciones.
{
	...
	// de acuerdo al tipo de operacion el sistema muestra unos menus y opciones en un formulario para adicionar datos adicionales relacionados al estado o ubicación del vehiculo.
	...
	echo "<script language='javascript'>
			...
			{
				modal('zbalance_estadoDev.php?Acc=mas_talleres',0,0,500,500,'mast');
			}
		</script><body>
		<form action='zbalance_estadoDev.php' target='Oculto_ctrl' method='POST' name='forma' id='forma'>
		<table>
...
function mas_talleres() // permite crear talleres. 
{
	...
	echo "<br>
	<form action='zbalance_estadoDev.php' target='_self' method='POST' name='forma' id='forma'>
		<h4>Creación de nuevo Taller</h4>
...
function crear_nuevo_taller() // inserta nuevo taller en la tabla de talleres
{
	...
	header('location:zbalance_estadoDev.php?Acc=mas_talleres');
}
...
function aprobacion_requisicion() // aprobacion de la requisicion 
{
	...
	header("location:zbalance_estadoDev.php?Acc=mensaje_operativo_alerta&Mensaje=$Mensaje");die();} // no deja aprobar sino una vez la requisicion
	if($D->estado==3) {$Mensaje=urlencode(base64_encode("El estado de esta requisición ya fue procesado y es: Rechazado." ));
	header("location:zbalance_estadoDev.php?Acc=mensaje_operativo_alerta&Mensaje=$Mensaje");die();} // si el estado es rechazado sale el aviso
	if($D->estado==4) {$Mensaje=urlencode(base64_encode("El estado de esta requisición ya fue procesado y es: Calificado." ));
	header("location:zbalance_estadoDev.php?Acc=mensaje_operativo_alerta&Mensaje=$Mensaje");die();} // si el estado es Calificado sale el aviso
	...
	$Ruta_aprobacion=base64_encode("\$Programa='$Ruta_correo&Acc=aprobar_requisicion&observaciones='.\$observaciones.'&cotapr='.\$cotapr;\$Fecha_control=date('Y-m-d');"); 
	// calcula una ruta de no aprobación o rechazo
	$Ruta_daprobacion=base64_encode("\$Programa='$Ruta_correo&Acc=daprobar_requisicion&observaciones='.\$observaciones;\$Fecha_control=date('Y-m-d');"); 
	$Fecha_control=date('Y-m-d',strtotime(aumentadias(date('Y-m-d'),30)));
	...
	$Det.="</table>";
	echo " <body><b>Solicitud de aprobación Requisición Número $id</b><br>
				<table><tr><td>Fecha de Requisición:</td><td><b>$D->fecha</b></td></tr>
				 <tr><td>Solicitado por: </td><td><b>$D->solicitado_por</b></td></tr>
				<tr><td>Placa: </td><td><b>$D->placa</b></td></tr>
				<tr><td>Ciudad: </td><td><b>$Ciu</b></td></tr>
				...
				<form action='http://app.aoacolombia.com/i.php' target='_blank' method='GET' name='forma' id='forma'>
					<select name='i'><option value=\"$Ruta_aprobacion\">Aprobar</option><option value=\"$Ruta_daprobacion\">Rechazar</option></select><br>
						No. Cotización Aprobada: <select name='cotapr'><option value=''></option><option value='1'>1</option><option value='2'>2</option><option value='2'>2</option></select><br>
						<br>Observaciones: <input type='text' name='observaciones' id='observaciones' value='' size='50' maxlength='200'><br>
						<br><input type='submit' value=' PROCEDER ' >
						<input type='hidden' name='Fecha_control' value='$Fecha_control'>
				</form><br><br></body>
...
function evaluar_requisicion() // formulario para evaluar una requisicion de un proveedor
{
        ...
        <form action='zbalance_estadoDev.php' target='_self' method='POST' name='forma' id='forma'>
                <table><tr><th>Criterio</th><th>Opción</th><th>Resultado</th></tr>";
                include('inc/link.php');
                $Js='';
...
function evaluar_requisicion_ok() // guarda la evaluación hecha por el funcionario
{
        ...
        graba_bitacora('requisicion','M',$id,'Evalua requisición'); // graba la bitacora de requisicion
		q("insert into $BDA.app_bitacora (ano,mes,dia,hora,minuto,segundo,nick,nombre,tabla,accion,registro,ip,detalle)
			values ('".date('Y')."','".date('m')."','".date('d')."','".date('G')."','".date('i')."','".date('s')."','$NICK','$NUSUARIO','requisicion','M','$id','".$_SERVER['REMOTE_ADDR']."','Evalua requisición.')"); // graba la bitacora de requisicion
        echo "<body><script language='javascript'>alert('Evaluación guardada satisfactoriamente.');window.close();void(null);opener.recargar();</script></body>";
}
...
function adicionar_observaciones()
{
	...
	echo "<title>ADICION DE OBSERVACIONES</title>
		...
		<body><script language='javascript'>centrar(600,400);</script>
			<form action='zbalance_estadoDev.php' target='Oculto_obsreq' method='POST' name='forma' id='forma'>
				Observaciones:<br>
...
function adicionar_observaciones_ok()
{
	...
	q("insert into $BDA.app_bitacora (ano,mes,dia,hora,minuto,segundo,nick,nombre,tabla,accion,registro,ip,detalle)
			values ('".date('Y')."','".date('m')."','".date('d')."','".date('G')."','".date('i')."','".date('s')."','$NICK','$NUSUARIO','requisicion','M','$id','".$_SERVER['REMOTE_ADDR']."','Adiciona observaciones')"); // graba la bitacora de requisicion
