...
<?php header('Content-Type: text/html; charset=utf-8'); ?>
				...
				<table>
					...
					<tbody>
						...
						<?php foreach($itemsFactura as $item){  
                            if($saldo>0){
								$item->unitario = $item->unitario - round($saldo/1.19);
							}
						...
						<tr>
							...
							<td>
								...
								<input class="quantity" value="<?php echo $item->cantidad ?>" type="number" max="<?php echo $item->cantidad ?>"  />  
							</td>
							<td>
							...
							<td>
								<span class="totalItem"> <?php echo ($item->cantidad*$item->unitario)+( ($item->porc_iva / 100 ) * ($item->cantidad*$item->unitario) ) ?>  </span>
							</td>
	...
	<script>
		...
		{			
			...
			let totalItem = parseInt( quantity *  unitaryValue ) + parseInt( quantity *  unitaryValue * (porc_iva/100) );
