...
function consultar_garantias()
{
	...
	$Consulta_garantias="select a.*,s.numero as nsin,s.asegurado_nombre,s.asegurado_id,t_aseguradora(s.aseguradora) as naseguradora,t_ciudad(ciudad) as nciudad,
								s.img_inv_salida_f as acta1,img_inv_entrada_f as acta2,t_franquisia_tarjeta(a.franquicia) as nfranq,s.fecha_final as fdevolvh,t_veh_ub(s.ubicacion) as placa
								FROM sin_autor a,siniestro s,franquisia_tarjeta frq
								WHERE a.siniestro=s.id and a.franquicia=frq.id  and s.fecha_final between '$FECHAI' and '$FECHAF' and a.estado='A'
								and a.id not in (select autorizacion from recibo_caja where garantia=0)
								".($OFIU?" and s.ciudad=$OFIU ":"")." $Filtro_query 
								ORDER BY fecha_solicitud ";

	if($IDsiniestro) $Consulta_garantias="select a.*,s.numero as nsin,s.asegurado_nombre,s.asegurado_id,t_aseguradora(s.aseguradora) as naseguradora,t_ciudad(ciudad) as nciudad,
								s.img_inv_salida_f as acta1,img_inv_entrada_f as acta2,t_franquisia_tarjeta(a.franquicia) as nfranq,s.fecha_final as fdevolvh,t_veh_ub(s.ubicacion) as placa
								FROM sin_autor a,siniestro s,franquisia_tarjeta frq
								WHERE a.siniestro=s.id and a.franquicia=frq.id  and a.estado='A' and s.id=$IDsiniestro
								and a.id not in (select autorizacion from recibo_caja where garantia=0)
								ORDER BY fecha_solicitud ";
	...
	//		if($Rcaja=qom("select id from recibo_caja where autorizacion=$G->id and garantia=0",$LINK))
	//		{

	//		}
	//		else
	//		{
				if($Foto=qo1m("select foto_f as foto from aoacol_administra.ingreso_recepcion where siniestro=$G->siniestro",$LINK))
					$Foto="<a onclick=\"modal('../../Administrativo/".$Foto."',0,0,400,400,'foto');\"><img src='../../Administrativo/".$Foto."' border='0' height='16'></a>";
				else $Foto='';
				$Contador++;
				echo "<tr ".($USUARIO==1?" ondblclick=\"modal('marcoindex.php?Acc=mod_reg&Num_Tabla=$NT&id=$G->id',0,0,500,500,'mod')\" ":"").">
						...
						<td bgcolor='ffffff' align='center'>$G->placa </td>
				...
				echo "
						...
						<td bgcolor='ffffff'>$G->nombre $Foto</td>
						...
						<td nowrap='yes' bgcolor='ffffff'>$G->nfranq ";

					if($RCP=qom("select ofi.sigla,rcp.consecutivo  from oficina ofi, recibo_caja_prov rcp where rcp.oficina=ofi.id and rcp.siniestro=$G->siniestro ",$LINK))
						echo "<br />RCP: <b>$RCP->sigla $RCP->consecutivo </b>";
					if($RC=qom("select rc.id,ofi.sigla,rc.consecutivo from recibo_caja rc,oficina ofi where rc.oficina=ofi.id and rc.autorizacion=$G->id and garantia=1",$LINK))
					{
						echo "<br />Recibo de Caja: <b>$RC->sigla $RC->consecutivo ";
						if(!$Excel) echo "<a class='info' style='cursor:pointer' onclick='ver_recibo($RC->id);'><img src='gifs/standar/Preview.png' border='0'><span>Ver Recibo</span></a>";
						echo "</b>";
					}
				...
				if($Facturado || $Por_facturar)
				{
					...
					echo "<td align='right' bgcolor='$Color_auditoria'>".coma_format($Facturado+$Por_facturar)."<br />";
					IF(!$Excel)
					...
					echo "<td  bgcolor='$Color_auditoria'></td><td  bgcolor='$Color_auditoria'></td><td  bgcolor='$Color_auditoria'></td>";
...
//			}
	...
	echo "<iframe name='Oculto_garantia' id='Oculto_garantia' style='visibility:hidden' height='1' width='1'></iframe><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /></body>";
}
...
//	}
	...
	$Mensaje1.="<br><br>Para ver el documento por favor <a href='http://www.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta_link';\$Fecha_control=date('Y-m-d');")."' target='_blank'><u> click aqui para descargar. </u></a> <br>";
