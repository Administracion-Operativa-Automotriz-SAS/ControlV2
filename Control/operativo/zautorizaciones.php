...
function inicio_autorizacion()  // Pinta el formulario de solicitud de informaci�n. Solicita el n�mero del siniestro y la placa del veh�culo para validar si el funcionario est� en el siniestro correcto.
{
	...
	echo "<script language='javascript'>
		...
		function buscar()
		{
			...
			{ alert('Debe escribir algun valor en el n�mero de siniestro para buscarlo y la placa del veh�culo del Asegurado');
	...
	<form action='zautorizaciones.php' method='post' target='Oculto_autorizaciones' name='forma' id='forma'>
		N�mero de Siniestro: <input type='text' name='sini' value='$sini' onfocus='this.select()'>
		Placa del veh�culo del Asegurado: <input type='text' name='placaaseg' id='placaaseg' value='' size='10' maxlength='10' style='font-size:14px' onkeyup='javascript:this.value=this.value.toUpperCase();'><br><br>
	...
	if($DesdeCall)  // si el formulario es activado desde call center, le da la opcion al funcionario de volver al m�dulo de call center.
...
function buscar_siniestro() // Despu�s de pedir el siniestro y la placa busca si hay una o varias coincidencias. si hay varias solicita al funcionario que escoja una
{
	...
	if($Siniestros=q("select id from siniestro where numero like '%$sini%' and estado in (5,3,7,8) and placa='$placaaseg' "))
	{
		...
		echo "<body><script language='javascript'>parent.oculta_autoriza();alert('No hay siniestros que cumplan con la informaci�n suministrada');</script></body>";
}

function presenta_siniestro() // muestra toda la informaci�n del siniestro y pide los datos del asegurado
{
	global $id,$USUARIO;
	...
	$Ahora=date('Y-m-d');
	//////  VALIDACION SI LA CITA DEL SINIESTRO YA TIENE MARCADO EL ARRIBO o si est� en estado pendiente.
	...
	$Oficina=qo("Select * from oficina where ciudad='$D->ciudad'"); // obtiene la informaci�n de la oficina
	
	//echo "Select * from oficina where ciudad='$D->ciudad'";
	
	$Aseguradora=qo("select * from aseguradora where id=$D->aseguradora"); // obtiene la informaci�n de la aseguradora
	$Autorizacion=q("select a.*,f.tipo from sin_autor a, franquisia_tarjeta f where a.siniestro=$id and a.franquicia=f.id order by a.fecha_solicitud"); // obtiene la informaci�n de autorizaciones anteriores
	html();
	echo "<script language='javascript'>	var Validado=false;";
	if($Fran) // rutinas de validaci�n en la captura de las tarjetas cr�dito
	{
		...
		while($F=mysql_fetch_object($Fran))
		{
			...
			echo "if(Franquicia==$F->id)
			{
				...
				{
					alert('Error en el n�mero de la tarjeta, no corresponde de acuerdo a la franquicia');
	...
	echo "
		function activa_n()
		{
			with(document.forma)
			{
				...
				if(franquicia.value==10) // sin garant�a
				{
					n1.style.visibility='hidden';n2.style.visibility='hidden';n3.style.visibility='hidden';n4.style.visibility='hidden';
					banco.style.visibility='hidden';vencimiento_mes.style.visibility='hidden';vencimiento_ano.style.visibility='hidden';
					codigo_seguridad.style.visibility='hidden';voucher.style.visibility='hidden';valor.value='0';valor.focus();return true;
		...
		function valida_envio() // validaci�n de todos los campos del formulario
		{
			with(document.forma)
			{
				...
				{
					if(!alltrim(n1.value)) {alert('Debe escribir el numero completo de la tarjeta de cr�dito'); n1.style.backgroundColor='ffff55';n1.focus(); return;}
					if(!alltrim(n2.value)) {alert('Debe escribir el numero completo de la tarjeta de cr�dito'); n2.style.backgroundColor='ffff55';n2.focus(); return;}
					if(!alltrim(n3.value)) {alert('Debe escribir el numero completo de la tarjeta de cr�dito'); n3.style.backgroundColor='ffff55';n3.focus(); return;}
					if(!alltrim(n4.value)) {alert('Debe escribir el numero completo de la tarjeta de cr�dito'); n4.style.backgroundColor='ffff55';n4.focus(); return;}
					...
					if(!alltrim(voucher.value)) {alert('Debe digitar el n�mero del voucher'); voucher.style.backgroundColor='ffff55';voucher.focus(); return;}
				}
				else
				{if(!alltrim(observaciones.value)) {alert('Debe digitar en el campo de observaciones la raz�n por la que se solicita una garant�a en efectivo');observaciones.style.backgroundColor='ffffdd';observaciones.focus();return false;}}
				...
				if(!Number(valor.value) && Number(valor.value)!=0) {alert('Debe escribir correctamente el valor de la garant�a o consumo, sin comas ni puntos');valor.style.backgroundColor='ffff55';valor.focus();return;}
				if(confirm('Desea enviar la solicitud de Autorizaci�n?'))
		...
		{if(confirm('Est� seguro de querer REASIGNAR esta Autorizaci�n al nuevo servicio?')) window.open('zautorizaciones.php?Acc=reasignar&auanterior='+auanterior+'&idsiniestro='+idsiniestro,'Oculto_autorizacion');	}
	...
	if($Autorizacion) // presenta las autorizaciones anteriores o en espera si ya est�n autorizadas o no.
	{
		echo "<table width='100%' bgcolor='dddddd'><tr>
						...
						<th style='font-size:14'>Autorizaci�n</th></tr>";
		while($A=mysql_fetch_object($Autorizacion))
		{
			...
			if(inlist($USUARIO,'1,5') && $A->tipo=='C' && $A->estado=='E') // si el usuario es de autorizaciones y el tipo de garantia es Tarjeta Cr�dito, puede procesar la autorizaci�n
				echo "<a style='cursor:pointer' onclick=\"parent.centrar(1000,800);window.open('zautorizaciones.php?Acc=estado','_self');\" class='info'>
							<img src='img/estado_autorizaciones.png' border='0' height='30'><span>Procesar Autorizaci�n</span></a>";
			if(inlist($USUARIO,'1,5,10') && $A->tipo=='P' && $A->estado=='E') // si el usuario es de autorizaciones y el tipo de garant�a des Pagar� puede procesar la autorizaci�n
				echo "<a style='cursor:pointer' onclick=\"window.open('zautorizaciones.php?Acc=asignar_pagare&id=$A->id','_self');\" class='info'>
						<img src='img/pagare.png' border='0' height='30'><span>Asignar Pagar�</span></a>";
			echo "</td></tr>";
		}
		echo "</table>";
	}
	// presenta todo el formulario para llenar la solicitud de una nueva autorizaci�n.
	echo "<hr>
	...
	<form action='zautorizaciones.php' method='post' target='Oculto_autorizacion' name='forma' id='forma'>
		...
		($D->no_garantia?" <a class='info'><img src='img/nogarantia.png' border='0' height='30px' alt='Servicio Sin Garantia' title='Servicio Sin Garantia' align='middle'> <b style='color:ff3333'>Servicio Sin Garantia</b><span><img src='img/nogarantia.png'><h3>Servicio Sin Garant�a</h3></span></a> ":"")
		."</h3>
		<table cellspacing=1 cellpadding=0>
		<tr ><td align='right'>N�mero de identificaci�n:</td><td ><input type='text' name='identificacion' class='numero' size='15' maxlength='15' onblur='validar_identificacion(this.value);'></td>
		<td align='right'>Lugar de expedici�n:</td><td ><input type='text' name='lugar_expdoc' onblur='this.value=this.value.toUpperCase();' ></td></tr>
		...
		<tr ><td align='right'>Tipo de identificaci�n:</td><td >".menu1('tipo_id',"select codigo,nombre from tipo_identificacion",'',1)."</td>
		...
		<tr ><td align='right'>Direcci�n Domicilio:</td><td ><input type='text' name='direccion' id='direccion' size='30' maxlength='50'></td>
		<td align='right'>Tel�fono Oficina:</td><td ><input type='text' name='telefono_oficina' id='telefono_oficina' size='30' maxlength='50'></td></tr>
		<tr ><td align='right'>Tel�fono Vivienda:</td><td ><input type='text' name='telefono_casa' id='telefono_casa' size='30' maxlength='50'></td>
	...
	// pinta las opciones de franquicias autorizadas de acuerdo al perfil del usuario, la oficina en la que est� y  la aseguradora
	if($D->no_garantia)
	{
		//echo "no garantia";	
	...
	echo "<br />N�mero de la tarjeta: <input type='text' name='n1' size=4 maxlength=4 class='numero' onblur=\"vfran(this.value);\" disabled> -
		...
		Banco que expidi� la tarjeta: ".menu1("banco","select id,nombre from codigo_ach order by nombre",0,1)."<br />
	...
	echo "</select><br />
		C�digo de seguridad: <input type='text' id='codigo_seguridad' name='codigo_seguridad' class='numero' size='4' maxlength='4'>
		N�mero de Voucher: <input type='text' id='voucher' name='voucher' class='numero' size='7' maxlength='7'>
		...
		Valor de la garant�a o consumo: $<input type='text' name='valor' id='valor' value='$Aseguradora->garantia' size='10' maxlength='10' class='numero' style='font-weight:bold'><br />
		Observaciones: <textarea name='observaciones' cols=80 rows=2></textarea><br /><br>

		<h3>Datos Financieros para devoluci�n de garant�as en Efectivo o  Consignaci�n Voucher</h3>
		...
		<tr><td>Identificaci�n: </td><td><input type='text' name='identificacion_devol' id='identificacion_devol' value='' size='20' maxlength=15></td></tr>
		</table></td></tr></table>
		<br>
		<b>Clave de confirmaci�n</b>: <input type='password' name='Clave' id='Clave' onKeyPress='bloqueo_mayusculas(event)'> Digite la clave del usuario ".$_SESSION['Nombre']."
				<div id='bloqueomayusculas' style='visibility:hidden'><b style='color:red'>El bloqueo de may�sculas est� activado</b></div><br><br>
	...
	{
		// busca autorizaciones donde coincida la c�dula del asegurado o la placa para dar posibilidad de reasignar una garant�a anterior al caso actual
		...
		if($Anterior=q($Consulta))
		{
			...
			while($Ant=mysql_fetch_object($Anterior)) // registro por registro pinta un link para la reasignaci�n de la garant�a anterior al registro actual
			{
...
function registrar_solicitud() // graba todos los datos de una nueva solicitud en la tabla de autorizaciones
{
	...
	if(verificar_password($_SESSION['Nick'],$Clave)) // Verifica el password del usuario actual para evitar que un usuario no autorizado grabe la informaci�n y por trazabilidad de proceso
	{
		...
		q("update cliente set nombre='$nombre',apellido='$apellido',lugar_expdoc='$lugar_expdoc',tipo_id='$tipo_id',sexo='$sexo',pais='$pais',ciudad='$ciudad',
			...
			// actualiza toda la informaci�n del cliente
		html();
...
function envio_solicitud_efectivo($id=0,$Nid=0)  // funci�n antigua que envia un correo de solicitud de autorizaci�n a un directivo de AOA para permitir la recepci�n de garant�as en efectivo esta rutina ya esta  fuera de uso
{
	global $idauto;
	$Nusuario=$_SESSION['Nombre'];
	$Hoy=date('Y-m-d H:i:s');
            ...
            $.ajax(
                    {
                        ...
                        success: function (response)
                        {
						  ...
						  alert('Mensaje enviado satisfactoriamente a  $Oficina->email_autorizacion  23');
        </script>
            ...
            $.ajax(
                    {
                        ...
                        success: function (response)
                        {
						  ...
						  alert('Mensaje enviado satisfactoriamente a  $Oficina->email_autorizacion 21 ');
        </script>
		</body>";	
...
function pide_siniestro() // presenta de varios siniestros encontrados para que el funcionario seleccione el que necesita procesar.
{
	...
	Seleccione el siniestro que est� buscando<br />
...
function estado() // presenta la lista de solicitudes del d�a, y las que falta por procesar.
{
	...
	$Hoy=date('Y-m-d');
	// echo $Hoy;
	if($Oficina)  // construye la consulta filtrando por la oficina de atenci�n basado en la tabla de citas.  Estado de citas: P:PROGRAMADA C:CUMPLIDA S:CUMPLIDA Y NO TOMA SERVICIO
	{
		...
		$Query_autorizaciones="select a.*,s.numero as nsin,fq.nombre as nfranq, fq.telefono as tfq,t_codigo_ach(banco) as nbanco,s.id as idsin,datediff(cs.fecha,'$Hoy') as fdias
										...
										and if(a.formulario_web=1,(cs.estado in ('P','S') and datediff(cs.fecha,'$Hoy')<2),1) and datediff('$Hoy',cs.fecha) <1 AND fq.nombre!='TARJETA CREDITO HUB'
										";
	...
	if($Autorizaciones=q($Query_autorizaciones)) // pinta las autorizaciones pendientes
	{
		...
		while($A=mysql_fetch_object($Autorizaciones))
		{
			...
			echo "<tr >
					<td style='font-size:12' ".($A->formulario_web?" bgcolor='ffddff' ":" bgcolor='ffffff' ")."><b>$A->nsin <a style='cursor:pointer' onclick='versin($A->idsin);'><img src='imagenes/000/580/dicono_f_grua_lupa.png' height=30></a><br />Identificaci�n: ".coma_format($Sincars->asegurado_id)."</b>
			...
			if($Cita)
			{
				...
				echo "<br><table align='center'>
					...
					<tr><td align='right' $Alerta><b>Fecha y Hora de Marcaci�n:</b></td><td $Alerta><b>$Cita->fecha $Hora_llamada</b></td></tr>
					</table>";
			}
			echo "</td>
					<td  bgcolor='ffffff'>
							...
							<tr bgcolor='eeffee'><td>Identificaci�n:</td><td style='font-size:14'><b>".coma_format($A->identificacion)."</b></td></tr>
							...
							<tr><td>C�digo Seguridad:</td><td style='font-size:14'><b>$A->codigo_seguridad</b></td></tr>
							<tr bgcolor='eeffee'><td>N�mero Voucher:</td><td style='font-size:14'>";
			...
			echo "</td></tr>
					...
					<td bgcolor='ffffff'>
							<form action='zautorizaciones.php' method='post' target='_self' name='forma$A->id' id='forma$A->id'>
								<table>";
								// por cada solicitud de autorizaci�n muestra un formulario para el proceso de la autorizaci�n ante la garant�a
			if($Sincars->estado!=8 && $Sincars->estado!=7)
				echo "<tr><td colspan=2><a onclick=\"modal('http://www.runt.com.co:7777/runt/ciudadanos/consultas/consulta_ciudadano_por_documento_final_public.jsf',100,100,600,800,'consulta');\"
                     style='cursor:pointer;color:blue;' class='info'>Consulta de estado de licencia de conducci�n RUNT<span style='width:200px'>Click para consultar el estado de la licencia de conducci�n en RUNT</span></a><br />
                     <a onclick=\"modal('http://web.mintransporte.gov.co/Consultas/transito/Consulta23122010.htm',100,100,600,800,'consulta');\"
                     style='cursor:pointer;color:blue;' class='info'>Consulta de estado de licencia de conducci�n MinTransporte<span style='width:200px'>Click para consultar el estado de la licencia de conducci�n en MINTRANSPORTE</span></a><br />
                     Marque si la licencia de conducci�n ha sido verificada correctamente : <input type='checkbox' name='estado_licencia'
			...
			echo "
									{
										...
										document.getElementById('txt1_$A->id').innerHTML='N�mero de autorizaci�n:';
										document.getElementById('txt2_$A->id').innerHTML='';
										if(document.getElementById('txt3_$A->id'))
										{
											document.getElementById('txt3_$A->id').innerHTML='Factura para aplicar esta autorizaci�n:';
			...
			// para los rechazos muestra las causales para una no adjudicaci�n del servicio
			...
			echo "<br /><a onclick=\"modal('marcoindex.php?Acc=mod_reg&Num_Tabla=$T&id=$A->siniestro',0,0,500,800,'eds');\"
		          style='cursor:pointer'>Subir las im�genes al Siniestro <img src='gifs/standar/Pencil.png' border='0'></a>
			...
			echo "<br /><center>
						<input type='button' value='Grabar informaci�n' style='font-size:12;font-weight:bold;width:200px' onclick=\"
						...
						{
							alert('Debe seleccionar el estado de la autorizaci�n');
						...
						{
							alert('Debe registar el n�mero de autorizaci�n');
			...
			echo "
				...
				<a class='info' style='cursor:pointer' onclick='actualiza_info($A->id);'><img src='img/actualizar.png' border='0' height='40'><span style='width:200px'>Actualizar informaci�n del asegurado</span></a>";
	...
	if($Oficina)  // construye la consulta filtrando por la oficina de atenci�n basado en la tabla de citas.
	{
			...
			else
			{
				...
				echo "<tr><td valign='top'>$Contador</td>
						...
						<td  bgcolor='ffffff' valign='top'>
								...
								<tr bgcolor='eeffee'><td>Franquicia:</td><td style='font-size:14'><b>$A->nfranq</b></td></tr>
								<tr bgcolor='eeffee'><td>N�mero Voucher:</td><td style='font-size:14'><b>$A->numero_voucher</b></td></tr>
								<tr><td>Fecha y hora de solicitud:</td><td style='font-size:14'><b>$A->fecha_solicitud</b></td></tr>
								</table>
						</td>
						<td bgcolor='ffffff'>
				...
				else
				{
					...
					if(inlist($USUARIO,'1,5'))
						echo "<br><a style='cursor:pointer;' onclick=\"modal('zautorizaciones.php?Acc=datos_autorizacion&id=$A->siniestro',0,0,500,500,'solicitud');\"><u>Solicitar Visualizaci�n</u></a>";
				}
				switch($Sincars->estado)
				{
				...
				echo "</td></tr><tr><td>Observaciones:</td><td>$A->observaciones</td></tr><tr><td>Numero Autorizaci�n:</td><td>$Aut</td></tr>";
				if($A->estado=='A')
					echo "<tr><td>Estado del siniestro:</td><td><b style='$sty'>&nbsp;$Sincars->nestado&nbsp;</b>
							<a class='info' style='cursor:pointer' onclick='actualiza_info($A->id);'><img src='img/actualizar.png' border='0' height='20'><span style='width:200px'>Actualizar informaci�n del asegurado</span></a>
							</td></tr>";
...
function cambio_estado()
{
	...
	if($estado=='A' && $Factura)  // generaci�n autom�tica de recibo de caja desde aqui
	{
...
function ver_autorizaciones()
{
	...
	html('HISTORICO DE AUTORIZACIONES Siniestro n�mero: '.$Sin->numero);
	echo "<html><body>
	<script language='javascript'>centrar(1000,700);</script>
	<h3>Siniestro n�mero: $Sin->numero - $Aseguradora->nombre </h3>";
...
function datos_autorizacion()
{
	...
	html('SOLICITUD DE INFORMACI�N DE UNA AUTORIZACION');
	echo "<script language='javascript'>
			function carga()
			{
				centrar(700,500);
	...
	if(!$Por_visualizar) echo "
					if(tipo.value=='2' && !alltrim(motivo.value))  {alert('Debe digitar el motivo cuando desea ver el detalle de la autorizaci�n');motivo.style.backgroundColor='ffdddd';motivo.focus();return false;}
					";
	echo "
					submit();
				}
		...
		<body onload='carga()'><h3>Solicitud de Informaci�n de Autorizaciones</h3>";
	...
	echo "<form action='zautorizaciones.php' method='post' target='_self' name='forma' id='forma'>
		<h3>Solicitud de Visualizaci�n de Garant�a.</h3>
		Estimado(a) Usuario(a) ".$_SESSION['Nombre'].", Usted va a solicitar una autorizaci�n electr�nica para poder visualizar una vez
		la informaci�n correspondiente a la garant�a de este siniestro. Por favor digite su contrase�a, llene la cadena de seguridad y de click en Enviar.<br><br>
		...
		<br><br>
		Tipo de Solicitud: <select name='tipo'><option value='1'>Informaci�n B�sica</option>
		<option value='2'>Informaci�n Completa</option>
		</select><br><br>";
	if($Por_visualizar) echo "<b style='color:00aa00;font-size:18px;'>Visualizaci�n Completa Autorizada<b><br><br>";
	else echo "Motivo de la revisi�n: <input type='text' name='motivo' id='motivo' value='' size='70'><br><br>";
...
function datos_autorizacion_valida_clave()
{
	...
	if(verificar_password($_SESSION['Nick'],$Clave))
	{
		if($cadena!=$_SESSION['CAPTCHAString'])
		{
			...
			graba_bitacora('siniestro','O',$id,'Solicitud de Informaci�n de Autorizaciones fallida por error en captcha.');
			echo "<script language='javascript'>
				function carga()
				{alert('C�digo de seguridad Incorrecto.');window.history.back();}
			</script>
		...
		else
		{
			...
			elseif($Visualizacion_completa)
			{
				...
				graba_bitacora('siniestro','O',$id,'Visualizaci�n Completa de Garantias autom�tica.');
				echo "<form action='zautorizaciones.php' method='post' target='_self' name='forma' id='forma'>
						<input type='hidden' name='Acc' value='datos_autorizacion_ok'>
						<input type='hidden' name='tip' value='$tipo'>
						<input type='hidden' name='id' value='$id'>
			...
			else
			{
				...
				{
					echo "<body><script language='javascript'>alert('Existe una solicitud de visualizaci�n pendiente de este Siniestro, pero no ha sido autorizada por el funcionario indicado');</script></body>";
				}
				else
				{
				...
				$data_mail = array(
					...
					"enviarEmail" => "true",
				     "contenido" => $contenido,
					"para" => "$Oficina->email_autorizacion",
					...
					"departamento" => $Ciudad->departamento,
					"sinumero" => $Sin->numero,
					"Siniestronaseg" => $Siniestro->naseg,
					"Siniestronumero" => $Siniestro->numero,
					"fec_autorizacion" => $Siniestro->fec_autorizacion,
	...
	{
		html('ERROR DE VALIDACION');
		graba_bitacora('siniestro','O',$id,'Solicitud de Informaci�n de Autorizaciones fallida por error en contrase�a.');
		echo "<script language='javascript'>
				function carga()
				{
...
function datos_autorizacion_ok()
{
	...
	graba_bitacora('siniestro','O',$id,'Soliciutd de Informaci�n de Autorizaciones satisfactoria.');
		...
		from sin_autor where siniestro=$id "))
	{
		while($A=mysql_fetch_object($Autorizaciones))
		{
			if($tip==1)
			{
				...
				N�mero de cuenta: <b>$A->devol_cuenta_banco</b><br>
				A nombre de: <b>$A->devol_ncuenta</b><br>
				Identificaci�n: <b>$A->identificacion_devol</b></br>
			...
			else
			{
				...
				Estado : ".($A->estado=='A'?"<b style='background-color:ddffdd;font-size:16px'>ACEPTADO</b>":"<b style='background-color:ffdddd;font-size:16px'>RECHAZADO</b>")."<br>
				Nombre Tarjeta habiente: <b>$A->nombre </b><br>
				Identificaci�n: <b>$A->identificacion</b><br>
				...
				Fecha Proceso: <b>$A->fecha_proceso</b><br>
				C�digo Seguridad: <b>$A->codigo_seguridad</b><br><br>";
			}
		}
	}
	else
	...
	<b>Estimado(a) ".$_SESSION['Nombre']." :<br><u>INCOCREDITO</u> es la entidad que vigila el correcto uso de la informaci�n de tarjetas de cr�dito y tarjeta habientes en los establecimientos comerciales.
	AOA no es la excepci�n y por lo tanto debemos cumplir los requisitos y sugerencias de seguridad que INCOCREDITO propone. <br>Una estrategia de seguridad es NO guardar, grabar o mantener en ning�n
	medio fisico o electr�nico cualquier informaci�n total o parcial de las tarjetas de cr�dito de los asegurados. Por lo tanto no copie ni guarde esta informaci�n que se est� presentando en pantalla. Solamente
	utilicela solamente para el <b>Motivo</b> por el cual fue autorizada su visualizaci�n. En el caso de ocurrir un fraude, ser� investigado cada funcionario que tiene acceso a la informaci�n o que
	haya sido autorizado para verla. Tan pronto sea utilizada debidamente esta informaci�n, se debe cerrar la ventana para que nadie m�s pueda hacer un mal uso de la misma.<br>
	</b>
	A continuaci�n se muestra un fragmento de la p�gina www.incocredito.com.co en la  que se expresa claramente las pol�ticas de seguridad que se deben mantener.<br>
...
function actualizar_info()
{
	...
	echo "<script language='javascript'>
			...
			function pasar_info()
			{
				...
				{
					alert('La informaci�n fue actualizada satisfactoriamente');
					window.close();void(null);
				}
			}
		</script>
	...
	if($Autorizaciones=q("select *, t_franquisia_tarjeta(franquicia) as nfr from sin_autor where siniestro=$Sin->id  and estado in ('A','E') "))
	{
		...
		while($A=mysql_fetch_object($Autorizaciones))
		{
			...
			{
				echo "<tr bgcolor='ddffdd'><td colspan=8>Esta garant�a ya fue enviada por email a $A->email</td></tr>";
			}
			else
			{
				echo "
					...
					Banco: <input type='text' name='nbanco$A->id' id='nbanco$A->id' ".($Soloemail=='SI'?" value='Informaci�n Protegida' disabled ":" value='$A->devol_banco' size=20 ondblclick='copia_campo(2,$A->id);' ").">
					Cuenta: <input type='text' name='cuenta$A->id' id='cuenta$A->id' ".($Soloemail=='SI'?" value='Informaci�n Protegida' disabled ":" value='$A->devol_cuenta_banco' size=20 ondblclick='copia_campo(3,$A->id);' ").">
					...
					A nombre de: <input type='text' name='beneficiario$A->id' id='beneficiario$A->id' ".($Soloemail=='SI'?" value='Informaci�n Protegida' disabled ":" value='$A->devol_ncuenta' size=30  ondblclick='copia_campo(4,$A->id);' ").">
		...
		<center><input type='button' value=' Grabar Informaci�n ' style='font-size:18px;font-weight:bold' onclick='validar_formulario();'></center>
...
function carga_imagenes()
{
		...
		from sin_autor where siniestro=$id and estado='A' "))
	{
		...
		while($A=mysql_fetch_object($Autorizaciones))
		{
			...
			$Resultado.="<table border cellspacing='0'><tr><td valign='top'>
				...
				Identificaci�n: <b>***********".r($A->identificacion,5)."</b><br>
				Numero Voucher: ";
			...
			$Resultado.="<br>
				...
				N�mero de cuenta: <b>$A->devol_cuenta_banco</b><br>
				A nombre de: <b>$A->devol_ncuenta</b><br>
				Identificaci�n: <b>*******".r($A->identificacion_devol,5)."</b></br>
				Tipo de cuenta: <b>".($A->devol_tipo_cuenta=='A'?'Ahorros':'Corriente')."</b><br>
				</td></tr></table>
				<br><br>";
		}
	...
	echo "<script language='javascript'>
		...
		<h3>Carga de Im�genes del siniestro n�mero $Sin->numero $Sin->asegurado_nombre</h3>
		$Resultado
		...
		<td valign=top align='right'>
				...
				<form name='forma2' id='forma2'>
					<input type=button id='bt1' value='Configuraci�n' onClick='webcam.configure()'>
				<!--	&nbsp;&nbsp;
...
function asignar_pagare()
{
	...
	echo "<script language='javascript'>
		...
		<b>Oficina: $Oficina->nombre Cliente: $Cliente->nombre $Cliente->apellido Identificaci�n: ".coma_format($Cliente->identificacion)."<br />
	...
	if($Pre_existe)
	{
		...
		echo "<iframe id='simg_imagen_f' name='simg_imagen_f' src='$Tumb' height='$Alto' width='$Ancho' frameborder='0' ></iframe></td><td valign='top' bgcolor='efefef'>
					...
					<td >
					<h3 align='center'>Toma de Imagen con c�mara Web</h3>
					<!-- inicio de las rutinas de la toma de imagen -->
						<script type='text/javascript' src='inc/js/webcam.js'></script>
						<script language='JavaScript'>
	...
	{
		echo "<br /><br />Por favor digite el consecutivo del pagar�: <input type='text' name='consecutivo' name='consecutivo' id='consecutivo' class='numero' size='10' maxlength='10'>
		<input type='button' value='Asignar Pagar�' onclick=\"asigna_pagare1('$Oficina->sigla');\">";
	}
}
...
function asignar_numero_pagare()
{
	...
	if($Previo=qo("select * from pagare where consecutivo='$Consecutivo'"))
	{
		...
		{
			echo "<body><script language='javascript'>alert('El consecutivo $Consecutivo fue asignado a otra autorizaci�n y fue anulado');</script></body>";die();
		}
		if($Previo->siniestro)
		{
			echo "<body><script language='javascript'>alert('El consecutivo $Consecutivo ya est� asignado a otra autorizaci�n');</script></body>";die();
		}
...
function imprimir_pagare()
{
	...
	"manifiesto, lo siguiente: PRIMERO: Que debo y pagar�, incondicional y solidariamente a la orden de ADMINISTRACION OPERATIVA AUTOMOTRIZ S.A. Nit. 900.174.552-5 o a la persona natural o ".
	"jur�dica a quien el mencionado acreedor ceda o endose sus derechos sobre este pagar�, la suma cierta de _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _".
	"PESOS MCTE. ($ _________________,oo), pesos moneda legal colombiana. SEGUNDO: Que el pago total de la mencionada obligaci�n se efectuar� en un s�lo contado, el d�a _____ del mes de  _______________ ".
	"del a�o _______ en las dependencias de ADMINISTRACION OPERATIVA AUTOMOTRIZ S.A. ubicada en la ciudad de $Oficina->nombre. TERCERO: Que en caso de mora pagar� a ADMINISTRACION OPERATIVA AUTOMOTRIZ S.A. o a la persona natural ".
	"o jur�dica a quien el mencionado acreedor ceda o endose sus derechos, intereses de mora a la m�s alta tasa permitida por la Ley, desde el d�a siguiente a la fecha de exigibilidad del presente pagar�, y hasta cuando ".
	"su pago total se efect�e. CUARTO: Expresamente declaro excusado el protesto del presente pagar� y los requerimientos judiciales o extrajudiciales para la constituci�n en mora. QUINTO: En caso de que haya lugar ".
	"al recaudo judicial o extrajudicial de la obligaci�n contenida en el presente t�tulo valor ser� a mi cargo las costas judiciales y/o los honorarios que se causen por tal raz�n. SEXTO: El tenedor del presente pagar� ".
	"podr� declarar vencidos la totalidad de los plazos de esta obligaci�n o de las cuotas que constituyan el saldo de lo debido y exigir su pago inmediato ya sea judicial o extrajudicialmente en los siguientes casos: ",0,'J',0);
	$ny=$P->y+5;
	$P->setxy(10,$ny);
	$P->multicell(190,5,"1. Cuando EL DEUDOR incumpla una cualquiera de las obligaciones derivadas del presente contrato, as� sea de manera parcial.");
	$ny=$P->y+5;
	$P->setxy(10,$ny);
	$P->cell(190,5,"2. Por muerte del DEUDOR.");
	$ny+=10;
	$P->setxy(10,$ny);
	$P->multicell(190,5,"3. Cuando el DEUDOR se declare en procedo de liquidaci�n obligatoria o convoque a concurso de acreedores.");
	$ny=$P->y+10;
	$P->setxy(10,$ny);
	$P->multicell(190,5,"En constancia de lo anterior, se suscribe en la ciudad de $Oficina->nombre, a los ".date('d')." d�as del mes de ".mes(date('m'))." del a�o ".date('Y').".");
	$ny=$P->y+10;
	$P->setxy(10,$ny);
	...
	$ny+=8;
	$P->setxy(10,$ny);
	$P->cell(190,5,"Domiciliado(a) en: Direcci�n: _________________________________   Ciudad: _______________");
	...
	"los negocios comerciales y contractuales bien sean verbales o escritos; sin previo aviso, proceda a llenar los espacios en blanco del pagar� No.$Pagare->consecutivo, que he suscrito en la fecha a su favor y que ".
	"se anexa, con el fin de convertir el pagar�, en un documento que presta m�rito ejecutivo y que est� sujeto a los par�metros legales del Art�culo 622 del C�digo de Comercio. ",0,'J',0);
	$ny=$P->y+5;
	$P->setxy(10,$ny);
	$P->multicell(190,5,"1. El espacio correspondiente a �la suma cierta de� se llenar� por una suma igual a la que resulte pendiente de pago de todas la obligaciones contra�das con el acreedor, por concepto de capital, ".
	"intereses, seguros, cobranza extrajudicial, seg�n la contabilidad del acreedor a la fecha en que sea llenado el pagare.",0,'J',0);
	$ny=$P->y+5;
	$P->setxy(10,$ny);
	$P->multicell(190,5,"2. El espacio correspondiente a la fecha en que se debe hacer el pago, se llenar� con la fecha correspondiente al d�a en que sea llenado el pagar�, fecha que se entiende que es la de su vencimiento.",0,'J',0);
	$ny=$P->y+5;
	$P->setxy(10,$ny);
	$P->multicell(190,5,"En constancia de lo anterior firmamos la presente autorizaci�n en la ciudad de $Oficina->nombre, a los ".date('d')." d�as del mes de ".mes(date('m'))." del a�o ".date('Y').".",0,'J',0);
	$ny=$P->y+10;
	$P->setxy(10,$ny);
	$P->cell(190,5,"EL DEUDOR,");
	$ny+=20;
...
function imprimir_pagare_old()
{
	...
	$P->SetFont("c128a","",10);$P->cell(70,12, uccean128('FA'.str_pad($Pagare->consecutivo,14,'0',STR_PAD_LEFT).str_pad($Pagare->codigo,10,'0',STR_PAD_LEFT)), 1, 0, 'C' );
	$ny+=16;
	$P->setxy(5,$ny);$P->setfont('Arial','B',10);$P->cell(80,5,"Lugar y fecha de suscripci�n: ".($Siniestro?"$Oficina->nombre - $Hoy ":"$Oficina->nombre  dia ___ del mes ___________________ del a�o: _______"));
	$ny+=5;
	...
	$P->setxy(5,$ny);$P->cell(80,5,'Ciudad y direcci�n donde se efectuar� el pago: ');$P->setfont('Arial','',10);$P->cell(80,5,"$Oficina->nombre $Oficina->direccion");
	$ny+=5;
	$P->setxy(5,$ny);$P->cell(80,5,"Barrio: $Oficina->barrio - Tel�fono: $Oficina->telefono ");
	$ny+=5;$P->setfont('Arial','B',10);
	...
	$P->setxy(5,$ny); $P->multicell(200,5,"DECLARO: PRIMERA.- OBJETO: Que por virtud del presente t�tulo valor pagar� incondicionalmente a la orden de ADMINISTRACION OPERATIVA AUTOMOTRIZ S.A. ".
		"con NIT 900.174.552-5 o a quien represente sus derechos, en la ciudad y direcci�n indicados la suma de: _________________________________________________________________________ ".
		" ($ _________________), mas los intereses se�alados en la Cl�usula segunda de este documento seg�n lo establecido en los t�rminos y condiciones del servicio. ".
		"SEGUNDA.- INTERESES: Que sobre la suma debida se reconocer�n Intereses Moratorios a la tasa m�xima legal vigente. TERCERA.- PLAZO: Que pagar� el capital indicado en la cl�usula primera de este ".
	    "pagar� el d�a _____________________________________________________________________.",0,'J',0);
	$ny=$P->y;
	$P->setxy(5,$ny); $P->multicell(200,5,"En constancia de lo anterior, se suscribe este documento en la ciudad de $Oficina->nombre el d�a ".($Siniestro?"$Hoy.":" _____ del mes ______________________ del a�o ______."),0,'J',0);
...
function autorizar_pagare()
{
	...
	{
		echo "<body><script language='javascript'>alert('No se ha cargado la imagen del pagar� firmado. No se puede autorizar aun este servicio');</script>";
	}
}
...
function generar_pagare_blanco_ok()
{
	...
	echo "La informaci�n es invalida.";
}
...
function reasignar()
{
	...
	q("update siniestro set observaciones=concat(observaciones,\"\nReasignacion de la autorizaci�n al siniestro $SinNue\") where id=$SinAnt");
	graba_bitacora('siniestro','M',$SinAnt,'Observaciones: Reasigna autorizaci�n');
	q("update sin_autor set siniestro='$idsiniestro',observaciones=concat(observaciones,\"\nReasignaci�n del siniestro no. $NSinAnt al no. $SinNue Fecha original de solicitud:\",fecha_solicitud,\" Fecha original de proceso: \",fecha_proceso ),
	fecha_solicitud='$Ahora',fecha_proceso='$Ahora' where id=$auanterior ");
	graba_bitacora('sin_autor','M',$auanterior,"Reasigna autorizaci�n del $NSinAnt al $SinNue");
	echo "<body><script language='javascript'>alert('Asignaci�n hecha satisfactoriamente.');</script>";
}
