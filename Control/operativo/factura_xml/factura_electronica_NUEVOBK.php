...
class factura_electronica
{
	...
	public function generate_xml_NC_with_taxes($nota_credito_id,$consecutive)
	{
        ...
        $sql = "select a.* , c.nombre AS concepto , c.porc_iva AS ivaperc FROM facturad a, nota_creditod b, concepto_fac c WHERE a.id=b.facturad AND a.concepto=c.id AND b.nota_credito=".$nota_credito->consecutivo;
		$query = $this->connect->query($sql);
		...
		{
			$c++;
			$query = $this->connect->query(" Select * from nota_creditod where nota_credito = ".$nota_credito->consecutivo);
