...
require_once($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/controllers/PTESA_ws_test2.php");

//json factura
include($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/factura_xml/aoa_json.php");

include($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/factura_xml/jSon_with_takes_NUEVO2.php");
...
class factura_electronica
{
	...
	public function generar_factura_electronica($amount_to_plus=1)
	{
		...
		header('Content-Type: text/plain');
			...
			$response = $this->ptesa_service->registro_documento_obligatorio_test_pruebas(base64_encode($xml_g),1,$consecutive);
	...
	/* Remplazo por la nueva funcionalidad*///$this->fact_obj->set_factItems($items);
		...
		$this->fact_obj->set_Extenciones($items,$arrayExtencions);
