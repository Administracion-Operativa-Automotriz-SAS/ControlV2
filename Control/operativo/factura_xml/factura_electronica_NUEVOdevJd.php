...
require_once($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/controllers/PTESA_ws_testDevJD.php");

//json factura
include($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/factura_xml/aoa_json.php");

include($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/factura_xml/jSon_with_takes_NUEVOdevJD.php");
...
class factura_electronica
{
	...
	public function generate_xml_with_taxes($idfac,$consecutive)
	{
		...
		$sql = "SELECT ase.txt_mandato,ase.mandato,ase.nombre,ase.nit,ase.dv_cliente_juridico,ase.tipo_mandato FROM factura fa
	...
	/* Remplazo por la nueva funcionalidad*///$this->fact_obj->set_factItems($items);
		...
		$this->fact_obj->set_InformacionClienteAdq($array_setInfartionCliente);
		 ...
		 $cliente->email_e ='davidduque@aoacolombia.com';
	...
	public function generate_xml_NC_with_taxes($nota_credito_id,$consecutive)
	{
		...
		//Aseguradora
		$sql = "SELECT ase.mandato,ase.nombre,ase.nit,ase.dv_cliente_juridico,ase.tipo_mandato FROM factura fa
               INNER JOIN aseguradora ase ON fa.aseguradora = ase.id 
               WHERE fa.id = ".$factura->id;
		$query = $this->connect->query($sql);
