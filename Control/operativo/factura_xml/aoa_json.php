...
class json_fact_electronica
{
	...
	public function consecutive($consecutive="FE28")
	{
		$this->json_object->{$this->fe_Invoice_key}->{$this->cbc_ID} = $consecutive;
	}
	
	public function set_InformacionDeDocumento($FechaVenc="2018/07/06",$VlrEnLetras="SON: TREINTA Y CINCO MIL PESOS MCTE CON CERO CENTAVOS",$OrdCompra="PA-SRP0000000005",$Resolucion_factura="Autorización Numeración de Facturación No.12354417 Factura por Computador de junio 21 de 2017 Prefijo FV. Rango 22351 al 30122 Vigencia por 12 Meses")
	{		
	...
	public function set_TaxTotal($TaxAmount="40000",$TaxableAmount="50000")
	{
		...
		$this->json_object->{$this->fe_Invoice_key}->{$this->fe_TaxTotal}->{$this->fe_TaxSubtotal}->{$this->cbc_TaxableAmount}->{$this->innerText} = number_format((float)$TaxableAmount, 2, '.', '');
		$this->json_object->{$this->fe_Invoice_key}->{$this->fe_TaxTotal}->{$this->fe_TaxSubtotal}->{$this->cbc_TaxAmount}->{$this->innerText} = number_format((float)$TaxAmount, 2, '.', ''); 
	}
	
	public function set_iva_percent($percent = "19")
	{
		$this->json_object->{$this->fe_Invoice_key}->{$this->fe_TaxTotal}->{$this->fe_TaxSubtotal}->{$this->cbc_Percent} = number_format((float)$percent, 2, '.', '');
	...
	public function set_LegalMonetaryTotal($LineExtensionAmount="10000",$TaxExclusiveAmount="20000",$AllowanceTotalAmount="10000")
	{
		...
		$this->json_object->{$this->fe_Invoice_key}->{$this->fe_LegalMonetaryTotal}->{$this->cbc_PayableAmount}->{$this->innerText} = number_format((float)(($LineExtensionAmount + $TaxExclusiveAmount)-$AllowanceTotalAmount), 2, '.', '');
		
	}
	
	public function set_factItems($array_of_items=array(array("cantidad"=>2,"valor"=>"3500","descripcion"=>"item no.1"),array("cantidad"=>3,"valor"=>"2000","descripcion"=>"item no.2")))
	{	
		...
		foreach($array_of_items as $item)
		{
			...
			//echo $item["cantidad"]*$item["valor"];
			
			//echo $new_item->{$this->cbc_LineExtensionAmount}->{$this->innerText};
			
			//echo number_format((float)($item["cantidad"]*$item["valor"]), 2, '.', '');
