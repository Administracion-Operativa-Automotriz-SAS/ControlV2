...
function informe_callcenter()
{
	...
	if($QPendientes=q("select s.aseguradora,o.id as idoficina,s.id,s.numero,s.placa,s.asegurado_nombre,s.ingreso,s.observaciones,s.chevyseguro,s.retencion,s.no_garantia  
							...
							WHERE s.ciudad=o.ciudad  and o.sucursal=0 ".($Filtro1?"":"and s.estado=5")." $Filtro1 order by s.aseguradora,o.id,s.ingreso"))
	...
	if($Aseguradoras=q("select distinct a.nombre,a.id from siniestro s,aseguradora a where s.aseguradora=a.id  ".($Filtro1?"":"and s.estado=5")." and a.id in ($Asegs) and s.ciudad in ($Ofics)  $Filtro1 order by a.nombre"))
	{
		...
		while($As=mysql_fetch_object($Aseguradoras))   ///  ASEGURADORAS //
		{
			...
			$Oficinas=mysql_query("select distinct o.id,o.nombre,o.ciudad from siniestro s, oficina o where s.ciudad=o.ciudad and s.aseguradora=$As->id  and o.sucursal=0 ".($Filtro1?"":"and s.estado=5")."
...
function inicio_proceso_call()
{
	...
	$Control_agente=($Siniestro->estado==5?'hidden':'visible');
	...
	echo "<script language='javascript'>
			...
			function activar_inicio_proceso()
			{
				document.getElementById('btn_guardar_obs').style.visibility='visible';
				document.getElementById('btn_ver_siniestro').style.visibility='visible';
				document.getElementById('sp_tel_declarante').style.visibility='visible';
				document.getElementById('sp_tel_conductor').style.visibility='visible';
				if(document.getElementById('sp_tel_actualizacion')) document.getElementById('sp_tel_actualizacion').style.visibility='visible';
			}
			</script>
			...
			<a class='info'  id='btn_ver_siniestro' onclick='ver_siniestro($id_siniestro);' style='cursor:pointer;visibility:$Control_agente;'><img src='img/versiniestro.png' border='0' height='30'><span>Ver el Siniestro</span></a>
	...
	echo "<div id='Plantilla_Observaciones' name='Plantilla_Observaciones' style='visibility:hidden;position:fixed;top:10;left:10;border-style:solid;border-width:2px;z-index:119;opacity: 0.90' border='1' frameborder='yes'>
				...
				<tr><td colspan='4' align='center'><b style='color:000099'>NUMERO DE DIAS DE SERVICIO: $Siniestro->dias_servicio</b></td></tr>
				</table>
				</td>
				<td rowspan=2 align='center' valign='top'>
					<form action='zcallcenter.php' method='post' target='Oculto_call' name='forma' id='forma'>
						...
						<textarea name='observaciones' id='observaciones' cols=70 rows=10 style='font-family:arial;font-size:14px;'></textarea><br />
						<input type='button' id='btn_guardar_obs' style='visibility:$Control_agente' value='Guardar Observaciones' onclick='guardar_observaciones();'>
				...
				<table bgcolor='eeeeff' width='100%'>
					...
					<tr><td>$Siniestro->declarante_nombre</td><td>Id:</td><td>$Siniestro->declarante_id</span></td></tr>
					<tr><td colspan='4'><span id='sp_tel_declarante' style='visibility:$Control_agente'><b>$Siniestro->declarante_telefono / $Siniestro->declarante_tel_resid / $Siniestro->declarante_tel_ofic / $Siniestro->declarante_celular / $Siniestro->declarate_tel_otro / $Siniestro->declarante_email</b></span></td></tr>
					<tr><th colspan=4>CONDUCTOR</th></tr>
					<tr><td>$Siniestro->conductor_nombre</td></tr>
					<tr><td colspan='4'><span id='sp_tel_conductor' style='visibility:$Control_agente'><b>$Siniestro->declarante_telefono / $Siniestro->conductor_tel_resid / $Siniestro->conductor_tel_ofic / $Siniestro->conductor_celular / $Siniestro->conductor_tel_otro</b></span></td></tr>";
	if($Siniestro->actualizacion_aseg)
	{
		echo "<tr><th colspan=4>ACTUALIZACION ASEGURADORA/TERCERA PERSONA</th></tr>
					<tr><td colspan=4><span id='sp_tel_actualizacion' style='visibility:$Control_agente'>$Siniestro->actualizacion_aseg</span><td></tr>";
	}
	echo "
				</table>
				</td></tr></table>";
...
function proceso_contacto()
{
	...
	echo "<script language='javascript'>
		...
		<body ><script language='javascript'>parent.activar_inicio_proceso();</script>
		<h3>Agente: $NUSUARIO - Inicio de Proceso</h3>
...
class Pdispon
{
	...
	{
		$Hoy=date('Y-m-d');
		echo "<tr><td align='right' nowrap='yes'><b style='font-size:14;color:0000ff'>$this->Placa</b><br />
		<b style='color:000077'>$this->Noficina</b> ".coma_format($this->Kilometraje)." </td>";
...
//										and (v.inactivo_desde='00000-00-00' or v.inactivo_desde>'$Ahora') and v.flota_distinta=1 ORDER BY kmf",$LINK))
		...
		if(!$Disponibles=mysql_query("select distinct v.id,v.placa,o.nombre as noficina,kilometraje(v.id) as kmf,v.ultima_ubicacion as oficina
										...
										v.ultima_ubicacion=o.id and o.ciudad='$S->ciudad' and (v.inactivo_desde='00000-00-00' or v.inactivo_desde>'$Ahora') and v.flota_distinta=1 ".
										($Filtro_placa?" and v.placa like '%$Filtro_placa%' ":"")." ORDER BY kmf",$LINK))
			die(mysql_error());	
		$id_disponibles='0';
		echo "Aseguradora: $S->aseguradora Ahora $Ahora Ciudad $S->ciudad ".date('Y-m-d');
		if(mysql_num_rows($Disponibles))
		{ $Contador=0; 
			while($D=mysql_fetch_object($Disponibles)) 
			{ echo " $D->placa"; $Disp_flota[$Contador]=new Pdispon($D,$S->aseguradora); $Contador++;$id_disponibles.=','.$D->id; } 
		}
		
		if($Disponibles=mysql_query("select distinct v.id,v.placa,o.nombre as noficina,kilometraje(v.id) as kmf,v.ultima_ubicacion as oficina
										FROM vehiculo v,oficina o WHERE v.ultima_ubicacion=o.id and o.ciudad='$S->ciudad' and (v.inactivo_desde='00000-00-00' or v.inactivo_desde>'$Ahora') 
		...
		else echo "No hay de AOA para esta ciudad ";
...
function solicita_activacion()
{
	...
	$Ruta1="utilidades/Operativo/operativo.php?Acc=activar_sinlogo&Placa=$Placa&Fecha=$Fecha&Usuario=ERNESTO GONZALEZ";
	$Ruta2="utilidades/Operativo/operativo.php?Acc=activar_sinlogo&Placa=$Placa&Fecha=$Fecha&Usuario=SANDRA OSORIO";
	$Ruta3="utilidades/Operativo/operativo.php?Acc=activar_sinlogo&Placa=$Placa&Fecha=$Fecha&Usuario=SERGIO CASTILLO CASTRO";
	...
	$Mensaje1="<body>Solicitud de Activación Vehículo de AOA<br><br>Placa: <b>$Placa</b><br>Fecha: <b>$Fecha</b><br>".
		...
		"Para activar haga click aquí: <a href='http://app.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta1';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Autorizar</a></body>";

	$Mensaje2="<body>Solicitud de Activación Vehículo de AOA<br><br>Placa: <b>$Placa</b><br>Fecha: <b>$Fecha</b><br>".
		"Funcionario que solicita: $NUSUARIO <br>Siniestro: <b>$Sin->numero $Sin->asegurado_nombre</b><br>Ciudad: <b>$Ciudad->nombre ($Ciudad->departamento)</b><br>".
		"Para activar haga click aquí: <a href='http://app.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta2';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Autorizar</a></body>";

	$Mensaje3="<body>Solicitud de Activación Vehículo de AOA<br><br>Placa: <b>$Placa</b><br>Fecha: <b>$Fecha</b><br>".
		"Funcionario que solicita: $NUSUARIO <br>Siniestro: <b>$Sin->numero $Sin->asegurado_nombre</b><br>Ciudad: <b>$Ciudad->nombre ($Ciudad->departamento)</b><br>".
		"Para activar haga click aquí: <a href='http://app.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta3';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Autorizar</a></body>";
	...
	$Envio2=enviar_gmail($Email_usuario /*de */,
										$NUSUARIO /*Nombre de */ ,
										"sandraosorio@aoacolombia.com,Sandra Osorio" /*para */,
										"$Email_usuario,$NUSUARIO" /*con copia*/,
										"Solicitud Activacion $Placa Flota AOA" /*Objeto */,
										$Mensaje2);
	...
	if($Envio1 && $Envio2 && $Envio3)
		echo "Envio exitoso a: ernestogonzalez@aoacolombia.com, sergiocastillo@aoacolombia.com, sandraosorio@aoacolombia.com, $Email_usuario ";
	else
...
function agendar_cita_ok()
{
	...
	if($Cita_Previa=qo("select *,t_siniestro(siniestro) as nsin from cita_servicio where (placa='$placa' and estado='P' and '$fecha' between fecha and adddate(fecha,dias_servicio) ) or 
								(placa='$paca'  and estadod='P' and '$fecha' <= adddate(fecha,dias_servicio))"))
	{
		echo "<body><script language='javascript'>alert('Ya existe una cita que cruza su tiempo con la fecha propuesta de esta adjudicación: $Cita_Previa->nsin de fecha: $Cita_Previa->fecha');</script></body>";
		die();
	}
	...
	if($S->declarante_email)
	{
		...
		else
		{
			$Correo="Respetado(a) Señor(a) $conductor <br><br>Reciba cordial saludo. <br><br>".
				...
				"<br>- Número de Cuenta Bancaria, A nombre de quien está registrada, Tipo de Cuenta Bancaria (Ahorros o Corriente) y el Nombre del Banco.".
...
function enviar_mail_servicio_html()
{
	...
	$htmlMail = include("CorreosHtml/citaProgramada.html");
	enviar_gmail('sistemas@aoacolombia.com',
				 'Sergio Castillo Castro',
				"sergio.castillo@helpnow.co",
				'',
				'Objeto del mensaje',
				'<BODY>Contenido del mensaje en <b>Html</b></body>',
				'',
...
function solicita_reactivacion_ok()
{
	...
	echo "<body>
		  ...
		  <script>
		  alert('Envio exitoso a: ernestogonzalez@aoacolombia.com, sandraosorio@aoacolombia.com, sergiocastillo@aoacolombia.com, $Email_usuario');
            $.ajax(
                    {
                        ...
                        dataType: 'text',
                        data: {
						    para:'ernestogonzalez@aoacolombia,sandraosorio@aoacolombia.com,sergiocastillo@aoacolombia.com',
							contenido:'9',
