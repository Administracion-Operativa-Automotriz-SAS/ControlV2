...
function citas_inicial() // funcion principal de citas
{
	...
	" Aseguradora: ".menu1('ASEG',"Select id,nombre from aseguradora where id not in (6) order by orden_monitor",$ASEG,1)."
		Ver: <select name='Modo'><option value='1' ".($Modo==1?"selected":"").">Solo Pendientes</option>
		 ...
		 <font style='font-size:8px'>Versión Mayo 2 de 2013</font>";
	...
	if (mysql_num_rows($Citas))
	{
		echo "<table border cellspacing=0 style='empty-cells:show' width='100%'><tr>
						...
						<th>Conductor</th>
						<th>Observaciones</th>
						<th>Vehículo</th>".($USUARIO!=32?"<th>Estado</th>":"")."
					...
					if ($Sin_conc == '' && $Futuro == '')/* si existen servicios sin concluir no permite cambiar ningun estado de la cita */
					{
						...
						if ($C->cestado == 'P')
						{
							...
							if($Autorizaciones[$C->siniestro])
							{
								...
								// boton para la impresión del acta de entrega/devolucion
...
*/
	...
	if(!$Devoluciones = mysql_query($Consulta,$LINK)) die(mysql_error()); // busca las citas de devolución
	if(mysql_num_rows($Devoluciones))
	{
		...
		while ($C = mysql_fetch_object($Devoluciones))
		{
			...
			if ($C->estadod == 'C') citas_devolucion_cumplido($C,$T); // presenta los estados de devolución cumplida
			echo "</td></tr>";
...
function citas_inicial_modo_entregas($Modo,$OFICINA,$ASEG,$Dia) // construcción del query de citas de acuerdo al modo que quier ver el usuario
{
	switch($Modo)
	{
		// solo pendientes
...
function citas_inicial_busqueda_futuros($LINK,$C) // busca ultimos estados, futuros o servicios sin concluir para mostrar un alerta
{
	...
	$Futuro='';
	if($Ultimos[$C->cplaca])  // busca el último estado hasta el dia de hoy, si es distinto a parqueadero, servicio concluido, servicio, o domicilio produce el alerta and u.estado not in (1,2,7,96)
	{
	...
	// si hay eventos futuros o últimos eventos distintos de parqueadero, presenta un alerta
...
function citas_inicial_estado_cumplido($LINK,$C,$T) // presenta las opciones de impresion y actualizaciones de una cita cumplida.
{
	...
	echo "<br /><a onclick=\"modal('zcitas.php?Acc=imprimir_acta&idc=$C->cid',0,0,900,900,'impa');\" style='cursor:pointer;font-weight:bold;'><img src='gifs/print.png' border='0'> Imprimir el Acta de Entrega</a><br />";
	if($T)
	{// botón para modificación del siniestro. para subir imagenes
		echo "<br />Hora: <a onclick=\"modal('marcoindex.php?Acc=mod_reg&Num_Tabla=$T&id=$C->siniestro',0,0,600,1000,'eds');\"
	...
	// si están subidas las imágenes del siniestro, pinta un chulo azul, de lo contrario pinta un trianculo de advertencia.
...
function citas_inicial_entrega_fecha($C) // presenta la fecha y el número de dias de servicio
{
	global $USUARIO;
	echo "<td nowrap='yes'>$C->fecha<br>Días de <br>Servicio: "; // si el usuario es coordinador de call center puede cambiar los dias de servicio
...
function citas_inicial_entrega_siniestro($con_of,$C,$Aseguradora,$T) // presenta el numero de siniestro
{
	...
	{ // presenta el ícono de inicio de proceso de facturación
	...
	{ // presenta el ícono de captura de nuevo recibo de garantía
	...
	if($PRE_AUTORIZA) // si es usuario de autorizaciones y puede preautorizar.. también lo pueden hacer los directores de oficina
	...
	// muestra el botón de consulta inteligente de siniestro
	...
	if(inlist($USUARIO,'1,2,5,6,13,33') && $C->cestado=='C') // muestra el botón de solicitud de visualización de la garantía
		echo "	<a onclick=\"modal('zautorizaciones.php?Acc=datos_autorizacion&id=$C->siniestro',0,0,600,1000,'eds');\"	style='cursor:pointer'><u>Visualización de Garantía</u></a>";
	echo "</td>";
}
...
function citas_inicial_tiempos($C,$LINK) // muestra el registro de tiempos de cada servicio desde el ingreso hasta la entrega del vehiculo
{
	...
	if($C->cestado=='P') // CONTROL DE TIEMPOS
	{
		...
		{
			if($Autorizacion = qom("select id,fecha_solicitud,fecha_proceso from sin_autor where siniestro='$C->siniestro' and estado!='R' ",$LINK)) // trae los datos de la autorización
			{
				// calcula el tiempo entre el arribo y la autorización
				...
				{
					// calcula el tiempo en tre la solicitud de autorización y el tiempo de solicitud
					...
					{
						//calcula el tiempo entre la aprobación de la autorización y la entrega del vehiculo
					...
					else
					{
						...
						{// para los operarios y auxiliares de información y directores de oficina: pinta el icono de entrega de vehiculo
				...
				{// en espera del proceso de autorización
			...
			echo "<br /><a onclick='ver_visitantes($C->siniestro);' style='cursor:pointer'>Visitantes..</a>"; // botón para ver los visitantes del servicio
		}
		elseif(inlist($USUARIO,'1,2,5,32,10'))
		{// pinta el botón de marcacion de arribo y que dispara el proceso de toma de la foto en recepción
		...
		if(!$C->dir_domicilio && $C->arribo=='0000-00-00 00:00:00')
		{
			...
			{// para los usuarios de call center  presenta el botón para insertar domicilio en el servicio
...
function citas_devolucion_siniestro($C,$Aseguradora,$T) // Pinta el siniestro en la devolucion
{
	...
	echo "<a onclick=\"modal('zsiniestro.php?Acc=buscar_siniestro&id=$C->siniestro',0,0,600,1000,'eds');\"	style='cursor:pointer'><u>Ver Siniestro</u></a>";
	echo "<br>";
	if(inlist($USUARIO,'1,6')) // para el perfil de facturacion pinta el botón de inicio de proceso de facturacion
	...
	if(inlist($USUARIO,'1,6,10')) // para facturación y oficinas, pinta solicitud de autorizacion para pago de facturas con tarjeta de credito.
...
function citas_devolucion_cumplido($C,$T) // pinta el formulario de cierre de devolución
{
	if($T) // pinta el lapiz de modificación del siniestro para cargar las imagenes.
	...
	else echo "<br />Hora: $C->hora_devol_real <br />"; // pinta la hora de devolución real
...
class d_efectivo // clase que se usa para pintar datos del tarjeta habiente en la cabecera del acta de entrega/devolucion para garantias de efectivo
{
	var $Nombre_tarjeta_habiente='';  // nombre del dueño de la tarjeta quien autoriza el congelamiento de cupo
	var $Identificacion=0; // identificación del tarjeta habiente quien autoriza el congelamiento de cupo
	var $Numero_cuenta=''; // número de cuenta
	var $Banco=''; // Banco
	var $Tipo_cuenta=''; // mes de vencimiento de la tarjeta de crédito
	var $Ciudad_cuenta='';  // año de vencimiento de la tarjeta de crédito
	var $Valor_garantia=''; // monto de la garantía a ser congelado
...
function imprimir_acta() // imprime el acta de entrega/devolucion
{
	...
	if(!$Fdevol)
	{ // formulario en el que solicita la fecha de devolución para tener encuenta si es domingo, festivo o pico y placa y correr la fecha y hora.
		...
		<form action='zcitas2.php' target='_self' method='POST' name='forma' id='forma'>
			Fecha de Devolución: ".pinta_fc('forma','Fdevol',date('Y-m-d',strtotime($Fec_entrega)))."
			Hora de Devolución: ".pinta_hora('forma','Hdevol',$Cita->hora)."
			<input type='submit' name='continuar' id='continuar' value=' IMPRIMIR '>
			<input type='hidden' name='Acc' value='imprimir_acta_test'>
			<input type='hidden' name='idc' value='$idc'><br><br>
			<b style='font-size:16px'>ESTIMADO (A) USUARIO (A) ".$_SESSION['Nombre']." Tenga en cuenta que si en el momento de la devolución del automovil hay restricción
			vehicular, debe modificar la entrega a la siguiente hora hábil la cual no tenga restricción según las leyes y normas vigentes en su ciudad. De lo contrario
			continúe con la impresión del acta sin hacer ninguna modificación. <br><br>Gracias.</b>
		</form></body>";
		die();
	}
	//echo "try";
	...
	$P->setxy(18,$P->y+5);$P->cell(32,5,'Luz de cortesía',1,0,'L');$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);
	...
	$P->setxy(18,$P->y+5);$P->cell(32,5,'Calefacción y A/A',1,0,'L');$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);
	...
	$P->setxy(18,$P->y+5);$P->cell(32,5,'Manuales y Garantía',1,0,'L');$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);$P->cell(10,5,' ',1,0);
	...
	if(inlist($Siniestro->aseguradora,'3,7'))
	{
		...
		$P->setxy(102,203);$P->cell(100,3,"1. De 0 a 10 donde 0 es nada probable y 10 es muy probable , recomendaría a Liberty Seguros",0,0,'L',1);
		...
		$P->setxy(102,215);$P->cell(100,3,"tan satisfecho se encuentra con el servicio de vehículo sustituto de Liberty Seguros? _____");
		$P->setxy(102,220);$P->cell(100,3,"En una escala de 0 a 10 (donde 0 = muy insatisfecho, 10 = muy satisfecho) en base a su",0,0,'L',1);
		$P->setxy(102,223);$P->cell(100,3,"experiencia de los servicios más recientes de vehículo sustituto por favor califique los",0,0,'L',1);
		...
		$P->setxy(102,236);$P->cell(100,3,"5. Claridad en la información recibida para la asignación de Vehículo Sustituto:_____",0,0,'L');
		$P->setxy(102,240);$P->cell(100,3,"6. Por favor indíquenos el número de veces que tuvo que llamar para recibir información del",0,0,'L',1);
		$P->setxy(102,243);$P->cell(100,3,"servicio y coordinar la entrega del vehículo: _____",0,0,'L',1);
		$P->setxy(102,247);$P->cell(100,3,"7. Tiempo para pa asignación del vehículo: _____",0,0,'L');
		$P->setxy(102,251);$P->cell(100,3,"8. Facilidad para la entrega del vehículo: _____   9. Calidad del Vehículo asignado:_____",0,0,'L');
		$P->setxy(102,255);$P->cell(100,3,"10. La facilidad para la devolución del Vehículo: _____",0,0,'L',1);
		//$P->SetFillColor(255,255,255);
	}
	else
	{
		...
		if($Siniestro->aseguradora != 55 or $date_now < '2019-04-24')
		{	
			$P->setxy(102,203);$P->cell(100,3,"1. Califique de 1 a 5 la información y orientación recibida en el primer contacto por el personal",0,0,'L',1);
			$P->setxy(102,206);$P->cell(100,3," de nuestro call center:",0,0,'L',1);
			$P->setfont('Arial','B',7);
			$P->rect(104,210,2,2);$P->setxy(107,210);$P->cell(13,2,"5 Excelente.",0,0,'L',1);
			$P->rect(124,210,2,2);$P->setxy(127,210);$P->cell(10,2,"4 Buena.",0,0,'L',1);
			$P->rect(140,210,2,2);$P->setxy(143,210);$P->cell(13,2,"3 Regular.",0,0,'L',1);
			$P->rect(158,210,2,2);$P->setxy(161,210);$P->cell(10,2,"2 Mala.",0,0,'L',1);
			$P->rect(173,210,2,2);$P->setxy(176,210);$P->cell(13,2,"1 Pesima.",0,0,'L',1);		
			$P->setfont('Arial','',7);
			$P->setxy(102,213);$P->cell(100,3,"2. Califique de 1 a 5 la gestión y agilidad de nuestras  auxiliares de servicio al  cliente en el");
			$P->setxy(102,216);$P->cell(100,3," momento de su ingreso a las instalaciones de AOA SAS:");
			$P->setfont('Arial','B',7);
			$P->rect(104,220,2,2);$P->setxy(107,220);$P->cell(13,2,"5 Excelente.",0,0,'L',1);
			$P->rect(124,220,2,2);$P->setxy(127,220);$P->cell(10,2,"4 Buena.",0,0,'L',1);
			$P->rect(140,220,2,2);$P->setxy(143,220);$P->cell(13,2,"3 Regular.",0,0,'L',1);
			$P->rect(158,220,2,2);$P->setxy(161,220);$P->cell(10,2,"2 Mala.",0,0,'L',1);
			$P->rect(173,220,2,2);$P->setxy(176,220);$P->cell(13,2,"1 Pesima.",0,0,'L',1);	
			$P->setfont('Arial','',7);
			$P->setxy(102,223);$P->cell(100,3,"3. Califique de 1 a 5 la gestión y agilidad de nuestros auxiliares operativos en el momento de",0,0,'L',1);
			$P->setxy(102,226);$P->cell(100,3,"la entrega del vehículo:",0,0,'L',1);
			$P->setfont('Arial','B',7);
			$P->rect(104,230,2,2);$P->setxy(107,230);$P->cell(13,2,"5 Excelente.",0,0,'L',1);
			$P->rect(124,230,2,2);$P->setxy(127,230);$P->cell(10,2,"4 Buena.",0,0,'L',1);
			$P->rect(140,230,2,2);$P->setxy(143,230);$P->cell(13,2,"3 Regular.",0,0,'L',1);
			$P->rect(158,230,2,2);$P->setxy(161,230);$P->cell(10,2,"2 Mala.",0,0,'L',1);
			$P->rect(173,230,2,2);$P->setxy(176,230);$P->cell(13,2,"1 Pesima.",0,0,'L',1);			
		}
		else{
			
			// this format is string comparable

			//if ($date_now > '2019-04-05') {
				
				$P->setxy(102,203);$P->cell(100,3,"1. ¿Cómo califica el servicio prestado por el agente que atendió su llamada?",0,0,'L',1);
				$P->setfont('Arial','B',7);
				$P->rect(104,207,2,2);$P->setxy(107,207);$P->cell(13,2,"5 Excelente.",0,0,'L',1);
				$P->rect(124,207,2,2);$P->setxy(127,207);$P->cell(10,2,"4 Buena.",0,0,'L',1);
				$P->rect(140,207,2,2);$P->setxy(143,207);$P->cell(13,2,"3 Regular.",0,0,'L',1);
				$P->rect(158,207,2,2);$P->setxy(161,207);$P->cell(10,2,"2 Mala.",0,0,'L',1);
				$P->rect(173,207,2,2);$P->setxy(176,207);$P->cell(13,2,"1 Pesima.",0,0,'L',1);		
				$P->setfont('Arial','',7);
				$P->setxy(102,210);$P->cell(100,3,"2. ¿Cómo califica el tiempo en qué fue otorgado el servicio?");				
				$P->setfont('Arial','B',7);
				$P->rect(104,214,2,2);$P->setxy(107,214);$P->cell(13,2,"5 Excelente.",0,0,'L',1);
				$P->rect(124,214,2,2);$P->setxy(127,214);$P->cell(10,2,"4 Buena.",0,0,'L',1);
				$P->rect(140,214,2,2);$P->setxy(143,214);$P->cell(13,2,"3 Regular.",0,0,'L',1);
				$P->rect(158,214,2,2);$P->setxy(161,214);$P->cell(10,2,"2 Mala.",0,0,'L',1);
				$P->rect(173,214,2,2);$P->setxy(176,214);$P->cell(13,2,"1 Pesima.",0,0,'L',1);	
				$P->setfont('Arial','',7);
				$P->setxy(102,217);$P->cell(100,3," 3. ¿Cómo califica la calidad del servicio prestado?",0,0,'L',1);
				$P->setfont('Arial','B',7);
				$P->rect(104,221,2,2);$P->setxy(107,221);$P->cell(13,2,"5 Excelente.",0,0,'L',1);
				$P->rect(124,221,2,2);$P->setxy(127,221);$P->cell(10,2,"4 Buena.",0,0,'L',1);
				$P->rect(140,221,2,2);$P->setxy(143,221);$P->cell(13,2,"3 Regular.",0,0,'L',1);
				$P->rect(158,221,2,2);$P->setxy(161,221);$P->cell(10,2,"2 Mala.",0,0,'L',1);
				$P->rect(173,221,2,2);$P->setxy(176,221);$P->cell(13,2,"1 Pesima.",0,0,'L',1);
		...
		{
			$P->setfont('Arial','',7);
			$P->setxy(102,233);$P->cell(100,3,"4. De ser necesario utilizaría nuevamente nuestros servicios?");
			$P->setfont('Arial','B',7);
			$P->rect(104,237,2,2);$P->setxy(107,237);$P->cell(13,2,"Definitivamente si.",0,0,'L',1);
			$P->rect(132,237,2,2);$P->setxy(135,237);$P->cell(10,2,"Probablemente si.",0,0,'L',1);
			$P->rect(160,237,2,2);$P->setxy(163,237);$P->cell(13,2,"No lo utilizaría.",0,0,'L',1);			
		}
		else
		{
			$P->setfont('Arial','',7);
			...
			{
				if($date_now < '2019-04-24')
				{
					$P->setxy(102,233);$P->cell(100,3,"4.En una escala de 0 a 10 siendo 0 muy improbable y 10 muy probable ¿Recomendaría a");
					$P->setxy(102,237);$P->cell(100,3,"Previsora Seguros?: _________");	
				}
				else
				{
					$P->setxy(102,224);$P->cell(100,3,"4.En una escala de 0 a 10 siendo 0 muy improbable y 10 muy probable ¿Recomendaría a");
					$P->setfont('Arial','B',7);
					$P->setxy(102,228);$P->cell(100,3,"Previsora Seguros?: _________");	
				}				
			...
			{
				$P->setxy(102,233);$P->cell(100,3,"4.En una escala de 0 a 10 siendo 0 muy improbable y 10 muy probable");
				$P->setxy(102,237);$P->cell(100,3,"¿Recomendaría HDI Seguros a su familia o amigos ?: _________");		
			}
		}		
		...
		{	
			$P->setfont('Arial','',7);
			$P->setxy(102,240);$P->cell(100,3,"5. Recomendaría usted los servicios prestados por AOA S.A.S. a sus familiares o conocidos",0,0,'L',1);
			...
			$P->setfont('Arial','',7);
			$P->setxy(102,250);$P->cell(100,3,"6. Califique de 1 a 5 en términos generales los servicios prestados por AOA S.A.S.");
			$P->setfont('Arial','B',7);
			...
			$P->rect(173,254,2,2);$P->setxy(176,254);$P->cell(13,2,"1 Pesima.",0,0,'L',1);	
			$P->setfont('Arial','',7);
		}
		else
		{			
			$P->setfont('Arial','',7);
			$P->setxy(102,231);$P->cell(100,3,"5.De acuerdo con la experiencia, nos recomendaría con familiares o amigos Cual es la principal",0,0,'L',1);
			$P->setxy(102,234);$P->cell(100,3,"razon por la cual nos dio este puntaje?  *Calificaciones menores de 8: Cual es la mejora más",0,0,'L',1); 
			$P->setxy(102,237);$P->cell(100,3,"importante que debemos realizar para tener un resultado cercano a 10?",0,0,'L',1);
			$P->setfont('Arial','B',7);
			$P->setxy(102,240);$P->cell(100,3,"Calificación: _________");			
			$P->setfont('Arial','',7);
			$P->setxy(102,243);$P->cell(100,3,"6. ¿Cuánto esfuerzo personal tuvo que invertir en la prestación del servicio? donde 1 es alto");
			$P->setxy(102,246);$P->cell(100,3,"esfuerzo y 10 es poco esfuerzo");			
			$P->setfont('Arial','B',7);
			$P->setxy(102,249);$P->cell(100,3,"Calificación: _________");
		}	
		//$P->SetFillColor(255,255,255);
	}
	...
	$P->cell(80,17,' ');
	$P->setxy(10,243);
	$P->cell(13,2,"SI AL MOMENTO DE LA DEVOLUCIÓN DEL VEHÍCULO SE",0,'L');
	$P->setxy(10,246);
	$P->cell(13,2,"PRESENTA ALGÚN COBRO, USTED DEBE EXIGIR LA COPIA DE SU ",0,'L');
	$P->setxy(10,249);
	...
	$P->setfont('Arial','',8);
	$P->setxy(10,263);$P->multicell(198,4,"Carrera 69 B No. 98 A - 10 • PBX: +(571) 756 0510 • Fax: 756 0510 Ext. 112 • Bogotá D.C., Colombia • www.aoacolombia.com",0,'C');
	...
	if($Linea->delineacion_n) $P->Image($Linea->delineacion_n,187,169,15,30);
	{
		...
		$P->setxy(20,10);
		$P->Cell(10,7,'ARRENDAMIENTO DE VEHÍCULOS',0,0,'L');
		...
		$Y=$P->y+$Incremento;
		//$P->setfont('Arial','',10);
		$P->cell(131.6,$Incremento,"$Siniestro->dias_servicio",1,0,'L');
		$Y=$P->y+$Incremento;
		//$P->setfont('Arial','B',10);
		$P->setxy(22,$Y);
		...
		$P->setfont('Arial','',9);
		$P->setxy(20,$Y+1);
		$P->multicell(180,3,"Cláusula 1: El  ARRENDATARIO y/o USUARIO  de manera voluntaria y dando certeza de que todo lo aquí consignado  es cierto, por medio del presente documento ".
		...
		$P->multicell(180,3,"Cláusula 2:  El  ARRENDATARIO y/o USUARIO  declara y reconoce que recibe el vehículo especificado en el estado de conservación y funcionamiento de acuerdo ".
			"con el Acta de Entrega.");
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 3: El  ARRENDATARIO y/o USUARIO  deberá hacer entrega del VEHÍCULO en las mismas condiciones mecánicas, de limpieza y de conservación en las ".
		...
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 4: Cuando se trate de un contrato de arrendamiento, bajo la modalidad de un vehículo en sustitución de las aseguradoras y el vehículo no fuera devuelto ".
		...
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 5: El  ARRENDATARIO y/o USUARIO  debe cumplir con las obligaciones establecidas por las normas vigentes de tránsito y toda la normatividad que se ".
		...
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 6: Suscripción Irrevocable: El  ARRENDATARIO y/o USUARIO  autoriza de manera expresa e irrevocable a la ARRENDADORA el congelamiento de cupo ".
		...
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 7: Autorizo de manera expresa e irrevocable a LA ARRENDADORA , en nombre propio y/o en representación tal y como aparece al pie de mi firma, a  ".
		...
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 8: Con la firma del presente anexo el Arrendatario/ Usuario acepta los términos establecidos en las condiciones CONTRATO DE ARRENDAMIENTO DE ".
			"VEHÍCULOS CONDICIONES GENERALES, el cual ha sido puesto a su disposición para su lectura y conocimiento.");
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 9: Aplican restricciones y condiciones para cada tarifa según su vigencia.");
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 10: El vehículo asignado para su servicio cuenta con un dispositivo de rastreo satélital, para salvaguardar los intereses de la compañía, cualquier alteración o daño de este dispositivo, será responsabilidad del Arrendatario y/o Usuario, que tenga el vehículo en custodia.");
		$Y=$P->y+2;$P->setxy(20,($Y-1));
		$P->multicell(180,3,"AUTORIZACIÓN TRATAMIENTO DE DATOS PERSONALES: En mi calidad de titular de la información AUTORIZO de manera previa, expresa y voluntaria, a ADMINISTRACIÓN OPERATIVA AUTOMOTRIZ S.A.S. (en adelante “AOA”) para que realice el tratamiento de mis datos personales, dentro del marco del objeto del presente CONTRATO. Asimismo, declaro que: (i) me han sido informadas las finalidades del tratamiento de mis datos (ii) conozco el “Manual de Políticas y Procedimientos para la Protección de Datos Personales” de la compañía, puede ingresar a nuestra página web www.aoacolombia.com Política de Protección de Datos Personales (iii) tengo claridad en relación con la existencia de los canales de atención dispuestos por AOA para efectos de ejercer los derechos que ostento como titular de la información.");
		$P->setfont('Arial','B',7);
		$Y=$P->y+1;
		$P->setxy(20,$Y);
		$P->multicell(180,3,"Por medio de la presente firma AUTORIZO de manera general a AOA S.A.S., a NOTIFICARME en la dirección de correo electrónico informada, todos los Actos, comunicaciones, decisiones y tramites en los cuales tengo interés, de carácter particular y concreto en relación con el contrato comercial suscrito.");		
		...
		$P->setxy(150,$Y);$P->cell(50,4,"Nombre Persona Jurídica");
		//$P->setfont('Arial','',7);
...
function insertar_domicilio() // formulario para insertar la información de un domicilio a la cita
{
	global $id;
...
function insertar_domicilio_ok() // graba la información del domicilio en la cita
{
...
function marcar_entregado() // hace la marcación de vehiculo entregado al asegurado - para medir tiempos de respuesta
{
	global $id;
...
function formulario_entrega() // Validación de la entrega para ve si permite ditgitar los datos de la entrega o cancela o reagenda o cumple y no toma serivcio - Formulario
{
	...
	$ultimokm=qo1("select kilometraje($Veh->id)"); // obtiene el ultimo kilometraje del vehículo
	if($estado=='C')
	{
	...
	if($estado=='C') // CUMPLIDA
	{
		...
		if($C->dir_domicilio) // cuando es domicilio, solicita un kilometraje adicional para determinar cuanto se gasto en el domicilio
			...
			echo "
						...
						if(Dato<$ultimokm) {alert('Debe escribir un kilometraje inicial válido igual que el último registrado ".($USUARIO==1?$ultimokm:'')."'); kmi.style.backgroundColor='ffff00';kmi.focus();continuar.style.visibility='hidden';return false;}
						if(Dato>$ultimokm)
						{
...
function cambia_estado() // cambia estado  de acuerdo al formulario de entrega
{
	...
	if ($D = qom("select * from cita_servicio where id=$id",$LINK)) // trae los datos de la cita
	{
		...
		$Fec_entrega = aumentadias($D->fecha, $D->dias_servicio); // calcula la fecha de devolución del vehículo
		...
		if ($estado == 'C') // CUMPLIDO
		{
			...
			if ($Ultimo = qom("select * from ubicacion where vehiculo=$idv and fecha_final > '$D->fecha' and estado=2",$LINK)) // trae la ultima ubicación del vehiculo
			{
				if ($Ultimo->fecha_inicial == $D->fecha) // si la fecha inicial y final coinciden dentro del mismo dia del cambio del nuevo estado, se elimina ese estado
					...
					if(!mysql_query("update ubicacion set fecha_final='$D->fecha' where id=$Ultimo->id",$LINK)) die(mysql_error()); // sino actualiza la ubicación actual
			}
			...
			if($Diferencia>$Oficina->km_domicilio)
			{
				...
				"ernestogonzalez@aoacolombia.com,ERNESTO GONZALEZ" /*para */ ,
		...
		elseif($estado!='P')   // programada
		{
			...
			// inserta en el seguimiento del caso la actualización de que se cancela por algun motivo
...
function generar_xml() // programa que genera un xml para graficos. no se está usando en el momento
{
	global $Hoy;
	...
	echo "<citas>";
	// pinta la información de las citas con su estado
	while($C=mysql_fetch_object($Citas))
	{
...
function insertar_domiciliod() // formulario para insertar un domicilio de devolución
{
	global $id;
...
function insertar_domiciliod_ok() // graba los datos de un domicilio de devolución
{
	...
	graba_bitacora('cita_servicio','M',$id,'Asigna Domicilio Devolución'); // graba la bitacora
	echo "<body><script language='javascript'>window.close();void(null);opener.location.reload();</script></body>";
}
...
function asigna_operario_devolucion() // asigna el operario de devolucion segun lo haya seleccionado el funcionario
{
	...
	graba_bitacora('cita_servicio','M',$cita,'Asigna operario para devolución.'); // graba la bitacora de la cita
	echo "<body><script language='javascript'>alert('Operario Asignado Satisfactoriamente');</script>";
}
...
function estado_operarios() // muestra diariamente, semanalmente y mensualmente la cantidad de entregas y solicitudes por oficina y por operario.
{
	...
	$Devoluciones=q("select concat(op.nombre,' ',op.apellido) as noperario, count(cs.id) as cantidad
							...
							GROUP BY noperario ORDER BY noperario");
	// inicia la creación de los xml para mostrar las gráficas
...
function formulario_devolucion() // formulario para capturar los datos de una devolucion
{
	...
	{ // verifica que tenga asignado ya un operario en la devolución del vehiculo
...
function cambia_estadod() // cambia el estado de devolución de acuerdo a los datos capturados en el formulario de devolución
{
	...
	if ($D = qom("select * from cita_servicio where id=$id",$LINK)) // trae los datos de la cita
	{
		...
		observaciones=\"$ords\",estado=7 where id=$Sincars->ubicacion",$LINK); // actualiza la ubicación
		...
		{
			enviar_gmail($Email_usuario /*de */ ,$Nusuario /*nombre de */ ,
			"ernestogonzalez@aoacolombia.com,ERNESTO GONZALEZ;sergiocastillo@aoacolombia.com,SERGIO CASTILLO" /*para */ ,
			""   /*Con copia*/ ,
		...
		if($kmd)   /// si hubo consumo de kilometraje por domicilio
		{
			$Estado_domicilio=96;
			$Diferencia=$kmd-$kmf;
			// inserta una ubiación de domicilio después de finalizacion del servicio
...
function desencripta_data($id) // desencripta datos de una garantía para mostrar en el acta de entrega/devolucion.
{
...
function imprimir_acta_test() // imprime el acta de entrega/devolucion
{
	...
	if($Linea->delineacion_n) $P->Image($Linea->delineacion_n,187,169,15,30);
	{
		...
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 8: Aplican restricciones y condiciones para cada tarifa según su vigencia.");
		
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 9: El vehículo asignado para su servicio cuenta con un dispositivo de rastreo satélital, para salvaguardar los intereses de la compañía, cualquier alteración o daño de este dispositivo, será responsabilidad del Arrendatario y/o Usuario, que tenga el vehículo en custodia.");
		
		
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 10: La mayoría de los vehículos cuentan con un dispositivo de rastreo satelital, para salvaguardar los intereses de la compañía, cualquier alteración o daño de este dispositivo, será responsabilidad del Arrendatario y/o Usuario, que tenga el vehículo en custodia. ");
		
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 11: Con la firma del presente anexo el ARRENDATARIO Y/O USUARIO acepta los términos establecidos en las condiciones CONTRATO DE ARRENDAMIENTO DE VEHÍCULOS CONDICIONES GENERALES, el cual ha sido puesto a su disposición para su lectura y conocimiento.");
		...
		$Y=$P->y+1;
		$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 12: Por medio de la presente firma AUTORIZO de manera general a AOA S.A.S., a NOTIFICARME en la dirección de correo electrónico informada, todos los Actos, comunicaciones, decisiones y tramites en los cuales tengo interés, de carácter particular y concreto en relación con el contrato comercial suscrito.");		
