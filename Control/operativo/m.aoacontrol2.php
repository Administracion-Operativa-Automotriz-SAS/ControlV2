...
function seleccionar_cita() // SELECCIONA UNA CITA PARA SU PROCESO
{
	...
	echo "
		...
		echo "
			...
			function firmar_factura(id){window.open('marcoindex.php?Acc=capturar_firma&app=m.aoacontrol2.php&retorno=seleccionar_cita~id=$id~tipo=$tipo&acc=firmar_factura_ok&id='+id,'_self');}
...
function guardar_entrega()
{
	...
	if($Diferencia>$Oficina->km_domicilio)
	{
		...
		"Exceso desplazamiento en domicilio $Cita->placa $Oficina->nombre"  /*OBJETO*/,
...
function guardar_devolucion()
{
	...
	if($aseguradora->id == 1 || $aseguradora->id == 8 || $aseguradora->id == 9){
			...
			$data_to_send = json_encode(array(
				...
				"meta_data"=> "{ \"numeroSiniestro\":\"$Sin->numero\", \"placaAsegurado\":\"$Sin->placa\", \"documentoAsegurado\":\"$Sin->asegurado_id\" , \"asd\":\"Gracias por visitarnos\" , \"first_name\":\"$Sin->asegurado_nombre\" , \"last_name\":\"$clienteSiniestro->apellido\" }",
		...
		if($aseguradora->id == 1 || $aseguradora->id == 8 || $aseguradora->id == 9){
			$Sincars = qo("select * from siniestro where id= $D->siniestro");
			//print_r($Sincars);
			echo "Select * from cliente where identificacion = '".$Sincars->asegurado_id."'";
			...
			$sqlMail = "INSERT INTO seguimiento (siniestro,fecha,hora,usuario,descripcion,tipo) VALUES ($D->siniestro,'$fechaMail','$horaMail','$usuarioMail','$responseMail',30)";
			qo($sqlMail);
			...
			$sqlSms = "INSERT INTO seguimiento (siniestro,fecha,hora,usuario,descripcion,tipo) VALUES ($D->siniestro,'$fechaSms','$horaSms','$usuarioSms','$responseSms',31)";
			qo($sqlSms);
		}
		*/
