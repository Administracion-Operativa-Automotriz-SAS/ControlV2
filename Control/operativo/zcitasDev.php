...
function citas_inicial() // funcion principal de citas
{
	...
	if (mysql_num_rows($Citas))
	{
		...
		while ($C = mysql_fetch_object($Citas))
		{
			...
			var_dump($_SESSION['excel_citas_entregas']);
			$con_of++;
			$countN++;
		}
		echo "</table>";
...
*/
	...
	if(mysql_num_rows($Devoluciones))
	{
		...
		while ($C = mysql_fetch_object($Devoluciones))
		{
			...
			var_dump($_SESSION['excel_citas_devoluciones']);
			$con_of++;
			$conuntNoDev++;
		}
		echo "</table>";
	...
	{ // formulario en el que solicita la fecha de devolución para tener encuenta si es domingo, festivo o pico y placa y correr la fecha y hora.
		...
		if($festivos->esFestivo(date('d', $myDate), date('m',$myDate))){
			$diaD =  "$style <style>.colorDiaFestivo{
				color: red;
			}</style> <h2>El dia de devolucion  es el: <div class='colorDia'>".$varDia."</div> ".date("Y-m-d", $myDate)."<div class='colorDiaFestivo'> Es un dia festivo.</div><h2>";
		}else{
			$diaD = "$style <h2>El dia de devolucion es el: <div class='colorDia'>".$varDia."</div> ".date("Y-m-d", $myDate)." No es un dia festivo</h2>";
		}
		...
		<form action='zcitas2.php' target='_self' method='POST' name='forma' id='forma'>
			...
			Hora de Devolución: ".pinta_hora('forma','Hdevol',$Cita->hora)." $diaD 
