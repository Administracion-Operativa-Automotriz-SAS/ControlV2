...
class Fact_electronica_angular
{
	...
	public function get_nd_electronica()
	{
	...
	public function make_elec_document()
	{
		...
		if($this->request->doc == 1)
		{
			...
			$response = $this->ptesa_service->registro_documento_obligatorio(base64_encode($xml),1,$register["consecutivo"]);
	...
	public function generate_fact_xml_for_m($json_input)
	{
		...
		$fact->set_InformacionDeDocumento($fecha_venc,$valor_letras,$orden,$resolucion,$observaciones);
		...
		$fact->set_InformacionDeAdquiriente($customer->email_e);
		
		//print_r($customer);
		
		//exit;
		
		$fact->set_InformacionCliente($customer->nombre,$customer->apellido,$city->departamento,$customer->barrio,$city->nombre,$customer->direccion,$customer->celular,$customer->identificacion,"");
		...
		//START especificación de impuestos
			...
			$fact->set_TaxTotal($doc_details->iva);
			
			$fact->set_tax_subtotals($array_of_taxes);
		
		
		//END especificación de impuestos
		
		$fact->set_LegalMonetaryTotal(str_replace(",","",$doc_details->subtotal),str_replace(",","",$doc_details->iva),0);
