...
function factura_general()
{
	...
	if($D->siniestro)
	{
		...
		{
			echo "<body><script language='javascript'>window.open('zfacturacionDev.php?Acc=generar_factura&identificacion=$identificacion&idCita=$idCita','_self');</script></body>";
			die();
		}
	}
	...
	echo "		
		...
		{
			window.open('zfacturacionDev.php?Acc=factura_general&id='+id,'_self');
		}
		function ingresar_al_detalle()
		{
			window.open('zfacturacionDev.php?Acc=detalle_factura&factura=$id&Aseguradora='+document.forma.aseguradora.value,'dfactura');
		}
		//function imprimir_factura()	{	modal('zfacturacionDev.php?Acc=imprimir_factura&id=$id',0,0,800,1000,'imp');}
		//function imprimir_factural()	{	modal('zfacturacionDev.php?Acc=imprimir_factural&id=$id',0,0,800,1000,'imp');}
		...
		{ 
			...
			{ window.open('zfacturacionDev.php?Acc=aprobar_factura&consecutivo='+document.forma.consecutivo.value+'&idCita=$idCita','Oculto_facturacion');
				//window.open('zfacturacionDev.php?Acc=aprobar_factura&consecutivo='+document.forma.consecutivo.value+'&idCita=$idCita');
		...
		function grabar_datos_factura()
		{
			...
			window.open('zfacturacionDev.php?Acc=grabar_datos_factura&fe='+fe+'&fv='+fv+'&co='+co+'&cl='+cl+'&ase='+ase,'Oculto_facturacion');
		}
		/*function borrar_factura()
		{
			if(confirm('Desea borrar esta factura y todo su detalle?'))
			{
				window.open('zfacturacionDev.php?Acc=borrar_factura&consecutivo='+document.forma.consecutivo.value+'&idCita=$idCita','Oculto_facturacion');
			}
		}*/
			function grabar_garantia()
			{
				...
				{
					window.open('zfacturacionDev.php?Acc=aplicar_garantia&idfactura=$id&idgarantia='+idga,'Oculto_facturacion');
				}
			}
	...
	if($id)
	{
		...
		{
			echo "<form action='zfacturacionDev.php' method='post' target='_self' name='forma' id='forma'><br />
			...
			</form><center><iframe name='dfactura' id='dfactura' src='zfacturacionDev.php?Acc=detalle_factura&factura=$id&Aseguradora=$F->aseguradora'
...
function inserta_desde_cita()
{
	...
	if($Cita=qo("select * from cita_servicio where id=$idCita"))
	{
		...
		<script language='javascript'>
			...
			function validar_nuevo_cliente()
			{
				with(document.forma)
				{
					...
					if(!alltrim(identificacion.value)) {alert('Debe digitar la identificación');identificacion.style.backgroundColor='ffff55';identificacion.focus(); return false;}
			...
			{
				window.open('zfacturacionDev.php?Acc=identifica_tercero&identificacion='+dato,'Oculto_facturacion');
			}
			function changeFunc(){
		...
		<body><script language='javascript'>centrar(800,700);</script>
			...
			<form action='zfacturacionDev.php' method='post' target='_self' name='forma' id='forma'>
...
function grabar_nuevo_cliente()
{
	...
	echo "<script language='javascript'>
		function carga()
		{
			...
			window.open('zfacturacionDev.php?Acc=generar_factura&idCita=$idCita&identificacion=$identificacion&VS=$VS&IDSF=$IDSF','_self');
		}
	</script>
	...
	if($Cita=qo("select * from cita_servicio where id=$idCita"))
	{
		...
		echo "
			function crear_consecutivo()
			{
				...
				window.open('zfacturacionDev.php?Acc=crear_consecutivo&fe='+fe+'&fv='+fv+'&cl='+cl+'&as=$Sin->aseguradora&sin=$Sin->id&IDSF=$IDSF','Oculto_facturacion');
			}
			
			
			function ingresar_al_detalle()
			{window.open('zfacturacionDev.php?Acc=detalle_factura&factura='+document.forma.idfactura.value+'&Aseguradora=$Sin->aseguradora','dfactura');}
			function traer_factura(id)
			{window.open('zfacturacionDev.php?Acc=traer_factura&id='+id,'Oculto_facturacion');	}
			function grabar_datos_factura()
			{
				with(document.forma)
				{
			...
			{ 
				...
				{ window.open('zfacturacionDev.php?Acc=aprobar_factura&consecutivo='+document.forma.consecutivo.value+'&idCita=$idCita','Oculto_facturacion');
					//window.open('zfacturacionDev.php?Acc=aprobar_factura&consecutivo='+document.forma.consecutivo.value+'&idCita=$idCita');
					document.querySelector('.loader').style.display = 'block';
					setTimeout('close_loader()', 1800000000);
				}
			}
			...
			function imprimir_factura()	{	modal('zfacturacionDev.php?Acc=imprimir_factura&consecutivo='+document.forma.consecutivo.value,0,0,800,1000,'imp');}
			function imprimir_factural()	{	modal('zfacturacionDev.php?Acc=imprimir_factural&consecutivo='+document.forma.consecutivo.value,0,0,800,1000,'imp');}
			...
			function genera_solicitud_autorizacion()
			{
				...
				with(document.forma)
				{
					window.open('zfacturacionDev.php?Acc=genera_solicitud_autorizacion&sin=$Sin->id&fac='+consecutivo.value+'&vou='+voucher.value,'Oculto_facturacion');
				}
			}
			function grabar_garantia()
			{
				...
				if(idfac)
				{
					if(confirm('Desea utilizar la garantía para pagar la factura?'))
					{
						window.open('zfacturacionDev.php?Acc=aplicar_garantia&idfactura='+idfac+'&idgarantia='+idga,'Oculto_facturacion');
					}
				}
				else
				{
		...
		<form action='zfacturacionDev.php' method='post' target='Oculto_facturacion' name='forma' id='forma'>
		...
		<center><iframe name='dfactura' id='dfactura' src='zfacturacionDev.php?Acc=detalle_factura&factura=0' height='200' width='750' border='0' frameborder='no' scrolling='auto'></iframe></center>
		<iframe name='Oculto_facturacion' style='visibility:hidden' height='1' width='1' frameborder='no'></iframe>
		Observaciones:<br />
		...
		window.open('zfacturacionDev.php?Acc=grabar_observaciones&id='+document.forma.idfactura.value+'&contenido='+document.getElementById('observaciones').value,'Oculto_facturacion');
		...
		if($factura=q("insert into factura (consecutivo,fecha_emision,fecha_vencimiento,cliente,aseguradora,siniestro) values ('$Consecutivo','$fe','$fv','$cl','$as','$sin')"))
		{
			echo "<script language='javascript'>
				function carga()
				{
					...
					window.open('zfacturacionDev.php?Acc=detalle_factura&factura=$factura&Aseguradora=$as','dfactura');
				}
			</script>
			<body onload='carga()'></body>";	
...
function traer_factura()
{
	...
	$iframe_Tag = '<iframe width="100%" height="100%" id="elec_frame"  src="https://app.aoacolombia.com/Control/operativo/zfacturacionDev.php?Acc=verificar_factura_electronica&id='.$D->id.'"  ></iframe>';
	...
	echo "<script language='javascript'>
		...
		//	parent.document.getElementById('observaciones').value=\"\";
			window.open('zfacturacionDev.php?Acc=detalle_factura&factura=$id&Aseguradora=$D->aseguradora','dfactura');
		}
	</script>
	<body onload='carga()'></body>";
}
	...
	if($factura)
	{
		echo "<script language='javascript'>
			function valida_concepto(concepto)
			{
				...
				if(Acc)
				{
					window.open('zfacturacionDev.php?Acc='+Acc+'&concepto='+concepto+'&Aseguradora=$Aseguradora&linea_vehiculo='+parent.Linea_vehiculo,'Oculto_detallefac');
				}
			}
			...
			function borrar_item(id)
			{
				...
				{
					window.open('zfacturacionDev.php?Acc=borrar_item&id='+id,'Oculto_detallefac');
				}
			}
		</script>
		<body bgcolor='ffffdd' topmargin='0' leftmargin='0' rightmargin='0'>";

		echo "<form action='zfacturacionDev.php' method='post' target='_self' name='forma_detalle' id='forma_detalle'>
		...
		if(!$Fac->autorizadopor)
		{
			...
			echo "<td bgcolor='ffffff' align='center'><textarea name='descripcion' id='descripcion' rows=2 cols=30 style='font-size:12px'></textarea></td>";
...
function borrar_factura()
{
	...
	echo "<script language='javascript'>
		function carga()
		{
			parent.location='zfacturacionDev.php?Acc=generar_factura&idCita=$idCita';
		}
	</script>
	<body onload='carga()'></body>";
}
...
function aprobar_factura() 
{
	...
	$Ahora=date('Y-m-d H:i:s');
		...
		$iframe_Tag = '<iframe width="100%" height="100%" src="https://app.aoacolombia.com/Control/operativo/zfacturacionDev.php?Acc=verificar_factura_electronica&id='.$Fac->id.'"  ></iframe>';
		...
		if($Solicitud=qo("select * from solicitud_factura where siniestro=$Fac->siniestro and procesado_por='' ")) echo "window.open('zfacturacionDev.php?Acc=marca_solicitud_procesada&id=$Solicitud->id','_self');";
