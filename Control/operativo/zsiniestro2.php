...
function inicial()
{
	...
	<form action='zsiniestro2.php' target='Tablero_busca_siniestro' method='POST' name='forma' id='forma'>
		...
		Id: <input type='text' style='font-size:16px;font-weight:bold;' class='numero' name='id' id='id' onfocus=\"document.forma.siniestro.value='';\"> 
		<input type='button' style='font-size:16px;font-weight:bold;' name='consultar' id='consultar' value=' Consultar ' onclick='enviar_consulta();' >
...
{  
	global $siniestro, $placa, $id, $Historico;
	sesion(); //verifica la sesion del usuario
	if($id)
	{
		if(!$Sins=q("select s.*,a.nombre as naseg  from siniestro s,aseguradora a where a.id=s.aseguradora and s.id=$id")) // busca por id en la tabla de siniestros
		{$Sins=q("select s.*,a.nombre as naseg  from siniestro_hst s,aseguradora a where a.id=s.aseguradora and s.id=$id");$Historico=1;} //  busca por id en la tabla de siniestros historicos
	}
	else 
	{
		if($siniestro)
		{// busca por numero de siniestro
			$Sins=q("select s.*,a.nombre as naseg from siniestro s,aseguradora a where s.aseguradora=a.id and s.numero like '%$siniestro%' order by s.ingreso desc");
			
			
			if(!$Sins) {$Sins=q("select s.*,a.nombre as naseg from siniestro_hst s,aseguradora a where s.aseguradora=a.id and s.numero like '%$siniestro%' order by s.ingreso desc");$Historico=1;}
		}
		elseif($placa)
		{ // busca por placa
			$Sins=q("select s.*,a.nombre as naseg from siniestro s,aseguradora a where s.aseguradora=a.id and s.placa like '%$placa%' order by s.ingreso desc");
			if(!$Sins) {$Sins=q("select s.*,a.nombre as naseg from siniestro_hst s,aseguradora a where s.aseguradora=a.id and s.placa like '%$placa%' order by s.ingreso desc");$Historico=1;}
		}
		else $Sins=false;
	}
	if($Sins)
	{
		...
		if(mysql_num_rows($Sins)>1) // si son varios siniestros.
		{
			...
			{window.open('zsiniestro2.php?Acc=buscar_siniestro&id='+id,'_self');}
			...
			<table border=0 cellspacing='5' ><tbody ><tr >
				...
				<th style='font-size:14px;'>Asegurado</th><th style='font-size:14px;'>Ingreso</th><th></th></tr>";
			...
			{	echo "<tr><td style='font-size:14px;'>$S->naseg</td><td style='font-size:14px;'>$S->numero</td>
							...
							<td style='font-size:14px;'>$S->asegurado_nombre</td><td style='font-size:14px;'>$S->ingreso</td>
		...
		else
		{
			...
			echo "<script language='javascript'>
				...
				function ver_seguimiento(id) {modal('zsiniestro2.php?Acc=ver_seguimiento&id='+id+'&Historico=$Historico',0,0,600,900,'vs');}
				function ver_observaciones(id) {modal('zsiniestro2.php?Acc=ver_observaciones&id='+id,50,50,600,900,'vo');}
				
				function insertarObservacion(id) {modal('zsiniestro2.php?Acc=insertarObservacion&id='+id,10,20,200,700,'vo');}
				...
				function re_agendar_cerrar() {alert('A continuación seleccione un vehículo para reagendar la cita');window.close();void(null);}
				...
				function facturar_conceptos()
				{
					...
					if(IDS_FACTURACION=='0') {alert('Debe tener marcado al menos una solicitud de facturación');return false;}
					if(IDCITA_FACTURACION==0) {alert('Debe marcar una cita de entrega o devolución para asociar la factura.'); return false;}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			echo "<tr><td bgcolor='ffffff' valign='top'>
						<table width='100%' bgcolor='dddddd'>
							...
							<tr><td bgcolor='ffffff'>Email Declarante:</td><td bgcolor='ffffff'><b>$S->declarante_email</b></td></tr>";
			...
			if($S->pqr_asociado != 0){
				 ...
				 $consultaPqr = qo($sql);
			 	 $htmlPqr = "<tr><td bgcolor='ffffff'>Hay un PQR en el siniestro:</td> <td bgcolor='ffffff' width='300'><b>Numero: $S->pqr_asociado</b></td>
							 ...
							 <tr><td bgcolor='ffffff'>Descripción PQR:</td> <td bgcolor='ffffff' width='300'><b>$consultaPqr->descripcion</b></td>
			...
			{
				echo "<tr><td colspan='2' bgcolor='ffffff'>No hay registro de ingreso en Recepción</td></tr>";
			}
			...
			echo "</tr>
					<tr><td align='right' bgcolor='ffffff'>Días de Servicio</td><td bgcolor='ffffff'> <b>$S->dias_servicio</b></td></tr>
					</table>";
			// Permite solicitar información de las garantias
			echo "<a onclick='solicitar_info_aut();'><u>Solicitar Información de Autorizaciones</u></a> 
				";
			if(inlist($USUARIO,'1,2,3,5,10')) // autorizaciones y oficinas: verificación de cartera y de garantias
			echo "<a onclick='verificar_cartera();'><u>Verificar Cartera</u></a> <a onclick='verificar_garantia();'><u>Verificar Garantía</u></a>";
			// datos del contrato
			echo "<br><br>Máximo kilometraje permitido: ".($Aseguradora->limite_kilometraje>10000?"ILIMITADO":coma_format($Aseguradora->limite_kilometraje))."<br>
				Valor de la garantía: ".coma_format($Aseguradora->garantia)." <a onclick='ver_contrato()'><u>Ver Contrato</u></a> ";
			// Si el siniestro está dentro de la tabla de historicos, permite retornarlo
			...
			// Busca la ubicación, que es el estado de servicio en la tabla de control
			...
			if($Ub=qo("select * from ubicacion where id=$S->ubicacion"))
			{
				...
				echo "<tr><td align='right'>Vehículo:</td><td colspan='2'>
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if($Garantias=q("select a.id,a.nombre,a.valor,a.estado,f.nombre as nfr,a.metodo_devol,
											...
											where a.franquicia=f.id and a.siniestro=$S->id"))
			{
				echo "<table bgcolor='dddddd'><tr>
					...
					<td bgcolor='eeeeee' align='center'><b>Responsable de la Garantía</b></td>
				...
				while($G =mysql_fetch_object($Garantias )) // pinta los detalles de las garantias
				{
					...
					<td bgcolor='$bgcolor'>$G->nombre</td>
					<td bgcolor='$bgcolor' align='right'><b>".coma_format($G->valor)."</b></td>
					<td bgcolor='$Color_Estado'>$Label_Estado".($USUARIO==1?" <a onclick='mod_sin_autor($G->id);'>[+]</a>":"")."</td>
					...
					if($G->estado=='A') echo "<tr><td bgcolor='$bgcolor' align='right'>Método de devolución:</td><td bgcolor='$bgcolor'>".
							($G->metodo_devol?$G->metodo_devol:"<span style='color:992222'>No ha sido devuelto</span>")."</td>
							<td bgcolor='$bgcolor' colspan=2>Fecha y Hora:</td><td colspan='2' bgcolor='$bgcolor'>".
					...
					{
						echo "<tr><td bgcolor='$bgcolor' colspan=6>
			...
			//// BUSQUEDA DE SOLICITUDES DE FACTURACION ////
				// verificación si el usuario esta en el perfil de facturacion  -------------------------------------------------------------------------
										...
										ORDER BY fecha_solicitud"))
			{
				...
				while($SF =mysql_fetch_object($Solicitudes_fac )) // solicitud por solicitud pinta los detalles
				{
					...
					<td align='center'>";
					if($F->procesado_por) echo "$SF->procesado_por";
					else 
					{
						if($Puede_facturar)
						{
			...
			if($Facturas=q("select * from factura where siniestro=$S->id"))
			{
				echo "<table bgcolor='dddddd' width='100%'><tr><td colspan='4' bgcolor='eeeeee' align='center'><b>FACTURAS</b></tr><tr>
				...
				while($Fa =mysql_fetch_object($Facturas )) // pinta los detalles de cada factura
				{
					...
					<td bgcolor='ffffff'>";
					// en caso de encontrar recibos de caja para cada factura pinta la información 
					if($Recibos=q("select rc.id,rc.consecutivo,rc.fecha,rc.valor,o.sigla,rc.consignacion_f from recibo_caja  rc,oficina o where o.id=rc.oficina and rc.factura=$Fa->id"))
					{
						...
						while($Rc =mysql_fetch_object($Recibos )) // pinta la información de cada recibo de caja
						{
							echo "<tr>
					...
					{
						echo "<table bgcolor='dddddd' width='100%'><tr><td bgcolor='eeeeee' colspan='3' align='center'><b>Notas Crédito</b></td></tr>
						...
						while($Ncr =mysql_fetch_object($Notas_credito )) // pinta la información de cada nota crédito
						{
							echo "<tr>
			...
			while($C2=mysql_fetch_object($C2q))
				echo "Cola2: $C2->fecha Código Único: <b>-$C2->codigo-</b> ".($C2->descargado!='0000-00-00 00:00:00'?"Contrato descargado: <b>$C2->descargado</b> ":"").
				" Aceptación:".($C2->aceptado?"<b>SI</b> Fecha aceptación: $C2->fecha_aceptacion ".($C2->ip?" <a onclick=\"modal('https://en.utrace.de/?query=$C2->ip',0,0,700,1000,'ubicacion');\">IP $C2->ip</a> ":"").$Estados_p[$C2->estado]:"<b>NO</b>")." <br>";
			...
			{
				echo "<br><b>Cola Información Erronea</b>
			...
			if($SV=q("select * from solicitud_dataautor where siniestro=$S->id"))
			{
				echo "<br><b>SOLICITUDES DE VISUALIZACION DE GARANTIAS</b>
					...
					<th>Fec.Visualización</th>
					</tr>";
				while($Sv=mysql_fetch_object($SV)) // pinta cada registro de solicitudes de visualización de la garantía
				{
					echo "<tr>
...
function ver_seguimiento() // función que permite ver el seguimiento en forma de matriz.
{
	global $id,$Historico;
...
function insertarObservacion(){
	...
	function adicionar_observaciones()
					{ var Obs=document.getElementById('nuevas_observaciones').value;
						...
						window.open('zsiniestro2.php?Acc=adicionar_observaciones&id=$id&d='+Obs,'Oculto_presentar_caso');
