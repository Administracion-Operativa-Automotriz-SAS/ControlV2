...
function inicio_callcenter()
{
	...
	echo "<script language='javascript'>
			...
			function recargar_control_compromisos()
			{
				...
				Recargar=setTimeout(recargar_control_compromisos,30000);
			}
...
function informe_callcenter()
{
	...
	echo "
		...
		<script language='javascript'>
			...
			function Callc(id_siniestro)
			{
				modal('zcallcenter.php?Acc=inicio_proceso_call&id_siniestro='+id_siniestro,0,0,600,900,'ca');
			}

			function Compr(id)
			{
				document.getElementById('if_compromisos').style.visibility='visible';
				window.open('zcompromiso.php?Acc=ver_compromisos&id='+id+'&DesdeCall=1','if_compromisos');
			}
			...
			function pinta_caso(oficina)
			{
				...
				for(var i=0;i<Casos_oficina[oficina].length;i++)
				{
					...
					{
						procesados++;
						document.getElementById('sp1_'+idcaso).innerHTML=\"<img src='gifs/standar/si.png' border='0'>\";
					}
			...
			function retira_compromiso(idSiniestro,idCompromiso)
			{
				...
				{
					if(Compromiso[idSiniestro][i].Id==idCompromiso)
					{
						Compromiso[idSiniestro].splice(i,1);
						return;
					}
				}
			}
	...
	if($av_asegurado)
	{
		$Filtro1.=" and (s.asegurado_nombre like '%$av_asegurado%' or s.declarante_nombre like '%$av_asegurado%' or s.conductor_nombre like '%$av_asegurado%' )";
		$Filtro2.=" and (si.asegurado_nombre like '%$av_asegurado%' or si.declarante_nombre like '%$av_asegurado%' or si.conductor_nombre like '%$av_asegurado%' )";
	}
	if($av_numero)
	{
	...
	if($Aseguradoras=q("select distinct a.nombre,a.id from siniestro s,aseguradora a where s.aseguradora=a.id  ".($Filtro1?"":"and s.estado=5")." and a.id in ($Asegs)
			and s.ciudad in ($Ofics)  $Filtro1
			order by a.nombre"))
	{
		if($Seguimientos=q("select si.id,si.ingreso,se.fecha,se.hora,se.usuario,se.tipo,ts.nombre as ntipo FROM seguimiento se,siniestro si,tipo_seguimiento ts
			WHERE si.id=se.siniestro  ".($Filtro2?"":"and si.estado=5")." and si.aseguradora in ($Asegs)
			and si.ciudad in ($Ofics) $Filtro2 and se.tipo=ts.id
			order by si.id,se.fecha,se.hora"))
		{
			echo "<script language='javascript'>";
			$Ultimo_id=0;
			$Contador=0;
			while($S=mysql_fetch_object($Seguimientos))
			{
				if($S->id!=$Ultimo_id)
				{
					$Ultimo_id=$S->id;
					echo "
							Caso[$Ultimo_id]=new Array();";
					$Contador=0;
				}
				//$Tiempo=segundos_habiles($S->ingreso,date('Y-m-d H:i:s'));
				echo "
							Caso[$Ultimo_id][$Contador]=new segclass($S->id,'$S->fecha','$S->hora','$S->usuario',$S->tipo,'$S->ntipo');";
				$Contador++;
			}
		}
		echo "
		...
		while($As=mysql_fetch_object($Aseguradoras))
		{
			...
			echo "<li class='depart' onclick=\"abre_cierra('$As->id','Ase')\"><b>$As->nombre </b> <span id='sp1$As->id' style='color:000099'></span></li>
						<ul id='Ase$As->id' style='visibility:hidden;position:absolute;cursor:pointer;' >";
			$Oficinas=mysql_query("select distinct o.id,o.nombre,o.ciudad from siniestro s, oficina o where s.ciudad=o.ciudad and s.aseguradora=$As->id  ".($Filtro1?"":"and s.estado=5")."
								and s.ciudad in ($Ofics) $Filtro1
								order by o.nombre",$LINK);
			$Cantidad_oficina=0;
			while($Of=mysql_fetch_object($Oficinas))
			{
				$Idofi=$As->id.'_'.$Of->id;
				echo "<li class='depart' onclick=\"abre_cierra('$Idofi','Ofi')\"><b>$Of->nombre</b> <span id='sp2_$Idofi' style='color:0000bb'></span> <span id='sp3_$Idofi' style='color:000000'></span></li>
							<ul id='Ofi$Idofi' style='visibility:hidden;position:absolute;cursor:pointer;' >";
				//  -*-*-*-*-*-**-**-*-*-*-*-*-*-*-*-*-*-**-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-
				$Pendientes=mysql_query("select s.id,s.numero,s.placa,s.asegurado_nombre,s.ingreso,s.observaciones,s.chevyseguro,s.retencion from siniestro s,oficina o
				...
				echo "<a style='cursor:pointer;font-size:14;font-weight:bold;color:0000cc;' onclick=\"modal('zcallcenter.php?Acc=verificacion_disponibilidad&Flota=$As->id&cCiudad=$Of->ciudad',0,0,600,600,'disponibles');\">Verificación de disponibilidad</a>
							<table border=0 cellspacing='1'><tr><th>Numero</th><th>Numero</th><th>Asegurado</th><th>Placa</th><th>Ingreso</th><th>Llamar</th><th>Iniciado</th>
				...
				{
					echo "<tr id='tr_$P->id'><td align='right'>$Contador</td><td nowrap='yes'><a class='info' style='cursor:pointer' onclick='ver_seguimiento($P->id);'>$P->numero ".
							($P->chevyseguro?" <img src='img/chevyseguro2.png' border='0' height='30px' alt='chevyseguro' title='chevyseguro'>":"").
	...
	{
		echo "No hay información con los criterios dados.";
	}
}
...
function inicio_proceso_call()
{
	...
	echo "<script language='javascript'>
			...
			function ver_seguimiento(id)
			{modal('marcoindex.php?Acc=abre_tabla&Num_Tabla=$Tseguimiento&VINCULOT='+id+'&VINCULOC=siniestro',0,0,500,1000,'seguimiento');}

			function ver_siniestro(id)
			{modal('marcoindex.php?Acc=mod_reg&Num_Tabla=$TSiniestro&id='+id,0,0,500,1000,'siniestro');}

			function cerrar()
			{
				window.close();void(null);
				opener.tachar($id_siniestro);
			}
			function cerrar1()
			{
				window.close();void(null);
			...
			{
				";
//			if($Aseguradora->pide_autorizacion)
//			{
//				echo "solicitar_autorizacion();";
//			}
//			else
//			{
				echo "window.close();void(null);tachar_adjudicado();";
//			}
			echo "
			}

			function tachar_adjudicado()
			{
				opener.tachar_adjudicado($id_siniestro);
				window.close();void(null);
			}
			...
			function recargar()
			{
				window.open('zcallcenter.php?Acc=inicio_proceso_call&id_siniestro=$id_siniestro','_self');
				recargar_observaciones();
			}

			function Compr(id)
			{
				modal('zcompromiso.php?Acc=ver_compromisos&id='+id+'&DesdeCall=0',0,0,400,700,'compromisos');
			}

			function marcar_procesado()
			{
				opener.marcar_procesado($id_siniestro);
			}

			function recargar_observaciones()
			{
				document.forma.observaciones.value='';
				document.getElementById('observaciones_grabadas').src='zcallcenter.php?Acc=traer_observaciones&id=$id_siniestro';
			}

			function solicitar_autorizacion()
			{modal('zautorizaciones.php?sini=$Siniestro->numero&DesdeCall=1',0,0,600,600,'call');}

			function activar_agenda()
			{
				modal('zcallcenter.php?Acc=proceso_adjudicacion&id=$id_siniestro',0,0,600,900,'wadj');
			}
...
//		if($Aseguradora->pide_autorizacion)
//		{
//		echo "<td align='center'><a class='info' onclick='solicitar_autorizacion();'><img src='img/solicita_autorizacion.png' border='0' height=30><span>Solicitar Autorización</span></a></td>";
//		}

		echo "
			 <br />
			Ciudad: <b>$Ciudad</b> ".($Ciudado?"Ciudad original: <b>$Ciudado</b>":"")."</h3>";
	echo "<table align='center'>
				<tr>
				<td align='center' valign='top' rowspan=2>
				Observaciones registradas:<br />
				<iframe height=200 width=400 name='observaciones_grabadas' id='observaciones_grabadas' src='zcallcenter.php?Acc=traer_observaciones&id=$id_siniestro'></iframe>
				</td>
...
function proceso_informacion_erronea_ok()
{
	...
	"$destinatario,$destinatario;gabrielsandoval@aoacolombia.com,Gabriel Sandoval;controloperativo@aoacolombia.com,Dervin Junior;oscargomez@aoacolombia.com,Oscar Gomez".($destino_copia?";$destino_copia":"") /*para */ ,
...
//	"<body>".nl2br($mensaje)."</body>" /*mensaje */);
	...
	{
		echo "Problemas con el envío a $destinatario,gabrielsandoval@aoacolombia.com,controloperativo@aoacolombia.com. ";
	}
}
...
function proceso_contacto_exitoso()
{
	...
	echo "<script language='javascript'>
		...
		{
			parent.agrandar();parent.activar_agenda();
			//modal('zcallcenter.php?Acc=proceso_adjudicacion&id=$id',0,0,600,900,'wadj');
		}
...
//	if($Aseguradora->pide_autorizacion)
//	{
//		echo "<td align='center'><a class='info' onclick='solicitar_autorizacion();'><img src='img/solicita_autorizacion.png' border='0'>Solicitar Autorización<span>Solicitar Autorización</span></a></td>";
//	}
		echo "<td align='center'>
			<a class='info' onclick='proceso_adjudicacion()'><img src='img/adjudicacion1.png' border='0'>
			<span style='width:200px'>Adjudicación</span><br>Adjudicación.</a></td>
		<td align='center'>
		<a class='info' onclick='proceso_compromiso()'><img src='img/compromiso.png' border='0'>
			<span style='width:200px'>Programar Compromiso.</span><br>Programar Compromiso.</a>
		</td>
...
class Pdispon
{
	...
	function pinta_celda_cita($Fecha,$Oficina,$Linea,$Flota,$LINK,$Solover)
	{
		...
		for($i=0;$i<count($Pyp);$i++)
		{
			if($Fecha>=$Pyp[$i]->Fecha_inicial && $Fecha<=$Pyp[$i]->Fecha_final )
			{
				...
				{ echo "<a class='info'><img src='gifs/picoplaca.png' border='0' height='26'><span>Pico y Placa</span></a>"; $Pico_y_placa=true;}
			}
		}
		if($Solover) echo "<div id='$id' style='visibility:visible'>";
		else echo "<div id='$id' style='visibility:hidden'>";
		...
		if($Permite)
		{
			if(!$Solover) echo "<select name='cita_".$Fecha.$this->Placa."' id='cita_".$Fecha.$this->Placa."'>";
			$Pinta=false;
			...
			for($H=$Hora_inicial;$H<=$Hora_final;$H++)
			{
				...
				if($Hora1>$h1)
				{
					if($Solover) { echo date('h:i A',strtotime($Fecha.' '.$Hora1));$Pinta=true;break; }
					else echo "<option value='$Hora1'>".date('h:i A',strtotime($Fecha.' '.$Hora1))."</option>";$Pinta=true;
				}
				//------------------  SEGUNDA MEDIA HORA ----------------------------------
				...
				if($Hora2<=$Hora_final)
				{
					if($Hora2>$h1)
					{
						if($Solover) { echo date('h:i A',strtotime($Fecha.' '.$Hora2));$Pinta=true;break; }
						else echo "<option value='$Hora2'>".date('h:i A',strtotime($Fecha.' '.$Hora2))."</option>";$Pinta=true;
					}
				}
				//------------------------------------------------------------------------
			}
			...
			else echo "<a class='info' style='cursor:pointer'><img src='gifs/standar/Cancel.png' border='0'><span style='width:100px'>No hay horario disponible para adjudicar</span></a>";
		}
		echo "</div>";
	}
	...
	function verifica_estado()
	{
		...
		$Ultimo=false;
		for($i=0; $i<count($Estado[$this->Id_vehiculo]);$i++)
		{
				if($this->Id_estado<$Estado[$this->Id_vehiculo][$i]->id)
				{
					$this->Id_estado=$Estado[$this->Id_vehiculo][$i]->id;
					$Ultimo=$Estado[$this->Id_vehiculo][$i];
				}
		}
		if($Ultimo)
		{
		...
		for($i=0; $i<count($Estado[$this->Id_vehiculo]);$i++)
		{
			if($Estado[$this->Id_vehiculo][$i]->Id_vehiculo==$this->Id_vehiculo && $Estado[$this->Id_vehiculo][$i]->Desde>date('Y-m-d'))
			{
				if($Estado[$this->Id_vehiculo][$i]->Estado==4 /*Mantenimiento*/ || $Estado[$this->Id_vehiculo][$i]->Estado==92 /*Mantenimiento programado*/ || $Estado[$this->Id_vehiculo][$i]->Estado==8) $this->Mantenimiento=1;
				if($Estado[$this->Id_vehiculo][$i]->Estado==5 /*Fuera de Servicio*/) $this->Fuera_servicio=1;
			}
		}
...
function proceso_adjudicacion()
{
	global $id,$Disp_flota,$Disp_aoa,$Estado,$Citado,$Pyp,$NUSUARIO;
...
function solicita_activacion()
{
	...
	$Mensaje1="<body>Solicitud de Activación Vehículo de AOA<br><br>Placa: <b>$Placa</b><br>Fecha: <b>$Fecha</b><br>".
		...
		"Para activar haga click aquí: <a href='http://www.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta1';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Autorizar</a></body>";

	$Mensaje2="<body>Solicitud de Activación Vehículo de AOA<br><br>Placa: <b>$Placa</b><br>Fecha: <b>$Fecha</b><br>".
		"Funcionario que solicita: $NUSUARIO <br>Siniestro: <b>$Sin->numero $Sin->asegurado_nombre</b><br>Ciudad: <b>$Ciudad->nombre ($Ciudad->departamento)</b><br>".
		"Para activar haga click aquí: <a href='http://www.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta2';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Autorizar</a></body>";

	$Mensaje3="<body>Solicitud de Activación Vehículo de AOA<br><br>Placa: <b>$Placa</b><br>Fecha: <b>$Fecha</b><br>".
		"Funcionario que solicita: $NUSUARIO <br>Siniestro: <b>$Sin->numero $Sin->asegurado_nombre</b><br>Ciudad: <b>$Ciudad->nombre ($Ciudad->departamento)</b><br>".
		"Para activar haga click aquí: <a href='http://www.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta3';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Autorizar</a></body>";
...
function solicita_reactivacion_ok()
{
	...
	"Fecha de autorización: $Sin->fec_autorizacion<br>";
		...
		$Mensaje.="Para activar haga click aquí: <a href='http://www.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta1';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Aprobar la modificación</a></body>";
		$Envio=enviar_gmail($Email_usuario /*de */,
		$_SESSION['Nombre'] /*Nombre de */ ,
		"gabrielsandoval@aoacolombia.com,Gabriel Sandoval;germangonzalez@aoacolombia.com,German Gonzalez;henrygonzalez@aoacolombia.com,Henry Gonzalez;arturoquintero@aoacolombia.com,Arturo Quintero Rodriguez" /*para */,
		"" /*con copia*/,
	...
	else
	{
		if($uno)
		{
			...
			q("update siniestro set estado=5 where id=$id ");
