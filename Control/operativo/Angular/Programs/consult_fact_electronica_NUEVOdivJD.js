...
app.controller("ConsultController", function ($scope,$http,$filter) {
	...
	$scope.init = function(){
		let request = $http.post("/Control/operativo/controllers/Fact_electronica_angular/Fact_electronica_angular_NUEVOdevJD.php",{acc:"get_fact_electronica",factura:findGetParameter("id")});
	...
	$scope.generate_xml = function(){
		window.open("/Control/operativo/controllers/Fact_electronica_angular/Fact_electronica_angular_NUEVOdevJD.php?XMLFACT="+$scope.fact_electronica.factura+"&XMLCONSE="+$scope.fact_electronica.ptesa_conse, '_blank');
	}
	
	$scope.graphic_representation = function(){
		document.querySelector(".loader").style.display = 'block';	
		let request = $http.post("/Control/operativo/controllers/Fact_electronica_angular/Fact_electronica_angular_NUEVOdevJD.php",{acc:"representacion_grafica",cufe:$scope.fact_electronica_estado.respuesta.cufe,tipo_documento:1});
		request.then(function(response){
	...
	$scope.resend_bill = function()
	{
		...
		{
			 request = $http.post("/Control/operativo/controllers/Fact_electronica_angular/Fact_electronica_angular_NUEVOdevJD.php",{acc:"enviar_factura",factura:$scope.fact_electronica.factura});
		
		}
		else{
			request = $http.post("/Control/operativo/controllers/Fact_electronica_angular/Fact_electronica_angular_NUEVOdevJD.php",{acc:"enviar_factura",factura:findGetParameter("id")});			
		}
		request.then(function(response){
			document.querySelector(".loader").style.display = 'none';
			if(response.data.status=="OK")
