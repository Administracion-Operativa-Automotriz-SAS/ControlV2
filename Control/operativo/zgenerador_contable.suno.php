...
function exportar_factura_uno() 
{
	...
	echo "<form action='zgenerador_contable.suno.php' target='_self' method='POST' name='forma' id='forma'>
	...
	//echo "select consecutivo,concat(consecutivo,' ',fecha_emision) from factura where fecha_emision between $interval_fecha1 order by id  desc limit 200"."<br>";
	echo "<body>
		<form action='zgenerador_contable.suno.php' target='_self' method='POST' name='forma' id='forma'>
...
function exportar_factura_uno_ok()
{
	...
	if($Facturas = q($sql_fechas))
	{
		...
		while($F=mysql_fetch_object($Facturas))
		{
			...
			if($F->siniestro)
			{
				$Siniestro=qo("select * from siniestro where id=$F->siniestro");
			...
			if($F->movilidad==0)
			{
				...
				if($Cliente->auto_retencion==1 && !$F->anulada)
				{
					...
					$I->Proyecto=$Proyecto;
					fwrite( $DD1,$I->genera());
					echo "<br>Tasa Base iva:|$I->Tasa_base_iva| ";
							...
							while ( $Cau = mysql_fetch_object( $Causacion ) )
							{
								...
								if($DVehiculos && !$F->anulada) // DISTRIBUCION ENTRE LOS VEHICULOS DE LA FLOTA 
								{
									...
									while($Dv=mysql_fetch_object($DVehiculos))
									{
										...
										echo "<li> $Dv->placa $Dv->centro_operacion ";
										$Acumulado+=$Cuota;
										...
										//$I->Centro_operacion_mov=$Ofic->centro_operacion;
										$I->Centro_operacion_mov=$Dv->centro_operacion;
										$I->Tercero_mov=$Cliente->identificacion;
										$I->Detalle1= l($Descripcion_factura,40);
										$I->Detalle2= substr($Descripcion_factura,40);
										$I->DC='C';
...
function preparacion_menus()
{
	...
	q("insert into usuario_tab (usuario,tipo,descripcion,destino,tabla) values (1,'INTERFASES S.UNO','Exportación de Facturas (Cuentas x Cobrar)','destino','zgenerador_contable.suno.php?Acc=exportar_factura_uno')");
