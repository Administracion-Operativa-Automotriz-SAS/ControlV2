...
function imprimir_acta() // imprime el acta de entrega/devolucion
{
	...
	$P->setfont('Arial','',9);
	$P->setxy(10,24);$P->cell(30,6,'PLACA ',1,0,'L');$P->cell(20,6,$Cita->placa,1);
	if($Siniestro->renta) $P->cell(40,6,'NUMERO SERVICIO ',1,0,'L'); else $P->cell(40,6,'NUMERO SINIESTRO ',1,0,'L');
	$P->cell(108,6,$Siniestro->numero.' '.$Siniestro->asegurado_nombre,1,0,'L');
	...
	if($Linea->delineacion_n) $P->Image($Linea->delineacion_n,187,169,15,30);
	{
		...
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y);$P->cell(60,$Incremento,"SERVICIO PRESTADO");$Y=$P->y+$Incremento;
		$P->setxy(70,($Y-4))+1;
		$P->setfont('Arial','',10);
		$Clase_servicios=q("select * from clase_servicio");
		while($cs=mysql_fetch_object($Clase_servicios))
		{
			$P->Cell(strlen($cs->nombre)*2.4,$Incremento,$cs->nombre);$P->cell(10,$Incremento,($Aseguradora->clase_servicio==$cs->id?" X ":"__"));
		}
		$P->setfont('Arial','B',10);
		$Y=$P->y+$Incremento;
		$P->setxy(20,$Y+1);$P->cell(60,$Incremento,"DATOS DEL USUARIO / ARRENDATARIO:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"CEDULA: ".coma_format($Autorizado_id) );$Y=$P->y+$Incremento;
		//$P->setxy(20,$Y);
		$P->cell(150,$Incremento,"NOMBRES Y APELLIDOS: $Autorizado_nombre");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"ASEGURADORA:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(150,$Incremento,"$Aseguradora->razon_social");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"DIAS DE SERVICIO:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(50,$Incremento,"$Siniestro->dias_servicio");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"OFICINA AOA QUE ATIENDE:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(40,$Incremento,"$Oficina->nombre");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->cell(43,$Incremento,"SINIESTRO O SERVICIO:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(50,$Incremento," $Siniestro->numero");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"TIPO DE GARANTIA: ");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(50,$Incremento,"$TG");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"VOUCHER No.: ");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(40,$Incremento,"$TGV");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->cell(50,$Incremento,"NUMERO DE CUOTAS:__");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"FECHA DE ENTREGA: ");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		;$P->cell(50,$Incremento,"".fecha_completa($Cita->fecha)." ".$Cita->hora);$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"FECHA DE DEVOLUCION: ");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(50,$Incremento,"".fecha_completa($Fdevol)." ".$Hdevol);$Y=$P->y+$Incremento;
		/*$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(90,$Incremento,"PICO Y PLACA: ");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);*/
		$P->setxy(20,$Y+1);
		$P->setfont('Arial','B',10);
		if($Aseguradora->limite_kilometraje == 0){$kilometrj="3000 Kilometros";}else{$kilometrj=coma_format($Aseguradora->limite_kilometraje);}
		$P->cell(50,$Incremento,"LIMITE DE KILOMETRAJE:     ".$kilometrj);$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"DOMICILIO ENTREGA:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(50,$Incremento,"SI __     NO__"			);$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"DOMICILIO DEVOLUCION:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(50,$Incremento,"SI __     NO__");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',10);
		$P->setxy(20,$Y+1);$P->cell(50,$Incremento,"COBERTURA ADICIONAL:");$Y=$P->y+$Incremento;
		$P->setfont('Arial','',10);
		$P->cell(50,$Incremento,"SI __     NO__");$Y=$P->y+$Incremento;
		$P->setfont('Arial','B',9);
        ...
        $P->setfont('Arial','B',8);
	    $P->setxy(10,261);$P->multicell(198,4,"____________________________________________________________________________________________________________________________",0,'C');
	    $P->setfont('Arial','',8);
	    $P->setxy(10,265);$P->multicell(198,4,"Carrera 69 B No. 98 A - 10 • PBX: +(571) 756 0510 • Fax: 756 0510 Ext. 112 • Bogotá D.C., Colombia • www.aoacolombia.com",0,'C');		
	}
...
function solicitar_factura() // formulario de solicitud de facturacion
{
	...
	echo "<script language='javascript'>
		...
		<form action='zcitas.php' method='post' target='_self' name='forma' id='forma'>
			...
			En caso de conocer el valor a ser cobrado al Usuario, por favor escriba o indique el valor que se
			debe facturar antes de impuestos:<input type='text' class='numero' name='valor'><br />
			<strong>NOTA: Los Proveedores de Régimen Simplificado no aplica IVA (impuesto a las ventas), sin embargo, AOA
			Colombia debe facturar el IVA sobre el valor cotizado por el proveedor , por tratarse de un servicio
			gravado con este impuesto de IVA del 19%.</strong>			
...
function solicitar_factura_test() // formulario de solicitud de facturacion
{
	global $cita;
	html('SOLICITUD DE FACTURA'); // pinta cabeceras html
	$Cita=qo("select * from cita_servicio where id=$cita"); // trae los datos de la cita
	...
	include("views/subviews/facturacion/solicitar_factura.html");
}

function datos_tarifa_aseg_concept()
{
	global $concepto,$aseguradora;
	$sql = "Select * from aoacol_aoacars.tarifa where concepto = '$concepto' and aseguradora = '$aseguradora' ";
	$tarifa = qo($sql);
	echo json_encode($tarifa);	
}

function concepto_fact_tarifa()
{
	global $concepto;
	$sql = "Select * from aoacol_aoacars.concepto_fac where id = '$concepto' ";
	$concepto = qo($sql);
