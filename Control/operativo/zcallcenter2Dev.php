...
$tokenPeril=$_SESSION['Nombre_Perfil'];
...
{
	global $USUARIO,$NUSUARIO,$NTusu,$NTsin,$NTap;
	...
	function tomar_nuevo()
	{
		if(!Tomando)
		{
			...
			window.open('zcallcenter2Dev.php?Acc=tomar_nuevo_caso','Oculto_perfil_agente');
		}
		else {alert('En este momento el sistema est· buscando un caso nuevo para ser tomado');}
	}
	function cargar_caso(id)
	{
		window.open('zcallcenter2Dev.php?Acc=presentar_caso&id='+id,'_self');
	}
	...
	function re_comenzar() {window.open('zcallcenter2Dev.php','_self');}
	function cerrar_caso(idc) {if(confirm('Desea cerrar el caso abierto? '+idc)) window.open('zcallcenter2Dev.php?Acc=cerrar_caso_abierto&idc='+idc,'Oculto_perfil_admin');}
	function recarga_accion() {window.open('zcallcenter2Dev.php?Acc=ver_accion_agentes&ag='+A_agentes,'Oculto_perfil_admin_recarga');}
	function tomar_renta(placa) {document.forma.placa.value=placa;}
	</script>
	<style tyle='text/css'>
		td.agente {font-size:9px;}
	</style>
	<body bgcolor='ffffff'><h3>AOA COLOMBIA S.A.  MODULO DE CALL CENTER 2 .:. Perfil: $Perfil->nombre Usuario: $NUSUARIO</h3>";
...
function modulo_perfil_administrador()
{
	...
	<table align='center' bgcolor='dddddd'>
		...
		<tr><td valign='top' bgcolor='ffffff'>
			<iframe name='cola1' id='cola1' style='visibility:visible' width='100%' height='500px' src='zcallcenter2Dev.php?Acc=pinta_cola1' frameborder='no'></iframe>
			<br>".admin_nivel(1)."</td>
			<td valign='top' bgcolor='ffffff'>
			<iframe name='cola2' id='cola2' style='visibility:visible' width='100%' height='500px' src='zcallcenter2Dev.php?Acc=pinta_cola2' frameborder='no'></iframe>
			".admin_nivel(3)."
			<iframe name='cola2a' id='cola2a' style='visibility:visible' width='100%' height='300px' src='zcallcenter2Dev.php?Acc=pinta_cola2a' frameborder='no'></iframe>
			</td><td valign='top' bgcolor='ffffff'>
			".admin_nivel(2)."
			<iframe name='cola3' id='cola3' style='visibility:visible' width='100%' height='250px' src='zcallcenter2Dev.php?Acc=pinta_cola3' frameborder='no'></iframe>
			</td>
			<td valign='top' bgcolor='ffffff'>".admin_nivel(4)."</td>
	</table>
	<a style='cursor:pointer' onclick=\"modal('zcall2estadistica.php',0,0,100,100,'call2estadistica');\">MÛdulo de EstadÌsticas</a>
...
function pinta_cola1()
{
	...
	function recargar() {document.getElementById('cargando').src='gifs/Loading.gif';window.open('zcallcenter2Dev.php?Acc=pinta_cola1','_self');}
	</script>
	...
	if($Cola1=q($sql))
	{
		...
		$Fecha='';$Bg='ffffff';$Bgr='ffbbbb';$Contador_fecha=0;$Contador_fecha_hoy=0;$Contador_infoerronea=0;
		...
		$A_singestion=explode(',',$Ids_singestion);$T_singestion=0;
		echo "<script language='javascript'>";
		$Ac_fecha=array();
		$Ac_fechat=array();
...
function pinta_cola2()
{
	...
	function ver_seguimiento(id) {modal('zsiniestro.php?Acc=ver_seguimiento&id='+id,0,0,600,900,'vs');}
	function ver_observaciones(id) {modal('zsiniestro.php?Acc=ver_observaciones&id='+id,50,50,600,900,'vo');}
	function recargar() {document.getElementById('cargando').src='gifs/Loading.gif';window.open('zcallcenter2Dev.php?Acc=pinta_cola2','_self');}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>";
	...
	if($Cola2=q($sql))
	{
		...
		<table border cellspacing='0' width='100%'><tr><th>#</th><th>Aseguradora</th><th>Siniestro</th><th>Hora</th><th>Opc</th></tr>";
		$Fecha='';$Bg='ffffff';$Bgr='ffbbbb';$Contador=0;
		...
		$A_aceptacion=array();
		while($Ac=mysql_fetch_object($Aceptaciones))	{$A_aceptacion[$Ac->siniestro]=$Ac->fec;}
...
function pinta_cola2a()
{
	...
	function ver_seguimiento(id) {modal('zsiniestro.php?Acc=ver_seguimiento&id='+id,0,0,600,900,'vs');}
	function ver_observaciones(id) {modal('zsiniestro.php?Acc=ver_observaciones&id='+id,50,50,600,900,'vo');}
	function re_enviar_correo(id) {modal('zcallcenter2Dev.php?Acc=caso_re_envia_correo_preadjudicacion&id='+id,0,0,600,900,'reenv');}
	function recargar() {document.getElementById('cargando').src='gifs/Loading.gif';window.open('zcallcenter2Dev.php?Acc=pinta_cola2a','_self');}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
	<h4>Correos de Pre-AdjudicaciÛn enviados</h4>";
	...
	{
		echo "<a style='cursor:pointer' onclick='recargar();'><img id='cargando' src='gifs/standar/Refresh.png' border='0' height='18px'></a>
		Numero de casos: ".mysql_num_rows($Cola2)." Mas de 2 DÌas: <span id='cantig'></span><br>
		<table border cellspacing='0' width='100%'><tr><th>#</th><th>Aseguradora</th><th>Siniestro</th><th>Hora</th><th>Opc</th></tr>";
		$Fecha='';$Bg='ffffff';$Bgr='ffbbbb';$Contador_c3=0;$Contador=0;$Contador_fecha=0;
		...
		while($C =mysql_fetch_object($Cola2))
		{
			if($Fecha!=date('Y-m-d',strtotime($C->envio)))
			{
				...
				$Bg=($Bg=='ffffff'?'ddddff':'ffffff');$sFec='f'.date('Ymd',strtotime($C->envio));
				echo "<tr><td align='center' colspan='5' bgcolor='$Bg'><b>$Fecha <span id='$sFec'></span></b></td></tr>";
				$Contador_fecha=0;
				
				
			}
			$Contador++;$Contador_fecha++;
			if(date('Y-m-d',strtotime($C->envio)) < $Fecha_cola2) {$Bg='E0AB77';$Contador_c3++;}
			echo "<tr ".($NTsin?"ondblclick='mod_sin($C->id);'":"")."><td align='center' bgcolor='$Bg'>$Contador</td>
...
function pinta_cola3()
{
	...
	function ver_seguimiento(id) {modal('zsiniestro.php?Acc=ver_seguimiento&id='+id,0,0,600,900,'vs');}
	function ver_observaciones(id) {modal('zsiniestro.php?Acc=ver_observaciones&id='+id,50,50,600,900,'vo');}
	function recargar() {document.getElementById('cargando').src='gifs/Loading.gif';window.open('zcallcenter2Dev.php?Acc=pinta_cola3','_self');}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>";
...
function admin_nivel($Nivel)
{
							...
							where u.estado=e.id and u.nivel=$Nivel and e.id not in (6) order by u.estado,u.nombre"))
	{
		...
		while($Ag =mysql_fetch_object($Agentes))
		{
			...
			if($Ag->estado==1) $Cantidad_activos++; else $Cantidad_inactivos++;
...
function modulo_perfil_agente()
{
	...
	$Escalafones=array();$Tescalafon=0;
	while($E=mysql_fetch_object($Escalafon)) {$Escalafones[$E->tipo]=$E->total;$Tescalafon+=$E->total;}
	...
	echo "<script>
		function send_siniestro()
		{
			...
			window.location.href = 'zcallcenter2Dev.php?Acc=presentar_caso&id='+siniestro;
		}
	</script>";
	...
	if($Caso_abierto && $Estado_Agente==1) // estado activo
	{
		echo "Redireccionando al caso abierto numero $Caso_abierto->siniestro
			...
			<script language='javascript'>window.open('zcallcenter2Dev.php?Acc=redireccionar_caso_abierto&id=$Caso_abierto->siniestro','Oculto_caso_abierto');</script></body>";
		die();
	}
	...
	<form action='zcallcenter2Dev.php' target='Oculto_perfil_agente' method='POST' name='forma' id='forma'>
	...
	<table cellspacing=4><tr><td align='center' bgcolor='dddddd'><a onclick='ver_apartados()' style='cursor:pointer;font-size:16px'><img src='img/vehiculo.png' border='0' height='60' align='bottom'><br>Ver vehÌculos apartados</a> </td>
...
function tomar_nuevo_caso()
{
	...
	if($Agente->nivel==2)
	{
		...
		if($Agente->nivel2==1)
		{
			...
			ELSE
			{
				...
				echo "<body><script language='javascript'>window.open('zcallcenter2Dev.php?Acc=tomar_nuevo_caso','_self');</script></body>";
			}
		}
		else
	...
	if($Agente->nivel==3)
	{
		...
		{
			$Hoy=date('Y-m-d H:i:s');$Fecha=date('Y-m-d');$Hora=date('H:i:s');
...
function toma_caso_nivel1($Temp,$Agente,$Ahora,$acumulado='',$aseguradora='')
{
	...
	{
		$Naseguradora='AOA';
		if($aseguradora==1 || $aseguradora==8 || $aseguradora==9) {$aseguradora='1,8,9';$Naseguradora='ALLIANZ';}
		elseif($aseguradora==2 || $aseguradora==5) {$aseguradora='2,5';$Naseguradora='RSA';}
		elseif($aseguradora==3 || $aseguradora==7) {$aseguradora='3,7';$Naseguradora='LIBERTY';}
		elseif($aseguradora==4 || $aseguradora==10) {$aseguradora='4,10';$Naseguradora='MAPFRE';}
	}
	include('inc/link.php');
	...
	if(!mysql_query("create table $Temp select s.id,s.ingreso,(0) as compromiso from siniestro s where s.estado=5
		and s.ingreso>='$Fecha_de_arranque' ".($aseguradora?" and s.aseguradora in ($aseguradora) and s.renta=0 ":""),$LINK)) { mysql_close($LINK); die('Error: '.mysql_error($LINK));}
	...
	$Ahora2=date('Y-m-d H:i:s');$idcompromiso=16;

	if($Agente->prioridad_nuevos) // si el agente tiene prioridad para solo nuevos
	{
		$Casos=mysql_query("select id from $Temp order by id",$LINK);$casos='0';while($Ca=mysql_fetch_object($Casos))$casos.=','.$Ca->id;
	...
	{
		if($acumulado) q("update usuario_callcenter set $acumulado=$acumulado+1 where id=$IDUSUARIO"); // incrementa el acumulador
		echo "<body><script language='javascript'>if(confirm('No hay casos nuevos para tomar en nivel 1. Desea intentar nuevamente?')){window.open('zcallcenter2Dev.php?Acc=tomar_nuevo_caso','_self');} else{parent.re_comenzar();}</script></body>";
		die();
	}
...
{
	// Esta rutina debe ser la que llama el modulo de voz ip , debe recibir el id del siniestro ($idsiniestro), y la extesiÛn marcada ($extension)
	// url automatica en VOZ IP: https://app.aoacolombia.com/Control/operativo/zcallcenter2Dev.php?Acc=toma_caso_vozip
...
&did_pattern=&did_description=&closecallid=&xfercallid=&agent_log_id=126&entry_list_id=2000&call_id=V
	...
	*/
	...
	if($AgenteCall=qo("select * from usuario_callcenter where extension='$extension' limit 1"))
	{
		...
		{
			//Agrege nueva condiciÛn para no cambiar como funciona el programa sin embargo es necesario revisar por que crea el caso dos veces
			//echo "new condition";
		...
		{	
			if($ID=q("insert into call2proceso (fecha,agente,siniestro,estado) values ('$Hoy','$idagente','$idsiniestro','A')"))
			{
				...
				echo "<body><script language='javascript'>window.open('zcallcenter2Dev.php?Acc=presentar_caso&id='+$idsiniestro,'_self');</script></body>"; // carga el caso para el agente de call center
			}
			else
			{
				html();
	...
	else
	{
		html();
		echo "<body><h3>FALLO EN REDIRECCION VOZ IP</h3>
			Error: No se encuentra el agente correspondiente a la extensiÛn. <br><br>
			Id Siniestro: $idsiniestro  <br>
			ExtensiÛn $extension
		</body>";
	}
...
function buscar_caso()
{
	...
	$Caso=false;$Encontrado=false;
	if($placa)
	{
		if($Caso=qo("select * from siniestro where placa = '$placa' order by ingreso desc"))	$Encontrado=true;
		else echo "<body><script language='javascript'>alert('No hay informaciÛn');</script></body>";
	}
	if($siniestro)
	{
	...
	if($Caso && $Encontrado)
	{
		...
		if($Caso->estado==5 /* Pendiente*/)
		{
			...
			if($ID=q("insert into call2proceso (fecha,agente,siniestro,estado) values ('$Hoy','$IDUSUARIO','$Caso->id','A')"))
			{
				...
				{ $_SESSION['nivelcaso']=1;q("delete from call2cola1 where siniestro=$Caso->id");}
				if($Idcaso=qo1("select id from call2cola2 where siniestro=$Caso->id"))
				{ $_SESSION['nivelcaso']=2;q("delete from call2cola1 where siniestro=$Caso->id");}
			}
		}
...
function presentar_caso()
{
	...
	html();
	echo "<script language='javascript'>
	function recargar() {window.open('zcallcenter2Dev.php?Acc=presentar_caso&id=$id','_self');}
	...
	if(alltrim(Obs)){
		window.open('zcallcenter2Dev.php?Acc=adicionar_observaciones&id=$id&d='+Obs+'&tipoCaja='+tipoCaja,'Oculto_presentar_caso');
		}
	}
	
	function re_comenzar() {window.open('zcallcenter2Dev.php','_self');}
	function solicitar_reactivacion() {window.open('zcallcenter2Dev.php?Acc=solicita_reactivacion&id=$id','_self');}
	function cambia_oficina_caso(dato) {if(confirm('Desea cambiar de oficina?')) window.open('zcallcenter2Dev.php?Acc=cambiar_ciudad_caso&id=$id&nueva_oficina='+dato,'Oculto_presentar_caso');}
	function asignar_transporte_vip() {window.open('zcallcenter2Dev.php?Acc=asignar_transporte_vip&id=$id','_self');}
	</script>";
  ...
  .botonRegistro:hover{
	...
	<a href='https://app.aoacolombia.com/Control/operativo/zcallcenter2Dev.php?Acc=pide_datos_financieros&ids=$Sin->id&cerrando=1&sinG1=$va'>
...
' row='50' cols='50'>$Ofic->picoyplaca</textarea></td>
	...
	<hr><iframe name='Control_caso' id='Control_caso' style='visibility:visible' width='800' height='180' frameborder='no' src='zcallcenter2Dev.php?Acc=control_caso&id=$id&estado=$Sin->estado'></iframe>
...
function control_caso()
{
	global $id,$USUARIO,$NUSUARIO,$IDUSUARIO,$estado,$tokenNick,$tokenPeril,$tokenUser;
        ...
        echo"select v.tipo_caja,v.placa,s.n_poliza,s.aseguradora_nombre,s.linea_asistencia,l.nombre nlinea
							FROM vehiculo v
							inner join seguros s on v.n_poliza = s.id
							inner join linea_vehiculo l on v.linea = l.id  
							where v.id=$ubicacion->vehiculo ";		
	...
	echo "<script language='javascript'> 
		 function ingresar_novedad(dato) {if(confirm('Desea ingresar una novedad del siniestro $id?')) 
			    window.open('https://app.aoacolombia.com/conVue/?#/home&url=$url_base&ciudad_siniestro=$Ofic->nombre&declarante_ciudad=$Ciuorig&declarante_email=$Sin->declarante_email&declarante_telefono=$Sin->declarante_telefono&declarante_nombre=$Sin->declarante_nombre&tokenNick=$tokenNick&siniestro=$Sin->numero&tokenPeril=$Perfil->idnombre&tokenUser=$NUSUARIO');
	...
	IF($Agente->nivel==1)
	{
		echo "<script language='javascript'> 
				...
				function reactivacion() { window.open('zcallcenter2.php?Acc=reactivar_caso&id=$id','_self');}
		...
		if($estado==5 /* Pendiente */)
		{
			if($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='0' and estado='0' "))   // SI ESTA EN PRE-ADJUDICACION Y NO HA ACEPTADO
				echo "<td colspan=3><h3>Este caso se encuentra en estado de espera de aceptaciÛn de TÈrminos y Condiciones.</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_email();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>ActualizaciÛn del correo electrÛnico</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($Cola2->id);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Reenviar el correo de Pre-AdjudicaciÛn</a></td>
			...
			elseif($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='1' and estado='0' "))  // SI ESTA EN PRE-ADJUDICACION PERO YA ACEPTO
				echo "<td colspan=2><h3>Este caso se encuentra en cola nivel 2 - AdjudicaciÛn del VehÌculo.</h3></td></tr><tr>
			...
			elseif($Cola3=qo("select * from call2cola3 where siniestro=$id"))  // SI ESTA EN COLA3
				echo "<td colspan=2><h3>Este caso se encuentra en cola nivel 3 - AdjudicaciÛn del VehÌculo.</h3></td></tr><tr>
			...
			else  // SI SIGUE PENDIENTE EN COLA 1
			{
				echo "<td bgcolor='ffffff' align='center'><a onclick='contacto_exitoso();' style='cursor:pointer'><img src='img/contacto_call_asegurado.png' height='80'><br>Contacto Exitoso</a></td>
					...
					<td bgcolor='ffffff' align='center'><a onclick='buzon();' style='cursor:pointer'><img src='img/buzon.png' height='80'><br>BuzÛn de Voz</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='informacion_erronea();' style='cursor:pointer'><img src='img/informacion_erronea.png' height='80'><br>InformaciÛn ErrÛnea</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='ingresar_novedad();' style='cursor:pointer'><img src='http://app.aoacolombia.com/img/Car-Repair-icon.png' height='80'><br>Ingresar novedad </a></td>";
				...
				if(($Cantidad_buzones>3 && !$Contacto_exitoso) || ($Cantidad_buzones>6))
					echo "<td bgcolor='ffffff' align='center'><a onclick='no_adjudica();' style='cursor:pointer'><img src='img/no_adjudicacion.png' height='80'><br>No AdjudicaciÛn<br>($Cantidad_buzones buzones)</a></td>";
				else
				...
				echo "<td bgcolor='ffffdd' align='center'><a onclick='marcacion_numero_alterno();' style='cursor:pointer'><img src='img/cambio_llamada.png' height='80'><br>Marcar N˙mero Alterno</a></td>";
			}
		...
		if($estado==1)
		{
			...
			<a onclick='solicitar_reactivacion();' style='cursor:pointer;font-size:14px;'><img src='img/reactivacion3.png' border='0' height='80'>
			<br>Solicitar Re-ActivaciÛn de Caso</a></td>
			<td bgcolor='ffffff' align='center'><a onclick='ingresar_novedad();' style='cursor:pointer'><img src='http://app.aoacolombia.com/img/Car-Repair-icon.png' height='80'><br>Ingresar novedad </a></td>";
		}
		echo "</tr></table></body>";
	}
	...
	echo "<script language='javascript'>
				...
				window.open('zcallcenter2Dev.php?Acc=cerrar_caso_abierto&idc='+idc,'Oculto_perfil_admin');
				 ...
				 function ingresar_novedad(dato) {
					 ...
					 if(confirm('Desea  ineegresar una novedad del siniestro $vehiculoData->placa $id?')) 
		...
		if($Sin->estado==5 /* Pendiente */)
		{
			if($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='0' and estado='0' "))
				echo "<td colspan=2><h3>Este caso se encuentra en estado de espera de aceptaciÛn de TÈrminos y Condiciones.</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_email();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>ActualizaciÛn del correo electrÛnico</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($Cola2->id);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Reenviar el correo de Pre-AdjudicaciÛn</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='ingresar_novedad();' style='cursor:pointer'><img src='http://app.aoacolombia.com/img/Car-Repair-icon.png' height='80'><br>Ingresar novedad </a></td>";
			...
			else
			{
				...
				echo "<td bgcolor='ffffff' align='center'><a onclick='asignar_vehiculo();' style='cursor:pointer'><img src='img/adjudicacion1.png' height='60'><br>Adjudicar Vehiculo</a></td>
					...
					<td bgcolor='ffffff' align='center'><a onclick='remarcacion();' style='cursor:pointer'><img src='img/buzon.png' height='60'><br>BuzÛn de Voz - RemarcaciÛn</a></td>
				...
				if($idc=qo1("select id from call2cola2 where siniestro=$id"))
					echo "<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($idc);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='60'><br>Reenviar el correo de Pre-AdjudicaciÛn</a></td>";
				else
					echo "<td bgcolor='ffffff' align='center'><a onclick='preadjudica();' style='cursor:pointer'><img src='img/preadjudicacion.png' height='60'><br>Reenviar el correo de Pre-AdjudicaciÛn</a></td>";
			//	if($_SESSION['nivelcaso']==1)
				echo "<td bgcolor='ffffff' align='center'><a onclick='actualizar_datos();' style='cursor:pointer'><img src='img/actualizacion.png' height='60'><br>ActualizaciÛn de datos</a></td>
								<td bgcolor='ffffff' align='center'><a onclick='informacion_erronea();' style='cursor:pointer'><img src='img/informacion_erronea.png' height='60'><br>InformaciÛn ErrÛnea</a></td>";
				// NUEVO BOTON MARCAR NUMERO ALTERNO
				echo "<td bgcolor='ffffdd' align='center'><a onclick='marcacion_numero_alterno();' style='cursor:pointer'><img src='img/cambio_llamada.png' height='80'><br>Marcar N˙mero Alterno</a></td>";
			}
			echo "<td bgcolor='ffffff' align='center'><a onclick='no_adjudica();' style='cursor:pointer'><img src='img/no_adjudicacion.png' height='80'><br>No AdjudicaciÛn</a></td>";
			echo "</tr></table>
					<iframe name='Oculto_asignacion' id='Oculto_asignacion' style='visibility:hidden' width='1' height='1'></iframe>";
		}
		elseif($Sin->estado==3 /* Adjudicado */ )
		{
			echo "<td colspan=2><h3>Este caso se encuentra Adjudicado</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_infofin();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>ActualizaciÛn de la InformaciÛn Financiera</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='ingresar_novedad();' style='cursor:pointer'><img src='http://app.aoacolombia.com/img/Car-Repair-icon.png' height='80'><br>Ingresar novedad </a></td>";
		}
		
		elseif($Sin->estado==7 /* SERVICIO */ )
		{
		$A=qo("select * from usuario_callcenter where   id=$IDUSUARIO");
		...
		elseif($Sin->estado==1 /* Pendiente */)
		{
			...
			NUMERO DE DIAS DEL CASO: <B>$Dias</B>.</td><td bgcolor='ffffff' ALIGN='CENTER'>
			<a onclick='solicitar_reactivacion();' style='cursor:pointer;font-size:14px;'><img src='img/reactivacion3.png' border='0' height='80'><br>Solicitar Re-ActivaciÛn de Caso</a></td>
				<td bgcolor='ffffff' align='center'><a onclick='ingresar_novedad();' style='cursor:pointer'><img src='http://app.aoacolombia.com/img/Car-Repair-icon.png' height='80'><br>Ingresar novedad </a></td>
			";
		}
		else
	...
	if($Agente->nivel>=3)
	{
		...
		echo "<script language='javascript'>
                ...
                function ingresar_novedad(dato) {if(confirm('Desea ingresar una novedad del siniestro $id?')) 
			    window.open('https://app.aoacolombia.com/conVue/?#/home?tokenNick=$tokenNick&tokenPeril=$Perfil->idnombre&tokenUser=$NUSUARIO');}
		...
		{
			if($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='0' and estado='0' "))
				echo "<td colspan=6><h3>Este caso se encuentra en estado de espera de aceptaciÛn de TÈrminos y Condiciones.</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_email();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>ActualizaciÛn del correo electrÛnico</a></td>
					...
					<td bgcolor='ffffff' align='center'><a onclick='remarcacion();' style='cursor:pointer'><img src='img/buzon.png' height='80'><br>BuzÛn de Voz - RemarcaciÛn</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($Cola2->id);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Reenviar el correo de Pre-AdjudicaciÛn</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='aceptacion_condiciones($Cola2->id);' style='cursor:pointer'><img src='img/aceptacionterminos.png' height='80'><br>AceptaciÛn de TÈrminos y Condiciones</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='ingresar_novedad();' style='cursor:pointer'><img src='http://app.aoacolombia.com/img/Car-Repair-icon.png' height='80'><br>Ingresar novedad </a></td>";
			else
			{
...
function asignar_transporte_vip2()
{
	...
	include('inc/gpos.php');
	$Codigo_seguimiento=29; // AdjudicaciÛn de Servicio VIP
	...
	echo "<body><script language='javascript'>window.open('zcallcenter2Dev.php','destino');</script></body>";
}
...
function caso_actualizar_email()
{
	...
	function validar_actualizacion()
	{
		with(document.forma)
		{
			if(!alltrim(correo.value)) { alert('Debe digitar el correo electrÛnico del asegurado');correo.style.backgroundColor='ffff99';correo.focus();return false;}
			submit();
		}
	}
	function finalizar()
	...
	<form action='' target='Oculto_actualizacion' method='POST' name='forma' id='forma'>
		Correo electrÛnico: <input type='text' name='correo' id='correo' value='$Sin->declarante_email' size='80' maxlength='200'><br>
...
function caso_info_erronea()
{
	...
	echo "<script language='javascript'>
		function enviar_mensaje()
		{
			...
			{
				alert('Debe seleccionar la tipificaciÛn correcta para este caso'); document.forma.tipificacion.style.backgroundColor='ffff44';document.forma.tipificacion.focus();return false;
			}
		...
		<form action='zcallcenter2Dev.php' method='post' target='Oculto_info_erronea' name='forma' id='forma'>
			La informaciÛn erronea es: <input type='text' name='info_erronea1' value='El n˙mero telefÛnico o celular' size=80><br>
			Seleccione la tipificaciÛn correcta para este caso: ".menu1("tipificacion","select id,nombre from tipifica_seguimiento where tipo='INFORMACION ERRONEA' ",0,1)."
			<input type='hidden' name='Acc' id='Acc' value='caso_info_erronea_ok'><br><br>";
	if($Siniestro->declarante_email)	echo "<input type='checkbox' name='email_inicial'> <b style='color:blue;font-size:14px;'>Enviar autom·ticamente un Correo Inicial a la direcciÛn $Siniestro->declarante_email </b><br><br>";
...
function caso_info_erronea_ok()
{
	...
	if($email_inicial)
	{
		...
		$Correo="
		Respetado(a) SeÒor(a) $Sin->asegurado_nombre

		Reciba cordial saludo.

		Somos AdministraciÛn Operativa Automotriz S.A. la empresa que le proporciona el $Aseguradora->nombre_servicio de $Aseguradora->razon_social.

		Usted tuvo un siniestro con su vehÌculo de placas $Sin->placa con el n˙mero $Sin->numero.

		$Aseguradora->razon_social le brinda a Usted el beneficio de disponer de un $Aseguradora->nombre_servicio por un tiempo determinado y AdministraciÛn Operativa Automotriz S.A. necesita contactarlo para bindarle este servicio.

		En vista de que la informaciÛn que tenemos para contactarlo no ha sido efectiva, le pedimos el favor de que nos proporcione un n˙mero de celular o un n˙mero fijo a donde nuestro Call Center pueda contactarlo y explicarle todo el proceso para acceder al servicio.

		La informaciÛn que nos proporcionÛ la aseguradora es la siguiente:

		Nombre: $Sin->asegurado_nombre  ".
		($Sin->declarante_telefono?" TelÈfono del declarante: $Sin->declarante_telefono":"").
		($Sin->declarante_celular?" Celular del declarante: $Sin->declarante_celular":"").
		($Sin->declarante_tel_resid?" TelÈfono de residencia del declarante: $Sin->declarante_tel_resid":"").
		($Sin->declarate_tel_otro?" TelÈfono adicional del declarante: $Sin->declarate_tel_otro":"").
		($Sin->declarate_email?" Email del declarante: $Sin->declarate_email":"").
		"
		Ciudad :".($Ciuo?$Ciuo:$Ciu)."
		Fecha de AutorizaciÛn: $Sin->fec_autorizacion

		Por favor puede enviarnos sus datos actualizados al correo siniestros@aoacolombia.com o llamar a nuestro Pbx en Bogot· D.C. al n˙mero $Aseguradora->numero_call .

		Agradecemos su atenciÛn.

		Cordialmente,

		DirecciÛn Call Center
		AdministraciÛn Operativa Automotriz S.A.
		Carrera 69B N˙mero 98A-10 Barrio Morato Bogot· D.C.
		Pbx. 7560510 Fax 7560512
		www.aoacolombia.com

		";
		$Email_usuario=usuario('email');
		$Envio=enviar_gmail("direccioncallcenter@aoacolombia.com" /*de */,
											"DirecciÛn Call Center AOA" /*Nombre de */ ,
											"$Sin->declarante_email,$Sin->declarante_nombre" /*para */,
											"$Email_usuario,$NUSUARIO" /*con copia*/,
											"AOA COLOMBIA S.A. $Aseguradora->nombre_servicio" /*Objeto */,
											nl2br($Correo));
...
function caso_contacto_exitoso()
{
	...
	html();
	echo "<script language='javascript'>
		function no_adjudica() { window.open('zcallcenter2Dev.php?Acc=caso_no_adjudica&id=$id','_self'); }
		function preadjudica() { window.open('zcallcenter2Dev.php?Acc=caso_preadjudica&id=$id','_self'); }
		function compromiso() { window.open('zcallcenter2Dev.php?Acc=caso_compromiso&id=$id','_self'); }
		</script>
		<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<table align='center' cellspacing='10' bgcolor='ffffff'><tr>
		<td bgcolor='ffffff' align='center'><a onclick='preadjudica();' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Solicitud de AdjudicaciÛn</a></td>
		<td bgcolor='ffffff' align='center'><a onclick='no_adjudica();' style='cursor:pointer'><img src='img/no_adjudicacion.png' height='80'><br>No AdjudicaciÛn</a></td>
		<td bgcolor='ffffff' align='center'><a onclick='compromiso();' style='cursor:pointer'><img src='img/compromiso.png' height='80'><br>Compromiso</a></td>
		</tr></table>
	</body>";
}
...
function caso_preadjudica()
{
		...
		{ with(document.forma)
			{
				...
				if(confirm('Est· seguro de Solicitar la Adjudicacion?'))
	...
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2Dev.php' method='post' target='Oculto_preadj' name='forma' id='forma'>
			<H4>SOLICITUD DE ADJUDICACION DE VEHÕçCULO</H4>
			Se enviar· un correo electrÛnico con la informaciÛn de requisitos y el anexo de tÈrminos y condiciones.<br><br>
			Correo electrÛnico del asegurado: <input type='text' name='correo_asegurado' id='correo_asegurado' size='80' value='$D->declarante_email'><br><br>
			<input type='checkbox' name='inmediato'> Marque esta casilla en caso de que el asegurado requiera la adjudicaciÛn inmediata sin leer el correo de pre-adjudicaciÛn.<br>
...
function caso_preadjudica_ok()
{
	...
	if(q("select id from call2cola2 where codigo='$Codigo' ")){$Codigo=round(rand($codigo_inicial,$codigo_final),0);$Codigo=str_pad($Codigo,$Longitud,'0',STR_PAD_LEFT);}
	if(q("select id from call2cola2 where codigo='$Codigo' ")){$Codigo=round(rand($codigo_inicial,$codigo_final),0);$Codigo=str_pad($Codigo,$Longitud,'0',STR_PAD_LEFT);}
	if(q("select id from call2cola2 where codigo='$Codigo' ")){$Codigo=round(rand($codigo_inicial,$codigo_final),0);$Codigo=str_pad($Codigo,$Longitud,'0',STR_PAD_LEFT);}
	if(q("select id from call2cola2 where codigo='$Codigo' ")){$Codigo=round(rand($codigo_inicial,$codigo_final),0);$Codigo=str_pad($Codigo,$Longitud,'0',STR_PAD_LEFT);}
	if(q("select id from call2cola2 where codigo='$Codigo' ")){$Codigo=round(rand($codigo_inicial,$codigo_final),0);$Codigo=str_pad($Codigo,$Longitud,'0',STR_PAD_LEFT);}
	...
	if(q("select id from call2cola2 where codigo='$Codigo' ")){$Codigo=round(rand($codigo_inicial,$codigo_final),0);$Codigo=str_pad($Codigo,$Longitud,'0',STR_PAD_LEFT);}
	if(q("select id from call2cola2 where codigo='$Codigo' ")){$Codigo=round(rand($codigo_inicial,$codigo_final),0);$Codigo=str_pad($Codigo,$Longitud,'0',STR_PAD_LEFT);}
	$Ahora=date('Y-m-d H:i:s');$Fecha=date('Y-m-d');$Hora=date('H:i:s');$Codigo_seguimiento=17; /*17: Correo pre-adjudicacion */
	...
	if($Envio=enviar_gmail($Email_usuario /*de */ ,$NUSUARIO /*nombre de */ ,	"$correo_asegurado,$Sin->asegurado_nombre" /*para */ ,
		"direccioncallcenter@aoacolombia.com,DIRECCION CALL CENTER"   /*Con copia*/ ,	"SOLICITUD VEHICULO DE REEMPLAZO"  /*OBJETO*/,
		nl2br("
		...
		Este es un correo automatico del AÅrea de Call Center cuyo objetivo es facilitar el acceso a nuestra plataforma para que
		...
		") /*mensaje */))
	{
		...
		if($inmediato)
		{
			...
			graba_bitacora('call2cola2','A',$Idn,'Adiciona Registro con aceptacion de tÈrminos y condiciones');
		}
		...
		guarda_escalafon(2);
		echo "<body><script language='javascript'>alert('EnvÌo del correo satisfactorio.');parent.recargar();</script></body>";
	}
	else
	{
		echo "<body><script language='javascript'>parent.getElementById('mensaje').innerHTML='NO SE PUDO ENVIAR EL CORREO, por favor verifique la informaciÛn e intentelo nuevamente.';
...
function caso_re_envia_correo_preadjudicacion()
{
	...
	<form action='zcallcenter2Dev.php' target='_self' method='POST' name='forma' id='forma'>
...
function caso_re_envia_correo_preadjudicacion_ok()
{
	...
	$Ase=qo("select nombre,razon_social,info_per,numero_call from aseguradora where id=$Sin->aseguradora");
	if($Envio=enviar_gmail($Email_usuario /*de */ ,$NUSUARIO /*nombre de */ ,	"$email,$Sin->asegurado_nombre" /*para */ ,
		"direccioncallcenter@aoacolombia.com,DIRECCION CALL CENTER"   /*Con copia*/ ,	"SOLICITUD VEHÕçCULO DE REEMPLAZO"  /*OBJETO*/,
		nl2br("
		SeÒor(a) Asegurado(a) $Sin->asegurado_nombre,
		...
		servicio incluyendo la validacion del proceso de garantias; informacion que corresponde a datos personales
		como son nombres, apellidos, numero de identificacion, telefono (s), direccion(es), correo(s) electronico(s), ciudad,
		informacion de tarjeta(s) de crÈdito y cuenta(s) bancaria(s) (cuando apliquen) entre otros.
		...
		") /*mensaje */))
	{
		...
		q("update call2proceso set fecha_cierre='$Ahora',estado='C' where agente='$IDUSUARIO' and siniestro='$Cola->siniestro' and estado='A' ");
		echo "<body><script language='javascript'>alert('EnvÌo del correo satisfactorio.');opener.re_comenzar();window.close();void(null);</script></body>";
	}
}
	...
	echo "<script language='javascript'>
		function validar_envio_compromiso()
		{
			with(document.forma)
			{
				...
				if(confirm('Est· seguro de crear el Compromiso?')) submit();
			}
		}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2Dev.php' method='post' target='_self' name='forma' id='forma'>
			<h4>REGISTRO DE COMPROMISO DE LLAMADA A FUTURO ($Cantidad_eventos eventos)</H4>
			Seleccione la tipificaciÛn seg˙n el caso: ".menu1("tipificacion","select id,nombre from tipo_compromiso where activo=1",0,1)."<br>
			Fecha del compromiso: <select name='fechacompromiso'>";
			$Hoy=date('Y-m-d');
			for($i=1;$i<20;$i++) { $nd=ndiasemana($Hoy);echo "<option value='$Hoy' ".($nd=='S·bado' || $nd=='Domingo'?"style='background-color:ffdddd'":"").">$Hoy [$nd]</option>";$Hoy=date('Y-m-d',strtotime(aumentadias($Hoy,1)));}
...
function caso_compromiso_ok()
{
	...
	$Fecha=date('Y-m-d');$Hora=date('H:i:s');$Codigo=16; /*16: Compromiso */
	$Idn=q("insert into seguimiento (siniestro,usuario,fecha,hora,descripcion,tipo,tipo_compromiso,fecha_compromiso) values ($id,'$NUSUARIO','$Fecha','$Hora','COMPROMISO: $fechacompromiso $horacompromiso ',$Codigo,'$tipificacion','$fechacompromiso $horacompromiso')");
	//graba_bitacora('seguimiento','A',$Idn,'Adiciona Registro');
	q("update siniestro set info_erronea = '',observaciones=concat(observaciones,\"\n$NUSUARIO [$Ahora]: COMPROMISO: $observaciones. ($Ntipificacion)\") where id=$id");
...
function caso_compromiso2()
{
	...
	echo "<script language='javascript'>
		...
		function busca_conteo()
		{
			...
			window.open('zcallcenter2Dev.php?Acc=busca_conteo_tipificacion&siniestro=$id&tipo='+Tipo,'Oculto_compromiso');
		}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2Dev.php' method='post' target='_self' name='forma' id='forma'>
			<h4>REGISTRO DE COMPROMISO DE ASIGNACION DE VEHÕçCULO A FUTURO ($Cantidad_eventos eventos)</H4>
			Seleccione la tipificaciÛn seg˙n el caso: ".menu1("tipificacion","select id,nombre from tipo_compromiso where activo=1",0,1,''," onchange='busca_conteo();' ")." <span id='conteo'></span><br>
			Fecha del compromiso: <select name='fechacompromiso'>";
			$Hoy=date('Y-m-d');
			for($i=1;$i<20;$i++) { $nd=ndiasemana($Hoy);echo "<option value='$Hoy' ".($nd=='S·bado' || $nd=='Domingo'?"style='background-color:ffdddd'":"").">$Hoy [$nd]</option>";$Hoy=date('Y-m-d',strtotime(aumentadias($Hoy,1)));}
			echo"</select> Hora: ".pinta_hora('forma','horacompromiso',date('H:i:s'))."<br>
...
function caso_compromiso2_ok()
{
	...
	{
		if($idc1=qo1("select id from call2cola1 where siniestro=$id")){	q("update call2cola1 set fecha='$fechacompromiso $horacompromiso' where id=$idc1");}
		else {q("insert into call2cola1(siniestro,fecha,compromiso) values ('$id','$fechacompromiso $horacompromiso',1)");}
		$Idcc=qo1("select id from call2cola1 where siniestro=$id");
		graba_bitacora('call2cola1','M',$Idcc,'Graba Compromiso 2. Cola 1');
	}
	else
	{
		if($idc2=qo1("select id from call2cola2 where siniestro=$id")) {q("update call2cola2 set fecha_aceptacion='$fechacompromiso $horacompromiso',estado='0' where id=$idc2");}
		else {q("insert into call2cola2(siniestro,fecha,estado) values ('$id','$fechacompromiso $horacompromiso',0)");}
...
function caso_compromiso3()
{
	...
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2Dev.php' method='post' target='_self' name='forma' id='forma'>
			<h4>REGISTRO DE COMPROMISO DE ASIGNACION DE VEHÕçCULO A FUTURO</H4>
			Seleccione la tipificaciÛn seg˙n el caso: ".menu1("tipificacion","select id,nombre from tipo_compromiso where activo=1",0,1,''," onchange='busca_conteo();' ")." <span id='conteo'></span><br>
			Fecha del compromiso: <select name='fechacompromiso'>";
			$Hoy=date('Y-m-d');
			for($i=1;$i<20;$i++) { $nd=ndiasemana($Hoy);echo "<option value='$Hoy' ".($nd=='S·bado' || $nd=='Domingo'?"style='background-color:ffdddd'":"").">$Hoy [$nd]</option>";$Hoy=date('Y-m-d',strtotime(aumentadias($Hoy,1)));}
...
function caso_contacto_tercera()
{
	...
	echo "<script language='javascript'>
		function compromiso() { window.open('zcallcenter2Dev.php?Acc=caso_compromiso&id=$id','_self'); }
		function buzon_tercera_persona() { window.open('zcallcenter2Dev.php?Acc=caso_buzon_tercera_persona&id=$id','_self'); }
		function actualizar_datos() {modal('zcallcenter2Dev.php?Acc=caso_actualizar_datos&id=$id','actualizar');}
		function re_comenzar() {parent.re_comenzar();}
		</script>
		<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<table align='center' cellspacing='10' bgcolor='ffffff'><tr>
		...
		<td bgcolor='ffffff' align='center'><a onclick='compromiso();' style='cursor:pointer'><img src='img/compromiso.png' height='80'><br>Compromiso</a></td>
		<td bgcolor='ffffff' align='center'><a onclick='actualizar_datos();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>ActualizaciÛn de datos</a></td>
		</tr></table>
	</body>";
}
...
function caso_buzon_voz()
{
	...
	echo "<script language='javascript'>
		function validar_envio_buzon()
		{
			with(document.forma)
			{
				if(!tipificacion.value) {alert('Debe seleccionar un tipo de buzÛn');tipificacion.style.backgroundColor='ffffaa';tipificacion.focus();return false;}
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Est· seguro de crear el Compromiso?')) submit();
			}
		}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'><h4>BuzÛn de Voz ($Cantidad_eventos eventos)</h4>
		<form action='zcallcenter2Dev.php' method='post' target='_self' name='forma' id='forma'>
			Seleccione la tipificaciÛn seg˙n el caso:
	...
	echo "</optgroup></select><br><br>
			<br>Observaciones: <input type='text' name='observaciones' id='observaciones' size='80'><br>
			<br>Cerrar el caso: <input type='checkbox' name='CERRAR_caso'>  Marque esta casilla si no hay mas n˙meros para marcar.<br><br>
...
function caso_buzon_ok()
{
	...
	$Fecha=date('Y-m-d');$Hora=date('H:i:s');$Codigo=12; /*12: Mensaje en BuzÛn de voz */
	$Idn=q("insert into seguimiento (siniestro,usuario,fecha,hora,descripcion,tipo,tipificacion) values ($id,'$NUSUARIO','$Fecha','$Hora','Se deja mensaje en buzÛn de voz.',$Codigo,'$tipificacion')");
	//graba_bitacora('seguimiento','A',$Idn,'Adiciona Registro');
...
function caso_buzon_tercera_persona()
{
	...
	echo "<script language='javascript'>
		function validar_envio_buzon()
		{
			...
			{
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Est· seguro de guardar el Mensaje?')) submit();
			}
		}
	</script>
...
function caso_no_adjudica()
{
	...
	<body><body ><h3>Agente: $NUSUARIO - Proceso No AdjudicaciÛn</h3>
	<form action='zcallcenter2Dev.php' target='_self' method='POST' name='forma' id='forma'>
		Seleccione la causal: ";
	$Causales=q("select s.id,c.nombre as causal,s.nombre from causal c,subcausal s where s.causal in (1,2,18) and s.activo=1 and s.causal=c.id order by c.nombre,s.nombre");
...
function caso_no_adjudicacion_ok()
{
	...
	$Fecha=date('Y-m-d');$Hora=date('H:i:s');$Codigo=6; /*6: No AdjudicaciÛn */
...
function adicionar_observaciones()
{
	global $id,$USUARIO,$NUSUARIO,$d,$tipoCaja;
...
function marcar_proceso_actualizacion()
{
	...
	q("update siniestro set observaciones=concat(observaciones,'\n$NUSUARIO [$Ahora]: Actualizacion InformaciÛn Erronea Procesada.') where id=$D->siniestro");
	$H1=date('Y-m-d'); $H2=date('H:i:s');
	$Idn=q("insert into seguimiento (siniestro,fecha,hora,usuario,descripcion,tipo) values ($D->siniestro,'$H1','$H2','$NUSUARIO','Se recibe actualizaciÛn por parte de la aseguradora.',8)"); // 8: ACTUALIZACION DE DATOS
...
function solicita_reactivacion()
{
	...
	html('Solicitud de ModificaciÛn');
	...
	echo "<script language='javascript'>
			...
			function enviar_solicitud()
			{
				with(document.forma)
				{
					if(uno.checked)
					{
						...
						{
							alert('Debe justificar por quÈ se desea pasar a Pendiente el estado de este siniestro');
					...
					if(dos.checked)
					{
						...
						{
							alert('Debe justificar por quÈ se desea cambiar de ciudad este siniestro');
							justificacion2.style.backgroundColor='ffffdd';
							return false;
						}
						if(!ciudad.value)
						{
							alert('Debe especificar una ciudad v·lida');
					...
					{
						alert('La solicitud debe contener uno o los dos conceptos que son ReactivaciÛn o Cambio de Ciudad');
						return false;
					}
				}
		...
		<form action='zcallcenter2Dev.php' method='post' target='_self' name='forma' id='forma'>
		<h3>Solicitud de ModificaciÛn de Siniestro</h3>
		Usuario: $NUSUARIO   Fecha: $Fecha <br />
		<table border cellspacing='0' width='100%'>
			<tr><td>ReactivaciÛn <input type='checkbox' name='uno' onchange='activaruno();' ".($Sin->estado==1?'':"disabled")."> El estado actual es: <b>".qo1("select t_estado_siniestro($Sin->estado)")."</b></td></tr>
			<tr><td  id='tduno' style='visibility:hidden;'>Esta opciÛn solicita pasar el estado a PENDIENTE por favor escriba la justificaciÛn: <br /><textarea name='justificacion1' style='font-family:arial;font-size:12' rows='4' cols='80' valign='top'></textarea></td></tr>
			<tr><td>Cambio de Ciudad <input type='checkbox' name='dos' onchange='activardos();'></td></tr>
			<tr><td id='tddos' style='visibility:hidden;'>Ciudad: $Ciudades JustificaciÛn: <br><textarea name='justificacion2' style='font-family:arial;font-size:12' rows='4' cols='80' valign='top'></textarea></td></tr>
		</table>
...
function solicita_reactivacion_ok()
{
	...
	"Fecha de autorizaciÛn: $Sin->fec_autorizacion<br>";
		if($uno)
		{
		...
		$Mensaje.="Para activar haga click aquÌ: <a href='https://app.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta1';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Aprobar la modificaciÛn</a></body>";
		$Envio=enviar_gmail($Email_usuario /*de */,
		$_SESSION['Nombre'] /*Nombre de */ ,
		"fernandauribe@aoacolombia.com,Luz Angel;sandraosorio@aoacolombia.com,Sandra Osorio;sergiocastillo@aoacolombia.com,Sergio Castillo Castro" /*para */,
		"" /*con copia*/,
		"Solicitud Modificacion Siniestro $Sin->numero" /*Objeto */,
		$Mensaje);
		if($Envio)
			echo "Envio exitoso a: fernandauribe@aoacolombia, sandraosorio@aoacolombia.com, sergiocastillo@aoacolombia.com, $Email_usuario";
		else
			echo "Falla en el envÌo del mail.";
	}
	else
	{
		if($uno)
		{
			...
			echo "ReactivaciÛn del Siniestro exitosa.";
		}
		if($dos)
		{
			q("update solicitud_modsin set aprobado_por='$NUSUARIO',fec_aprobacion='$Ahora' where id=$IDM");
...
{
	global $f,$v,$NUSUARIO;
	html('ADJUDICACION DE VEHÕçCULO');
	
	if(!($_SESSION['Adjudicacion_SINIESTRO'] && $_SESSION['Adjudicacion_OFICINA'] && $_SESSION['Adjudicacion_ASEGURADORA']))
	{echo "<body><script language='javascript'>centrar(400,200);alert('AdjudicaciÛn caida, por favor empiece nuevamente desde el mÛdulo de Call Center');window.close();void(null);</script></body>";die();}
	...
	if(!$_SESSION['Adjudicacion_READJUDICAR'])
	{
		...
		{
			echo "<body>";
			echo "<script language='javascript'>alert('Aun existe una cita pendiente del dia $Cita_pendiente->fecha $Cita_pendiente->hora con el vehÌculo $Cita_pendiente->placa');window.close();void(null);</script>";
			echo "</body>";
			die();
		}
	}
	...
	if($V->flota!=$Sin->aseguradora)
	{
		if( ($V->flota==8 && $Sin->aseguradora==1) ||
			($V->flota==9 && $Sin->aseguradora==8) )
		{
			$Ahora=date('Y-m-d H:i:s');
			...
			('$v','$V->flota','$Sin->aseguradora','AUTOMATICO','$Ahora','$NUSUARIO',\"Cambio autom·tico de gamas Allianz.\")");
		}
		elseif($Q=qo("select * from cambio_flota where date_format(fecha,'%Y-%m-%d')='".date('Y-m-d')."'  and vehiculo='$v' and flota_original='$V->flota' and flota_temporal='$Sin->aseguradora' "))
		{
		...
		else
		{
			echo "
				...
				<form action='zcallcenter2Dev.php' target='Oculto_autorizacion_cambio_flota' method='POST' name='forma' id='forma'>
					Por Orden de Gerencia, se debe justificar todo cambio de flota.<br><br>
					JustificaciÛn: <input type='text' name='justificacion' id='justificacion' value='' size='80' maxlength='200'><br><br>
					...
					<input type='button' name='autorizar' id='autorizar' value=' AUTORIZAR CAMBIO DE FLOTA ' style='font-size:16px' onclick='validar_cambio_flota();' ><br><br>
	...
	if($Cita=qom("select c.id,c.fecha,c.hora from cita_servicio c , vehiculo v where c.placa=v.placa and v.id=$v and c.estado='P' limit 1",$LINK))
	{
		if($_SESSION['Adjudicacion_READJUDICAR'])
		{
			echo "<body><script language='javascript'>centrar(400,200);
				if(confirm('Aun tiene una cita pendiente el dia ".fecha_completa($Cita->fecha)." a las $Cita->hora. Desea Cancelar la re-asignaciÛn de vehÌculo en proceso?'))
					window.open('zcallcenter2Dev.php?Acc=mata_reasignacion','_self');
	...
	if($Nivel>2) $Pasa_estado= inlist($Ultimo->estado,'1,2,4,5,7,8,92,94,96,102');
	else $Pasa_estado=inlist($Ultimo->estado,'1,2,8,102');
	if($Pasa_estado)
	{
		echo "<script language='javascript'>
		...
		<iframe name='Accion_cita' id='Accion_cita' style='visibility:hidden;position:absolute;' width='98%' height='90%' frameborder='no'></iframe>
			<form action='zcallcenter2Dev.php' target='Accion_cita' method='POST' name='forma' id='forma'>
		...
		while($Co=mysql_fetch_object($Concurrencias)){	$Concur[$Co->hora]=$Co->cantidad;	}
		...
		if($Ultimo->estado==1) // estado en servicio
		{
			...
			{
				echo "<h3>ASIGNACION DE VEHÕçCULO .:. OFICINA: $Oficina->nombre</h3>
				Placa: <b style='font-size:16px'>$V->placa</b> Linea: $V->nlinea <br>Fecha de Asignacion: <b>$f - ".ndiasemana($f)."</b><br>
				<table bgcolor='ffffff'><tr><td valign='top'><table border cellspacing='0' bgcolor='ffffff'><tr><th>Hora</th><th>Concurrencia</th><th>Observaciones</th><th>OpciÛn</th></tr>";
				while($Hora<=$Oficina->hora_final)
				{
					...
					// VERIFICACION DE SI EL VEHICULO TIENE FECHA DE DEVOLUCI”N EN UNA HORA DETERMINADA.. BLOQUEA LAS HORAS HASTA LA ENTREGA Y 1 HORA MAS PARA EL ALISTAMIENTO
					if($Cita->fec_devolucion==$f)	
					{
						if($Hora<=$Cita->hora_devol) {	$Bloqueo='aaffaa';  }
						else if($Hora<=aumentaminutos($Cita->hora_devol,60))  { $Bloqueo='aaffdd'; }
					}
					...
					if($Hora=='13:00:00') echo "</table></td><td valign='top'><table border cellspacing='0' bgcolor='ffffff'><tr><th>Hora</th><th>Concurrencia</th><th>Observaciones</th><th>OpciÛn</th></tr>";
				}
		...
		{
			$Bloqueo='';
			$Concurrencia=$Concur[$Hora];if($Concurrencia>=$Oficina->concurrencia) $Bloqueo='ffdddd';
...
function pre_activar(){html();echo "<body><script language='javascript'>opener.activar();</script></body>";}
...
function asigna_cita()
{
	...
	echo "<script language='javascript'>
		...
		{
			window.open('zcallcenter2Dev.php?Acc=pide_datos_financieros&ids='+ids,'_self');
		}
		function cerrar_re_asignacion()
		{
			alert('Re-AsignaciÛn Satisfactoria');
			window.open('zcallcenter2Dev.php?Acc=recargar_control','Oculto_control');
	...
	<h3>ASIGNACION DE VEHÕçCULO Y CITA DE ENTREGA</h3>";
	if($_SESSION['Adjudicacion_READJUDICAR'])
	{
	...
	echo "<form action='zcallcenter2Dev.php' method='post' target='Oculto_agendar' name='forma' id='forma'>
	...
	<br /><br />VehÌculo asignado: <input type='text' name='placa' value='$Veh->placa' readonly style='font-size:12;font-weight:bold' size=7>
	...
	<br /><br /><b style='font-size:16;color:0000ff;text-decoration:blink;'>Persona quien va a recoger el vehÌculo o conductor: </b>
	<input type='text' name='conductor' value='$conductor' style='font-size:12;font-weight:bold' size='50'><br />
	Este ser· el nombre que aparezca en el ACTA DE ENTREGA. Si quien recoge el vehÌculo es un TERCERO, indique los requisitos adicionales.
	<br />Observaciones:
	...
	<table bgcolor='ddddff'><tr><td align='center' colspan=2 style='font-size:16'><B>DOMICILIO ENREGA</B></td></tr>
	<tr><td align='right' style='font-size:14'>DirecciÛn Domicilio entrega:</td><td><input type='text' style='font-size:14' name='dir_domicilio' value='$dir_domicilio' size='100' maxlength='200' onblur='valida_domicilio();'></td></tr>
	<tr><td align='right' style='font-size:14'>TelÈfono Domicilio entrega:</td><td><input type='text' style='font-size:14' name='tel_domicilio' value='$tel_domicilio' size='30' maxlength='50' onblur='valida_domicilio();'></td></tr>
	...
	<br>
	<table bgcolor='ddddff'><tr><td align='center' colspan=2 style='font-size:16'><B>DOMICILIO DEVOLUCI”N</B></td></tr>
	<tr><td align='right' style='font-size:14'>DirecciÛn domicilio devoluciÛn:</td><td><input type='text' style='font-size:14' name='dir_domiciliod' value='$dir_domiciliod' size='100' maxlength='200' onblur='valida_domiciliod();'></td></tr>
	<tr><td align='right' style='font-size:14'>TelÈfono Domicilio devoluciÛn:</td><td><input type='text' style='font-size:14' name='tel_domiciliod' value='$tel_domiciliod' size='30' maxlength='50' onblur='valida_domiciliod();'></td></tr>
	<tr ><td align='center' colspan='2'><input type='button' id='btn_envio_aviso'  name='btn_envio_aviso' value=' Enviar Aviso de Llamada a Autorizaciones ' style='visibility:hidden;font-weight:bold;font-size:14px;'	onclick='aviso_autorizacion();'></td></tr>
	</table>
	
	<br /><br />
	SeÒor usuario $NUSUARIO, el agendamiento de la cita quedar· registrada a nombre suyo con fecha y hora de registro.<br /><br />
	...
	<br><br>
	La informaciÛn de esta cita ser· enviada al correo <b>$Sin->declarante_email </b>
...
function enviar_mail_servicio_html()
{
	...
	$envio = enviar_gmail('sistemas@aoacolombia.com','Sergio Castillo Castro',
...
function asigna_cita_ok_testHtml()
{
	...
	global $oficina,$siniestro,$flota,$placa,$fecha,$hora,$conductor,$observaciones,$NUSUARIO,$dir_domicilio,$tel_domicilio,$tipogarantia;
	...
	// se busca en la tabla de citas alguna que estÈ en estado programada
		...
		if($S->declarante_email)
		{
			if($S->renta)
			{
				if($dir_domicilio)
				{
					...
					Este es un correo automatico del Area de Call Center cuyo objetivo es confirmar  la cita programada para el proximo ".fecha_completa($fecha)." a las $Nhora a fin de hacer entrega de un VehÌculo Rentado en la direccion: $dir_domicilio Telefono: $tel_domicilio,  solicitado por $conductor.
					...
					* La hora pactada para la devolucion corresponde a la hora en la cual se suministro el vehiculo o en su defecto la establecida por la compaÒÌa ajustandose a los horarios de pico y placa establecidos en la ciudad (en caso de aplicar). Una vez cumplido el tiempo de gracia (3 horas) previamente informado para la devoluciÛn, se cobrara un dia adicional. Si desea prorrogar el servicio por m·s dÌas deber· informarlo con 12 horas de anterioridad.
				...
				else
				{
					...
					/*$Correo="
					Respetado(a) SeÒor(a) $conductor

					Reciba cordial saludo.

					Bienvenido(a) al sistema de adjudicacion del Servicio de Renta de AOA Colombia.

					Este es un correo autom·tico del Area de Call Center cuyo objetivo es confirmar  la cita programada para el proximo ".fecha_completa($fecha)." a las $Nhora en las instalaciones de AOA Colombia, Sucursal:  $OF->nombre, ubicada en la  $OF->direccion en $OF->barrio con el fin de realizar entrega de un VehÌculo Rentado,  solicitado por $conductor.
					...
					* Recuerde que despues de cumplido el tiempo del servicio, usted debe devolver el vehiculo en las mismas condiciones en que le fue entregado, incluyendo el combustible.
					* La hora pactada para la devolucion corresponde a la hora en la cual se suministro el vehiculo o en su defecto la establecida por la compaÒia ajustandose a los horarios de pico y placa establecidos en la ciudad (en caso de aplicar). Una vez cumplido el tiempo de gracia (3 horas) previamente informado para la devolucion, se cobrara un dia adicional. Si desea prorrogar el servicio por m·s dÌas debera informarlo con 12 horas de anterioridad.
					*(Para cliente natural) La Garantia constituida mediante voucher sera	devuelta a traves de la direccion de correo electronico suministrada en un termino aproximado de 10 dias despues de finalizado el servicio con el fin verificar pagos pendientes. No obstante es importante tener en cuenta que en caso de no recibirlo por algun motivo concerniente a fallas tecnicas, errores de conectividad, entre otros, usted podra acercarse durante los primeros 60 dias de transcurrido el servicio a la oficina en la cual constituyo dicha garantia y reclamar el original. Pasado este tiempo, debera solicitarlo con antelacion para realizar el proceso de alistamiento y entrega del mismo desde nuestra central de archivo.
					...
					Ante cualquier inquietud puede escribirnos al correo electronico sac@aoacolombia.com. Nuestro horario de atenciÛn es de lunes a viernes de 8:00 a.m. a 5:00 p.m. y s·bados de 9:00a.m. a 12:00 p.m.
			...
			else
			{
				if($dir_domicilio)
				{
					...
					- Numero de una cuenta bancaria, nombre de cuenta habiente, tipo de cuenta bancaria (Ahorros o Corriente) y el nombre de la entidad bancaria. Esta cuenta sera utilizada exclusivamente en caso de verse afectada la garantia constituida mediante congelamiento por un valor inferior a la misma, realizaremos la devolucion del excedente en el numero de cuenta por usted suministrado. (Este requisito no aplica para la garantia No Reembolsable). Existan cobros inferiores a la garantÌa y se requiera devolver algun excedente.
					- Si usted es un tercero autorizado (a), debe presentar una carta diligenciada por el asegurado(a) indicando que usted est· autorizado(a) para proceder con la gestiÛn del servicio (no requiere autenticaciÛn) y adjunto a esta debe presentar una fotocopia de la Cedula de CiudadanÌa del (la) asegurado(a) o fotocopia de Camara y Comercio (no mayor a 30 dÌas) de la Entidad asegurada.
					- Haber leido y aceptado los terminos y condiciones del servicio.
					 -$Aseguradora->info_per
					...
					*La hora pactada para la devolucion corresponde a la hora en la cual se suministro el vehiculo o en su defecto la establecida por la compania ajustandose a los horarios de pico y placa establecidos en la ciudad (en caso de aplicar). Una vez cumplido el tiempo de gracia previamente informado para la devoluciÛn, se incurrira en penalidad sin excepcion alguna. Recuerde que para su mayor comodidad y evitar incurrir en dicha penalidad, puede retornar el vehiculo a traves de un tercero por usted autorizado. Hora pactada devolucion: $Nhora.
...
function asigna_cita_ok()
{
	...
	// VALIDACION PARA EVITAR QUE UN VEHICULO NO SEA ASIGNADO A DOS SERVICIOS EN EL MISMO LAPSO DE TIEMPO
	// se busca en la tabla de citas alguna que estÈ en estado programada
	if($Cita_Previa=qo("select c.id,s.numero
	...
	{
		echo "<body><script language='javascript'>alert('Ya existe una cita que cruza su tiempo con la fecha propuesta de esta adjudicaciÛn: $Cita_Previa->numero de fecha: $Cita_Previa->fecha');</script></body>";
		die();
	}
	...
	if($tipogarantia=='1') $tg='No reembolsable'; elseif($tipogarantia==2) $tg='Reembolsable'; else $tg='No definida';
	if($tipogarantia == 3){
		$tg='SIN GARANTIA';
	} 
	$H1=date('Y-m-d'); $H2=date('H:i:s');
	mysql_query("insert into seguimiento (siniestro,fecha,hora,usuario,descripcion,tipo) values ($siniestro,'$H1','$H2','$NUSUARIO','Agenda cita para $Dia $fecha a la(s) $hora con el vehÌculo $placa en la ciudad de $Nciudad Tipo de Garantia: $tg ".
	...
	{
		mysql_query("insert into cita_servicio (oficina,siniestro,flota,placa,fecha,hora,conductor,observaciones,agendada_por,fecha_agenda,estado,dir_domicilio,tel_domicilio,dias_servicio)
			values ('$oficina','$siniestro','$flota','$placa','$fecha','$hora','$conductor','$observaciones','$NUSUARIO','".date('Y-m-d H:i:s')."','P','$dir_domicilio','$tel_domicilio','$S->dias_servicio')",$LINK);
		$Idn=mysql_insert_id($LINK);
		...
		else
		{
			...
			graba_bitacora('call2cola2','A',$Idcc,'Inserta con aceptaciÛn de tÈrminos y condiciones',$LINK);
		}
	}
	if($_SESSION['Adjudicacion_OFICINA'])
	{
	...
	if(!$_SESSION['Adjudicacion_READJUDICAR'])
	{
		...
		if($S->declarante_email)
		{
			...
			$Envio3=enviar_gmail("direccioncallcenter@aoacolombia.com" /*de */,
					"Call Center AOA" /*Nombre de */ ,
					"$S->declarante_email,$conductor" /*para */,
					"$Email_usuario,$NUSUARIO,pqr@aoacolombia.com,pqr" /*con copia*/,
					"AOA COLOMBIA S.A. - Cita programada para  $Aseguradora->nombre_servicio" /*Objeto */,
					utf8_decode($buffer));
		}
	}
	if($_SESSION['Adjudicacion_READJUDICAR'])
	...
	else
	{
		...
		parent.adjudicacion_datos_financieros($siniestro);
		</script></body>";
	}
}
	...
	if($S->declarante_email)
	{
		if($Cita->dir_domicilio)
		{
			...
			Reciba cordial saludo.
				...
				Este es un correo autom·tico del Area de Call Center cuyo objetivo es confirmar  la cita programada para el prÛximo ".fecha_completa($fecha)." a las $Nhora a fin de hacer entrega de un Vehiculo de Sustituto en la direcciÛn: $dir_domicilio TelÈfono: $tel_domicilio,  derivado de la reclamacion que presento ante  $Aseguradora->razon_social referente al vehiculo de placas $S->placa y cuyo numero de siniestro es $S->numero .

				Es importante que tenga en cuenta los requisitos previamente informados:

				- Documento de IdentificaciÛn original en el momento de la entrega de vehiculo.
				- Licencia de ConducciÛn original debidamente registrada (ante el Runt o Ministerio de Transporte).
				- Tarjeta de crÈdito con el cupo disponible previamente informado por el consultor y el cual corresponde a una suma de:  $Aseguradora->garantia pesos para constituir la garantÌa, requisito indispensable para la prestaciÛn del servicio. En caso de no poseer una tarjeta de crÈdito propia o respaldada por una entidad bancaria cuya franquicia corresponda a las aceptadas y previamente informadas por el consultor que le atendio telefÛnicamente, usted puede solicitar a un tercero que le sea garante mediante tarjeta de crÈdito para constituir la garantÌa. En este caso el Tarjetahabiente debe acompaÒarlo(a) a la cita de entrega del vehiculo para el congelamiento de cupo de la garantÌa, asi como los demas documentos que soportar·n la cobertura mediante dicha tarjeta.
				- N˙mero de una cuenta bancaria, nombre de cuenta habiente, tipo de cuenta bancaria (Ahorros o Corriente) y el nombre de la entidad bancaria. Esta cuenta ser· utilizada exclusivamente en caso de verse afectada la garantÌa constituida mediante voucher por un valor inferior a la misma, realizaremos la devoluciÛn del excedente en el n˙mero de cuenta por usted suministrado. (Este requisito no aplica para la garantÌa No Reembolsable).
				existan cobros inferiores a la garantÌa y se requiera devolver alg˙n excedente.
				- Si usted es un tercero autorizado (a), debe presentar una carta diligenciada por el asegurado(a) indicando que usted est· autorizado(a) para proceder con la gestiÛn del servicio (no requiere autenticaciÛn) y adjunto a esta debe presentar una fotocopia de la CÈdula de CiudadanÌa del (la) asegurado(a) o fotocopia de C·mara y Comercio (no mayor a 30 dÌas) de la Entidad asegurada.
				- Haber leido y aceptado los terminos y condiciones del servicio.
				 -$Aseguradora->info_per
				...
				*Asistir puntual a la hora de la cita pactada. Una vez cumplido el tiempo acordado sera  cancelada automaticamente por nuestra plataforma y se debera volver a reagendar una nueva cita, la cual sera asignada de acuerdo con la prioridad de entrega de vehiculos por usuario.
				* Recuerde que despues de cumplido el tiempo del servicio, usted debe devolver el vehiculo en las mismas condiciones en que le fue entregado, incluyendo el lavado y tanqueado.
				* La hora pactada para la devolucion corresponde a la hora en la cual se suministro el vehiculo o en su defecto la establecida por la compaÒÌa ajustandose a los horarios de pico y placa establecidos en la ciudad (en caso de aplicar). Una vez cumplido el tiempo de gracia previamente informado para la devoluciÛn, se incurrira en penalidad sin excepcion alguna. Recuerde que para su mayor comodidad y evitar incurrir en dicha penalidad, puede retornar el vehiculo a travÈs de un tercero por usted autorizado. Hora pactada devolucion: $Nhora.
				*La Garantia constituida mediante voucher sera devuelta a travÈs de la direccion de correo electrÛnico suministrada en un termino aproximado de 10 dÌas despues de finalizado el servicio con el fin  verificar pagos pendientes. No obstante es importante tener en cuenta que en caso de no recibirlo por alg˙n motivo concerniente a fallas tecnicas, errores de conectividad, entre otros, usted podra acercarse durante los primeros  60 dÌas de transcurrido el servicio a la oficina en la cual constituyÛ dicha garantÌa y reclamar el original. Pasado este tiempo, deber· solicitarlo con antelaciÛn para realizar el proceso de alistamiento y entrega del mismo desde nuestra central de archivo.

				Importante: La anulacion o devolucion de la garantia se realiza sin perjuicio a que ADMINISTRACI”N OPERATIVA AUTOMOTRIZ S.A. acuda a otras vias legales (inclusive reporte a Centrales de Riesgo)  para el pago de saldos pendientes conforme a los Terminos y Condiciones del servicio de vehiculo sustituto.

				Nota: respecto al suministro de sus datos personales (informaciÛn no sensible)

				Es importante que tenga en cuenta que para acceder al servicio de vehiculo sustituto, usted puede haber
				suministrado y/o suministrar· de manera voluntaria y libre informaciÛn para ser utilizada exclusivamente durante
				las etapas de contactabilidad, agendamiento, adjudicacion, prestaciÛn, devolucion del vehÌculo y cierre del
				servicio incluyendo la validaciÛn del proceso de garantias; informacion que corresponde a datos personales
				como son nombres, apellidos, numero de identificacion, telÈfono (s), direcciÛn(es), correo(s) electrÛnico(s), ciudad, paÌs, informaciÛn de tarjeta(s) de crÈdito y cuenta(s) bancaria(s) (cuando apliquen) entre otros.
		...
		else
		{
			...
			Respetado(a) SeÒor(a) $conductor
				...
				Este es un correo autom·tico del Area de Call Center cuyo objetivo es confirmar  la cita programada para el prÛximo ".fecha_completa($fecha)." a las $Nhora  en las instalaciones de AOA Colombia, Sucursal:  $OF->nombre, ubicada en la  $OF->direccion en $OF->barrio con el fin de realizar entrega del  vehiculo sustituto derivado de la reclamacion que presento ante  $Aseguradora->razon_social referente al vehiculo de placas $S->placa y cuyo numero de siniestro es $S->numero .

				Es importante que tenga en cuenta los requisitos previamente informados:

				- Documento de IdentificaciÛn original en el momento de la entrega de vehiculo.
				- Licencia de ConducciÛn original debidamente registrada (ante el Runt o Ministerio de Transporte).
				- Tarjeta de crÈdito con el cupo disponible previamente informado por el consultor y el cual corresponde a una suma de:  $Aseguradora->garantia pesos para constituir la garantÌa, requisito indispensable para la prestaciÛn del servicio. En caso de no poseer una tarjeta de crÈdito propia o respaldada por una entidad bancaria cuya franquicia corresponda a las aceptadas y previamente informadas por el consultor que le atendio telefÛnicamente, usted puede solicitar a un tercero que le sea garante mediante tarjeta de crÈdito para constituir la garantÌa. En este caso el Tarjetahabiente debe acompaÒarlo(a) a la cita de entrega del vehiculo para congelamiento de cupo de la garantÌa, asi como los demas documentos que soportar·n la cobertura mediante dicha tarjeta.
				- N˙mero de una cuenta bancaria, nombre de cuenta habiente, tipo de cuenta bancaria (Ahorros o Corriente) y el nombre de la entidad bancaria. Esta cuenta ser· utilizada exclusivamente en caso de verse afectada la garantÌa constituida mediante voucher por un valor inferior a la misma, realizaremos la devoluciÛn del excedente en el n˙mero de cuenta por usted suministrado. (Este requisito no aplica para la garantÌa No Reembolsable).
				existan cobros inferiores a la garantÌa y se requiera devolver alg˙n excedente.
				- Si usted es un tercero autorizado (a), debe presentar una carta diligenciada por el asegurado(a) indicando que usted est· autorizado(a) para proceder con la gestiÛn del servicio (no requiere autenticaciÛn) y adjunto a esta debe presentar una fotocopia de la CÈdula de CiudadanÌa del (la) asegurado(a) o fotocopia de C·mara y Comercio (no mayor a 30 dÌas) de la Entidad asegurada.
				- Haber leido y aceptado los terminos y condiciones del servicio.
		...
		$Envio3=enviar_gmail("direccioncallcenter@aoacolombia.com" /*de */,
										"Call Center AOA" /*Nombre de */ ,
										"$S->declarante_email,$Cita->conductor" /*para */,
										"$Email_usuario,$NUSUARIO" /*con copia*/,
										"AOA COLOMBIA S.A. - Cita programada para  $Aseguradora->nombre_servicio" /*Objeto */,
										nl2br($Correo));
...
function escalar_caso()
{
	...
	echo "<script language='javascript'>
		function validar_escalar()
		{
			...
			{
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Est· seguro de escalar el caso?')) submit();
			}
		}
	</script>
...
function escalar_caso_ok()
{
	...
	$Ahora=date('Y-m-d H:i:s');
	$Luego=date('Y-m-d').' '.aumentaminutos(date('H:i:s'),$minutos);
	$Fecha=date('Y-m-d');$Hora=date('H:i:s');$Codigo=19; /*19: Escalamiento de caso a un nivel superior */
...
function caso_actualizar_datos()
{
	...
	function validar_actualizacion()
	{
		...
		{
			//if(!alltrim(correo.value)) { alert('Debe digitar el correo electrÛnico del asegurado');correo.style.backgroundColor='ffff99';correo.focus();return false;}
			if(!alltrim(actualizacion_aseg.value)) { alert('Debe digitar la informaciÛn correspondiente a la actualizaciÛn de datos de contacto');actualizacion_aseg.style.backgroundColor='ffff99';actualizacion_aseg.focus();return false;}
			submit();
		}
	}
	...
	<form action='zcallcenter2Dev.php' target='Oculto_actualizacion' method='POST' name='forma' id='forma'>
		<h3>ActualizaciÛn de datos de contacto:</h3>
		Datos de contacto: <br>
		TelÈfono Celular de contacto: <input type='text' name='telefono_contacto' id='telefono_contacto' size='10' maxlength='10'> A este n˙mero se enviar· el mensaje de Texto (SMS) autom·tico para recordar las citas de entrega y devoluciÛn.<br>

		<textarea name='actualizacion_aseg' id='actualizacion_aseg' cols=80 rows=5></textarea><br>
		Correo electrÛnico: <input type='text' name='correo' id='correo' value='$Sin->declarante_email' size='60' maxlength='200'><br>
		<input type='button' name='continuar' id='continuar' value=' ACTUALIZAR INFORMACION ' onclick='validar_actualizacion()'>
...
function caso_actualizar_datos_ok()
{
	...
	$H1=date('Y-m-d'); $H2=date('H:i:s');
	$Idn=q("insert into seguimiento (siniestro,fecha,hora,usuario,descripcion,tipo) values ($id,'$H1','$H2','$NUSUARIO','ActualizaciÛn de datos de contacto: Telefono Anterior <h2>$varTelefonoCel</h2> ',8)");
	$sql = "delete from call2infoerronea where siniestro = $id";
	q($sql);
...
function pide_datos_financieros(){
	...
	if($sinG == 1){
		...
		$titulo = " .:. AOA COLOMBIA S.A. .:. SERVICIO DE VEHÕçCULO DE REEMPLAZO .:. FORMULARIO DE GARANTÕçA";
		$varFranquicia = "<tr><td align='right'>Franquicia</td><td>";
		$ScriptFranquicia = "if(franquicia.value) {alert('Debe seleccionar la franquicia de la tarjeta de crÈdito'); franquicia.style.backgroundColor='ffff55';franquicia.focus(); return false;}franquicia.disabled=false;";
	}	
	...
	<script language='javascript'>
		function valida_envio()
		{
			with(document.forma)
			{
				if(!Number(identificacion.value)) {alert('Debe escribir correctamente el n˙mero de identificaciÛn, sin comas ni puntos');identificacion.style.backgroundColor='ffff55';identificacion.focus();return false;}
				...
				if(!tipo_id.value) {alert('Debe seleccionar el tipo de identificaciÛn'); tipo_id.style.backgroundColor='ffff55';tipo_id.focus();return false;}
				if(!alltrim(lugar_expdoc.value)) {alert('Debe escribir lugar de expediciÛn de la IdentificaciÛn'); lugar_expdoc.style.backgroundColor='ffff55';lugar_expdoc.focus(); return false;}
				...
				if(!alltrim(direccion.value)) { alert('Debe digitar la direcciÛn de residencia.'); direccion.style.backgroundColor='ffff44'; direccion.focus(); return false; }
				...
				$ScriptFranquicia
				if(confirm('Confirma el envÌo de esta informaciÛn?'))
		...
		{
			if(!id) {alert('Debe digitar una identifcaciÛn valida');   document.forma.Enviar.style.visibility='hidden';document.forma.identificacion.focus();return false; }
			else {document.forma.Enviar.style.visibility='visible';}
			window.open('zcallcenter2Dev.php?Acc=valida_identificacion&id='+id,'Oculto_forma');
		}
		...
		function validar_id()
		{
			window.open('zcallcenter2Dev.php?Acc=valida_identificacion&id='+document.forma.identificacion.value,'Oculto_forma');
		}
		function changeFunc(){
	...
	<body style='font-size:16px' bgcolor='ffffff'><script language='javascript'>centrar();</script>
		...
		<br><br>
		<form action='zcallcenter2Dev.php' target='Oculto_forma' method='POST' name='forma' id='forma'>
			<table>
				<tr><td align='right'>N˙mero de IdentificaciÛn</td><td><input type='text' class='numero' name='identificacion' id='identificacion' value='' size='15' maxlength='15'>
					<input type='button' name='validar' id='validar' value=' Validar IdentificaciÛn ' onclick='validar_id();'></td></tr>
				<tr><td align='right'>Tipo de IdentificaciÛn</td><td>".menu1('tipo_id',"select codigo,nombre from tipo_identificacion",'',1)."</td></tr>
				<tr><td align='right'>Lugar de ExpediciÛn de la IdentificaciÛn</td><td><input type='text' name='lugar_expdoc' id='lugar_expdoc' value='' size='50' maxlength='50' onkeyup='javascript:this.value=this.value.toUpperCase();'></td></tr>
				<tr><td align='right'>Selecciona si es juridica o natural</td><td><select id='seleCode' name='seleCode' onchange='changeFunc();'><option>Seleccione</option><option value='1'>Juridica</option><option value='2'>Natural</option></select></td></tr>
				<tr id='trPadre' style='display:none'><td align='right'>Coloca el  codigo de verificacion</td><td><input type='text' style='width: 2%;' name='codigoVerificacion' maxlength='1' id='codigoVerificacionId' ></td></tr>
				...
				<tr><td align='right'>DirecciÛn Domicilio:</td><td ><input type='text' name='direccion' id='direccion' size='30' maxlength='50' onkeyup='javascript:this.value=this.value.toUpperCase();'></td></tr>
				<tr><td align='right'>TelÈfono Oficina:</td><td ><input type='text' class='numero' name='telefono_oficina' id='telefono_oficina' size='30' maxlength='50'></td></tr>
				<tr><td align='right'>TelÈfono Vivienda:</td><td ><input type='text' class='numero' name='telefono_casa' id='telefono_casa' size='30' maxlength='50'></td></tr>
	...
	echo "</select></td></tr>
				<tr><td align='right'>CÛdigo de Seguridad<br>(Tres ˙ltimos digitos al respaldo de la tarjeta)</td><td><input type='password' name='codigo_seguridad' id='codigo_seguridad' value='' size='4' maxlength='4'></td></tr>
			</table><br><br>
			En caso de hacer cobros con la garantÌa y si hay que devolver un dinero se hace mediante transferencia electrÛnica. A continuaciÛn por favor diligencie los datos
			...
			<table>
				...
				<tr><td align='right'>Documento de IdentificaciÛn</td><td><input type='text' class='numero' name='identificacion_devol' id='identificacion_devol' value='' size='20' maxlength=15></td></tr>
			</table>";
	echo $varScript = "<script language='javascript'>
		function valida_envio()
		{
			with(document.forma)
			{
				if(!alltrim(numero_tarjeta.value)) {alert('Debe escribir el numero completo de la tarjeta de crÈdito'); numero_tarjeta.style.backgroundColor='ffff55';numero_tarjeta.focus(); return false;}
				if(!Number(numero_tarjeta.value)) {alert('Debe escribir el numero completo de la tarjeta de crÈdito'); numero_tarjeta.style.backgroundColor='ffff55';numero_tarjeta.focus(); return false;}
				if(!banco.value) {alert('Debe seleccionar el banco que expidiÛ la tarjeta de crÈdito'); banco.style.backgroundColor='ffff55';banco.focus(); return;}
				if(!alltrim(vencimiento_mes.value)) {alert('Debe seleccionar el mes de vencimiento de la tarjeta de crÈdito'); vencimiento_mes.style.backgroundColor='ffff55';vencimiento_mes.focus(); return;}
				if(!alltrim(vencimiento_ano.value)) {alert('Debe seleccionar el aÒo de vencimiento de la tarjeta de crÈdito');vencimiento_ano.style.backgroundColor='ffff55';vencimiento_ano.focus(); return;}
				if(!alltrim(codigo_seguridad.value)) {alert('Debe digitar el cÛdigo de seguridad de la tarjeta de crÈdito'); codigo_seguridad.style.backgroundColor='ffff55';codigo_seguridad.focus(); return;}
				if(codigo_seguridad.value!='000') if(!Number(codigo_seguridad.value)) {alert('Debe digitar el cÛdigo de seguridad de la tarjeta de crÈdito'); codigo_seguridad.style.backgroundColor='ffff55';codigo_seguridad.focus(); return;}
				...
				if(!Number(devol_cuenta_banco.value)) {alert('Numero de cuenta bancaria para devoluciones de garantÌa inv·lido'); devol_cuenta_banco.style.backgroundColor='ffff55';devol_cuenta_banco.focus();return false;}
				if(devol_cuenta_banco.value.length<8) {alert('Numero de cuenta bancaria para devoluciones de garantÌa inv·lido'); devol_cuenta_banco.style.backgroundColor='ffff55';devol_cuenta_banco.focus();return false;}
				if(devol_cuenta_banco.value.indexOf('0000000')>-1) {alert('Numero de cuenta bancaria para devoluciones de garantÌa inv·lido'); devol_cuenta_banco.style.backgroundColor='ffff55';devol_cuenta_banco.focus();return false;}
				...
				if(!alltrim(identificacion_devol.value)) {alert('Debe digitar  la identificaciÛn a la que pertenece la cuenta para devoluciones');identificacion_devol.style.backgroundColor='ffff55';identificacion_devol.focus();return false;}
				if(!Number(identificacion_devol.value)) {alert('Debe digitar  la identificaciÛn a la que pertenece la cuenta para devoluciones');identificacion_devol.style.backgroundColor='ffff55';identificacion_devol.focus();return false;}
		...
		function valida_vencimiento()
		{
			...
			if(Vm && Va)
			{
				...
				{alert('No se puede aceptar una tarjeta vencida. Verifique la informaciÛn o intente con otra tarjeta.');document.forma.Enviar.disabled=true;return false;}
	...
	if($C=qo("select * from cliente where identificacion='$id' "))
	{
		...
		echo "
				with(parent.document.forma)
				{
					...
					sexo.value='$C->sexo';codigoVerificacion.value='$C->dv';email_e.value='$C->email_e';
...
function registrar_solicitud()
{
	...
	global $devol_ncuenta,$devol_cuenta_banco,$devol_tipo_cuenta,$devol_banco,$apellido,$lugar_expdoc,$tipo_id,$sexo,$ciudad,$direccion,
		$identificacion_devol,$telefono_oficina,$telefono_casa,$celular,$pais,$NUSUARIO,$IDUSUARIO,$codigoVerificacion;
	...
	if($sinG == 1){
		...
		echo "update cliente set nombre='$nombre',apellido='$apellido',lugar_expdoc='$lugar_expdoc',tipo_id='$tipo_id',sexo='$sexo',pais='$pais',ciudad='$ciudad',
			direccion='$direccion',telefono_oficina='$telefono_oficina',telefono_casa='$telefono_casa',celular='$celular',email_e='$email_e',dv='$codigoVerificacion' where identificacion='$identificacion'";
...
function aceptacion_condiciones()
{
	...
	q("update siniestro set observaciones=concat(observaciones,'\n".$NUSUARIO.' '.date('Y-m-d H:i')." AceptaciÛn de tÈrminos y condiciones por el Agente de Call Center.') where id=$id");
	echo "<body><script language='javascript'>parent.re_comenzar();</script></body>";
}

function redireccionar_caso_abierto() {global $id;sleep(1);echo "<body><script language='javascript'>parent.cargar_caso($id);</script></body>";}

function adjudicacion_finalizada() {echo "<body><script language='javascript'>opener.adjudicacion_finalizada();window.close();void(null);</script></body>";}
...
function recargar_control(){echo "<body><script language='javascript'>parent.recargar_datos();</script></body>";}
...
function mata_reasignacion()
{
	...
	$_SESSION['Adjudicacion_SINIESTRO']=false;
	$_SESSION['Adjudicacion_READJUDICAR']=false;
	echo "<body><script language='javascript'>window.open('zcallcenter2Dev.php?Acc=recargar_control','Oculto_control');window.close();void(null);</script></body>";
}
...
function marcacion_numero_alterno()
{
	...
	echo "<script language='javascript'>
		function validar_envio_marcacion()
		{
			...
			{
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Est· seguro de Marcar a un n˙mero alterno?')) submit();
			}
		}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'><h4>MarcaciÛn N˙mero Alterno</h4>
		<form action='zcallcenter2Dev.php' method='post' target='_self' name='forma' id='forma'>
			<br>
			<br>Observaciones: <input type='text' name='observaciones' id='observaciones' size='80'><br>
...
function marcacion_numero_alterno_ok()
{
	global $id,$NUSUARIO,$IDUSUARIO,$observaciones;
	$Ahora=date('Y-m-d H:i:s');
	$Fecha=date('Y-m-d');$Hora=date('H:i:s');$Codigo=26; /*Marcacion a numero alterno */
	$Idn=q("insert into seguimiento (siniestro,usuario,fecha,hora,descripcion,tipo) values ($id,'$NUSUARIO','$Fecha','$Hora','MarcaciÛn a N˙mero alterno.',$Codigo)");
	q("update siniestro set observaciones=concat(observaciones,\"\n$NUSUARIO [$Ahora]: MarcaciÛn a Numero Alterno: $observaciones. \") where id=$id");
