...
$Rango_colores[1]['ini']=1;        $Rango_colores[1]['fin']=600;          $Rango_colores[1]['color']='C6FFC2';    $Rango_colores[1]['cantidad']=0; $Rango_colores[1]['tag']='Primeros 10 minutos';
$Rango_colores[2]['ini']=601;     $Rango_colores[2]['fin']=3600;        $Rango_colores[2]['color']='FFFFA4';    $Rango_colores[2]['cantidad']=0; $Rango_colores[2]['tag']='de 10 min. a 1 hora';
$Rango_colores[3]['ini']=3601;   $Rango_colores[3]['fin']=28800;       $Rango_colores[3]['color']='FFC1A3';   $Rango_colores[3]['cantidad']=0; $Rango_colores[3]['tag']='de 1 a 8 horas';
$Rango_colores[4]['ini']=28801; $Rango_colores[4]['fin']=86400;       $Rango_colores[4]['color']='C8F9FF';   $Rango_colores[4]['cantidad']=0; $Rango_colores[4]['tag']='de 8 a 24 horas';
$Rango_colores[5]['ini']=86401; $Rango_colores[5]['fin']=172800;     $Rango_colores[5]['color']='63BBFF';    $Rango_colores[5]['cantidad']=0; $Rango_colores[5]['tag']='de 24 a 48 horas';
$Rango_colores[6]['ini']=172801;$Rango_colores[6]['fin']=345602;    $Rango_colores[6]['color']='D30000';     $Rango_colores[6]['cantidad']=0; $Rango_colores[6]['tag']='de 48 a 96 horas';
$Rango_colores[7]['ini']=345602;$Rango_colores[7]['fin']=99999999; $Rango_colores[7]['color']='C64FFF';     $Rango_colores[6]['cantidad']=0; $Rango_colores[7]['tag']='Más de 96 horas';
*/
$Rango_colores[0]['ini']=0;        $Rango_colores[0]['fin']=0;             $Rango_colores[0]['color']='ffffff';            $Rango_colores[0]['cantidad']=0; $Rango_colores[0]['tag']='Pendiente';
...
echo "<script language='javascript'>
		...
		{
			modal('zcalculo_tiempos_gerencia.php?Acc=verseguimiento&id='+id+'&Contador='+contador,0,0,10,10,'seg');
		}
		function reconstruye_seguimiento(id)
		{
			modal('zcalculo_tiempos_gerencia.php?Acc=reconstruye_seguimiento&id='+id,0,0,10,10,'seg2');
		}
...
q("drop table if exists tmpi_tiemposg");
q("create table tmpi_tiemposg select id,numero,fec_autorizacion,estado,t_estado_siniestro(estado) as nestado,ingreso, ingreso as contacto, t_causal(causal) as ncausal,
		observaciones
...
q("alter table tmpi_tiemposg add column remitido datetime default '0000-00-00 00:00:00' ");
q("alter table tmpi_tiemposg add column actual datetime default '0000-00-00 00:00:00' ");
q("alter table tmpi_tiemposg add unique index id (id) ");

if($Siniestros=q("select * from tmpi_tiemposg"))
{
	include('inc/link.php');
	while($S=mysql_fetch_object($Siniestros))
	{
		if($Cox=qom("select concat(fecha,' ',hora) as momento from seguimiento where siniestro=$S->id and tipo in (3,12,13) order by fecha,hora limit 1",$LINK))
		{
			mysql_query("update tmpi_tiemposg set contacto='$Cox->momento' where id=$S->id",$LINK);
		}
		if($Rem=qom("select concat(fecha,' ',hora) as momento from seguimiento where siniestro=$S->id and tipo=4",$LINK))
		{
			mysql_query("update tmpi_tiemposg set remitido='$Rem->momento' where id=$S->id",$LINK);
		}
		if($Act=qom("select concat(fecha,' ',hora) as momento from seguimiento where siniestro=$S->id and tipo=8",$LINK))
		{
			mysql_query("update tmpi_tiemposg set actual='$Act->momento' where id=$S->id",$LINK);
		}
...
function pide_datos()
{
	...
	echo "<script language='javascript'>
			function carga()
			{
				document.getElementById('IResultado').style.height=document.body.clientHeight-190;
			}
			function validar()
			{
				carga();
		...
		<form action='zcalculo_tiempos_gerencia.php' method='post' target='IResultado' name='forma' id='forma'>
	...
	echo " <li>Si no se selecciona un <i>Estado</i>, se muestran todos los registros.
		...
		<iframe name='IResultado' id='IResultado' width='100%' height='400px' border='0' frameborder='no'></iframe>
		<script language='javascript'>document.getElementById('IResultado').style.height=document.body.clientHeight-180;</script>
		</body>";
}
...
function verseguimiento()
{
	...
	{
		modal('zcalculo_tiempos_gerencia.php?Acc=convierte_a_exitoso&id='+id,0,0,10,10,'seg2');
	}
	function marca_fecha(id)
	{
		...
		else
		{
			if(Marca2==0)
			{
				...
				modal('zcalculo_tiempos_gerencia.php?Acc=cambia_fechas&id1='+Marca1+'&id2='+Marca2,0,0,10,10,'cf');
			}
		}
	}
...
function reconstruye_seguimiento($LINK=0,$id1=0)
{
	...
	if(!qo1m("select id from seguimiento where siniestro=$Sin->id and tipo=1",$LINK))
		mysql_query("insert into seguimiento (siniestro,fecha,hora,descripcion,tipo) values ($Sin->id,'$Fechai','$Horai','Ingresa a la base de datos',1)",$LINK);
	$Arreglo=explode("\n",$Sin->observaciones);
	$Usuario='';
	$Consulto=false;
	$Stop=false;
