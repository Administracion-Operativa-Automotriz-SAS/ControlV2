...
function menu_w_principal() // formulario de entrada para validar el ingreso del usuario
{
	global $c;
	...
	<body style='font-size:20px'>
		...
		<br><BR>
		<a onclick='entrar();' class='sinfo' style='cursor:pointer'><img src='img/LOGO_AOA.png' height='260px' BORDER=5><br><b style='font-size:18px;color:000099;'>CLICK EN LA IMAGEN PARA ENTRAR</b><span style='font-size:70px;font-weight:bold;'>INGRESAR</span></A>
		</center><BR>
		No presione la tecla de Bloqueo de Mayusculas, no utilice guiones, puntos, comas ni cualquier separador. Solamente digite las letras y los números de la placa. El código de servicio son solamente números. No utilice la tecla Enter. Sólamente utilice el mouse. 
...
function validar_ingreso() // validacion del ingreso del usuario
{
	global $p,$c;
	if($id=qo1("select c.id from siniestro s,call2cola2 c where c.siniestro=s.id and s.placa='$p' and c.codigo='$c'")) // busca en la cola2 de call center la coincidencia de la placa y el codigo unico
...
function presenta_caso() // muestra el caso al asegurado
{
	global $id;
	html('AOA COLOMBIA S.A. - SERVICIO DE VEHÍCULO DE REEMPLAZO');
	if($D=qo("select * from call2cola2 where id='$id'")) // trae todos los datos de la cola2 de call center
	{
		$Sin=qo("select * from siniestro where id=$D->siniestro"); // trae todos los datos del siniestro
		$Ase=qo("select * from aseguradora where id=$Sin->aseguradora"); // trae todos los datos de la aseguradora
		if(inlist($Ase->id,'1,8,9')) // si la aseguradora es allianz 
		{
			if($Sin->dias_servicio==7) {$Ase->archivo_contrato='contrato_allianz7.pdf';$Ase->archivo_contrato_php='aseguradora/contrato_allianz7.php';}
			elseif($Sin->dias_servicio>7) {$Ase->archivo_contrato='contrato_allianz10.pdf';$Ase->archivo_contrato_php='aseguradora/contrato_allianz10.php';}
		}
		...
		if($D->aceptado) // en caso de que el asegurado ya haya aceptado terminos y condiciones con anterioridad
		{
		...
		// pinta todos los requisitos para el servicio
		echo "<br><br>
			...
			<li> Haber leído los términos y condiciones del servicio.<br>
			<center><b>TERMINOS Y CONDICIONES DEL SERVICIO:</b></center><br>";
			require($Ase->archivo_contrato_php); // la asguradora controla cada archivo de contrato.
			echo "<br><br>
		...
		{
			// presenta los botones de aceptación y no aceptación de terminos y condiciones
			echo "Señor(a) Asegurado(a), por favor lea los términos y condiciones y si está de acuerdo por favor de click en el  botón <b>ACEPTO TERMINOS Y CONDICIONES</b>. <br><br>
...
function acepta_terminos() // aceptacion de terminos y condiciones por el asegurado
{
	global $id;
	html('AOA COLOMBIA S.A. - SERVICIO DE VEHÍCULO DE REEMPLAZO');
	if($D=qo("select * from call2cola2 where id='$id'"))
	{
		...
		q("update call2cola2 set aceptado=1,fecha_aceptacion='$Ahora',ip='$Ip' where id=$id"); // actualiza la cola 2 de call center
		graba_bitacora('call2cola2','M',$id,'Acepta Terminos y Condiciones'); // graba la bitacora de la cola 2
		...
		$Ase=qo("select * from aseguradora where id=$Sin->aseguradora"); // vuelve a cargar los datos de la aseguradora
		$correo_asegurado=$Sin->declarante_email; // obtiene el correo del asegurado
		html('AOA COLOMBIA S.A. SERVICIO DE VEHÍCULO DE REEMPLAZO');
		echo "<body style='font-size:16px'><h3>AOA COLOMBIA S.A. SERVICIO DE VEHÍCULO DE REEMPLAZO</h3>
		Señor(a) Asegurado(a) $Sin->asegurado_nombre, usted ha aceptado los Términos y Condiciones del Servicio de Vehículo de Reemplazo de Administración Operativa Automotriz S.A. versión (2013-05-30)<br><br>
		...
		En cualquier momento puede volver a consultar su servicio ingresando a http://app.aoacolombia.com/servicio y digitando su placa y <i>Código Único de Servicio</i>.<br><br>
		...
		// envia el correo al asegurado infomrando la aceptacion de terminos y condiciones
		...
		"direccioncallcenter@aoacolombia.com,DIRECCION CALL CENTER"   /*Con copia*/ ,	"ACEPTACION TERMINOS Y CONDICIONES"  /*OBJETO*/,
		nl2br("
		Señor(a) Asegurado(a) $Sin->asegurado_nombre,
		
		Reciba cordial saludo. 
		
		Usted ha ACEPTADO los Términos y Condiciones del Servicio de Vehículo de Reemplazo de Administración Operativa Automotriz S.A. (versión 2013-05-30).
		...
		En cualquier momento puede volver a consultar su servicio ingresando a http://app.aoacolombia.com/servicio y digitando su placa y Código Único de Servicio.
...
function noacepta_terminos() // cuando no acepta terminos y condiciones
{
	global $id;
	if($D=qo("select * from call2cola2 where id='$id'")) // obtiene la informacion de la cola 2 de call center
	{
		$Sin=qo("select * from siniestro where id=$D->siniestro"); // trae la informacion del siniestro
		html('AOA COLOMBIA S.A. SERVICIO DE VEHÍCULO DE REEMPLAZO');
		echo "<script language='javascript'>
		...
		// muestra las causales de no adjudicacion para que el usuario pueda seleccionar alguna
...
function noacepta_terminos_ok() // finalizacion de la no aceptacion de terminos y condiciones
{
	global $id,$motivo_no_adjudicacion,$idsin,$naseg;
	$Ahora=date('Y-m-d H:i:s');
	$Subc=qo("select * from subcausal where id='$motivo_no_adjudicacion' "); // obtiene los datos de la subcausal
	$ncausal=qo1("select nombre from causal where id=$Subc->causal").' - '.$Subc->nombre; // obtiene los datos de la causal
	$Sin=qo("select * from siniestro where id=$idsin"); // trae los datos del siniestro
	$Ase=qo("select nombre,razon_social,numero_call from aseguradora where id=$Sin->aseguradora"); // trae información de la aseguradora
	...
	q("update call2cola2 set estado=6 where siniestro=$idsin"); // cambia el estado de la cola 2 de call center 
	q("delete from call2cola1 where siniestro=$idsin"); // borra el registro de la cola 1 para disminuir consultas
	q("delete from call2cola3 where siniestro=$idsin"); // borra el registro de la cola 3
	q("delete from call2cola4 where siniestro=$idsin"); // borra el registro de la cola 4
	html();
	echo "<body style='font-size:16px'><h3>AOA COLOMBIA S.A. SERVICIO DE VEHÍCULO DE REEMPLAZO</h3>
...
function formulario_garantia() // formulario para capturar datos de la garantia ( no se esta usando)
{
	html('AOACOLOMBIA.COM - VEHÍCULO DE REEMPLAZO - FORMULARIO DE GARANTÍA');
	echo "<body><h3>Este módulo está en evaluación. No hay acceso temporalmente al módulo de captura de datos de garantías de los asegurados.</h3>
	<b>Con todo respeto, se ofrecen las respectivas disculpas. <br><br>
	El procedimiento de <i>Constitución de Garantía</i> se debe hacer directamente en la oficina asignada al momento de la cita de entrega del vehículo.<br><br>
	Si tiene algún comentario o sugerencia por favor comuniquese a <i>pqr@aoacolombia.com</i>.<br><br>
	Cordialmente,<br><br>
	DEPARTAMENTO DE TECNOLOGIA E INFORMACION.<br>
...
function validacion_formulario_garantia() // validacion de datos para el formulario de garantia (no se esta usando)
{
	global $p,$c;
...
function formulario_garantia1() // segunda fase del formulario de garantia (no se esta usando)
{
	global $id;
...
function registrar_solicitud() // registro de solicitud de una autorizacion a partir del formulario de garantias (no se esta usando)
{
	...
	q("insert into app_bitacora (ano,mes,dia,hora,minuto,segundo,nick,nombre,tabla,accion,registro,ip,detalle)
				values ('".date('Y')."','".date('m')."','".date('d')."','".date('G')."','".date('i')."','".date('s')."','$identificacion','$nombre $apellido','cliente','A','$nuevoc','".$_SERVER['REMOTE_ADDR']."','Adiciona registro')");
...
function bajar_archivo_contrato() // Bajar un archivo de contratos (no se esta usando)
{
