...
function factura_general()
{
	...
	echo "		
		...
		{ 
			alert('Cuando se aprueba la factura se genera una factura electr nica debe esperar a que el sistema la genere para terminar el proceso');
			prevent_leave = true;
			console.log('Got process');if(confirm('Desea aprobar la factura? Nota: Esta opci n es requerida para poder imprimirla y es irreversible.'))
		...
		/*function borrar_factura()
		{
			...
			function grabar_garantia()
			{
				...
				if(confirm('Desea utilizar la garant a para pagar la factura?'))
				{
	...
	if($id)
	{
		if($F=qo("select * from factura where id=$id"))
		{
			...
			echo "</td></tr>
			<td>Fecha de emisi n: ".pinta_FC('forma','fecha_emision',$F->fecha_emision)." Fecha de vencimiento: ".
			...
			&nbsp;&nbsp;<a style='cursor:pointer' onclick='imprimir_factural();' class='info'><img src='gifs/Print.png' border='0' height='16'  align='absmiddle' >Imprimirr<span style='width:100px'>Imprimir la Factura - Preimpreso</span></a>
				<br /><b>Cruzar esta Factura con la Garantia :  </b>  ".menu1("Garantia","select id,concat(nombre,' - ',fecha_solicitud,' - ',num_autorizacion,' - ',estado,' - $',valor) from sin_autor where siniestro=$F->siniestro and estado='A' ",0,1,'width:200px')."
			&nbsp;&nbsp;<a style='cursor:pointer' onclick='grabar_garantia();' class='info'><img src='gifs/standar/aplicar.png' border='0' height='16'  align='absmiddle' >Aplicar Garant a<span style='width:100px'>Pagar esta factura con la Garant a.</span></a>
...
function inserta_desde_cita()
{
	...
	if($Cita=qo("select * from cita_servicio where id=$idCita"))
	{
		...
		<script language='javascript'>
			...
			function validar_nuevo_cliente()
			{
				with(document.forma)
				{
					if(!validateEmail(email_e.value)){
						alert('El email no es valido');
						email_e.focus();
						return false;
					}
					var tipovalida = document.getElementById('tipo_id').value;
					console.log(tipovalida);
					if(tipovalida == ''){
					...
					if(!alltrim(email_e.value)) {alert('Es obligatorio tener un correo electr nico');email_e.style.backgroundColor='ffff55';email_e.value.focus(); return false;}					
					
					if(!alltrim(identificacion.value)) {
					alert('Debe digitar la identificaci n');
					...
					if(identificacion.value < 6){
						alert('El numero de identificaci n debe de tenre minimo 6 caracteres');
						identificacion.style.backgroundColor='ffff55';
						identificacion.focus(); 
						return false;
					}
		...
		<body><script language='javascript'>centrar(800,700);</script>
			...
			<td><a class='info' href='$Sin->img_odo_entrada_f' target='vimg'><img src='gifs/camara.png' border='0'><span>Imagen Odometro Devoluci n</span></a>
			...
			<tr><td align='right'>Kilometraje utilizado:</td><td colspan=2><b>$Ubicacion->odometro_inicial - $Ubicacion->odometro_final Kil metros: $Cantidad_kilometros ".($Kilometros_exceso>0?" <font color='red'>kil metros de exceso: $Kilometros_exceso":"")."</b></td></tr>
			<tr><td align='right'>Veh culo prestado:</td><td><b>$Cita->placa</b></td><td>Flota: <b>$Flota->nombre</b></td></tr>
			<tr><td align='right'>Tipo de identificaci n del Asegurado:</td><td colspan='2'>".menu1("tipo_id","select codigo,nombre from tipo_identificacion",'',$D->tipo_id)."</td></tr>";
		
		echo "
		
		<tr><td align='right'>Identificaci n del Asegurado:</td><td colspan='2'><input type='text' name='identificacion' value='$Sin->asegurado_id' size=15 class='numero' onblur='identifica_tercero(this.value);'></td></tr>";
		...
		echo  "<tr><td align='right'>Selecciona si es juridica o natural</td><td><select id='seleCode' name='seleCode' onchange='changeFunc();'><option>Seleccione</option><option value='02' $select2>Juridica</option><option value='01' $select1>Natural</option></select></td></tr>";
		 ...
		 echo	"<tr id='trPadre' style='display:$style'><td align='right'>Digito de verificaci n</td><td><input type='text' style='width: 5%;' name='codigoVerificacion' id='codigoVerificacionId' maxlength='1' value='".($D->dv)."' id='codigoVerificacionId' ></td></tr>";	
		
		
		echo "	<tr><td align='right'>Lugar de expedici n de la Identificaci n:</td><td colspan='2'><input type='text' name='lugar_expdoc' value='$D->lugar_expdoc' size=50></td></tr>
		...
		echo "<tr><td align='right'>Ciudad:</td><td colspan='2'><input type='text' name='_ciudad' id='_ciudad' value='$NCiudad' size='70' onclick=\"busqueda_ciudad('ciudad','$Ciudad');\" readonly><input type='hidden' name=ciudad id=ciudad value='$Ciudad'></td></tr>
			...
			<tr><td align='right'>Direcci n Domicilio</td><td colspan='2'><input type='text' name='direccion' value='$D->direccion' size='50'></td></tr>
			<tr><td align='right'>Tel fono Oficina</td><td colspan='2'><input type='text' name='telefono_oficina' value='".($D?$D->telefono_oficina:$Sin->declarante_tel_ofic)."' size='50'></td></tr>
			<tr><td align='right'>Tel fono Casa</td><td colspan='2'><input type='text' name='telefono_casa' value='".($D?$D->telefono_casa:$Sin->declarante_telefono)."' size='50'></td></tr>
			...
			<tr><td align='right'>Direcci n de correo electr nico</td><td colspan='2'><input type='text' name='email_e' value='".($D?$D->email_e:$Sin->declarante_email)."' size='50'></td></tr>
	...
	else
	{
		...
		echo "<body><script language='javascript'>
			centrar(400,300);
		</script><br /><br /><h3 align='center'><font color='red'>No se encuentra la informaci n de la cita $idCita</font></h3></body>";
	}
}
...
function generar_factura()
{
	...
	if($Cita=qo("select * from cita_servicio where id=$idCita"))
	{
		...
		echo "
			...
			function grabar_datos_factura()
			{
				if(document.forma.consecutivo.value==0)
				{
			...
			function genera_solicitud_autorizacion()
			{
				...
				if(!alltrim(document.forma.voucher.value)) {alert('Debe digitar el numero del voucher para realizar la solicitud de autorizaci n de cupo');
		...
		"</td></tr>
		<tr><td align='right'>Tiempo del Servicio:</td><td><b>$Ubicacion->fecha_inicial - $Ubicacion->fecha_final Dias: $Cantidad_dias ".($Dias_exceso>0?" <font color='red'>Dias de exceso: $Dias_exceso</font>":"")."</b></td>
		<td>".($Sin->img_odo_entrada_f?"<a class='info' href='$Sin->img_odo_entrada_f' target='vimg'><img src='gifs/camara.png' border='0'><span>Imagen Odometro Devoluci n</span></a>":"").
				...
				($Sin->fotovh9?"<a class='info' href='$Sin->fotovh9_f' target='vimg'><img src='gifs/camara.png' border='0'><span>Imagen Adicional</span></a>":"")."</td></tr>
				<tr><td align='right'>Kilometraje utilizado:</td><td colspan=2><b>$Ubicacion->odometro_inicial - $Ubicacion->odometro_final Kilometros: $Cantidad_kilometros ".($Kilometros_exceso>0?" <font color='red'>kil metros de exceso: $Kilometros_exceso":"")."</b></td></tr>
				<tr><td align='right'>Vehiculo prestado:</td><td><b>$Cita->placa</b></td><td>Flota: <b>$Flota->nombre</b></td></tr>
		</table>
		<form action='zfacturacion.php' method='post' target='Oculto_facturacion' name='forma' id='forma'>
			<table><tr><td align='right'>Consecutivo de Factura:</td><td colspan=3><input type='text' name='consecutivo' id='consecutivo' value='0' class='numero' size='10' readonly style='font-size:14;color:aa0000'>
				...
				Facturas anteriores: ".menu1('fac_ant',"Select id,t_factura(id) from factura where cliente=$D->id order by id desc",0,1,"","onchange='traer_factura(this.value);' ");
				echo $D->tipo_id;
				$date_future = date('Y-m-d');
				if ($D->tipo_id=="NIT") {
					echo $Aseguradora->nit."<br>";
					echo $Sin->asegurado_id;
					if ($Aseguradora->nit == $Sin->asegurado_id) {
						$date_future = strtotime("+".$Aseguradora->termino_pago_dias." day", strtotime(date('Y-m-d')));
		...
		echo "</td></tr>
			<tr><td align='right'>Cliente:</td><td colspan='3'><input type='text' name='ncliente' id='ncliente' value='$D->apellido $D->nombre' size='80' readonly>
					<input type='hidden' name='cliente' value='$D->id'><input type='hidden' name='aseguradora' value='$Flota->id'></td></tr>
			<tr><td align='right'>Fecha de emision:</td><td><input type='text' name='fecha_emision' value='".date('Y-m-d')."' size='10' readonly > Fecha de vencimiento: ".pinta_FC('forma','fecha_vencimiento',$date_future)."Terminos de pago dias: ";
			if ($D->tipo_id=="NIT") {
				echo $Aseguradora->termino_pago_dias;
			}
			echo"
			...
			<!--&nbsp;&nbsp;<a style='cursor:pointer' onclick='imprimir_factura();' class='info'><img src='gifs/print.png' border='0' height='16'  align='absmiddle' >Imprimirr<span style='width:100px'>Imprimir la Factura</span></a>-->
			<br /><b>Solicitar Nueva autorizacion : </b> Numero de voucher: <input type='text' name='voucher' size=10 class='numero'>&nbsp;&nbsp;
			<a style='cursor:pointer' onclick='genera_solicitud_autorizacion();' class='info'><img src='img/solicita_autorizacion.png' border='0' height='16'  align='absmiddle' >Generar Autorizacion<span style='width:100px'>Solicitar nueva Autorizaci n</span></a>
			<br /><b>Cruzar esta Factura con la Garantia :  </b>  ".menu1("Garantia","select s.id,concat(s.nombre,' - ',s.num_autorizacion,' - ',s.estado,' - $',s.valor,' ',f.nombre)
				from sin_autor s,franquisia_tarjeta f where s.siniestro=$Sin->id and s.estado='A' and s.franquicia=f.id ",0,1,'width:200px')."
			&nbsp;&nbsp;<a style='cursor:pointer' onclick='grabar_garantia();' class='info'><img src='gifs/standar/aplicar.png' border='0' height='16'  align='absmiddle' >Aplicar Garantia<span style='width:100px'>Pagar esta factura con la Garant a.</span></a>
			</td></tr>
			</table>
...
function crear_consecutivo()
{
	...
	if($resolucion_pre->prefijo != null)
	{
		...
		{
                  //if($_SESSION['Nick']=='jemes.navarro' || $_SESSION['Nick']=='sergio.castillo'){
                    IF($IDSF)
			{
				...
				for($i=0;$i<=count($Conceptos_a_facturar);$i++)
				{
					if($Conceptos_a_facturar[$i]>0)
					{
						...
						$neto=$Solicitud->cantidad*$Solicitud->valor;
						
                                                
                                                
                                                if($Solicitud->canon!=0){
                                                    $iva = $Solicitud->iva; 
                                                    $a = $neto + $Solicitud->seguro;
                                                    $base = $iva / 0.19;
                                                    $c = $a - $base;
                                                    $total = $iva + $base + $c;
                                                    $valorund = $Solicitud->valor + $Solicitud->seguro;
                                                    $canon = $Solicitud->valor;
                      
                                                }else{
                                                    $iva=round($neto*$concepto->porc_iva/100,0);
                                                    $total=$neto+$iva+$Solicitud->seguro;
                                                    $base = $neto;
                                                    $valorund = $Solicitud->valor;
                                                    $canon = 0;
                                                }

						mysql_query("insert into facturad (factura,concepto,cantidad,unitario,iva,total,descripcion,base,seguro,canon) values ('$factura','$concepto->id','$Solicitud->cantidad','$valorund','$iva','$total','$Solicitud->descripcion','$base','$Solicitud->seguro','$canon')",$LINK);
						
                                                $Nusuario = $_SESSION['Nombre'];
                                                mysql_query("update solicitud_factura set factura='$factura', procesado_por='$Nusuario', fecha_proceso='$fe' where id='$id_solicitud' ",$LINK);
                                                $inserto_items=true;
					}
				}
                                
				$Totales=qom("select sum(unitario*cantidad) as subtotal,sum(iva) as iva,sum(total) as total, sum(base) as base from facturad where factura='$factura' ",$LINK);
				mysql_query("update factura set subtotal='$Totales->subtotal',iva='$Totales->iva',total='$Totales->total' where id='$factura' ",$LINK); // , base='$Totales->base'
				mysql_close($LINK);
			//}
                    }
			echo "<script language='javascript'>
	...
	else
	{
		...
		if($factura=q("insert into factura (consecutivo,fecha_emision,fecha_vencimiento,cliente,aseguradora,siniestro) values ('$Consecutivo','$fe','$fv','$cl','$as','$sin')"))
		{
			...
			// Inserci n autom tica de detalle cuando la factura se elabora desde la vista general de SINIESTRO
			IF($IDSF)
			{
				$Conceptos_a_facturar=explode(',',$IDSF);
				include('inc/link.php');
				...
				for($i=0;$i<=count($Conceptos_a_facturar);$i++)
				{
					if($Conceptos_a_facturar[$i]>0)
					{
						...
						$inserto_items=true;
                                ...
                                mysql_query("update solicitud_factura set factura='$factura' where siniestro='$sin' ",$LINK);
		...
		echo "
			<script>
				...
				function load_iframe()
				{
					...
					console.log(new_element);
					alert('Verifique el proceso de factura electr nica');
				}
...
function detalle_factura()
{
	...
	if($factura)
	{
		...
		{
			$Subtotal=$Iva=$Total=0;$Seguro=0;
                        $baset = 0;
			while($D=mysql_fetch_object($Detalle))
			{
                            if($D->canon!=0){
                                $a = ($D->unitario * $D->cantidad);
                                $base = $D->iva / 0.19;
                                $c = $a - $base;
                                $total = $D->iva + $base + $c;
                                $und = $a;
                            }else{
                                $base = $D->unitario * $D->cantidad;
                                $total = $base + $D->iva;
                                $und = $D->unitario;
                            }
				...
				echo "<tr><td bgcolor='ffffff'>$D->nconcepto</td>
							...
							<td bgcolor='ffffff' align='right'>".coma_format($und)."</td>
                                                        <td bgcolor='ffffff' align='right'>".coma_format($base)."</td>
							<td bgcolor='ffffff' align='right'>".coma_format($D->iva)."</td>
                                                            <td bgcolor='ffffff' align='right'>".coma_format($D->seguro)."</td>
							<td bgcolor='ffffff' align='right'>".coma_format($total)."</td><td bgcolor='ffffff'>$D->descripcion</td>";
			...
			echo "<tr bgcolor='dddddd'><td><b>TOTALES</b></td>
							<td align='right' colspan=2><b>Subtotal: ".coma_formatd($Subtotal,2)."</b></td>
                                                            <td align='right'><b>".coma_format($baset)."</b></td>							
                                                            <td align='right'><b>".coma_format($Iva)."</b></td>
                                                            <td align='right'><b>".coma_format($Seguro)."</b></td>
		...
		if(!$Fac->autorizadopor)
		{
			...
			echo "<td bgcolor='ffffff' align='right'><input type='text' class='numero' name='unitario' id='unitario' value='' size='8' onblur='liquida_item();'></td>";
                        echo "<td bgcolor='ffffff' align='right'><input type='text' class='numero' name='base' id='base' value='' size='5' readonly></td>";
			echo "<td bgcolor='ffffff' align='right'><input type='text' class='numero' name='iva' id='iva' value='' size='5' readonly></td>";
                        echo "<td bgcolor='ffffff' align='right'><input type='text' class='numero' name='seguro' id='seguro' value='' size='5' ></td>";
		...
		if(count($rows) > 0 )
		{
			...
			foreach($rows as $row)
			{
				if($row->iva_percent != $concept_to_fact->porc_iva)
				{
					...
					//Innecesario con la ultima versi n de facturaci n electr nica
				}					
			}*/
...
//			$P->Cell(90,5,'ADMINISTRACI N OPERATIVA AUTOMOTRIZ S.A.',0,0,'C');
//			$P->setxy(100,9);
//			$P->Cell(90,5,'NIT.: 900.174.552-5 - I.V.A. R GIMEN COM N',0,0,'C');
//			$P->setfont('Arial','',8);
//			$P->setxy(100,14);
//			$P->Cell(90,5,'ACTIVIDADES ECON MICAS No. 7710 Y 7020',0,0,'C');
//			$P->setxy(100,17);
//			$P->Cell(90,5,'NO SOMOS GRANDES CONTRIBUYENTES - NO SOMOS AUTORRETENEDORES',0,0,'C');
//			$P->setxy(100,21);
//			$P->Cell(90,5,'CENTRO COMERCIAL HACIENDA SANTA B RBARA OFICINA D-503',0,0,'C');
//			$P->SETXY(100,24);
//			$P->CELL(90,5,'PBX: (571) 629 3096 - FAX (571) 620 0967',0,0,'C');
//			$P->SETXY(100,27);
//			$P->CELL(90,5,'www.aoacolombia.com - Bogot , D.C. - Colombia',0,0,'C');
//			$P->SETFONT('Arial','B',8);
...
//			$P->setfont('Arial','B',6);
//			$P->setxy(13,$Y+28);$P->cell(30,4,'ADMINISTRACI N OPERATIVA AUTOMOTRIZ');
//			$P->setxy(13,$Y+54);$P->cell(90,4,'FIRMA Y SELLO AUTORIZADOS',0,0,'C');
//			$P->setxy(15,$Y+58);$P->cell(90,4,'Esta Factura de Venta es un T tulo Valor de conformidad con lo establecido en',0,0,'C');
//			$P->setxy(15,$Y+60);$P->cell(90,4,'la Ley 231 de julio 17 de 2008 y dem s normas que lo complementan.',0,0,'C');
...
/*			$P->setfont('Arial','',10);$P->SetTextColor(0,0,0);
   ...
   $P->setxy(35,$P->y+6);$P->SetFont('Arial','',8);$P->Cell(180,4,'Se or Proveedor: Favor hacer referencia del n mero de esta Orden de Servicio en su factura.');
   ...
   "Orden_$id.pdf",$Archivo	);
		...
		echo "<script language='javascript'>
			...
			{
				centrar(10,10);
				alert('No se encuentra informaci n de la factura n mero $consecutivo');
				window.close();
				void(null);
			}
		</script>
...
function aprobar_factura() 
{
	...
	if($Fac->id == 43408)
	{
		/*echo "Modo de prueba para fact electron ca";	
	...
	$Ahora=date('Y-m-d H:i:s');
		...
		//q("update siniestro set obsconclusion=concat(obsconclusion, \"\n$Nusuario [$Ahora]: Elabora Factura de Reintegro n mero $Fac->consecutivo de fecha $Fac->fecha_emision por valor de $ ".coma_format($Fac->total)."\") where id=$Fac->siniestro ");
		...
		$m="<body><table border cellspacing='0'><td>Factura N mero</td><td colspan='3'>$consecutivo</td></tr>".
		...
		$m.="<table><tr><td>Subtotal:</td><td align='right'>".coma_format($Fac->subtotal)."</td></tr>".
			...
			"<br>Este es un correo automatico generado por el Sistema de Control.</body>";
...
function aprobar_factura_ANTERIOR_19_11_30()
{
	...
	{	
		...
		<body>
			...
			<script language='javascript'>console.log('Make another process');   alert('Aprobaci n satisfactoria por $Nusuario , enviado a $Email_usuario'); window.scrollTo(0,parent.document.body.scrollHeight); ";	
...
function anular_factura()
{
	...
	echo "<script language='javascript'>
		...
		<form action='zfacturacion.php' method='post' target='_self' name='forma' id='forma'>
			...
			Proceso de anulaci n de la factura: <b>$F->consecutivo</b><br />
			Cliente: <b>$F->ncliente</b><br />
			Aseguradora: <b>$F->naseg</b><br />
			<br />Digite por favor el motivo de la anulaci n:<br /></font>
...
function anular_factura_ok()
{
	...
	"<table border cellspacing='0'><td>Factura N mero</td><td colspan='3'>$Fac->consecutivo</td></tr>".
	"<tr><td>Fecha de Factura:</td><td>$Fac->fecha_emision</td><td>Vencimiento:</td><td>$Fac->fecha_vencimiento</td></tr>".
	"<tr><td>Cliente:</td><td colspan='3'>".qo1("select concat(apellido,' ',nombre) from cliente where id=$Fac->cliente")."</td></tr>".
	"<tr><td>Aseguradora:</td><td colspan=3>".qo1("select nombre from aseguradora where id=$Fac->aseguradora")."</td></tr>";
	if($Fac->siniestro)
	...
	"<br>Este es un correo autom tico generado por el Sistema de Control.</body>";
	...
	echo "<script language='javascript'>function carga() {alert('Anulaci n satisfactoria por $Nusuario');window.close();void(null);} </script><body onload='carga()'></body>";
}
...
function aplicar_garantia()
{
	...
	if($F->garantia)
	{
		echo "<body><script language='javascript'>alert('Ya fue efectuada la aplicaci n de pago de esta factura con la Garant a');</script>";
		die();
	}
...
function marca_solicitud_procesada()
{
	...
	echo "<body><h3>Marcaci n Solicitud de Facturaci n Procesada</h3>
		<form action='zfacturacion.php' method='post' target='_self' name='forma' id='forma'>
			<table>
				<tr ><td >Siniestro</td><tr >$S->nsiniestro</tr></tr>
				<tr ><td >Concepto</td><td >$S->nconcepto</td></tr>
				<tr ><td >Descripci n</td><td >$S->descripcion</td></tr>
...
function marca_solicitud_procesada_ok()
{
	...
	echo "<body><script language='javascript'>alert('Marcaci n satisfactoria');window.close();void(null);</script></body>";
}
...
function verificar_factura_electronica()
{
	...
	header('Content-Type: text/html; charset=utf-8');
	include($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/views/factura_electronica/interfazNUEVO.html");
}
