...
function imprimir_acta() // imprime el acta de entrega/devolucion
{
	...
	$texto="Me fueron explicados los mecanismos de encendido, bloqueo central y apertura de puertas del vehículo";
	...
	if(inlist($Siniestro->aseguradora,'3,7'))
	{
		...
		$P->setxy(102,231);$P->cell(100,3,"experiencia de los servicios más recientes de vehículo sustituto por favor califique los",0,0,'L',1);
		...
		$P->setxy(102,242);$P->cell(100,3,"5. Claridad en la información recibida para la asignación de Vehículo Sustituto:_____",0,0,'L');
		$P->setxy(102,247);$P->cell(100,3,"6. Por favor indíquenos el número de veces que tuvo que llamar para recibir información del",0,0,'L',1);
		$P->setxy(102,250);$P->cell(100,3,"servicio y coordinar la entrega del vehículo: _____",0,0,'L',1);
		$P->setxy(102,253);$P->cell(100,3,"7. Tiempo para pa asignación del vehículo: _____",0,0,'L');
		$P->setxy(102,259);$P->cell(100,3,"8. Facilidad para la entrega del vehículo: _____   9. Calidad del Vehículo asignado:_____",0,0,'L');
		$P->setxy(102,261);$P->cell(100,3,"10. La facilidad para la devolución del Vehículo: _____",0,0,'L',1);
		//$P->SetFillColor(255,255,255);
	}
	else
	{
		...
		{			
			$P->setxy(102,212);$P->cell(100,3,"1. ¿Cómo califica el servicio prestado por el agente que atendió su llamada?",0,0,'L',1);			
			$currentY = 218;
		}
		else{
			$P->setxy(102,212);$P->cell(100,3,"1. Califique de 1 a 5 la información y orientación recibida en el primer contacto por el personal",0,0,'L',1);
		...
		$P->setfont('Arial','',7);
		$P->setxy(102,$currentY+7);$P->cell(100,3,"2. Califique de 1 a 5 la gestión y agilidad de nuestras  auxiliares de servicio al  cliente en el");
		...
		if($Siniestro->aseguradora == 55) 
		{			
			$P->setxy(102,235);$P->cell(100,3,"3. ¿Cómo califica la calidad del servicio prestado ? ",0,0,'L',1);			
			$currentY = 239;
		}
		else{
			$P->setxy(102,235);$P->cell(100,3,"3. Califique de 1 a 5 la gestión y agilidad de nuestros auxiliares operativos en el momento de",0,0,'L',1);
			$P->setxy(102,238);$P->cell(100,3,"la entrega del vehículo:",0,0,'L',1);
			$currentY = 239;	
		}
		...
		{
			$P->setfont('Arial','',7);
			$P->setxy(102,247);$P->cell(100,3,"4. De ser necesario utilizaría nuevamente nuestros servicios?");
			$P->setfont('Arial','B',7);
			...
			$P->rect(160,250,2,2);$P->setxy(163,250);$P->cell(13,2,"No lo utilizaría.",0,0,'L',1);			
		}
		else
		{
			$P->setfont('Arial','',7);
			...
			{
				$P->setxy(102,248);$P->cell(100,3,"4.En una escala de 0 a 10 siendo 0 muy improbable y 10 muy probable");
				$P->setxy(102,251);$P->cell(100,3,"¿Recomendaría HDI Seguros a su familia o amigos ?: _________");		
			}
		}
		$P->setfont('Arial','',7);
		...
		if($Siniestro->aseguradora == 55) 
		{			
			$P->setxy(102,254);$P->cell(100,3,"5. De acuerdo con la experiencia, nos recomendaria con familiares o amigos. ¿Cual es la",0,0,'L',1);
			$P->setxy(102,257);$P->cell(100,3,"pricipal razón por la que nos dio este puntaje? *Calificaciones menores a 8.",0,0,'L',1);
			$P->setxy(102,247);$P->cell(100,3,"mejora  mas importante que debemos realizar para tener un resultado cercano a 10?",0,0,'L',1);			
			$P->setfont('Arial','B',7);	
			$P->setxy(102,260);$P->cell(100,3,"Calificacion: _________");
			//$P->setxy(130,250);$P->cell(100,3,"¿Por que ?:");			
			$P->setfont('Arial','',7);
		}
		else{
			$P->setxy(102,254);$P->cell(100,3,"5. Recomendaría usted los servicios prestados por AOA S.A.S. a sus familiares o conocidos",0,0,'L',1);
		...
		if($Siniestro->aseguradora == 55) 
		{			
			$P->setxy(102,264);$P->cell(100,3,"6. ¿Cuanto esfuerzo personal tuvo que invertir en la prestación del servicio?");
			...
			$P->setxy(102,264);$P->cell(100,3,"6. Califique de 1 a 5 en términos generales los servicios prestados por AOA S.A.S.");
			$P->setfont('Arial','B',7);
	...
	//$P->cell(80,17,' ');
	$P->setxy(10,240);
	$P->cell(13,1,"SI AL MOMENTO DE LA DEVOLUCIÓN DEL VEHÍCULO SE PRESENTA",0,'L');
	$P->setxy(10,243);
	$P->cell(13,1,"UN DAÑO CON RESPECTO A COMO FUE ENTREGADO INICIALMENTE,",0,'L');
	$P->setxy(10,246);
	$P->cell(13,1,"EL COSTO DE LA REPARACIÓN PARA LLEVARLO A SU ESTADO INICIAL, ",0,'L');
	$P->setxy(10,249);
	$P->cell(13,1,"CORRERÁ A CARGO DEL CLIENTE O USUARIO FIRMANTE DE PRESENTE",0,'L');
	$P->setxy(10,252);
	$P->cell(13,1,"ACTA,SI NO SE CONOCE EL VALOR LOS DAÑOS SE SOLICITARÁ UNA",0,'L');
	$P->setxy(10,255);
	$P->cell(13,1,"COTIZACIÓN AL PROVEEDOR AUTORIZADO DE LA COMPAÑÍA Y UNA VEZ",0,'L');
	$P->setxy(10,258);
	$P->cell(13,1,"CUENTE CON LA COTIZACIÓN SE DEBERÁ CANCELAR EL VALOR DE",0,'L');
	$P->setxy(10,261);
	$P->cell(13,1,"MANERA INMEDIATA Y/O REALIZAR EL COBRO CONTRA LA GARANTÍA",0,'L');
	$P->setxy(10,264);
	$P->cell(13,1,"QUE HAYA SIDO DEJADA A AOA. PARA LOS DEMÁS CASOS USTED DEBE",0,'L');
	$P->setxy(10,267);
	$P->cell(13,1,"EXIGIR LA COPIA DE SU FACTURA Y/O RECIBO DE CAJA , DE LO CONTRARIO",0,'L');
	$P->setxy(10,270);
	$P->cell(13,1,"NO DEBERÁ CANCELAR NINGÚN VALOR.",0,'L');
	$P->setxy(1,270);
	$P->cell(13,2,". ",0,'L');
	...
	$P->setfont('Arial','',8);
	$P->setxy(10,274);$P->multicell(199,4,"Carrera 69 B No. 98 A - 10 • PBX: +(571) 756 0510 • Fax: 756 0510 Ext. 112 • Bogotá D.C., Colombia • www.aoacolombia.com",0,'C');
	...
	if($Linea->delineacion_n) $P->Image($Linea->delineacion_n,187,186,15,20);
	{
		...
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 9: La mayoría de los vehículos cuentan con un dispositivo de rastreo satelital, para salvaguardar los intereses de la compañía, cualquier alteración o daño de este dispositivo, será responsabilidad del Arrendatario y/o Usuario, que tenga el vehículo en custodia. ");
		
		$Y=$P->y+2;$P->setxy(20,$Y);
		$P->multicell(180,3,"Cláusula 10: Con la firma del presente anexo el ARRENDATARIO Y/O USUARIO acepta los términos establecidos en las condiciones CONTRATO DE ARRENDAMIENTO DE VEHÍCULOS CONDICIONES GENERALES, el cual ha sido puesto a su disposición para su lectura y conocimiento.");
...
function imprimir_acta_traslado() // imprime el acta de entrega/devolucion
{
	...
	$P->AddFont("c128a","","c128a.php"); // incluye fuentes para codigo de barras
	$P->AliasNbPages();
	$P->setTitle("ACTA DE TRASLADO DE VEHÍCULO ");
	$P->setAuthor("Tecnologia AOA it@aoacolombia.co");
	$P->Numeracion=false;
	$P->SetAutoPageBreak(false);
	$P->SetTopMargin('5');
	...
	$P->setfont('Arial','B',13);
	$P->setxy(10,8);
	$P->Cell(110,5,'ACTA DE TRASLADO DE VEHÍCULO ',0,0,'L');
	$P->setfont('Arial','',8);
	$P->setxy(10,20);$P->multicell(198,4,$TH,1,'L');
	$P->setfont('Arial','',9);
	$P->setxy(10,24);
...
function imprimir_acta_test() // imprime el acta de entrega/devolucion
{
	...
	else
	{
		...
		if($Siniestro->aseguradora == 55) 
		{			
			$P->setxy(102,215);$P->cell(100,3,"1. ¿Cómo califica el servicio prestado por el agente que atendió su llamada?",0,0,'L',1);			
			$currentY = 215;
		}
		else{
			$P->setxy(102,216);$P->cell(100,3,"1. Califique de 1 a 5 la información y orientación recibida en el primer contacto por el personal",0,0,'L',1);
		...
		if($Siniestro->aseguradora == 55) 
		{		
			$P->setxy(102,228);$P->cell(100,3,"3. ¿Cómo califica la calidad del servicio prestado ? ",0,0,'L',1);			
			$currentY = 220;
		}
		else{
			$P->setxy(102,231);$P->cell(100,3,"3. Califique de 1 a 5 la gestión y agilidad de nuestros auxiliares operativos en el momento de",0,0,'L',1);
			$P->setxy(102,233);$P->cell(100,3,"la entrega del vehículo:",0,0,'L',1);
			$currentY = 226;	
		}
		...
		{
			$P->setfont('Arial','',7);
			$P->setxy(102,236);$P->cell(100,3,"4. De ser necesario utilizaría nuevamente nuestros servicios?");
			$P->setfont('Arial','B',7);
			...
			$P->rect(160,239,2,2);$P->setxy(163,239);$P->cell(13,2,"No lo utilizaría.",0,0,'L',1);			
		}
		else
		{
			$P->setfont('Arial','',7);
			if($Siniestro->aseguradora == 55)
			{
				$P->setxy(102,236);$P->cell(100,3,"4.En una escala de 0 a 10 siendo 0 muy improbable y 10 muy probable ¿Recomendaría a");
			...
			if($Siniestro->aseguradora == 93)
			{
				...
				$P->setxy(102,238);$P->cell(100,3,"¿Recomendaría HDI Seguros a su familia o amigos ?: _________");		
			}
		}
		$P->setfont('Arial','',7);
		...
		if($Siniestro->aseguradora == 55) 
		{			
			$P->setxy(102,240);$P->cell(100,3,"5. De acuerdo con la experiencia, nos recomendaria con familiares o amigos. ¿Cual es la",0,0,'L',1);
			$P->setxy(102,243);$P->cell(100,3,"pricipal razón por la que nos dio este puntaje? *Calificaciones menores a 8 ¿Cual es la",0,0,'L',1);
		...
		if($Siniestro->aseguradora == 55) 
		{			
			$P->setxy(102,250);$P->cell(100,3,"6. ¿Cuanto esfuerzo personal tuvo que invertir en la prestación del servicio?, donde 1");
