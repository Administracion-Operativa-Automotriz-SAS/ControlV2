...
function factura_general()
{
	...
	if($id)
	{
		if($F=qo("select * from factura where id=$id"))
		{
			...
			&nbsp;&nbsp;<a style='cursor:pointer' onclick='imprimir_factura();' class='info'><img src='gifs/Print.png' border='0' height='16'  align='absmiddle' >Imprimir<span style='width:100px'>Imprimir la Factura</span></a>
			&nbsp;&nbsp;<a style='cursor:pointer' onclick='imprimir_factural();' class='info'><img src='gifs/Print.png' border='0' height='16'  align='absmiddle' >Imprimir<span style='width:100px'>Imprimir la Factura - Preimpreso</span></a>
...
function inserta_desde_cita()
{
	...
	if($Cita=qo("select * from cita_servicio where id=$idCita"))
	{
		...
		echo "<tr><td align='right'>Identificación del Asegurado:</td><td colspan='2'><input type='text' name='identificacion' value='$Sin->asegurado_id' size=15 class='numero' onblur='identifica_tercero(this.value);'></td></tr>
			<tr><td align='right'>Lugar de expedición de la Identificación:</td><td colspan='2'><input type='text' name='lugar_expdoc' value='$D->lugar_expdoc' size=50></td></tr>
...
function grabar_nuevo_cliente()
{
	...
	q("update cliente set tipo_id='$tipo_id',lugar_expdoc='$lugar_expdoc',nombre='$nombre',apellido='$apellido',pais='$pais',ciudad='$ciudad',
		...
		observaciones='$observaciones' where identificacion='$identificacion' ");
	...
	if($Cita=qo("select * from cita_servicio where id=$idCita"))
	{
		...
		echo "
			function crear_consecutivo()
			{
				...
				window.open('zfacturacion_electronica.php?Acc=crear_consecutivo&fe='+fe+'&fv='+fv+'&cl='+cl+'&as=$Sin->aseguradora&sin=$Sin->id&IDSF=$IDSF','Oculto_facturacion');
			}
			function ingresar_al_detalle()
...
function grabar_observaciones()
{
	...
	echo "<body><script language='javascript'>alert('Observacioens grabadas correctamente');</script>";
}
		...
		if($factura=q("insert into factura (consecutivo,fecha_emision,fecha_vencimiento,cliente,aseguradora,siniestro) values ('$Consecutivo','$fe','$fv','$cl','$as','$sin')"))
		{
			echo "<script language='javascript'>
				function carga()
				{
					...
					window.open('zfacturacion_electronica.php?Acc=detalle_factura&factura=$factura&Aseguradora=$as','dfactura');
				}
			</script>
			<body onload='carga()'></body>";	
...
function aprobar_factura()
{
	...
	$cliente = qo("select * from cliente where id='".$Fac->cliente."'");
	
	if($cliente->emaile == null)
	{
		echo "<body><script language='javascript'>alert('No se puede facturar si el cliente no tiene dentro del sistema un correo electrónico');</script></body>";
		exit;
	}
	...
	{	
		...
		$Envio=enviar_gmail($Email_usuario /*de */,
						$Nusuario /*Nombre de */ ,
						"claudiacastro@aoacolombia.com,Claudia Castro;sergiocastillo@aoacolombia.com,Sergio Castillo;s.ospina@valps.com,Contabilidad" /*para */,
		...
		<div>
			...
			<!--<iframe width='100%' height='100%' src='https://app.aoacolombia.com/Control/operativo/zfacturacion.php?Acc=verificar_factura_electronica&id=".$Fac->id."'></iframe>-->
		</div>
		<script language='javascript'>console.log('Make another process');   alert('Aprobación satisfactoria por $Nusuario'); window.open(src='https://app.aoacolombia.com/Control/operativo/zfacturacion.php?Acc=verificar_factura_electronica&id=".$Fac->id."')";
...
function anular_factura_ok()
{
	...
	$Envio=enviar_gmail($Email_usuario /*de */,
									$Nusuario /*Nombre de */ ,
									"shurtado@aoacolombia.com,Sebastian Hurtado;claudiacastro@aoacolombia.com,Claudia Castro;s.ospina@valps.com,Sandra Ospina;i.vargas@valps.com,Johanna Vargas;sergio.castillo@aoacolombia.com,Sergio Castillo Castro" /*para */,
									"$Email_usuario,$Nusuario" /*con copia*/,
