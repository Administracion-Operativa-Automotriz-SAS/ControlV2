 ...
 * OCTUBRE 29. Se excluira del calculo de tiempos para Liberty los casos que no tengan datos de contacto ni actualizaci�n de datos. 
		Se toma en cuenta cuando el filtro de estado EST est� en ceros.
...
else $Aseg=qo("select * from aseguradora where id in ($ASEG)");
...
echo "<script language='javascript'>
		function verseguimiento(id,contador) { modal('zcalculo_tiempos2.php?Acc=verseguimiento&id='+id+'&Contador='+contador,0,0,10,10,'seg'); }
		function reconstruye_seguimiento(id) { alert('funci�n deshabilitada'); }
...
echo "
	<h3 align='center'>ADMINISTRACION OPERATIVA AUTOMOTRIZ S.A. - C�lculo de Tiempos - Aseguradora: $Aseg->nombre</h3>";
...
if($Siniestros=mysql_query("SELECT si.perdida_total,si.vigencia_desde,si.vigencia_hasta,tmp.* FROM 
							tmpi_tiempos tmp
							INNER JOIN siniestro si ON tmp.numero = si.numero",$LINK))
{
	$A_titulos=array();
	$A_titulos['contador']='#';$A_titulos['numero']='Numero'; $A_titulos['Gama']='Gama';$A_titulos['fau']='Fecha Autorizacion'; $A_titulos['estado']='Estado'; $A_titulos['ingreso']='Ingreso'; $A_titulos['perdida_total']='Perdida total'; $A_titulos['Vigencia_poliza_desde']='Vigencia poliza desde'; 
    $A_titulos['Vigencia_poliza_hasta']='Vigencia poliza hasta';	$A_titulos['gestion']='Gestion';
	$A_titulos['contacto']='Contacto'; $A_titulos['dgestion']='Dif. Gestion'; $A_titulos['dcontacto']='Dif. Contacto'; $A_titulos['causal']='Causal'; $A_titulos['subcausal']='Subcausal';
	$A_titulos['remitido']='Remitido'; $A_titulos['actualizacion']='Actualizacion'; $A_titulos['diferencia']='Diferencia'; $A_titulos['placa']='Placa'; $A_titulos['ciudad']='Ciudad';
	$A_titulos['finicial']='Fec.Ini.Serv.';$A_titulos['dias']='Dias.Serv.';$A_titulos['sucursal']='Sucursal.';
	if($_Exel) echo "<table border cellspacing='0' style='empty-cells:show'><tr><th>#</th><th>Numero</th><th>Gama</th><th>Fecha Autorizacion</th><th>Estado</th>
				<th>Ingreso</th><th>Perdida total</th><th>Vigencia poliza (desde)</th><th>Vigencia poliza (hasta)</th><th>Gestion</th><th>Contacto</th><th>Dif. Gestion</th><th>Dif. Contacto</th><th>Causal</th><th>Subcausal</th>
				<th>Remitido</th><th>Actualizacion</th><th>Diferencia</th><th>Placa</th><th>Ciudad</th><th>Observaciones</th><th>Rango Gestion</th><th>Rango Contacto</th>
				<th>Fec.Ini.Serv.</th><th>Dias.Serv.</th></tr>";
	...
	if($Siniestros2=mysql_query("select si.perdida_total,si.vigencia_desde,si.vigencia_hasta,tmp.* from tmpi_tiempos2 tmp inner join siniestro si on tmp.numero=si.numero"))
	{
	...
	/* echo "<table border cellspacing='0' width='100%'><tr ><td>
					<table width='100%' cellspacing='0'><tr ><td bgcolor='505099' style='color:ffffff'>N�mero de siniestros reportados a AOA en el periodo</td>
					...
					<tr ><td bgcolor='505099' style='color:ffffff'>% de participaci�n</td><td align='right' bgcolor='505099' style='color:ffffff'>$Porcentaje_pendientes %</td></tr>
					...
					<tr ><td bgcolor='505099' style='color:ffffff'>% de participaci�n</td><td align='right' bgcolor='505099' style='color:ffffff'>$Porcentaje_noadj %</td></tr>
	 				</table></td></tr></table>";*/
	...
	echo "<table cellspacing='5' width='100%'>
			...
			<td><table border cellspacing='0'><tr><td colspan=3 bgcolor='505099'  style='color:ffffff'></b>Distribuci�n<b></td></tr>
	...
	echo "<table><tr><th>Informaci�n erronea:</th></tr><tr><td> $Erroneos de $Solicitudes_mes = $Porcentaje % </td></tr>
...
function pinta_detalle($S,$BGC='ffffff',$Acumula=true)
{
	...
	echo ">$S->nestado </td><td align='center' nowrap='yes'>$S->ingreso</td><td align='center' nowrap='yes'>$S->perdida_total</td><td align='center' nowrap='yes'>$S->vigencia_desde</td><td align='center' nowrap='yes'>$S->vigencia_hasta</td><td align='center' nowrap='yes'>$S->gestion</td><td align='center' nowrap='yes'>".($S->contacto>$S->ingreso?$S->contacto:'')."</td>";
	////////// GESTION
...
siniestro.ubicacion=ubicacion.id and ubicacion.fecha_inicial >= '$Fecha_verificacion' ");
		...
		echo "<form action='zcalculo_tiempos2.php' method='post' target='IResultado' name='forma' id='forma'>
			Aseguradora: ";
	if($_SESSION['User']==11 /*Aseguradora nivel avanzado 1*/)
	{
		$Aseg=qo1("select aseguradora from usuario_aseguradora1 where id=".$_SESSION['Id_alterno']);
	...
	else
	{
		...
		echo "<option value='3,7'>LIBERTY GA y GM</option>
				<option value='4,262,10'>MAPFRE todas</option>
				<option value='1,8,9'>ALLIANZ todas</option>
				<option value='1,4,6,8,9,10,11,15,39,48,53,55,59,60,66,70,71,72,73,74,75,76,
				77,78,81,83,84,88,89,91,92,93,94,95,105,110,111,143,144,145,146,147,148,173,181,
				182,183,184,186,188,192,193,194,198,199,200,201,202,203,204,206,208,209,210,211,212,213,
				215,216,218,220,223,224,225,227,228,229'>Todas</option></select>";
	}
