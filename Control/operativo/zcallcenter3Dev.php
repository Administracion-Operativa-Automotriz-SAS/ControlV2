...
function iniciar_callcenter2()
{
	...
	function tomar_nuevo()
	{
		...
		else {alert('En este momento el sistema está buscando un caso nuevo para ser tomado');}
	}
...
function validationDays($deliveryDate,$Cita,$Oficina){
	...
	$arrayDate = array();
			...
			for($y = 0; $y < count($arrayDate); $y ++){
				...
				if($dayName != "Domingo" and $dayName != "Sabado"){
...
function modulo_perfil_administrador()
{
	...
	<a style='cursor:pointer' onclick=\"modal('zcall2estadistica.php',0,0,100,100,'call2estadistica');\">Módulo de Estadísticas</a>
...
function pinta_cola2a()
{
	...
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
	<h4>Correos de Pre-Adjudicación enviados</h4>";
	...
	{
		echo "<a style='cursor:pointer' onclick='recargar();'><img id='cargando' src='gifs/standar/Refresh.png' border='0' height='18px'></a>
		Numero de casos: ".mysql_num_rows($Cola2)." Mas de 2 Días: <span id='cantig'></span><br>
		<table border cellspacing='0' width='100%'><tr><th>#</th><th>Aseguradora</th><th>Siniestro</th><th>Hora</th><th>Opc</th></tr>";
...
function modulo_perfil_agente()
{
	...
	if($Dagente->nivel>1) echo "
          ...
          <script language='javascript'>
				 ...
				 function ingresar_pa(dato) {if(confirm('Desea ingresar a Pequeños accesoríos ?')) 
			    window.open('https://app.aoacolombia.com/conVue/?#/inicio_call&PA_USUSE=$PA_USUSE&ciudad_siniestro=$Ofic->nombre&declarante_ciudad=$Ciuorig&declarante_email=$Sin->declarante_email&declarante_telefono=$Sin->declarante_telefono&declarante_nombre=$Sin->declarante_nombre&tokenNick=$tokenNick&siniestro=$Sin->numero&tokenPeril=$Perfil->idnombre&tokenUser=$NUSUARIO');
		
			}
		</script>	
			
	<table cellspacing=4><tr><td align='center' bgcolor='dddddd'><a onclick='ver_apartados()' style='cursor:pointer;font-size:16px'><img src='img/vehiculo.png' border='0' height='60' align='bottom'><br>Ver vehículos apartados</a> </td>
	...
	<td align='center' bgcolor='dddddd'><a style='cursor:pointer;font-size:16px' href='https://app.aoacolombia.com/conVue/?#/inicio_call&PA_USUSE=$PA_USUSE&ciudad_siniestro=$Ofic->nombre&declarante_ciudad=$Ciuorig&declarante_email=$Sin->declarante_email&declarante_telefono=$Sin->declarante_telefono&declarante_nombre=$Sin->declarante_nombre&tokenNick=$tokenNick&siniestro=$Sin->numero&tokenPeril=$Perfil->idnombre&tokenUser=$NUSUARIO'  \"><img src='http://app.aoacolombia.com/img/Car-Repair-icon.png' border='0' height='60'><br>PEQUEÑOS ACCESORIOS</a></td>
...
{
	// Esta rutina debe ser la que llama el modulo de voz ip , debe recibir el id del siniestro ($idsiniestro), y la extesión marcada ($extension)
...
&did_pattern=&did_description=&closecallid=&xfercallid=&agent_log_id=126&entry_list_id=2000&call_id=V
	...
	*/
	...
	if($AgenteCall=qo("select * from usuario_callcenter where extension='$extension' limit 1"))
	{
		...
		{
			//Agrege nueva condición para no cambiar como funciona el programa sin embargo es necesario revisar por que crea el caso dos veces
			//echo "new condition";
	...
	else
	{
		html();
		echo "<body><h3>FALLO EN REDIRECCION VOZ IP</h3>
			Error: No se encuentra el agente correspondiente a la extensión. <br><br>
			Id Siniestro: $idsiniestro  <br>
			Extensión $extension
		</body>";
	}
...
function buscar_caso()
{
	...
	if($placa)
	{
		...
		else echo "<body><script language='javascript'>alert('No hay información');</script></body>";
	}
	if($siniestro)
	{
...
function presentar_caso()
{
	...
	if(alltrim(tipoCaja)=== 'Seleccione'){
		alert('Debe seleccionar un tipo de caja ');		
			...
			window.open('zcallcenter2.php?Acc=adicionar_observaciones&id=$id&d='+Obs+'&tipoCaja='+tipoCaja,'Oculto_presentar_caso');
	}
		}else{
...
' row='50' cols='50'>$Ofic->picoyplaca</textarea></td>
		...
		$sql = "SELECT pqso.fecha,pqso.descripcion,est.nombre estado, est.color_co color FROM pqr_solicitud pqso 
				...
				WHERE pqso.id = $Sin->pqr_asociado";
				 ...
				 <tr><td class='rojo'>Hay un PQR en el siniestro:</td> <td  width='300'><b>Numero: $Sin->pqr_asociado</b></td>
							 ...
							 <tr><td class='rojo'>Descripción PQR:</td> <td bgcolor='ffffff' width='300'><b>$consultaPqr->descripcion</b></td>
	...
	if($Ofic->envio_adjudicacion){$varCheked = "checked";}else{$varCheked = "";}
  ...
  <div class='modal-dialog' role='document'>
    <div class='modal-content'>
      ...
      <div class='modal-body'>
		...
		<b>Teléfono de contacto:</b> $Ofic->telefono<br>
		<hr>
		...
		<b>Restricción Almuerzo:</b> $Ofic->restriccion_almuerzo<br>
...
function control_caso()
{
	...
	IF($Agente->nivel==1)
	{
		...
		if($estado==5 /* Pendiente */)
		{
			if($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='0' and estado='0' "))   // SI ESTA EN PRE-ADJUDICACION Y NO HA ACEPTADO
				echo "<td colspan=3><h3>Este caso se encuentra en estado de espera de aceptación de Términos y Condiciones.</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_email();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>Actualización del correo electrónico</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($Cola2->id);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Reenviar el correo de Pre-Adjudicación</a></td>
			...
			elseif($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='1' and estado='0' "))  // SI ESTA EN PRE-ADJUDICACION PERO YA ACEPTO
				echo "<td colspan=2><h3>Este caso se encuentra en cola nivel 2 - Adjudicación del Vehículo.</h3></td></tr><tr>
			...
			elseif($Cola3=qo("select * from call2cola3 where siniestro=$id"))  // SI ESTA EN COLA3
				echo "<td colspan=2><h3>Este caso se encuentra en cola nivel 3 - Adjudicación del Vehículo.</h3></td></tr><tr>
			...
			else  // SI SIGUE PENDIENTE EN COLA 1
			{
				echo "<td bgcolor='ffffff' align='center'><a onclick='contacto_exitoso();' style='cursor:pointer'><img src='img/contacto_call_asegurado.png' height='80'><br>Contacto Exitoso</a></td>
					...
					<td bgcolor='ffffff' align='center'><a onclick='buzon();' style='cursor:pointer'><img src='img/buzon.png' height='80'><br>Buzón de Voz</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='informacion_erronea();' style='cursor:pointer'><img src='img/informacion_erronea.png' height='80'><br>Información Errónea</a></td>";
				...
				if(($Cantidad_buzones>3 && !$Contacto_exitoso) || ($Cantidad_buzones>6))
					echo "<td bgcolor='ffffff' align='center'><a onclick='no_adjudica();' style='cursor:pointer'><img src='img/no_adjudicacion.png' height='80'><br>No Adjudicación<br>($Cantidad_buzones buzones)</a></td>";
				else
				...
				echo "<td bgcolor='ffffdd' align='center'><a onclick='marcacion_numero_alterno();' style='cursor:pointer'><img src='img/cambio_llamada.png' height='80'><br>Marcar Número Alterno</a></td>";
			}
		...
		if($estado==1)
		{
			...
			<a onclick='solicitar_reactivacion();' style='cursor:pointer;font-size:14px;'><img src='img/reactivacion3.png' border='0' height='80'><br>Solicitar Re-Activación de Caso</a></td>";
		}
		echo "</tr></table></body>";
	}
	if($Agente->nivel==2)
	{
		...
		echo "<script language='javascript'>
				...
				function no_adjudica() { window.open('zcallcenter2.php?Acc=caso_no_adjudica&id=$id','_self'); }
		         ...
		         function ingresar_novedad(dato) {if(confirm('Desea ingresar una novedad del siniestro $id?')) 
					 window.open('https://app.aoacolombia.com/conVue/?#/home?declarante=$Sin->declarante_email?tele=$Sin->declarante_telefono?Declarante=$Sin->declarante_nombre?tokenNick=$tokenNick&tokenPeril=$tokenPeril&tokenUser=$USUARIO');}
		...
		if($Sin->estado==5 /* Pendiente */)
		{
			if($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='0' and estado='0' "))
				echo "<td colspan=2><h3>Este caso se encuentra en estado de espera de aceptación de Términos y Condiciones.</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_email();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>Actualización del correo electrónico</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($Cola2->id);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Reenviar el correo de Pre-Adjudicación</a></td>";
			else
			{
				...
				echo "<td bgcolor='ffffff' align='center'><a onclick='asignar_vehiculo();' style='cursor:pointer'><img src='img/adjudicacion1.png' height='60'><br>Adjudicar Vehiculo</a></td>
					...
					<td bgcolor='ffffff' align='center'><a onclick='remarcacion();' style='cursor:pointer'><img src='img/buzon.png' height='60'><br>Buzón de Voz - Remarcación</a></td>
				...
				if($idc=qo1("select id from call2cola2 where siniestro=$id"))
					echo "<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($idc);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='60'><br>Reenviar el correo de Pre-Adjudicación</a></td>";
				else
					echo "<td bgcolor='ffffff' align='center'><a onclick='preadjudica();' style='cursor:pointer'><img src='img/preadjudicacion.png' height='60'><br>Reenviar el correo de Pre-Adjudicación</a></td>";
			//	if($_SESSION['nivelcaso']==1)
				echo "<td bgcolor='ffffff' align='center'><a onclick='actualizar_datos();' style='cursor:pointer'><img src='img/actualizacion.png' height='60'><br>Actualización de datos</a></td>
								<td bgcolor='ffffff' align='center'><a onclick='informacion_erronea();' style='cursor:pointer'><img src='img/informacion_erronea.png' height='60'><br>Información Errónea</a></td>";
				// NUEVO BOTON MARCAR NUMERO ALTERNO
				echo "<td bgcolor='ffffdd' align='center'><a onclick='marcacion_numero_alterno();' style='cursor:pointer'><img src='img/cambio_llamada.png' height='80'><br>Marcar Número Alterno</a></td>";
			}
			echo "<td bgcolor='ffffff' align='center'><a onclick='no_adjudica();' style='cursor:pointer'><img src='img/no_adjudicacion.png' height='80'><br>No Adjudicación</a></td>";
			echo "</tr></table>
					<iframe name='Oculto_asignacion' id='Oculto_asignacion' style='visibility:hidden' width='1' height='1'></iframe>";
		}
		elseif($Sin->estado==3 /* Adjudicado */ )
		{
			echo "<td colspan=2><h3>Este caso se encuentra Adjudicado</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_infofin();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>Actualización de la Información Financiera</a></td>
	...
	if($Agente->nivel>=3)
	{
		...
		echo "<script language='javascript'>
                ...
                function ingresar_novedad(dato) {if(confirm('Desea ingresar una novedad del siniestro $id?')) 
			    window.open('https://app.aoacolombia.com/conVue/?#/home?tokenNick=$tokenNick&tokenPeril=$tokenPeril&tokenUser=$tokenUser');}
		...
		{
			if($Cola2=qo("select * from call2cola2 where siniestro=$id and aceptado='0' and estado='0' "))
				echo "<td colspan=6><h3>Este caso se encuentra en estado de espera de aceptación de Términos y Condiciones.</h3></td></tr><tr>
					<td bgcolor='ffffff' align='center'><a onclick='actualizar_email();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>Actualización del correo electrónico</a></td>
					...
					<td bgcolor='ffffff' align='center'><a onclick='remarcacion();' style='cursor:pointer'><img src='img/buzon.png' height='80'><br>Buzón de Voz - Remarcación</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='re_enviar_correo($Cola2->id);' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Reenviar el correo de Pre-Adjudicación</a></td>
					<td bgcolor='ffffff' align='center'><a onclick='aceptacion_condiciones($Cola2->id);' style='cursor:pointer'><img src='img/aceptacionterminos.png' height='80'><br>Aceptación de Términos y Condiciones</a></td>
					";
			else
			{
...
function asignar_transporte_vip2()
{
	...
	include('inc/gpos.php');
	$Codigo_seguimiento=29; // Adjudicación de Servicio VIP
...
function caso_actualizar_email()
{
	...
	function validar_actualizacion()
	{
		with(document.forma)
		{
			if(!alltrim(correo.value)) { alert('Debe digitar el correo electrónico del asegurado');correo.style.backgroundColor='ffff99';correo.focus();return false;}
			submit();
		}
	}
	function finalizar()
	...
	<form action='' target='Oculto_actualizacion' method='POST' name='forma' id='forma'>
		Correo electrónico: <input type='text' name='correo' id='correo' value='$Sin->declarante_email' size='80' maxlength='200'><br>
...
function caso_info_erronea()
{
	...
	echo "<script language='javascript'>
		...
		<form action='zcallcenter2.php' method='post' target='Oculto_info_erronea' name='forma' id='forma'>
			...
			Seleccione la tipificación correcta para este caso: ".menu1("tipificacion","select id,nombre from tipifica_seguimiento where tipo='INFORMACION ERRONEA' ",0,1)."
			<input type='hidden' name='Acc' id='Acc' value='caso_info_erronea_ok'><br><br>";
	if($Siniestro->declarante_email)	echo "<input type='checkbox' name='email_inicial'> <b style='color:blue;font-size:14px;'>Enviar automáticamente un Correo Inicial a la dirección $Siniestro->declarante_email </b><br><br>";
...
function caso_info_erronea_ok()
{
	...
	if($email_inicial)
	{
		...
		$Correo="
		Respetado(a) Señor(a) $Sin->asegurado_nombre

		Reciba cordial saludo.

		Somos Administración Operativa Automotriz S.A. la empresa que le proporciona el $Aseguradora->nombre_servicio de $Aseguradora->razon_social.

		Usted tuvo un siniestro con su vehículo de placas $Sin->placa con el número $Sin->numero.

		$Aseguradora->razon_social le brinda a Usted el beneficio de disponer de un $Aseguradora->nombre_servicio por un tiempo determinado y Administración Operativa Automotriz S.A. necesita contactarlo para bindarle este servicio.

		En vista de que la información que tenemos para contactarlo no ha sido efectiva, le pedimos el favor de que nos proporcione un número de celular o un número fijo a donde nuestro Call Center pueda contactarlo y explicarle todo el proceso para acceder al servicio.

		La información que nos proporcionó la aseguradora es la siguiente:

		Nombre: $Sin->asegurado_nombre  ".
		($Sin->declarante_telefono?" Teléfono del declarante: $Sin->declarante_telefono":"").
		($Sin->declarante_celular?" Celular del declarante: $Sin->declarante_celular":"").
		($Sin->declarante_tel_resid?" Teléfono de residencia del declarante: $Sin->declarante_tel_resid":"").
		($Sin->declarate_tel_otro?" Teléfono adicional del declarante: $Sin->declarate_tel_otro":"").
		($Sin->declarate_email?" Email del declarante: $Sin->declarate_email":"").
		"
		Ciudad :".($Ciuo?$Ciuo:$Ciu)."
		Fecha de Autorización: $Sin->fec_autorizacion

		Por favor puede enviarnos sus datos actualizados al correo siniestros@aoacolombia.com o llamar a nuestro Pbx en Bogotá D.C. al número $Aseguradora->numero_call .
		...
		Carrera 69B Número 98A-10 Barrio Morato Bogotá D.C.
		...
		$Envio=enviar_gmail("direccioncallcenter@aoacolombia.com" /*de */,
											"Dirección Call Center AOA" /*Nombre de */ ,
...
function caso_contacto_exitoso()
{
	...
	echo "<script language='javascript'>
		...
		<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<table align='center' cellspacing='10' bgcolor='ffffff'><tr>
		<td bgcolor='ffffff' align='center'><a onclick='preadjudica();' style='cursor:pointer'><img src='img/preadjudicacion.png' height='80'><br>Solicitud de Adjudicación</a></td>
		<td bgcolor='ffffff' align='center'><a onclick='no_adjudica();' style='cursor:pointer'><img src='img/no_adjudicacion.png' height='80'><br>No Adjudicación</a></td>
		<td bgcolor='ffffff' align='center'><a onclick='compromiso();' style='cursor:pointer'><img src='img/compromiso.png' height='80'><br>Compromiso</a></td>
		</tr></table>
	</body>";
}
...
function caso_preadjudica()
{
		...
		{ with(document.forma)
			{
				...
				if(confirm('Está seguro de Solicitar la Adjudicacion?'))
	...
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2.php' method='post' target='Oculto_preadj' name='forma' id='forma'>
			<H4>SOLICITUD DE ADJUDICACION DE VEHÍCULO</H4>
			Se enviará un correo electrónico con la información de requisitos y el anexo de términos y condiciones.<br><br>
			Correo electrónico del asegurado: <input type='text' name='correo_asegurado' id='correo_asegurado' size='80' value='$D->declarante_email'><br><br>
			<input type='checkbox' name='inmediato'> Marque esta casilla en caso de que el asegurado requiera la adjudicación inmediata sin leer el correo de pre-adjudicación.<br>
...
function caso_preadjudica_ok()
{
	...
	if($Envio=enviar_gmail($Email_usuario /*de */ ,$NUSUARIO /*nombre de */ ,	"$correo_asegurado,$Sin->asegurado_nombre" /*para */ ,
		...
		Este es un correo automatico del Area de Call Center cuyo objetivo es facilitar el acceso a nuestra plataforma para que
		...
		") /*mensaje */))
	{
		...
		if($inmediato)
		{
			...
			graba_bitacora('call2cola2','A',$Idn,'Adiciona Registro con aceptacion de términos y condiciones');
		}
		...
		guarda_escalafon(2);
		echo "<body><script language='javascript'>alert('Envío del correo satisfactorio.');parent.recargar();</script></body>";
	}
	else
	{
		echo "<body><script language='javascript'>parent.getElementById('mensaje').innerHTML='NO SE PUDO ENVIAR EL CORREO, por favor verifique la información e intentelo nuevamente.';
...
function caso_re_envia_correo_preadjudicacion_ok()
{
	...
	if($Envio=enviar_gmail($Email_usuario /*de */ ,$NUSUARIO /*nombre de */ ,	"$email,$Sin->asegurado_nombre" /*para */ ,
		"direccioncallcenter@aoacolombia.com,DIRECCION CALL CENTER"   /*Con copia*/ ,	"SOLICITUD VEHÍCULO DE REEMPLAZO"  /*OBJETO*/,
		nl2br("
		...
		servicio incluyendo la validacion del proceso de garantias; informacion que corresponde a datos personales
		como son nombres, apellidos, numero de identificacion, telefono (s), direccion(es), correo(s) electronico(s), ciudad,
		informacion de tarjeta(s) de crédito y cuenta(s) bancaria(s) (cuando apliquen) entre otros.
		...
		") /*mensaje */))
	{
		...
		q("update call2proceso set fecha_cierre='$Ahora',estado='C' where agente='$IDUSUARIO' and siniestro='$Cola->siniestro' and estado='A' ");
		echo "<body><script language='javascript'>alert('Envío del correo satisfactorio.');opener.re_comenzar();window.close();void(null);</script></body>";
	}
}
	...
	echo "<script language='javascript'>
		function validar_envio_compromiso()
		{
			with(document.forma)
			{
				...
				if(confirm('Está seguro de crear el Compromiso?')) submit();
			}
		}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2.php' method='post' target='_self' name='forma' id='forma'>
			...
			Seleccione la tipificación según el caso: ".menu1("tipificacion","select id,nombre from tipo_compromiso where activo=1",0,1)."<br>
			Fecha del compromiso: <select name='fechacompromiso'>";
			$Hoy=date('Y-m-d');
			for($i=1;$i<20;$i++) { $nd=ndiasemana($Hoy);echo "<option value='$Hoy' ".($nd=='Sábado' || $nd=='Domingo'?"style='background-color:ffdddd'":"").">$Hoy [$nd]</option>";$Hoy=date('Y-m-d',strtotime(aumentadias($Hoy,1)));}
...
function caso_compromiso2()
{
	...
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2.php' method='post' target='_self' name='forma' id='forma'>
			<h4>REGISTRO DE COMPROMISO DE ASIGNACION DE VEHÍCULO A FUTURO ($Cantidad_eventos eventos)</H4>
			Seleccione la tipificación según el caso: ".menu1("tipificacion","select id,nombre from tipo_compromiso where activo=1",0,1,''," onchange='busca_conteo();' ")." <span id='conteo'></span><br>
			Fecha del compromiso: <select name='fechacompromiso'>";
			$Hoy=date('Y-m-d');
			for($i=1;$i<20;$i++) { $nd=ndiasemana($Hoy);echo "<option value='$Hoy' ".($nd=='Sábado' || $nd=='Domingo'?"style='background-color:ffdddd'":"").">$Hoy [$nd]</option>";$Hoy=date('Y-m-d',strtotime(aumentadias($Hoy,1)));}
			echo"</select> Hora: ".pinta_hora('forma','horacompromiso',date('H:i:s'))."<br>
...
function caso_compromiso3()
{
	...
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'>
		<form action='zcallcenter2.php' method='post' target='_self' name='forma' id='forma'>
			<h4>REGISTRO DE COMPROMISO DE ASIGNACION DE VEHÍCULO A FUTURO</H4>
			Seleccione la tipificación según el caso: ".menu1("tipificacion","select id,nombre from tipo_compromiso where activo=1",0,1,''," onchange='busca_conteo();' ")." <span id='conteo'></span><br>
			Fecha del compromiso: <select name='fechacompromiso'>";
			$Hoy=date('Y-m-d');
			for($i=1;$i<20;$i++) { $nd=ndiasemana($Hoy);echo "<option value='$Hoy' ".($nd=='Sábado' || $nd=='Domingo'?"style='background-color:ffdddd'":"").">$Hoy [$nd]</option>";$Hoy=date('Y-m-d',strtotime(aumentadias($Hoy,1)));}
...
function caso_contacto_tercera()
{
	...
	echo "<script language='javascript'>
		...
		<td bgcolor='ffffff' align='center'><a onclick='compromiso();' style='cursor:pointer'><img src='img/compromiso.png' height='80'><br>Compromiso</a></td>
		<td bgcolor='ffffff' align='center'><a onclick='actualizar_datos();' style='cursor:pointer'><img src='img/actualizacion.png' height='80'><br>Actualización de datos</a></td>
		</tr></table>
	</body>";
}
...
function caso_buzon_voz()
{
	...
	echo "<script language='javascript'>
		function validar_envio_buzon()
		{
			with(document.forma)
			{
				if(!tipificacion.value) {alert('Debe seleccionar un tipo de buzón');tipificacion.style.backgroundColor='ffffaa';tipificacion.focus();return false;}
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Está seguro de crear el Compromiso?')) submit();
			}
		}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'><h4>Buzón de Voz ($Cantidad_eventos eventos)</h4>
		<form action='zcallcenter2.php' method='post' target='_self' name='forma' id='forma'>
			Seleccione la tipificación según el caso:
	...
	echo "</optgroup></select><br><br>
			<br>Observaciones: <input type='text' name='observaciones' id='observaciones' size='80'><br>
			<br>Cerrar el caso: <input type='checkbox' name='CERRAR_caso'>  Marque esta casilla si no hay mas números para marcar.<br><br>
...
function caso_buzon_tercera_persona()
{
	...
	echo "<script language='javascript'>
		function validar_envio_buzon()
		{
			...
			{
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Está seguro de guardar el Mensaje?')) submit();
			}
		}
	</script>
...
function caso_no_adjudica()
{
	...
	<body><body ><h3>Agente: $NUSUARIO - Proceso No Adjudicación</h3>
	<form action='zcallcenter2.php' target='_self' method='POST' name='forma' id='forma'>
...
function marcar_proceso_actualizacion()
{
	...
	q("update siniestro set observaciones=concat(observaciones,'\n$NUSUARIO [$Ahora]: Actualizacion Información Erronea Procesada.') where id=$D->siniestro");
	$H1=date('Y-m-d'); $H2=date('H:i:s');
	$Idn=q("insert into seguimiento (siniestro,fecha,hora,usuario,descripcion,tipo) values ($D->siniestro,'$H1','$H2','$NUSUARIO','Se recibe actualización por parte de la aseguradora.',8)"); // 8: ACTUALIZACION DE DATOS
...
function solicita_reactivacion_ok()
{
	...
	"Fecha de autorización: $Sin->fec_autorizacion<br>";
		...
		$Mensaje.="Para activar haga click aquí: <a href='https://app.aoacolombia.com/i.php?i=".base64_encode("\$Programa='$Ruta1';\$Fecha_control='".date('Y-m-d')."';")."' target='_blank'>Aprobar la modificación</a></body>";
	     ...
	     $data_mail = array(
					...
					"id" => $id,
				     "contenido" => "9",
					"para" =>  "sandraosorio@aoacolombia.com",
					"copia" => "danielsuarez@aoacolombia.com",
...
{
	global $f,$v,$NUSUARIO;
	html('ADJUDICACION DE VEHÍCULO');
	
	if(!($_SESSION['Adjudicacion_SINIESTRO'] && $_SESSION['Adjudicacion_OFICINA'] && $_SESSION['Adjudicacion_ASEGURADORA']))
	{echo "<body><script language='javascript'>centrar(400,200);alert('Adjudicación caida, por favor empiece nuevamente desde el módulo de Call Center');window.close();void(null);</script></body>";die();}
	...
	if(!$_SESSION['Adjudicacion_READJUDICAR'])
	{
		...
		{
			echo "<body>";
			echo "<script language='javascript'>alert('Aun existe una cita pendiente del dia $Cita_pendiente->fecha $Cita_pendiente->hora con el vehículo $Cita_pendiente->placa');window.close();void(null);</script>";
			echo "</body>";
			die();
		}
	}
	...
	if($V->flota!=$Sin->aseguradora)
	{
			...
			($V->flota==9 && $Sin->aseguradora==8) )
		{
			...
			('$v','$V->flota','$Sin->aseguradora','AUTOMATICO','$Ahora','$NUSUARIO',\"Cambio automático de gamas Allianz.\")");
		}
		...
		else
		{
			echo "
				...
				<form action='zcallcenter2.php' target='Oculto_autorizacion_cambio_flota' method='POST' name='forma' id='forma'>
					Por Orden de Gerencia, se debe justificar todo cambio de flota.<br><br>
					Justificación: <input type='text' name='justificacion' id='justificacion' value='' size='80' maxlength='200'><br><br>
	...
	if($Cita=qom("select c.id,c.fecha,c.hora from cita_servicio c , vehiculo v where c.placa=v.placa and v.id=$v and c.estado='P' limit 1",$LINK))
	{
		if($_SESSION['Adjudicacion_READJUDICAR'])
		{
			echo "<body><script language='javascript'>centrar(400,200);
				if(confirm('Aun tiene una cita pendiente el dia ".fecha_completa($Cita->fecha)." a las $Cita->hora. Desea Cancelar la re-asignación de vehículo en proceso?'))
	...
	if($Pasa_estado)
	{
		...
		if($Ultimo->estado==1) // estado en servicio
		{
			...
			{
				echo "<h3>ASIGNACION DE VEHÍCULO .:. OFICINA: $Oficina->nombre</h3>
				Placa: <b style='font-size:16px'>$V->placa</b> Linea: $V->nlinea <br>Fecha de Asignacion: <b>$f - ".ndiasemana($f)."</b><br>
				<table bgcolor='ffffff'><tr><td valign='top'><table border cellspacing='0' bgcolor='ffffff'><tr><th>Hora</th><th>Concurrencia</th><th>Observaciones</th><th>Opción</th></tr>";
				while($Hora<=$Oficina->hora_final)
				{
					...
					// VERIFICACION DE SI EL VEHICULO TIENE FECHA DE DEVOLUCIÓN EN UNA HORA DETERMINADA.. BLOQUEA LAS HORAS HASTA LA ENTREGA Y 1 HORA MAS PARA EL ALISTAMIENTO
					...
					if($Hora=='13:00:00') echo "</table></td><td valign='top'><table border cellspacing='0' bgcolor='ffffff'><tr><th>Hora</th><th>Concurrencia</th><th>Observaciones</th><th>Opción</th></tr>";
				}
...
function asigna_cita()
{
	...
	echo "<script language='javascript'>
		...
		{
			alert('Re-Asignación Satisfactoria');
	...
	<h3>ASIGNACION DE VEHÍCULO Y CITA DE ENTREGA</h3>";
	if($_SESSION['Adjudicacion_READJUDICAR'])
	{
	...
	<br /><br />Vehículo asignado: <input type='text' name='placa' value='$Veh->placa' readonly style='font-size:12;font-weight:bold' size=7>
	...
	<tr><td align='right' style='font-size:14'>Dirección Domicilio:</td><td><input type='text' style='font-size:14' name='dir_domicilio' value='$dir_domicilio' size='100' maxlength='200' onblur='valida_domicilio();'></td></tr>
	<tr><td align='right' style='font-size:14'>Teléfono Domicilio:</td><td><input type='text' style='font-size:14' name='tel_domicilio' value='$tel_domicilio' size='30' maxlength='50' onblur='valida_domicilio();'></td></tr>
	...
	<br>
	<table bgcolor='ddddff'><tr><td align='center' colspan=2 style='font-size:16'><B>DOMICILIO DEVOLUCIÓN</B></td></tr>
	<tr><td align='right' style='font-size:14'>Dirección domicilio devolución:</td><td><input type='text' style='font-size:14' name='dir_domiciliod' value='$dir_domiciliod' size='100' maxlength='200' onblur='valida_domiciliod();'></td></tr>
	<tr><td align='right' style='font-size:14'>Teléfono Domicilio devolución:</td><td><input type='text' style='font-size:14' name='tel_domiciliod' value='$tel_domiciliod' size='30' maxlength='50' onblur='valida_domiciliod();'></td></tr>
	<tr ><td align='center' colspan='2'><input type='button' id='btn_envio_aviso'  name='btn_envio_aviso' value=' Enviar Aviso de Llamada a Autorizaciones ' style='visibility:hidden;font-weight:bold;font-size:14px;'	onclick='aviso_autorizacion();'></td></tr>
	</table>
	
	<br /><br />
	Señor usuario $NUSUARIO, el agendamiento de la cita quedará registrada a nombre suyo con fecha y hora de registro.<br /><br />
	...
	<br><br>
	La información de esta cita será enviada al correo <b>$Sin->declarante_email </b>
...
function asigna_cita_ok_testHtml()
{
	...
	// se busca en la tabla de citas alguna que esté en estado programada
		...
		if($S->declarante_email)
		{
			if($S->renta)
			{
				if($dir_domicilio)
				{
					...
					Este es un correo automatico del Area de Call Center cuyo objetivo es confirmar  la cita programada para el proximo ".fecha_completa($fecha)." a las $Nhora a fin de hacer entrega de un Vehículo Rentado en la direccion: $dir_domicilio Telefono: $tel_domicilio,  solicitado por $conductor.
					...
					* La hora pactada para la devolucion corresponde a la hora en la cual se suministro el vehiculo o en su defecto la establecida por la compañía ajustandose a los horarios de pico y placa establecidos en la ciudad (en caso de aplicar). Una vez cumplido el tiempo de gracia (3 horas) previamente informado para la devolución, se cobrara un dia adicional. Si desea prorrogar el servicio por más días deberá informarlo con 12 horas de anterioridad.
				...
				else
				{
					...
					/*$Correo="
					Respetado(a) Señor(a) $conductor

					Reciba cordial saludo.

					Bienvenido(a) al sistema de adjudicacion del Servicio de Renta de AOA Colombia.

					Este es un correo automático del Area de Call Center cuyo objetivo es confirmar  la cita programada para el proximo ".fecha_completa($fecha)." a las $Nhora en las instalaciones de AOA Colombia, Sucursal:  $OF->nombre, ubicada en la  $OF->direccion en $OF->barrio con el fin de realizar entrega de un Vehículo Rentado,  solicitado por $conductor.
					...
					* Recuerde que despues de cumplido el tiempo del servicio, usted debe devolver el vehiculo en las mismas condiciones en que le fue entregado, incluyendo el combustible.
					* La hora pactada para la devolucion corresponde a la hora en la cual se suministro el vehiculo o en su defecto la establecida por la compañia ajustandose a los horarios de pico y placa establecidos en la ciudad (en caso de aplicar). Una vez cumplido el tiempo de gracia (3 horas) previamente informado para la devolucion, se cobrara un dia adicional. Si desea prorrogar el servicio por más días debera informarlo con 12 horas de anterioridad.
					*(Para cliente natural) La Garantia constituida mediante voucher sera	devuelta a traves de la direccion de correo electronico suministrada en un termino aproximado de 10 dias despues de finalizado el servicio con el fin verificar pagos pendientes. No obstante es importante tener en cuenta que en caso de no recibirlo por algun motivo concerniente a fallas tecnicas, errores de conectividad, entre otros, usted podra acercarse durante los primeros 60 dias de transcurrido el servicio a la oficina en la cual constituyo dicha garantia y reclamar el original. Pasado este tiempo, debera solicitarlo con antelacion para realizar el proceso de alistamiento y entrega del mismo desde nuestra central de archivo.
					...
					Ante cualquier inquietud puede escribirnos al correo electronico sac@aoacolombia.com. Nuestro horario de atención es de lunes a viernes de 8:00 a.m. a 5:00 p.m. y sábados de 9:00a.m. a 12:00 p.m.
			...
			else
			{
				if($dir_domicilio)
				{
					...
					- Numero de una cuenta bancaria, nombre de cuenta habiente, tipo de cuenta bancaria (Ahorros o Corriente) y el nombre de la entidad bancaria. Esta cuenta sera utilizada exclusivamente en caso de verse afectada la garantia constituida mediante congelamiento por un valor inferior a la misma, realizaremos la devolucion del excedente en el numero de cuenta por usted suministrado. (Este requisito no aplica para la garantia No Reembolsable). Existan cobros inferiores a la garantía y se requiera devolver algun excedente.
					- Si usted es un tercero autorizado (a), debe presentar una carta diligenciada por el asegurado(a) indicando que usted está autorizado(a) para proceder con la gestión del servicio (no requiere autenticación) y adjunto a esta debe presentar una fotocopia de la Cedula de Ciudadanía del (la) asegurado(a) o fotocopia de Camara y Comercio (no mayor a 30 días) de la Entidad asegurada.
					- Haber leido y aceptado los terminos y condiciones del servicio.
					 -$Aseguradora->info_per
					...
					*La hora pactada para la devolucion corresponde a la hora en la cual se suministro el vehiculo o en su defecto la establecida por la compania ajustandose a los horarios de pico y placa establecidos en la ciudad (en caso de aplicar). Una vez cumplido el tiempo de gracia previamente informado para la devolución, se incurrira en penalidad sin excepcion alguna. Recuerde que para su mayor comodidad y evitar incurrir en dicha penalidad, puede retornar el vehiculo a traves de un tercero por usted autorizado. Hora pactada devolucion: $Nhora.
...
function asigna_cita_ok()
{
	...
	{
		echo "<body><script language='javascript'>alert('Ya existe una cita que cruza su tiempo con la fecha propuesta de esta adjudicación: $Cita_Previa->numero de fecha: $Cita_Previa->fecha');</script></body>";
		die();
	}
	include('inc/link.php');
	$OF=qom("select * from oficina where id=$oficina",$LINK);
	...
	$H1=date('Y-m-d'); $H2=date('H:i:s');
	mysql_query("insert into seguimiento (siniestro,fecha,hora,usuario,descripcion,tipo) values ($siniestro,'$H1','$H2','$NUSUARIO','Agenda cita para $Dia $fecha a la(s) $hora con el vehículo $placa en la ciudad de $Nciudad Tipo de Garantia: $tg ".
	...
	else
	{
		...
		$arrayValidation =  validationDays($Fec_entrega,1086508,$OF);
		...
		$observations = "¡Se añaden $diff->days dias por horarios de la oficina, fecha anterior de devolución: $Fec_entrega, fecha nueva: $dateDelivery $hour!";
		...
		else
		{
			...
			graba_bitacora('call2cola2','A',$Idcc,'Inserta con aceptación de términos y condiciones',$LINK);
		}
	}
	if($_SESSION['Adjudicacion_OFICINA'])
	{
...
function escalar_caso()
{
	...
	echo "<script language='javascript'>
		function validar_escalar()
		{
			...
			{
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Está seguro de escalar el caso?')) submit();
			}
		}
	</script>
...
function caso_actualizar_datos()
{
	...
	<form action='zcallcenter2.php' target='Oculto_actualizacion' method='POST' name='forma' id='forma'>
		...
		Teléfono Celular de contacto: <input type='text' name='telefono_contacto' id='telefono_contacto' size='10' maxlength='10'> A este número se enviará el mensaje de Texto (SMS) automático para recordar las citas de entrega y devolución.<br>
...
function pide_datos_financieros(){
	...
	if($sinG == 1){
		...
		$titulo = " .:. AOA COLOMBIA S.A.S .:. SERVICIO DE VEHÍCULO DE REEMPLAZO .:. FORMULARIO DE GARANTÍA";
		$varFranquicia = "<tr><td align='right'>Franquicia</td><td>";
		$ScriptFranquicia = "if(franquicia.value) {alert('Debe seleccionar la franquicia de la tarjeta de crédito'); franquicia.style.backgroundColor='ffff55';franquicia.focus(); return false;}franquicia.disabled=false;";
	}	
	...
	<body style='font-size:16px' bgcolor='ffffff'><script language='javascript'>centrar();</script>
		...
		<form action='zcallcenter2.php' target='Oculto_forma' method='POST' name='forma' id='forma'>
			<table>
				<tr><td align='right'>Número de Identificación</td><td><input type='text' class='numero' name='identificacion' id='identificacion' value='' size='15' maxlength='15'>
					<input type='button' name='validar' id='validar' value=' Validar Identificación ' onclick='validar_id();'></td></tr>
				...
				<tr id='trPadre' style='display:none'><td align='right'>Digito de verificaciòn</td><td><input type='text' style='width: 2%;' name='codigoVerificacion' maxlength='1' id='codigoVerificacionId' ></td></tr>
	...
	echo $varScript = "<script language='javascript'>
		function valida_envio()
		{
			with(document.forma)
			{
				...
				if(codigo_seguridad.value!='000') if(!Number(codigo_seguridad.value)) {alert('Debe digitar el código de seguridad de la tarjeta de crédito'); codigo_seguridad.style.backgroundColor='ffff55';codigo_seguridad.focus(); return;}
				...
				if(devol_cuenta_banco.value.length<8) {alert('Numero de cuenta bancaria para devoluciones de garantía inválido'); devol_cuenta_banco.style.backgroundColor='ffff55';devol_cuenta_banco.focus();return false;}
...
function aceptacion_condiciones()
{
	...
	q("update siniestro set observaciones=concat(observaciones,'\n".$NUSUARIO.' '.date('Y-m-d H:i')." Aceptación de términos y condiciones por el Agente de Call Center.') where id=$id");
	echo "<body><script language='javascript'>parent.re_comenzar();</script></body>";
}
...
function marcacion_numero_alterno()
{
	...
	echo "<script language='javascript'>
		function validar_envio_marcacion()
		{
			...
			{
				if(!alltrim(observaciones.value)) {alert('Debe digitar las observaciones');observaciones.style.backgroundColor='ffffaa';observaciones.focus();return false;}
				if(confirm('Está seguro de Marcar a un número alterno?')) submit();
			}
		}
	</script>
	<body topmargin='0' leftmargin='0' rightmargin='0' bottommargin='0' bgcolor='ffffff'><h4>Marcación Número Alterno</h4>
		<form action='zcallcenter2.php' method='post' target='_self' name='forma' id='forma'>
			<br>
			<br>Observaciones: <input type='text' name='observaciones' id='observaciones' size='80'><br>
...
function marcacion_numero_alterno_ok()
{
	...
	$Idn=q("insert into seguimiento (siniestro,usuario,fecha,hora,descripcion,tipo) values ($id,'$NUSUARIO','$Fecha','$Hora','Marcación a Número alterno.',$Codigo)");
	q("update siniestro set observaciones=concat(observaciones,\"\n$NUSUARIO [$Ahora]: Marcación a Numero Alterno: $observaciones. \") where id=$id");
