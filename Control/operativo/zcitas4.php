...
function citas_inicial_busqueda_futuros($LINK,$C) // busca ultimos estados, futuros o servicios sin concluir para mostrar un alerta
{
	...
	$Futuro='';
	if($Ultimos[$C->cplaca])  // busca el último estado hasta el dia de hoy, si es distinto a 
...
.closebtn:hover {
  color: black;
		...
		if($Cita->aplazar_entrega){
			...
			echo "<h2 style='color:blue;'>Esta cita cuenta con un aplazo en la fecha de devolución: <br> $seguimientoText->descripcion<h2>";
		}		
				
		$arrayValidation =  validationDays($Fec_entrega,$Cita,$Oficina);
		
		if($arrayValidation['date'] == date('Y-m-d', $myDate)){
		  $isPostpone = "<h2 style='color: blue;'>La devolución no fue aplazada segun el horario de la oficina $Oficina->nombre</h2>";
		...
		<form action='zcitas2.php' target='_self' method='POST' name='forma' id='forma'>
			Fecha de Devolución: ".pinta_fc('forma','Fdevol',$dateDelivery)."
			Hora de Devolución: ".pinta_hora('forma','Hdevol',$hour)." $diaD 
...
function cambia_estado() // cambia estado  de acuerdo al formulario de entrega
{
	...
	if ($D = qom("select * from cita_servicio where id=$id",$LINK)) // trae los datos de la cita
	{
		...
		{
			$sqlDateAppointment = "SELECT * FROM temp_date_appointment WHERE id_cita = $id ORDER BY id DESC LIMIT 1";
			
			$appointmentTemp = qo($sqlDateAppointment);
			
			if($appointmentTemp){
			
			$onlyDesc = $appointmentTemp->observaciones;
			$obsPostpone =  $appointmentTemp->observaciones." 
			".$D->observaciones;
			
			$sqlInsetSeguimiento = "insert into seguimiento 
			(siniestro,fecha,hora,usuario,descripcion,tipo)  values 
			('$D->siniestro','$Ahora','$Hora','$Nusuario','$onlyDesc',32)";
			q($sqlInsetSeguimiento);
			$sqlUpdateAppointment = "UPDATE cita_servicio SET dias_servicio = $appointmentTemp->dias_servicio, 
			observaciones = '$obsPostpone', hora = '$appointmentTemp->hora', aplazar_entrega = 1 
			WHERE id = $appointmentTemp->id_cita";
			q($sqlUpdateAppointment);
			...
			function valida_km_final2()
			{
				...
				{ 	
					...
					{	currentProcess = true; 
						...
						alert('Debe escribir un odometro mayor que el odómetro inicial');					
					    return false;
					}
	...
	echo "
			...
			function prevalida_ords()
			{
				if(document.forma.Nuevo_estado.value==0) {
					...
					{	
						currentProcess = true;					
						alert('Debe primero seleccionar el nuevo estado en que quedará el vehículo');
