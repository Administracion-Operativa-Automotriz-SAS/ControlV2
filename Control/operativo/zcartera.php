...
function elaborar_recibo_ok() // graba el recibo de caja
{
	...
	// si la factura es masiva actualiza el id_recibo de caja
	q("update factura_masiva set id_recibo= $IDN where id_factura ='$Factura' ");

	if($Siniestro=='0'){
        
		$query = q("select * from factura_masiva where id_factura='$Factura' ");
