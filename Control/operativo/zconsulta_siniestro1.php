...
sesion();
if(inlist($_SESSION['User'],'8,11' /*aseguradora y aseguradora 1*/))
{
...
function pide_datos()
{
	...
	echo "<form action='zconsulta_siniestro1.php' method='post' target='_self' name='forma' id='forma'>
		Señor Usuario: Puede hacer la búsqueda por alguno de los siguientes criterios:<br>
		<br>
		<table>
		<tr><td>Número de siniestro</td><td><input type='text' name='numero' id='numero'></td></tr>
		<tr><td>Número de póliza</td><td><input type='text' name='poliza' id='poliza'></td></tr>
		<tr><td>Placa del vehículo</td><td><input type='text' name='placa' id='placa'></td></tr>
		<tr><td>Nombre del asegurado</td><td><input type='text' name='asegurado' id='asegurado'></td></tr>
		<tr><td>Identificación del asegurado</td><td><input type='text' name='idasegurado' id='idasegurado'></td></tr>
		<tr><td>Nombre del declarante</td><td><input type='text' name='declarante' id='declarante'></td></tr>
		<tr><td>Identificación del declarante</td><td><input type='text' name='iddeclarante' id='iddeclarante'></td></tr>
		<tr><td colspan=2 align='center'><input type='button' value='CONSULTAR' style='width:200;height:60;font-weight:bold'
		onclick=\"if(document.forma.numero.value || document.forma.poliza.value || document.forma.placa.value ||
		document.forma.asegurado.value || document.forma.idasegurado.value || document.forma.declarante.value || document.forma.iddeclarante.value)
		this.form.submit(); else alert('No ha digitado ningún criterio de busqueda');\"></td></tr></table>
...
function buscar()
{
	global $numero,$poliza,$placa,$asegurado,$idasegurado,$declarante,$iddeclarante,$Aseguradora;
	if($numero) {$Cond="numero like '%$numero%' "; $Consulta=" Número de siniestro = $numero ";}
	elseif($poliza) {$Cond="poliza like '%$poliza%' "; $Consulta=" Número de póliza = $poliza ";}
	elseif($placa) {$Cond="placa like '%$placa%' "; $Consulta=" Placa = $placa ";}
	elseif($asegurado) {$Cond="asegurado_nombre like '%$asegurado%' "; $Consulta=" Asegurado = $asegurado ";}
	elseif($idasegurado) {$Cond="asegurado_id like '%$idasegurado%' "; $Consulta=" Identificación asegurado = $idasegurado ";}
	elseif($declarante) {$Cond="declarante_nombre like '%$declarante%' "; $Consulta=" Declarante = $declarante ";}
	elseif($iddeclarante) {$Cond="declarante_id like '%$iddeclarante%' "; $Consulta=" Identificación declarante = $iddeclarante ";}
	html();
