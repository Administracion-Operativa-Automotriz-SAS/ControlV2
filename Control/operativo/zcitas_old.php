...
function citas_inicial()
{
	...
	<h3>CONTROL OPERATIVO - CITAS DE ENTREGA Y DEVOLUCIONES</h3>
	...
	if ($Citas = q($Consulta))
	{
		...
		while ($C = mysql_fetch_object($Citas))
		{
			...
			$Bc=qo1m("select color_co from estado_citas where codigo='$C->estado'",$LINK);
			$Sincars = qom("select *,t_ciudad(ciudad) as nciudad,t_ciudad(ciudad_original) as nciudado,t_estado_siniestro(estado) as nestado from siniestro where id=$C->siniestro",$LINK);
			$Aseguradora = qom("select * from aseguradora where id=$Sincars->aseguradora",$LINK);
			$Vehiculo=qom("select nombre_propietario,id_propietario from vehiculo where placa='$C->placa' ",$LINK);

			echo "<tr bgcolor='$Bc' ";
			if(inlist($USUARIO, '1,2,7,4') && ($C->estado != 'C' || $USUARIO==1))  // SI ES USUARIO GERENCIA, OPERATIVO, CALL CENTER
				echo "ondblclick='modifica_cita($C->id);' ";
			if($C->estado == 'C' ) echo " ondblclick=\"alert('Despues de cumplida la cita no se puede modificar');\" ";
			echo ">";
			citas_inicial_entrega_siniestro($con_of,$C,$Sincars,$Aseguradora,$T);
			citas_inicial_entrega_fecha($C);
			citas_inicial_entrega_hora($C,$Aseguradora,$Sincars,$Vehiculo);
			citas_inicial_entrega_conductor($C,$LINK);
			citas_inicial_tiempos($C,$LINK);
			if($C->estado=='C')	echo "<br /><a onclick='ver_visitantes($C->siniestro);' style='cursor:pointer'>Visitantes..</a>";
			echo "</td><td>" . nl2br($C->observaciones) . ($C->observaciones?" ":"") . "<a class='info' style='cursor:pointer' href=\"javascript:modal('zcitas.php?Acc=inserta_obs&id=$C->id',0,0,500,500,'obs');\">
							<img src='gifs/mas.gif' border='0'><span>Insertar observaciones</span></a><br />";

			echo "</td><td align='center' class='placa' style='background-image:url(img/placa.jpg);' width='80px'>$C->placa</td><td align='center' width='20%'>";
			...
			if($C->estado!='P')
			{
				if($C->estado=='C') citas_inicial_estado_cumplido($LINK,$C,$T,$Sincars);
				else echo menu1('estado', "Select codigo,nombre from estado_citas", $C->estado, 0, "font-size:12", " disabled onchange=''; ",$LINK);
			}
...
//							if(inlist($USUARIO,'1,2')) echo "<a onclick=\"modal('zautorizaciones.php?Acc=ver_autorizaciones&id=$C->siniestro',0,0,600,1000,'eds');\"	style='cursor:pointer'><u>Ver Autorizaciones</u></a>";
//							echo "<BR>";
//							if($Recibo_Caja=qom("Select recibo_caja_prov.* from recibo_caja_prov,sin_autor where recibo_caja_prov.autorizacion=sin_autor.id and sin_autor.estado='A' and recibo_caja_prov.siniestro=$C->siniestro
//																and sin_autor.siniestro=$C->siniestro  ",$LINK))
//							{
//								echo "<a style='cursor:pointer' onclick='imprimir_rcp($Recibo_Caja->id,$C->id)'>Imprimir Recibo de Caja Provisional</a><br />";
//							}
							echo "<br /><a onclick=\"modal('zcitas.php?Acc=imprimir_acta&idc=$C->id',0,0,900,900,'impa');\" style='cursor:pointer;font-weight:bold;'><img src='gifs/print.png' border='0'> Imprimir el Acta de Entrega</a><br />";
							if($Entregado=qo1m("select id from cita_servicio where siniestro='$C->siniestro' and arribo!='0000-00-00 00:00:00' and estado='P' and
															momento_entrega!='0000-00-00 00:00:00' ",$LINK))
						...
						else  ///  SI NO TIENE AUTORIZACION
						{
							...
							if($C->dir_domicilio)
							echo "<BR> <a onclick=\"modal('zcitas.php?Acc=imprimir_acta&idc=$C->id',0,0,900,900,'impa');\" style='cursor:pointer;font-weight:bold;'><img src='gifs/print.png' border='0'> Imprimir el Acta de Entrega</a>";
						...
						if (inlist($_SESSION['User'], '1,2,4,10,13,26'))
						{
							...
							if ($Ult = qo1m("select if(odometro_final!=0,odometro_final,odometro_inicial) as uodo from ubicacion u,vehiculo v where v.id=u.vehiculo and v.placa='$C->placa'
	             			 and u.fecha_final<='$Fec_inicial' and odometro_inicial!=0 order by u.fecha_final desc,u.id desc limit 1",$LINK))
							{
								echo menu1('estado', $EST_CITA, $C->estado, 0, "font-size:12; width:200px;"," onchange=\"formulario_entrega(this.value,$C->id,$Ult);this.value='P';\"",$LINK);
							}
							else
							{
								echo "Existe algun error en la placa asignada o la flota definida en la cita.";
							}
						}
						else
						{
							echo menu1('estado', "Select codigo,nombre from estado_citas", $C->estado, 0, "font-size:12;width:200px;", " disabled onchange=''; ",$LINK);
						}
					}
					else
					{
				...
				else // CUANDO AUN HAY SERVICIOS SIN CONCLUIR O ESTADOS FUTUROS
				{
					...
					{
						$Sin_observaciones = ($C->observaciones?0:1);
						echo menu1('estado', "Select codigo,nombre from estado_citas where codigo!='C'", $C->estado, 0, "font-size:12;width:200px;"," onchange=\"formulario_entrega(this.value,$C->id,0);this.value='P';\"",$LINK);
					}
					else // SI LOS USUARION NO SON LOS PERMITDOS, SIMPLEMENTE MUESTRA EL ESTADO PROTEJIDO
						echo menu1('estado', "select codigo,nombre from estado_citas", $C->estado,0," font-size:12;width:200px;"," disabled ",$LINK);
				}
			}
			echo "</td></tr>";
			// ////////  CONTADOR DE OFICINA
	...
	if ($Devoluciones = q($Consulta))
	{
		...
		while ($C = mysql_fetch_object($Devoluciones))
		{
			...
			$T = qo1m("select id from usuario_tab where usuario=$USUARIO and tabla='siniestro' ",$LINK);
			$Aseguradora = qom("select * from aseguradora where id=$Sincars->aseguradora",$LINK);
			$Vehiculo=qom("select id,nombre_propietario,id_propietario from vehiculo where placa='$C->placa' ",$LINK);
			if ($C->estadod == 'C') $Bc = 'ccffcc';	else $Bc = 'ffffff';
			echo "<tr bgcolor='$Bc' " . (inlist($USUARIO, '1,2,4')?"ondblclick='modifica_cita($C->id);' ":"") . " ><td align='center'>$con_of</td>";
			citas_devolucion_siniestro($C,$Sincars,$Aseguradora,$T);
			citas_devolucion_fechahora($C,$Aseguradora,$Sincars,$Vehiculo);
			...
			if (inlist($_SESSION['User'], '1,2,4,10,13')) // call center y Autorizaciones
				echo menu3('estadod', $ESTADOS_CITA_DEVOLUCION, $C->estadod, 0, "font-size:12", ($C->estadod == 'C'?" disabled":"") . " onchange=\"formulario_devolucion($C->id);this.value='P';\" ");
			else echo menu3('estadod', $ESTADOS_CITA_DEVOLUCION, $C->estadod, 0, "font-size:12", " disabled onchange=''; ");
			if ($C->estadod == 'C') citas_devolucion_cumplido($C,$T,$Sincars);
			echo "</td></tr>";
			$con_of++;
		}
		mysql_close($LINK);
...
function citas_inicial_modo_entregas($Modo,$OFICINA,$ASEG,$Dia)
{
	switch($Modo)
	{
		case 1:  return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota  from cita_servicio c,siniestro s where
			c.siniestro=s.id and c.fecha='$Dia' " .	($OFICINA?" and c.oficina='$OFICINA' ":" ")." and c.estado='P' ".($ASEG?" and s.aseguradora=$ASEG":"")." order by noficina,c.hora";
			break;
		case 2: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota  from cita_servicio c,siniestro s where
			c.siniestro=s.id and c.fecha='$Dia' " .	($OFICINA?" and c.oficina='$OFICINA' ":" ")." and c.estado in  ('S','X','Y','N') ".($ASEG?" and s.aseguradora=$ASEG":"")." order by noficina,c.hora";
			break;
		case 3: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota  from cita_servicio c,siniestro s where
			c.siniestro=s.id and c.fecha='$Dia' " .	($OFICINA?" and c.oficina='$OFICINA' ":" ")." and c.estado='C' ".($ASEG?" and s.aseguradora=$ASEG":"")." order by noficina,c.hora";
			break;
		case 4: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota  from cita_servicio c,siniestro s where
			c.siniestro=s.id and c.fecha='$Dia' " .	($OFICINA?" and c.oficina='$OFICINA' ":" ")." and c.estado in  ('P','C') ".($ASEG?" and s.aseguradora=$ASEG":"")." order by noficina,c.hora";
			break;
		case 5: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota  from cita_servicio c,siniestro s where
			c.siniestro=s.id and c.fecha='$Dia' " .	($OFICINA?" and c.oficina='$OFICINA' ":" ")." ".($ASEG?" and s.aseguradora=$ASEG":"")." order by noficina,c.hora";
			break;
	}
}
...
function citas_inicial_busqueda_futuros($LINK,$C)
{
	...
	if ($Futuros = mysql_query("select t_estado_vehiculo(u.estado) as ne,u.fecha_inicial,u.fecha_final from ubicacion u ,vehiculo v where
												v.id=u.vehiculo and v.placa='$C->placa' and u.estado!=1and u.estado!=96 and u.fecha_final>'$Hoy' and
	...
	if ($Sinconcluir = mysql_query("select u.id,u.fecha_inicial,u.fecha_final from ubicacion u,vehiculo v where
														v.id=u.vehiculo and v.placa='$C->placa' and u.fecha_final <= '$Fec_inicial' and u.estado=1",$LINK))
	{
...
function citas_inicial_estado_cumplido($LINK,$C,$T,$Sincars)
{
	echo menu1('estado', "Select codigo,nombre from estado_citas", $C->estado, 0, "font-size:12", " disabled onchange=''; ",$LINK);
	echo "<br /><a onclick=\"modal('zcitas.php?Acc=imprimir_acta&idc=$C->id',0,0,900,900,'impa');\" style='cursor:pointer;font-weight:bold;'><img src='gifs/print.png' border='0'> Imprimir el Acta de Entrega</a><br />";
	if($T)
	{
		echo "<br />Hora: <a onclick=\"modal('marcoindex.php?Acc=mod_reg&Num_Tabla=$T&id=$C->siniestro',0,0,600,1000,'eds');\"
	...
	if($Sincars->img_cedula_f) echo $okimg1.$Sincars->img_cedula_f.$okimg2." alt='Cédula' title='Cédula'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Cédula' title='Cédula'>";
	if($Sincars->img_pase_f) echo $okimg1.$Sincars->img_pase_f.$okimg2." alt='Reverso Cédula' title='Reverso Cédula'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Reverso Cédula' title='Reverso Cédula'>";
	if($Sincars->adicional1_f) echo $okimg1.$Sincars->acidional1_f.$okimg2." alt='Licencia' title='Licencia'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Licencia' title='Licencia'>";
	if($Sincars->adicional2_f) echo $okimg1.$Sincars->acidional2_f.$okimg2." alt='Reverso Licencia' title='Reverso Licencia'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Reverso Licencia' title='Reverso Licencia'>";
	if($Sincars->adicional3_f) echo $okimg1.$Sincars->acidional3_f.$okimg2." alt='Doc.Adicional 1' title='Doc.Adicional 1'>"; else echo "<img src='gifs/standar/marco.png' border='0' alt='Doc.Adicional 1' title='Doc.Adicional 1'>";
	if($Sincars->adicional4_f) echo $okimg1.$Sincars->acidional4_f.$okimg2." alt='Doc.Adicional 2' title='Doc.Adicional 2'>"; else echo "<img src='gifs/standar/marco.png' border='0' alt='Doc.Adicional 2' title='Doc.Adicional 2'>";
	echo "<br>";
	if($Sincars->img_odo_salida_f) echo $okimg1.$Sincars->img_odo_salida_f.$okimg2." alt='Odometro' title='Odometro'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Odometro' title='Odometro'>";
	if($Sincars->img_inv_salida_f) echo $okimg1.$Sincars->img_inv_salida_f.$okimg2." alt='Acta de Entrega' title='Acta de Entrega'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Acta de Entrega' title='Acta de Entrega'>";
	if($Sincars->fotovh1_f) echo $okimg1.$Sincars->fotovh1_f.$okimg2." alt='Frente' title='Frente'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Frente' title='Frente'>";
	if($Sincars->fotovh2_f) echo $okimg1.$Sincars->fotovh2_f.$okimg2." alt='Izquierda' title='Izquierda'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Izquierda' title='Izquierda'>";
	if($Sincars->fotovh3_f) echo $okimg1.$Sincars->fotovh3_f.$okimg2." alt='Derecha' title='Derecha'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Derecha' title='Derecha'>";
	if($Sincars->fotovh4_f) echo $okimg1.$Sincars->fotovh4_f.$okimg2." alt='Atras' title='Atras'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Atras' title='Atras'>";
	if($Sincars->img_contrato_f) echo $okimg1.$Sincars->img_contrato_f.$okimg2." alt='Contrato' title='Contrato'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Contrato' title='Contrato'>";
}

function citas_inicial_entrega_hora($C,$Aseguradora,$Sincars,$Vehiculo)
{
	global $Hora;
	echo "<td align='center'><a class='info'>" . ($C->hora < $Hora && $C->estado == 'P' && $C->arribo=='0000-00-00 00:00:00'?
				...
				date('h:i A', strtotime($C->hora))) . "<span style='width:700px'>
								<h3>Control de Seguimiento de Citas <i style='color:00000'>$Aseguradora->nombre  Póliza Número: $Sincars->poliza Siniestro No. $Sincars->numero </i></h3>
								Ciudad: <b>$Sincars->nciudad</b> " . ($Sincars->nciudado?"Ciudad original: <b>$Sincars->nciudado</b>":"") .
				                "<br />Fecha del siniestro: <b>$Sincars->fec_siniestro</b> Fecha de declaración del siniestro: <b>$Sincars->fec_declaracion</b>
								FECHA DE AUTORIZACION: <b style='color:ff0000'>$Sincars->fec_autorizacion</b>
								<br />Vigencia de la póliza:  Desde <b>$Sincars->vigencia_desde</b> hasta <b>$Sincars->vigencia_hasta</b>
								<br />Placa: <b>$Sincars->placa</b> Marca: <b>$Sincars->marca</b> Tipo: <b>$Sincars->tipo</b> Línea: <b>$Sincars->linea</b>
								Modelo: <b>$Sincars->modelo</b> Clase: <b>$Sincars->clase</b>
								<hr color='eeeeee'><b><u>ASEGURADO:</u></b> Nombre: <b>$Sincars->asegurado_nombre</b> Identificación: <b>$Sincars->asegurado_id</b>
								<hr color='eeeeee'><b><u>Declarante:</u></b> Nombre: <b>$Sincars->declarante_nombre</b> Identificación: <b>$Sincars->declarante_id</b>
								Telefonos: <b>$Sincars->declarante_telefono / $Sincars->declarante_tel_resid
								/ $Sincars->declarante_tel_ofic / $Sincars->declarante_celular</b>
								<hr color='eeeeee'><b><u>Conductor:</u></b> Nombre: <b>$Sincars->conductor_nombre</b> Telefonos: <b>$Sincars->declarante_telefono / $Sincars->conductor_tel_resid
								/ $Sincars->conductor_tel_ofic / $Sincars->conductor_celular / $Sincars->conductor_tel_otro</b><hr>
								Observaciones: $Sincars->observaciones</span></a><br />FLOTA: $C->nflota
								<br>Propietario: $Vehiculo->nombre_propietario Nit: ".coma_format($Vehiculo->id_propietario)."
						</td>";
...
function citas_inicial_entrega_fecha($C)
{
	...
	if((($USUARIO==26 && $_SESSION['Id_alterno']==5) || $USUARIO==1) && $C->estado=='P') echo "<a class='info' style='cursor:pointer' onclick='cambia_dias_servicio($C->id)'>";
	echo "<b>$C->dias_servicio</b>";
	if((($USUARIO==26 && $_SESSION['Id_alterno']==5) || $USUARIO==1) && $C->estado=='P') echo "</a>";
	echo "</td>";
}

function citas_inicial_entrega_siniestro($con_of,$C,$Sincars,$Aseguradora,$T)
{
	global $USUARIO,$PRE_AUTORIZA;
	echo "<td align='center'>$con_of</td><td>$C->nsiniestro <br>";
	if($C->estado != 'C' && inlist($USUARIO,'1,2,7,4')) // SI ES USUARIO GERENCIA, OPERATIVO, CALL CENTER
	...
	if($C->estado == 'P' && inlist($USUARIO,'1,2,5,7,10')) // SI ES USUARIO GERENCIA, AUTORIZACIONES, OPERATIVO, OFICINA
		echo "<a class='info' href=\"javascript:modal('zautorizaciones.php?sini=$Sincars->numero',0,0,600,600,'call');\" alt='Solicitar Autorizacion' title='Solicitar Autorizacion'>
	...
	{
		echo "<a href=\"javascript:inicia_facturacion($C->id);void(null);\" class='info'><img src='img/facturar.png' height='20' border='0' align='middle'><span style='width:200px'>Iniciar proceso de facturación</span></a>&nbsp;";
	}
	if(inlist($Us,'1,10,33'))  // SI EL USUARIO ES CAJERO
	{
		echo "<a href=\"javascript:nuevo_recibo_garantia($C->id);void(null);\" class='info'><img src='img/caja_registradora.png' height='20' border='0' align='middle'><span style='width:200px'>Recibo Caja x Garantia</span></a>&nbsp;";
	}
	if($PRE_AUTORIZA)
	{
		echo "<a href=\"javascript:nuevo_recibo_garantia($C->id);void(null);\" class='info'><img src='img/caja_registradora.png' height='20' border='0' align='middle'><span style='width:200px'>PRE-Autorización Efectivo o TD</span></a>&nbsp;";
	}
	echo "[<font color='blue'>$Aseguradora->nombre</font>]<br />Estado: $Sincars->nestado<br />";
	if($T)
	{
	...
	if(inlist($USUARIO,'1,2,5,6,13,33') && $C->estado=='C')
...
{
	global $USUARIO;
	if($C->estado=='P') // CONTROL DE TIEMPOS
	{
			...
			if($Autorizacion = qom("select id,fecha_solicitud,fecha_proceso from sin_autor where siniestro='$C->siniestro' and estado!='R' ",$LINK))
			{
				...
				if($Autorizacion->fecha_proceso!='0000-00-00 00:00:00')
				{
					...
					else
					{
						...
						{
							echo "<br><a class='info' style='cursor:pointer' onclick='marcar_entrega($C->id);'><img src='img/vehiculo.png' border='0' height='30'> Marcar Entrega del Vehículo <span  style='width:200px'>Marcar Entrega del Vehiculo</span></a>";
						}
					}
				}
				else
		...
		{
			echo "<br><a class='info' style='cursor:pointer' onclick='arribo_asegurado($C->id);'><img src='img/arribo_asegurado.png' border='0' height='30'><span>Marcar Arribo de Asegurado</span></a>&nbsp; ";
		}
		if(!$C->dir_domicilio && $C->arribo=='0000-00-00 00:00:00')
		{
			if(inlist($USUARIO,'1,2,4,13'))
			{
				echo "<br><a class='info' style='cursor:pointer' onclick='insertar_domicilio($C->id);'><img src='gifs/standar/seguir_amarillo.png' border='0'><span style='width:200px'>Crear Domicilio Entrega</span></a>&nbsp; ";
			}
		}
	}
	else
...
function citas_inicial_entrega_conductor($C,$LINK)
{
	...
	if(inlist($USUARIO,'1,2,3,5,6,7,10,13,23,27'))
		echo "<br /><a class='info' style='cursor:pointer' onclick='solicitar_factura($C->id);'><img src='gifs/standar/nuevo_ovr.png' border='0' height='18px'><span>Solicitar Factura</span></a>&nbsp;";
	if(inlist($USUARIO,'1,2,10,13,27'))
		echo menu1("operario_entrega","select id,concat(apellido,' ',nombre) from operario where inactivo=0 and oficina=$C->oficina order by apellido,nombre",$C->operario_domicilio,1,'',($C->operario_domicilio?"disabled ":"onchange='asigna_operario_entrega(this.value,$C->id,this);' "),$LINK);
	else
		echo menu1("operario_entrega","select id,concat(apellido,' ',nombre) from operario where inactivo=0 and oficina=$C->oficina order by apellido,nombre",$C->operario_domicilio,1,'',"disabled",$LINK);
}

function citas_devolucion_conductor($C,$LINK)
{
	global $USUARIO;
	echo "<td>$C->conductor<br />";
	if(inlist($USUARIO,'1,2,3,5,6,7,10,13,23,27'))
		echo "<a class='info' style='cursor:pointer' onclick='solicitar_factura($C->id);'><img src='gifs/standar/nuevo_ovr.png' border='0'><span>Solicitar Factura</span></a>&nbsp;";
	if($C->dir_domiciliod)
	...
	elseif(inlist($USUARIO,'1,2,4,13'))
		echo "&nbsp;<a class='info' style='cursor:pointer' onclick='insertar_domiciliod($C->id);'><img src='gifs/standar/seguir_amarillo.png' border='0'><span style='width:200px'>Crear Domicilio Devolución</span></a>&nbsp; ";
	if(inlist($USUARIO,'1,2,10,13,27'))
		echo "<br />".menu1("operario_devolucion","select id,concat(apellido,' ',nombre) from operario where inactivo=0 and oficina=$C->oficina order by apellido,nombre",$C->operario_domiciliod,1,'',($C->operario_domiciliod?"disabled ":"onchange='asigna_operario_devolucion(this.value,$C->id,this);' "),$LINK);
	else
		echo "<br />".menu1("operario_devolucion","select id,concat(apellido,' ',nombre) from operario where inactivo=0 and oficina=$C->oficina order by apellido,nombre",$C->operario_domiciliod,1,'',"disabled ",$LINK);
	echo "</td>";
}
...
function citas_devolucion_modo($Modo,$OFICINA,$ASEG,$Dia)
{
	...
	{
		case 1: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota from cita_servicio c,siniestro s where
					c.siniestro=s.id and  c.fec_devolucion='$Dia' and c.estado='C'  " . ($OFICINA?" and c.oficina='$OFICINA' ":" ") ." and c.estadod='P' ".($ASEG?"and s.aseguradora=$ASEG ":""). " order by noficina,hora_devol";
			break;
		case 2: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota from cita_servicio c,siniestro s where
			c.siniestro=s.id and  c.fec_devolucion='$Dia' and c.estado='C'  " . ($OFICINA?" and c.oficina='$OFICINA' ":" ") ." and c.estadod='X' ".($ASEG?"and s.aseguradora=$ASEG ":""). " order by noficina,hora_devol";
			break;
		case 3: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota from cita_servicio c,siniestro s where
					c.siniestro=s.id and  c.fec_devolucion='$Dia' and c.estado='C'  " . ($OFICINA?" and c.oficina='$OFICINA' ":" ") ." and c.estadod='C' ".($ASEG?"and s.aseguradora=$ASEG ":""). " order by noficina,hora_devol";
			break;
		case 4: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota from cita_servicio c,siniestro s where
					c.siniestro=s.id and  c.fec_devolucion='$Dia' and c.estado='C'  " . ($OFICINA?" and c.oficina='$OFICINA' ":" ") ." and c.estadod in ('P','C') ".($ASEG?"and s.aseguradora=$ASEG ":""). " order by noficina,hora_devol";
			break;
		case 5: return "select c.*,t_oficina(c.oficina) as noficina,t_siniestro(c.siniestro) as nsiniestro,t_aseguradora(flota) as nflota from cita_servicio c,siniestro s where
					c.siniestro=s.id and  c.fec_devolucion='$Dia' and c.estado='C'  " . ($OFICINA?" and c.oficina='$OFICINA' ":" ") ."  ".($ASEG?"and s.aseguradora=$ASEG ":""). " order by noficina,hora_devol";
			break;
	}

}

function citas_devolucion_siniestro($C,$Sincars,$Aseguradora,$T)
{
	global $USUARIO;
	echo "<td>$C->nsiniestro<br>";
	if(inlist($USUARIO,'1,6'))
		echo "<a href=\"javascript:inicia_facturacion($C->id);void(null);\" class='info'><img src='img/facturar.png' height='24' border='0' align='middle'><span style='width:200px'>Iniciar proceso de facturación</span></a>&nbsp;";
	if(inlist($USUARIO,'1,6,10'))
		echo "&nbsp;<a class='info' href=\"javascript:modal('zautorizaciones.php?sini=$Sincars->numero',0,0,600,600,'call');\" alt='Solicitar Autorizacion' title='Solicitar Autorizacion'><img src='img/solicita_autorizacion.png' border='0' height='30' align='middle'><span style='width:100px'>Solicitar Autorización</span></a> ";
	echo "[<font color='red'>$Aseguradora->nombre</font>]<br />	Estado: $Sincars->nestado";
...
function citas_devolucion_fechahora($C,$Aseguradora,$Sincars,$Vehiculo)
{
	global $Hora;
	echo "<td nowrap='yes'>$C->fec_devolucion</td><td align='center'><a class='info'>" . ($C->hora_devol < $Hora && $C->estadod == 'P'?
                ...
                "<br />Fecha del siniestro: <b>$Sincars->fec_siniestro</b> Fecha de declaración del siniestro: <b>$Sincars->fec_declaracion</b>
								...
								/ $Sincars->conductor_tel_ofic / $Sincars->conductor_celular / $Sincars->conductor_tel_otro</b><hr>
								Observaciones: $Sincars->observaciones</span></a><br />FLOTA: $C->nflota
								<br>Propietario: $Vehiculo->nombre_propietario Nit: ".coma_format($Vehiculo->id_propietario)."</td>";
}

function citas_devolucion_observaciones($C)
{
	echo "<td>".nl2br($C->obs_devolucion)."<br><a class='info' style='cursor:pointer' href=\"javascript:modal('zcitas.php?Acc=inserta_obsd&id=$C->id',0,0,500,500,'obs');\"><img src='gifs/mas.gif' border='0'><span>Insertar observaciones</span></a><br /></td><td align='center' class='placa' style='background-image:url(img/placa.jpg);' width='80px'>$C->placa</td>";
}

function citas_devolucion_cumplido($C,$T,$Sincars)
{
	if($T)
	...
	$okimg1="<img src='gifs/standar/si.png' border='0' style='cursor:pointer' onclick=\"modal('";
	$okimg2="',0,0,500,500,'foto');\" border='0' ";
	if($Sincars->img_odo_entrada_f) echo $okimg1.$Sincars->img_odo_entrada_f.$okimg2." alt='Odometro' title='Odometro'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Odometro' title='Odometro'>";
	if($Sincars->img_inv_entrada_f) echo $okimg1.$Sincars->img_inv_entrada_f.$okimg2." alt='Acta de Entrega' title='Acta de Entrega'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Acta de Entrega' title='Acta de Entrega'>";
	if($Sincars->fotovh5_f) echo $okimg1.$Sincars->fotovh5_f.$okimg2." alt='Frente' title='Frente'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Frente' title='Frente'>";
	if($Sincars->fotovh6_f) echo $okimg1.$Sincars->fotovh6_f.$okimg2." alt='Izquierda' title='Izquierda'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Izquierda' title='Izquierda'>";
	if($Sincars->fotovh7_f) echo $okimg1.$Sincars->fotovh7_f.$okimg2." alt='Derecha' title='Derecha'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Derecha' title='Derecha'>";
	if($Sincars->fotovh8_f) echo $okimg1.$Sincars->fotovh8_f.$okimg2." alt='Atras' title='Atras'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Atras' title='Atras'>";
	if($Sincars->img_encuesta_f) echo $okimg1.$Sincars->img_encuesta_f.$okimg2." alt='Encuesta' title='Encuesta'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Encuesta' title='Encuesta'>";
//	$Uu=qom("select * from ubicacion where vehiculo=$Vehiculo->id order by id desc limit 1",$LINK);
//	if($Uu->estado==8 /*alistamiento*/ && $Uu->fecha_final==date('Y-m-d'))
//	{
//		if($UA=qo1m("select id from alistamiento where vehiculo=$Vehiculo->id and fecha_cumplimiento='0000-00-00 00:00:00' order by id desc limit 1 ",$LINK))
//		{
//			echo "<br><a style='cursor:pointer' onclick=\"modal('zalistamiento.php?Acc=imprimir_formato&id=$UA',0,0,700,700,'fa');\"><img src='gifs/print.png' border='0'> Imprimir Formato de Alistamiento</a>";
//		}
//	}
