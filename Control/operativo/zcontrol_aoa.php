...
function verificar_hoy($base,$LINK)
{
	$Hoy=date('Y-m-d');
	if($Vehiculos=mysql_query("select id from $base.vehiculo order by id",$LINK))
	{
		while($V=mysql_fetch_object($Vehiculos))
		{
			if($Ultimo_estado=mysql_query("select * from $base.ubicacion where vehiculo=$V->id and fecha_inicial<='$Hoy' order by fecha_final desc,fecha_inicial desc limit 1"))
			{
				if($UE=mysql_fetch_object($Ultimo_estado))
				{
					if($UE->fecha_final<$Hoy)
					{
						if($UE->estado!=7)
						{
							mysql_query("update $base.ubicacion set fecha_final='$Hoy' where id=$UE->id",$LINK);
						}
						else
						{
							$FechaI=aumentadias($UE->fecha_final,1);
							mysql_query("insert into $base.ubicacion (oficina,vehiculo,fecha_inicial,fecha_final,odometro_inicial,odometro_final,odometro_diferencia,observaciones,estado) values
...
function control()
{
	...
	<script language='javascript'>
		...
		{
			modal('zcontrol_aoa.php?Acc=asignar_estado&fecha='+Fecha+'&Vehiculo='+Vehiculo+'&FI=$FI&FF=$FF&OFICINA=$OFICINA',50,50,500,850,'Nestado');
		}
		...
		function cierre1(Fecha,Id,Ids)  // cerrar un servicio
		{
			...
			{
				modal('zcontrol_aoa.php?Acc=cierra_servicio&id='+Id+'&fecha='+Fecha+'&FI=$FI&FF=$FF&OFICINA=$OFICINA',0,0,500,500,'cs');
				return;
			}
			else
			{
		...
		function cierre2(Fecha,Id)  // cerrar un mantenimiento  recibe la fecha y el id de la ubicacion
		{
			...
			{
				modal('zcontrol_aoa.php?Acc=cierra_mantenimiento&id='+Id+'&fecha='+Fecha+'&FI=$FI&FF=$FF&OFICINA=$OFICINA',0,0,500,500,'cs');
				return;
			}
			else
			{
				...
				{
					modal('zcontrol_aoa.php?Acc=observaciones_mantenimiento&id='+Id+'&fecha='+Fecha+'&FI=$FI&FF=$FF&OFICINA=$OFICINA',0,0,500,500,'cs');
					return;
				}
			}
		...
		function asi(Id)
		{
			...
			{
				modal('zcontrol_aoa.php?Acc=reasigna_siniestro&id='+Id+'&FI=$FI&FF=$FF&OFICINA=$OFICINA',0,0,500,500,'cs');
				return;
			}
		}
	...
	echo "<form action='zcontrol_aoa.php' target='_self' method='post' name='forma' id='forma'>
...
function cuadricula()
{
	...
	q("create table $tControl select ub.vehiculo,ub.id,ev.nombre as estado,ev.sigla,ev.color_co as cestado,ub.fecha_inicial,ub.fecha_final,
		...
		from ubicacion ub,estado_vehiculo ev, oficina ofi
		where ub.fecha_inicial <= '$FF' and ub.fecha_final >= '$FI' and ub.estado=ev.id and ub.oficina=ofi.id ".($OFICINA?"and ofi.id=$OFICINA":""),1);
	q("create table $tServicio select t_aseguradora(aseguradora) as aseguradora,sn.numero,sn.declarante_nombre as declarante ,
    esn.nombre as estado,sn.observaciones,sn.ubicacion,sn.id
	...
	IF($Vehiculos=q("select id,placa,flota_distinta from vehiculo where
	(inactivo_desde='0000-00-00' || inactivo_desde>'$FI') 	".($OFICINA?"and id in (select vehiculo from $tControl)":"")." order by flota_distinta,placa "))
	{
		...
		echo "<iframe style='position:Absolute;top:$Top1;left:0;z-index:101;border-width:0px;' id='Titulos' width='".($Ancho_total+10)."' HEIGHT='34' border=0 frameborder='no' scroll='no'
				src='zcontrol_aoa.php?Acc=pinta_titulos_fechas&Ancho_total=$Ancho_total&Ancho_celda=$Ancho_celda&FI=$FI&FF=$FF&tControl=$tControl&OFICINA=$OFICINA'></iframe>";
		...
		echo "<iframe style='position:Absolute;top:$Top2;left:0;z-index:101;border-width:0px;' id='Titulos2' width='83' HEIGHT='$Alto_frame' border=0 frameborder='no' scroll='no'
				src='zcontrol_aoa.php?Acc=pinta_titulos_placas&tControl=$tControl&OFICINA=$OFICINA&FI=$FI'></iframe>";
...
function pinta_estado($E,$LINK,$fecha,$Vehiculo,$Ac,$Excluye=false,$Colspan=0)
{
	...
	{
		$SS=qom("Select concat('Número de Siniestro:',numero,' - <font style=\'font-weight:bold;background-color:333388;color:ffffbb;\'>',
		aseguradora,'</font><br>Declarante:',declarante,'<br>Obs:',observaciones,'<br>Estado:',estado) as span,id from
		...
		IF($E->est==1)  // En servicio
		{
			...
			if($USUARIO!=4 /*call center*/ && $USUARIO!=8 /*aseguradora*/)
			{
				if($Span) /*si tiene servicio asignado */
				{
					if($Estado_concluido)
					{
						if($USUARIO==7 /* control operativo*/)
						{
						...
						{
							$ONCLICK="alert('Servicio ya concluido');";  /*cierra servicio*/
						}
					}
					else
					{
	...
	elseif(($E->est==4 /*estado en mantenimiento */ || $E->est==5 /* estado fuera de servicio*/)
			 && $USUARIO<=3 /*captura y administradores*/ && $fecha==date('Y-m-d') ) /* si es usuario de captura debe poder cerrar el mantenimiento/fuera de servicio */
	{
		$Resultado2.="<td style='cursor:pointer;' bgcolor='$E->cestado' width='$Ac' colspan='$Colspan' ";
...
function cierra_servicio()
{
	...
	echo "Oficina: <b>".qo1("select nombre from oficina where id=$U->oficina")." </b>
		...
		<form action='zcontrol_aoa.php' method='post' target='_self' name='forma' id='forma'>
...
function concluir_servicio()
{
	...
	$B=qo("select * from aseguradora where id=$SN->aseguradora");
	// actualizacion del siniestro en la base de datos de la aseguradora
	q("update $B->base.siniestro set estado=8,obsconclusion=\"$SN->obsconclusion\",fecha_final='$fecha_devolucion' where numero='$SN->numero'");
	if(!$Ubicacion_Aseguradora=qo1("select ubicacion from $B->base.siniestro where numero='$SN->numero' ")) { echo "Error 2 select de ubicacion: $B->base ";die();}
	q("update $B->base.ubicacion set fecha_final='$fecha_devolucion',estado=7,odometro_final='$ofinal',odometro_diferencia='$odometro',
	observaciones=\"$UB->observaciones\",obs_mantenimiento=\"$UB->obs_mantenimiento\" where id=$Ubicacion_Aseguradora");
	...
	{
		echo "<body onload=\"window.close();void(null);opener.location='zcontrol_aoa.php?FI=$FI&FF=$FF&OFICINA=$OFICINA';\"></body>";
	}
}
...
function pinta_titulos_placas()
{
	...
	(inactivo_desde='0000-00-00' || inactivo_desde>'$FI') ".($OFICINA?"and id in (select vehiculo from $tControl)":"")." order by flota_distinta,placa"))
	{
		...
		while($V=mysql_fetch_object($Vehiculos))
		{
			echo "<tr><td width='82' ".($V->flota_distinta?"bgcolor='444455' alt='FLOTA DISTINTA DE AOA' title='FLOTA DISTINTA DE AOA'":"").">
...
{
	global $Vehiculo,$fecha,$FI,$FF,$OFICINA;
	...
	echo "<form action='zcontrol_aoa.php' target='_self' method='post' name='mod' id='mod'>
	...
	echo "</td>
		...
		<tr><td align='right'>Estado</td><td>".menu1('estado','select id,nombre from estado_vehiculo order by id',0,1,'',
		...
		menu1("siniestro","select id,concat(numero,' ',declarante_nombre) from siniestro sn where estado=3 and ubicacion=0 order by numero ",0,1,'')."</td></tr></table>";
		fincapa();
		capa('control_odometros',1,'Relative','');
		echo "<table border cellspacing=0 ><tr><th colspan=6>Control de Odometro</th></tr>
		...
		echo "<table border cellspacing=0 ><tr><th colspan=2>Observaciones</th></tr>
...
function asignar_estado_grabar()
{
	global $vehiculo,$oficina,$fecha_inicial,$fecha_final,$estado,$siniestro,$odometro_entrega,$odometro_devolucion,
			$FI,$FF,$FP1,$FP2,$observaciones,$obs_mantenimiento,$odometro_diferencia,$OFICINA;
	...
	//---- inserta en la base de datos del cliente, el vehiculo si no existe y la ubicación.
	if($siniestro) $Aseguradora=qo1("select aseguradora from siniestro where id=$siniestro"); else $Aseguradora=0;
	if($Aseguradora) $B=qo("select * from aseguradora where id=$Aseguradora"); else $B=false;
	$Vh=qo("select * from vehiculo where id=$vehiculo");
	if($B)
	{
		$Obase=$B->base;

	    q("insert ignore into $Obase.vehiculo (placa,linea,cilindraje,modelo,color,clase,servicio,carroceria,puertas,numero_motor,numero_serie,numero_chasis,
	      pasajeros,ciudad_matrricula,fecha_matricula,fecha_soat,flota_aoa) values ('$Vh->placa','$Vh->linea','$Vh->cilindraje','$Vh->modelo','$Vh->color','$Vh->clase',
	      '$Vh->servicio','$Vh->carroceria','$Vh->puertas','$Vh->numero_motor','$Vh->numero_serie','$Vh->numero_chasis','$Vh->pasajeros','$Vh->ciudad_matrricula',
	      '$Vh->fecha_matricula','$Vh->fecha_soat',1)");
	    //-- obtiene el id del vehiculo en la base cliente
	    $IdVh=qo1("select id from $Obase.vehiculo where placa='$Vh->placa' ");
	    //-- inserta la ubicacion en la base cliente
	    $IdUb=q("insert into $Obase.ubicacion (oficina,vehiculo,fecha_inicial,fecha_final,estado,odometro_inicial,odometro_final,observaciones,obs_mantenimiento,odometro_diferencia) values
	        ('$oficina','$IdVh','$fecha_inicial','$fecha_final','$estado','$odometro_entrega','$odometro_devolucion','$observaciones','$obs_mantenimiento','$odometro_diferencia')");
	    //-- averigua el id de la ubicacion para ser usado en la actualizacion del registro de siniestro mas adelante
	    ...
	    if(($estado==1 /*No adjudicado */ || $estado==7 /* servicio */) && $siniestro)
	    {
	      ...
	      //-- actualiza el siniestro en la base cliente si existe un siniestro de igual numero y de la misma empresa
	      $Numero=qo1("select numero from siniestro where id=$siniestro");
	      q("update $Obase.siniestro set ubicacion='$IdUb',estado=7,fecha_inicial='$fecha_inicial',fecha_final='$fecha_final' where numero='$Numero'");
	      $idosini=qo1("select id from $Obase.siniestro where numero='$Numero' ");
	      // inserta la bitacora del siniestro
	      q("insert into $Obase.app_bitacora (ano,mes,dia,hora,minuto,segundo,nick,nombre,tabla,accion,registro,ip,detalle)
	        values ('".date('Y')."','".date('m')."','".date('d')."','".date('G')."','".date('i')."','".date('s')."','".$_SESSION['Nick']."','".$_SESSION['Nombre']."','siniestro','M','$idosini','".$_SERVER['REMOTE_ADDR']."','Asigna Servicio')");
	      q("insert into app_bitacora (ano,mes,dia,hora,minuto,segundo,nick,nombre,tabla,accion,registro,ip,detalle)
...
function reasigna_siniestro()
{
	...
	echo "<form action='zcontrol_aoa.php' method='post' target='_self' name='cap' id='cap'>
		...
		".menu1("siniestro","select id,concat(numero,' ',declarante_nombre) from siniestro sn where estado IN (3,7,8) and ubicacion=0 order by numero ",0,1,'')."<br>
