...
function factura_general()
{
	...
	//sesion();
	$USUARIO = 'WEBUSER';//$_SESSION['User'];
	$Nusuario = 'CLIENTE WEB';//$_SESSION['Nombre'];
	$Nick = 'WEB';//$_SESSION['Nick'];
	...
	if($id)
	{
		if($F=qo("select * from factura where id=$id"))
		{
			...
			&nbsp;&nbsp;<a style='cursor:pointer' onclick='imprimir_factural();' class='info'><img src='gifs/Print.png' border='0' height='16'  align='absmiddle' >Imprimirr<span style='width:100px'>Imprimir la Factura - Preimpreso</span></a>
				<br /><b>Cruzar esta Factura con la Garant�a :  </b>  ".menu1("Garantia","select id,concat(nombre,' - ',fecha_solicitud,' - ',num_autorizacion,' - ',estado,' - $',valor) from sin_autor where siniestro=$F->siniestro and estado='A' ",0,1,'width:200px')."
...
function generar_factura()
{
	...
	if($Cita=qo("select * from cita_servicio where id=$idCita"))
	{
		...
		echo "</td></tr>
			...
			<tr><td align='right'>Fecha de emisi�n:</td><td><input type='text' name='fecha_emision' value='".date('Y-m-d')."' size='10' readonly > Fecha de vencimiento: ".pinta_FC('forma','fecha_vencimiento',date('Y-m-d'))."
			...
			<!--&nbsp;&nbsp;<a style='cursor:pointer' onclick='imprimir_factura();' class='info'><img src='gifs/print.png' border='0' height='16'  align='absmiddle' >Imprimirr<span style='width:100px'>Imprimir la Factura</span></a>-->
			<br /><b>Solicitar Nueva autorizaci�n : </b> N�mero de voucher: <input type='text' name='voucher' size=10 class='numero'>&nbsp;&nbsp;
			<a style='cursor:pointer' onclick='genera_solicitud_autorizacion();' class='info'><img src='img/solicita_autorizacion.png' border='0' height='16'  align='absmiddle' >Generar Autorizaci�n<span style='width:100px'>Solicitar nueva Autorizaci�n</span></a>
			<br /><b>Cruzar esta Factura con la Garant�a :  </b>  ".menu1("Garantia","select s.id,concat(s.nombre,' - ',s.num_autorizacion,' - ',s.estado,' - $',s.valor,' ',f.nombre)
...
function traer_factura()
{
	...
	$iframe_Tag = '<iframe width="100%" height="100%" id="elec_frame"  src="https://app.aoacolombia.com/Control/operativo/zfacturacion_new.php?Acc=verificar_factura_electronica&id='.$D->id.'"  ></iframe>';
...
function aprobar_factura() 
{
	...
	$Ahora=date('Y-m-d H:i:s');
		...
		$iframe_Tag = '<iframe width="100%" height="100%" src="https://app.aoacolombia.com/Control/operativo/zfacturacion_new.php?Acc=verificar_factura_electronica&id='.$Fac->id.'"  ></iframe>';
...
function verificar_factura_electronica()
{
	...
	header('Content-Type: text/html; charset=utf-8');
	include($_SERVER["DOCUMENT_ROOT"]."/Control/operativo/views/factura_electronica/interfazfact_new.html");
}
