...
function ejecutar()
{
	...
	if($Reporte->redirect_url != null or $Reporte->redirect_url != "")
	{
	    header("Location: $Reporte->redirect_url ");
		die();
	...
	if($Reporte->pre && !$_Previo_Ok)
	{
		...
		<script language='javascript'>
				...
				{	if(document.forma.Cambiatarget.checked) document.forma.target='_blank'; else document.forma.target='_self';}
		</script>
	...
	if($_REQUEST["ID"] == 92 and (empty($_REQUEST['ASEG']) or  empty($_REQUEST['FI']) or empty($_REQUEST['FF'])))
	{
		echo "Debe mandar parametros para generar el informe";
		EXIT;
	}
	...
	if(!$Reporte->construye)
	{
		$Comando_Sql=$Reporte->instruccion;
	}
	else
	{
		$Comando_Sql="Create Table $_Tabla_temporal Select ".($Reporte->distintos?"DISTINCT ":" ").ejec_campos($ID,strlen($_Agrupaciones)).
	" From ".ejec_tablas($ID).$_Where.$_Agrupaciones.ejec_orden($ID);
	}		# TERMINA DE CONSTRUIR TODA LA INSTRUCCION SQL
		
		$condition = false;
		if(strlen($_REQUEST["ASEG"])>2)
		{
			$aseguradoras_array = explode(",",$_REQUEST["ASEG"]);
			if(in_array("55",$aseguradoras_array) or in_array("93",$aseguradoras_array))
			{
				$condition = true;
			}
		}
		else{
			if($_REQUEST["ASEG"] == 55 or $_REQUEST["ASEG"] == 93)
			{
				$condition =  true;				
			}
		}
		
		//Se puso asi ya que tocaba forzar el comportamiento de esta manera
		$mydate = "03/07/2018";
		...
		if($_REQUEST["ID"] == 92 and $condition and !empty($_REQUEST['FI']) and !empty($_REQUEST['FF']) and $todaysdate >= $mydate and $_REQUEST['FI'] <= "2019-04-24")
		{
			$Comando_Sql= "Create Table tmpi__92 Select  t_aseguradora(si.aseguradora) as aseguradora, concat(year(si.fecha_final),'-',month(si.fecha_final)) as periodo, AVG(va1.valor) as va1_valor, AVG(va2.valor) as va2_valor, AVG(va3.valor) as va3_valor, AVG(va4.valor) as va4_valor, AVG(va5.valor) as va5_valor, AVG(va6.valor) as va6_valor, AVG(round((va1.valor+va2.valor+va3.valor+(case when (t_aseguradora(si.aseguradora) = 'LA PREVISORA' ) then (va4.valor)/2 else va4.valor end)+va5.valor+va6.valor)/6,2)) as total_encuesta, COUNT(si.id) as cantidad From siniestro si, valor_encuesta va1, valor_encuesta va2, valor_encuesta va3, valor_encuesta va4, valor_encuesta va5, valor_encuesta va6 Where va1.id = si.encuesta_11 and va2.id = si.encuesta_12 and va3.id = si.encuesta_13 and va4.id = si.encuesta_14 and va5.id = si.encuesta_15 and va6.id = si.encuesta_16 and si.fecha_final between '".$_REQUEST['FI']."' and '".$_REQUEST['FF']."' and si.aseguradora in (".$_REQUEST['ASEG'].") Group by aseguradora ,periodo  Order by aseguradora ASC,periodo ASC";
		}
		
		if($_REQUEST["ID"] == 92 and $condition and !empty($_REQUEST['FI']) and !empty($_REQUEST['FF']) and $todaysdate >= $mydate and $_REQUEST['FI'] >= "2019-04-24")
		{
			$Comando_Sql = "Create Table tmpi__92	Select  t_aseguradora(si.aseguradora) as aseguradora, 
			concat(year(si.fecha_final),'-',month(si.fecha_final)) as periodo,
			 AVG(va1.valor) as va1_valor,
			 AVG(va2.valor) as va2_valor,
			 AVG(va3.valor) as va3_valor,
			 AVG(va4.valor) as va4_valor,
			 AVG(va5.valor) as va5_valor,
			 AVG(va6.valor) as va6_valor,
			 AVG(round((va1.valor+va2.valor+va3.valor+(case when (t_aseguradora(si.aseguradora) = 'LA PREVISORA' )
			 then
			 (va4.valor)/2 else va4.valor end)+(case when (t_aseguradora(si.aseguradora) = 'LA PREVISORA' )
			 then
			 (va5.valor)/2 else va5.valor end)+(case when (t_aseguradora(si.aseguradora) = 'LA PREVISORA' )
			 then
			 (va6.valor)/2 else va6.valor end))/6,2)) as total_encuesta,
			 COUNT(si.id) as cantidad 
			 From siniestro si, valor_encuesta va1, valor_encuesta va2, valor_encuesta va3,
			 valor_encuesta va4, valor_encuesta va5, valor_encuesta va6 
			 Where va1.id = si.encuesta_11 and va2.id = si.encuesta_12 and va3.id = si.encuesta_13 
			 and va4.id = si.encuesta_14 and va5.id = si.encuesta_15 and va6.id = si.encuesta_16 
			 and si.fecha_final between '".$_REQUEST['FI']."' and '".$_REQUEST['FF']."' and
			 si.aseguradora in (".$_REQUEST['ASEG'].") Group by aseguradora ,periodo  Order by aseguradora ASC,periodo ASC
				";
