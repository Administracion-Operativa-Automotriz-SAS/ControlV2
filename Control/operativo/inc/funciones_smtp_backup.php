...
function intercept_mail_to_new_library($mail_object){
	...
	$mail->SMTPAuth = true;
	$mail->Username = $mail_object->user;
	$mail->Password = $mail_object->password;
	$mail->SMTPSecure = 'tls';
	$mail->Port = 25;
	echo $mail_object->from;
	
	//print_r($mail);
	
	$mail->setFrom($mail_object->from);
	$mail->addAddress($mail_object->destinatary);
	$mail->isHTML(true);
							 ...
							 $Password='jl6316!' )
{
	...
	intercept_mail_to_new_library($mail_object);
