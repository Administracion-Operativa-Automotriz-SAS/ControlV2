...
function validaemail(dato)
{
	...
	if (enc1==0)
	{
		alert('Debe escribir una direcci�n de correo electr�nico v�lida, hace falta @');
		dato.focus();
	}
	if (enc2==0)
	{
	...
	if(enc1==0 || enc2==0) return false; else return true;
}
...
{
  if(confirm('Realmente desea cerrar sesi�n ?'))
...
function verificanumero(Evento,Nodo,entero)
{
	...
	if(keynum==8 || keynum==13  || keynum==37 || keynum==39 || keynum==46 || keynum==9 ||
		...
		keynum==110 || keynum==190 || keynum==113 ||
	...
	else
	{
		...
		if(document.getElementById(Nodo).id == "llave"){
		...
		alert('Caracer no permitido !! debe escribir solamente n�meros ');
...
function Sumar(){
		...
		let var1 = document.getElementById("impuesto_venta").value;
		let var2 = document.getElementById("impuesto_consumo").value;
		
		let rest = parseInt(var1) + parseInt(var2);
		console.log(rest);
		
		document.getElementById("total_impuesto").value = rest;
		
	
}

function SumarTotaFactura(){
...
function SumarPrecioCompraTotalImpuestos(){
	
	let var1 = document.getElementById("valorcompra").value;
	let var2 = document.getElementById("total_impuesto").value;
	
	let rest = parseInt(var1) + parseInt(var2);
	
	console.log(rest);
		
	document.getElementById("vaolor_de_compra").value = rest;
	
}

function ValorDeLaLista(){
	let valor_lista = document.getElementById("valor_lista").value;
	
	let total_factura = document.getElementById("vaolor_de_compra").value;
	
	let rest2 = (((parseInt(valor_lista) -  parseInt(total_factura)) / parseInt(valor_lista)) * parseInt(100));
	
	console.log(rest2);
	
	document.getElementById("descuento_otorgado").value = rest2;
	
}
function ImpuestoVentas(){
	
	let porcentage_iva = document.getElementById("porcentage_iva").value;
	let precio_compra = document.getElementById("valorcompra").value;
	
	let rest2 = parseInt(precio_compra) * parseInt(porcentage_iva) / 100;  
	
	console.log(rest2);
	
	document.getElementById("impuesto_venta").value = rest2;
	
}

function ImpuestoConsumo(){
	
	let precio_compra = document.getElementById("valorcompra").value;
	let porcentage_consumo = document.getElementById("porcentage_ica").value;
	
	let rest2 = parseInt(precio_compra) * parseInt(porcentage_consumo) / 100;  
	
	console.log(rest2);
	
	document.getElementById("impuesto_consumo").value = rest2;
	
}

function verPoliza(){
	var npoliza = document.getElementById('n_poliza').value;
	/*let x = document.getElementById("n_poliza").selectedIndex;
	let y = document.getElementById("n_poliza").options;
	Con este codigo podremos obtener el texto del option
	*/
	//let valorPoliza =  y[x].text;
	//inputValor.value = npoliza;
	
	searchData(npoliza);

}

function RegistroSelect(){
	let select = document.getElementById('n_poliza');
    select.addEventListener('change',function(){
	
    var selectedOption = this.options[select.selectedIndex];
    let varValue = selectedOption.value;
	searchData(varValue);
...
function guardarImpuesto(){
  console.log('entra')
  let placaImpuesto = document.getElementById('placa').value
  
  guardarImpuestoHistorial(placaImpuesto)
}


let guardarImpuestoHistorial = (placaImpuesto) =>  {
	let xhr = new XMLHttpRequest()
	xhr.open('POST', '/Control/operativo/controllers/WebServices.php');
	...
	xhr.onload = function() {
			if (xhr.status == 200 || xhr.responseText) {
				Swal.fire({
					  ...
					  type: 'info',
					  html:
					  '<ul style="text-align: left;margin-left: 50;">'+
						'<b>Guarde de poliza con exito</b>'+
						'</ul>',
					});
			}
		}
...
function guardarAnteriorPoliza(){
	var npoliza2 = document.getElementById('n_poliza').value;
	let placa2 = document.getElementById('placa').value;
	
	guardarPoliza(npoliza2,placa2);
}
   
	
 let guardarPoliza = (varValueDos,placa) =>{
	  let xhr = new XMLHttpRequest();
	    xhr.open('POST', '/Control/operativo/controllers/WebServices.php');
		xhr.setRequestHeader('Content-Type', 'application/json');
		...
		xhr.onload = function() {
			if (xhr.status == 200 || xhr.responseText) {
				...
				Swal.fire({
					  ...
					  html:
					  '<ul style="text-align: left;margin-left: 50;">'+
						'<b>Guardada historial con exito</b>'+
						'</ul>',
					});
			}
		}
		...
		xhr.send(JSON.stringify({
				acc:"registroTablaTemp",
				id:varValueDos,
				placa:placa
			}));
	}
	function verHistorialSoat(){
		let placaSoat = document.getElementById('placa').value;
		
		verUtimoHistorialSoat(placaSoat)
	}
	
	var isFetchingTres = false;
	
	
	let verUtimoHistorialSoat = (placaSoat) => {
		let xhr = new XMLHttpRequest()
		xhr.open('POST', '/Control/operativo/controllers/WebServices.php')
		xhr.setRequestHeader('Content-Type', 'application/json')
		...
		xhr.onload = function() {
			...
			if(xhr.status == 200 || xhr.responseText) {
				...
				let objTres = JSON.parse(xhr.responseText);
				
				if(objTres.historyGetSoat == null){
					Swal.fire({
					  title: '<strong>No hay SOAT aun cargado</strong>',
					  type: 'info',
					  html: '<h3>No hay informacion aun</h3>',
					});
				}else{
					var lista = ''
				
				for(var i = 0; i < objTres.historyGetSoat.length; i++){
					if(objTres.historyGetSoat[i].caratula_soat_f == null){
						objTres.historyGetSoat[i].caratula_soat_f = ''
					}else{
						objTres.historyGetSoat[i].caratula_soat_f
					}
			...
			var fecha_inicio = objTres.historyGetSoat[i].fecha_inicio_vijencia
			var fecha_fin  = objTres.historyGetSoat[i].fecha_fin_vigencia
			var hoyFormateada = formatDate(hoy)
			var estado = objTres.historyGetSoat[i].estado
					
					if(fecha_inicio > hoyFormateada){
						estado = "EN PROCESO DE VIGcontrENCIA"
					}
					
					
					
					lista += `<tr><td>${objTres.historyGetSoat[i].numero_soat}</td>
					<td>${objTres.historyGetSoat[i].fecha_expedicion}</td>
					<td>${objTres.historyGetSoat[i].fecha_inicio_vijencia}</td>
					<td>${objTres.historyGetSoat[i].fecha_fin_vigencia}</td>
					<td>${objTres.historyGetSoat[i].entida_expide}</td>
					<td>${estado}</td>
					<td><a href="https://app.aoacolombia.com/Control/operativo/${objTres.historyGetSoat[i].caratula_soat_f}" target="_blank"><img class="zoom" src='${objTres.historyGetSoat[i].caratula_soat_f}' alt='' width='50' height='50'></a></td>
					</tr>`
				}
				
				Swal.fire({
					  title: '<strong>Historial de soat</strong>',
					  type: 'info',
					  html: 
					  '<table>'
					    +'<tr><th>Numero de Poliza</th><th>Fecha de expedicion</th><th>Fecha inicio vijencia</th><th>Fecha fin vijencia</th><th>Entidad expide SOAT</th><th>Estado</th><th>Imagen</th></tr>'
						+lista+
						'</table>',
					});
				}
			...
			isFetchingTres = false;		
 	};
		if(isFetchingTres == false){
			xhr.send(JSON.stringify({
				acc:"get_history_soat",
				placaHistorySoat:placaSoat
			}));
			isFetchingTres = true;	
		}
	...
	function verImpuesto(){
		...
		verHistorialImpuesto(placa)
	}
		var isFetchingCuatro = false;
	let verHistorialImpuesto = (placa) => {
		let xhr = new XMLHttpRequest()
	    xhr.open('POST', '/Control/operativo/controllers/WebServices.php')
		xhr.setRequestHeader('Content-Type', 'application/json')
		...
		xhr.onload = function(){
			if(xhr.status == 200 || xhr.responseText) {
				let objDos = JSON.parse(xhr.responseText);
				var  row = ''
				for(var i = 0; i < objDos.historyGetImpuesto.length; i++ ){
					row += `<tr><td>${objDos.historyGetImpuesto[i].fecha_de_pago}</td>
					<td>${objDos.historyGetImpuesto[i].secretaria_hacienda}</td>
					<td>${objDos.historyGetImpuesto[i].avalulo_comercial}</td>
					<td>${objDos.historyGetImpuesto[i].valor_impuesto_cargo}</td>
					<td>${objDos.historyGetImpuesto[i].sanciones_vs}</td>
					<td>${objDos.historyGetImpuesto[i].valor_semaforizacion}</td>
					<td>${objDos.historyGetImpuesto[i].descuento_pronto_pago}</td>
					<td>${objDos.historyGetImpuesto[i].interes_de_mora}</td>
					<td>${objDos.historyGetImpuesto[i].valor_a_pagar}</td>
					<td>${objDos.historyGetImpuesto[i].pago_voluntario}</td>
					<td>${objDos.historyGetImpuesto[i].total_con_pago_voluntario}</td>
					<td>${objDos.historyGetImpuesto[i].numero_referencia_recaudo}</td>
					<td>${objDos.historyGetImpuesto[i].primera_fecha_hasta}</td>
					<td>${objDos.historyGetImpuesto[i].segunda_fecha_hasta}</td>
					<td>${objDos.historyGetImpuesto[i].total_a_pagar}</td>
					<td><a href="https://app.aoacolombia.com/Control/operativo/${objDos.historyGetImpuesto[i].caratura_impuesto_f}" target="_blank"><img class="zoom" src='${objDos.historyGetImpuesto[i].caratura_impuesto_f}' alt='' width='50' height='50'></a></td>
					</tr>` 
				}
			   Swal.fire({
					  title: '<strong>Historial de impuestos</strong>',
					  type: 'info',
					  html: 
					  '<table>'
					    +'<tr><th>Fecha de pago</th><th>Secretaria de hacienda</th><th>Avaluo comercia</th><th>Valor Impuesto</th><th>Sansiones VS</th>'
						+'<th>Valor Semaforizacion</th><th>Descuento por pronto pago</th><th>Interes de mora</th><th>Valor a pagar</th>'
						+'<th>Pago voluntario</th><th>Total con pago voluntario</th><th>Numero de referencia del recaudo</th>'
						+'<th>Primera fecha hasta</th><th>Segunda fecha hasta</th><th>Total a pagar</th><th>Caratula Impuesto</th></tr>'
						+row+
						'</table>',
						width: '850px'
					});
			}
			isFetchingCuatro = false
		}
		
		if(isFetchingCuatro == false){
			xhr.send(JSON.stringify({
				acc:"get_history_impuesto",
				placaHistoryImpuesto:placa
			}));
			isFetchingCuatro = true;	
		}
	}
	...
	function verHistorial(){
		let placa = document.getElementById('placa').value;
		let idSelect = document.getElementById('history_poliza');
		verUtimoHistorial(placa);
		
	}
	
	
	var isFetchingDos = false;

let verUtimoHistorial = (placa) => 
{
	   let xhr = new XMLHttpRequest();
	    xhr.open('POST', '/Control/operativo/controllers/WebServices.php');
		xhr.setRequestHeader('Content-Type', 'application/json');
		...
		xhr.onload = function() {
			...
			if(xhr.status == 200 || xhr.responseText) {
				...
				console.log(objDos.historyGet);
				var  row = ''
				for(var i = 0; i < objDos.historyGet.length; i++ ){
					row += `<tr><td>${objDos.historyGet[i].numero_poliza}</td>
					<td>${objDos.historyGet[i].aseguradora_nombre}</td>
					<td>${objDos.historyGet[i].fecha_cambio}</td>
					<td>${objDos.historyGet[i].vigencia_desde}</td>
					<td>${objDos.historyGet[i].vigencia_hasta}</td>
					</tr>` 
				}
			   Swal.fire({
					  title: '<strong>Historial de polizas</strong>',
					  type: 'info',
					  html: 
					  '<table>'
					    +'<tr><th>Numero de Poliza</th><th>Nombre de poliza</th><th>Fecha de cambio</th><th>Desde</th><th>Hasta</th></tr>'
						+row+
						'</table>',
					});
		...
		if(isFetchingDos == false){
			xhr.send(JSON.stringify({
				acc:"get_history_poliza",
				placaHistory:placa
			}));
			isFetchingDos = true;	
		}
	   ...
	   console.log(event.target.value);
		...
		xhr.onload = function() {
			...
			if (xhr.status == 200 || xhr.responseText) {
				...
				if(obj.seguros.cobertura_uno == ''){
					var civil = obj.seguros.cobertura_dos;
				}else{
					var civil = obj.seguros.cobertura_uno;
				}
				console.log(obj);
			
				Swal.fire({
					  title: '<strong>Datos Aseguradora</strong>',
					  type: 'info',
					  html:
					  '<ul style="text-align: left;margin-left: 50;">'+
						'<li>'+'<b>Numero de poliza:</b>'+obj.seguros.n_poliza+'</li>'+
						'<li>'+'<b>Nombre de Aseguradora:</b>'+obj.seguros.aseguradora_nombre+'</li>'+
						'<li>'+'<b>Certificado Anexo:</b>'+obj.seguros.certificado_anexo+'</li>'+
						'<li>'+'<b>Responsabilidad civil:</b>'+civil+'</li>'+
						'<li>'+'<b>Comentarios:</b>'+obj.seguros.comentarios+'</li>'+
						'<li>'+'<b>Corredor:</b>'+obj.seguros.corredor+'</li>'+
						'<li>'+'<b>Fecha envio:</b>'+obj.seguros.fecha_envio+'</li>'+
						'<li>'+'<b>Vigencia desde:</b>'+obj.seguros.vigencia_desde+'</li>'+
						'<li>'+'<b>Vigencia hasta:</b>'+obj.seguros.vigencia_hasta+'</li>'+
						'<li>'+'<b>Linea de asistencia:</b>'+obj.seguros.linea_asistencia+'</li>'+
						'</ul>',
					});
				console.log(xhr.responseText);
			}
...
function  cambiarValidacion(){
		...
		let fecha_inicio_vijencia = document.getElementById('fecha_desde_soat');
		
		let fecha_final_vijencia = document.getElementById('fecha_hasta_soat');
		
		let estilosValidarFecha = document.createElement('style');
		console.log(fecha_inicio_vijencia.value)
		var d = new Date(fecha_inicio_vijencia.value);
		
		let sumaFecha =  sumarDias(d,1)
		let dateAllFinSoat = formatDate(sumaFecha) 
		
		
		fecha_final_vijencia.value = dateAllFinSoat
		
		fecha_final_vijencia.readOnly = true
		
		fecha_final_vijencia.parentNode.insertBefore( estilosValidarFecha, fecha_final_vijencia.nextSibling );
		
		
			var f = new Date();
			var hoy = f.getFullYear()  + "-" + (f.getMonth() +1) + "-" + f.getDate()
			var fecha_final = fecha_final_vijencia.value
			...
			let estadoSoat =  document.getElementById('estado_soat') 
			
			if(hoyFormateada > fecha_final){
				console.log(`${hoyFormateada} es mayor a ${fecha_final} soat`)
				estadoSoat.setAttribute("class","validar")
				estadoSoat.readOnly = true
				estilosValidarFecha.innerHTML = ".validar{background-color: #ff2f00 !important;}"
				estadoSoat.value = "NO VIGENTE"
			}else if(fecha_inicio_vijencia.value > hoyFormateada){
				estadoSoat.setAttribute("class","validar2")
				estadoSoat.readOnly = true
				estilosValidarFecha.innerHTML = ".validar2{background-color: #6d6dff4f !important;}"
				estadoSoat.value = "EN PROCESO DE VIGENCIA"
			
			}else{
				estadoSoat.setAttribute("class","validar1")
				estadoSoat.readOnly = true
				estilosValidarFecha.innerHTML = ".validar1{background-color: #04ff00 !important;}"
				estadoSoat.value = "VIGENTE"
				console.log(`${hoyFormateada} es menor a ${fecha_final} soat`)
			}
...
document.addEventListener("DOMContentLoaded", function() {
  //alert("loaded");
  console.log(document.getElementById('n_poliza'));
  
  if(document.getElementById('n_poliza')){
		...
		let estilosAler = document.createElement('style');

		// Add content to the new element
		div.innerHTML = 'Ver poliza';
		
		div.setAttribute("id","btn");
		div.setAttribute("class","boton");
		estilos.innerHTML = ".boton{color: #0099CC;background: transparent;border: 2px solid #0099CC;border-radius: 6px;}"
		+"border: none;color: white;padding: 16px 32px;text-align: center;display: inline-block;font-size: 16px;margin: 4px 2px;"
		+"transition-duration: 0.4s;cursor: pointer;text-decoration: none;text-transform: uppercase;";
		div.setAttribute("onclick","verPoliza()");
		
		
		
		div2.innerHTML = 'Guardar historial poliza';
		
		div2.setAttribute("id","btn2");
		div2.setAttribute("class","boton");
		estilos.innerHTML = ".boton{color: #0099CC;background: transparent;border: 2px solid #0099CC;border-radius: 6px;}"
		+"border: none;color: white;padding: 16px 32px;text-align: center;display: inline-block;font-size: 16px;margin: 4px 2px;"
		+"transition-duration: 0.4s;cursor: pointer;text-decoration: none;text-transform: uppercase;";
		div2.setAttribute("onclick","guardarAnteriorPoliza()");
		
		div3.innerHTML = 'Ver Historial poliza';
		
		div3.setAttribute("id","btn3");
		div3.setAttribute("class","boton");
		estilos.innerHTML = ".boton{color: #0099CC;background: transparent;border: 2px solid #0099CC;border-radius: 6px;}"
		+"border: none;color: white;padding: 16px 32px;text-align: center;display: inline-block;font-size: 16px;margin: 4px 2px;"
		+"transition-duration: 0.4s;cursor: pointer;text-decoration: none;text-transform: uppercase;";
		div3.setAttribute("onclick","verHistorial()");
		...
		select.parentNode.insertBefore( estilos, select );

		// Insert the element after our target element
		select.parentNode.insertBefore( div, select.nextSibling );
		
		select.parentNode.insertBefore( estilos, select.nextSibling );
		
		//////////////////////////////////////////////////////////////////
		
		select.parentNode.insertBefore( div2, select );
		
		select.parentNode.insertBefore( estilos, select );

		// Insert the element after our target element
		select.parentNode.insertBefore( div2, select.nextSibling );
		...
		select.parentNode.insertBefore( div3, select );
		
		select.parentNode.insertBefore( estilos, select );

		// Insert the element after our target element
		select.parentNode.insertBefore( div3, select.nextSibling );
		
		select.parentNode.insertBefore( estilos, select.nextSibling );
		
		
		let  fecha_desde_soat = document.getElementById('fecha_desde_soat')
		
		let secretaria_hacienda = document.getElementById('secretaria_hacienda')
		
		
		
		
		let div4 = document.createElement('button');
		div4.innerHTML = 'Consultar historial SOAT';
		div4.setAttribute("id","btn4");
		div4.setAttribute("class","boton");
		estilos.innerHTML = ".boton{color: #0099CC;background: transparent;border: 2px solid #0099CC;border-radius: 6px;}"
		+"border: none;color: white;padding: 16px 32px;text-align: center;display: inline-block;font-size: 16px;margin: 4px 2px;"
		+"transition-duration: 0.4s;cursor: pointer;text-decoration: none;text-transform: uppercase;";
		div4.setAttribute("onclick","verHistorialSoat()")
		
		
		/*
		let div5 = document.createElement('button');
		
		div5.innerHTML = 'Actualizar SOAT';
		div5.setAttribute("id","btn5");
		div5.setAttribute("class","boton");
		estilos.innerHTML = ".boton{color: #0099CC;background: transparent;border: 2px solid #0099CC;border-radius: 6px;}"
		+"border: none;color: white;padding: 16px 32px;text-align: center;display: inline-block;font-size: 16px;margin: 4px 2px;"
		+"transition-duration: 0.4s;cursor: pointer;text-decoration: none;text-transform: uppercase;";
		div5.setAttribute("onclick","guardarSoat()");
		*/
		
		fecha_desde_soat.parentNode.insertBefore( div4, fecha_desde_soat );
		
		fecha_desde_soat.parentNode.insertBefore( estilos, fecha_desde_soat );

		// Insert the element after our target element
		fecha_desde_soat.parentNode.insertBefore( div4, fecha_desde_soat.nextSibling );
		
		fecha_desde_soat.parentNode.insertBefore( estilos, fecha_desde_soat.nextSibling );
		...
		/*
		fecha_desde_soat.parentNode.insertBefore( div5, fecha_desde_soat );
		
		fecha_desde_soat.parentNode.insertBefore( estilos, fecha_desde_soat );

		// Insert the element after our target element
		fecha_desde_soat.parentNode.insertBefore( div5, fecha_desde_soat.nextSibling );
		
		fecha_desde_soat.parentNode.insertBefore( estilos, fecha_desde_soat.nextSibling );
		
		fecha_desde_soat.parentNode.insertBefore( estilosValidarFecha, fecha_desde_soat.nextSibling );
		
		fecha_desde_soat.parentNode.insertBefore( estilosAler, fecha_desde_soat.nextSibling );
		*/
		
		//Boton registrar Seguro
		
		
		let btnImpuestoUpdate = document.createElement('button')
		btnImpuestoUpdate.innerHTML = 'Actualizar impuesto'
		btnImpuestoUpdate.setAttribute("id","btn6");
		btnImpuestoUpdate.setAttribute("class","boton");
		estilos.innerHTML = ".boton{color: #0099CC;background: transparent;border: 2px solid #0099CC;border-radius: 6px;}"
		+"border: none;color: white;padding: 16px 32px;text-align: center;display: inline-block;font-size: 16px;margin: 4px 2px;"
		+"transition-duration: 0.4s;cursor: pointer;text-decoration: none;text-transform: uppercase;";
		btnImpuestoUpdate.setAttribute("onclick","guardarImpuesto()");
		
		
		fecha_de_pago.parentNode.insertBefore( btnImpuestoUpdate, fecha_de_pago )
		
		fecha_de_pago.parentNode.insertBefore( estilos, fecha_de_pago )

		// Insert the element after our target element
		fecha_de_pago.parentNode.insertBefore( btnImpuestoUpdate, fecha_de_pago.nextSibling )
		
		fecha_de_pago.parentNode.insertBefore( estilos, fecha_de_pago.nextSibling )
		
		
		
		let btnImpuestoUpdateDos = document.createElement('button')
		btnImpuestoUpdateDos.innerHTML = 'Ver impuestos'
		btnImpuestoUpdateDos.setAttribute("id","btn7")
		btnImpuestoUpdateDos.setAttribute("class","boton")
		estilos.innerHTML = ".boton{color: #0099CC;background: transparent;border: 2px solid #0099CC;border-radius: 6px;}"
		+"border: none;color: white;padding: 16px 32px;text-align: center;display: inline-block;font-size: 16px;margin: 4px 2px;"
		+"transition-duration: 0.4s;cursor: pointer;text-decoration: none;text-transform: uppercase;"
		btnImpuestoUpdateDos.setAttribute("onclick","verImpuesto()")
		
		
		fecha_de_pago.parentNode.insertBefore( btnImpuestoUpdateDos, fecha_de_pago );
		
		fecha_de_pago.parentNode.insertBefore( estilos, fecha_de_pago );

		// Insert the element after our target element
		fecha_de_pago.parentNode.insertBefore( btnImpuestoUpdateDos, fecha_de_pago.nextSibling );
		
		fecha_de_pago.parentNode.insertBefore( estilos, fecha_de_pago.nextSibling );
		
		
		///////Validacion fecha SOAT ///////
		...
		fecha_inicio_vijencia.setAttribute("type","date")
		fecha_inicio_vijencia.setAttribute("onchange","cambiarValidacion()")
		
		
		let fecha_final_vijencia = document.getElementById('fecha_hasta_soat');
		
		fecha_final_vijencia.setAttribute("type","date")
		fecha_de_pago.setAttribute("type","date")
		
		
		
		
		if(fecha_final_vijencia){
			var f = new Date();
			var hoy = f.getFullYear()  + "-" + (f.getMonth() +1) + "-" + f.getDate()
			var fecha_final = fecha_final_vijencia.value
			...
			if(hoyFormateada > fecha_final){
				...
				estadoSoat.setAttribute("class","validar")
				estadoSoat.readOnly = true
				estilosValidarFecha.innerHTML = ".validar{background-color: #6d6dff4f !important;}"
				estadoSoat.value = "EN PROCESO DE VIGENCIA"
                  			
			}else{
				estadoSoat.setAttribute("class","validar")
				estadoSoat.readOnly = true
				estilosValidarFecha.innerHTML = ".validar{background-color: #04ff00 !important;}"
				estadoSoat.value = "VIGENTE"
				//console.log(`${hoyFormateada} es menor a ${fecha_final} soat`)
			}
		...
		let switAlert = document.getElementsByClassName('swal2-icon swal2-info swal2-animate-info-icon')
		
		
		
		console.log(switAlert)
		
		
		estilosAler.innerHTML = ".swal2-icon.swal2-info{border-color: #A8AD00 !important;color: #A8AD00 !important;}.swal2-popup .swal2-styled.swal2-confirm{    border: 0;border-radius: 0.25em ;background: initial;background-color: #A8AD00 !important;color: #fff;font-size: 1.0625em;}"
