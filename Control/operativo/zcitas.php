...
function citas_inicial() // funcion principal de citas
{
	...
	echo "
			...
			function valida3(forma)
			{
				...
				if(!Number(forma.kmf.value)) { alert('Debe escribir el kilometraje final v&aacute;lido'); forma.kmf.style.backgroundColor='ffff00';forma.kmf.value='';forma.kmf.focus(); return false;}
				if(!alltrim(forma.obs.value)) { alert('Debe escribir las observaciones en la conclusi&oacute;n del servicio'); forma.obs.style.backgroundColor='ffff00';forma.obs.value='';forma.obs.focus(); return false;}
				if(!forma.Nuevo_estado.value) {alert('Debe seleccionar el nuevo estado en el que va a quedar el veh&iacute;culo.'); forma.Nuevo_estado.style.backgroundColor='ffffdd';return false; }
			...
			{ if(confirm('".$_SESSION['Nombre'].": Desea marcar el momento de la Entrega de este Veh&iacute;culo?')) window.open('zcitas.php?Acc=marcar_entregado&id='+id,'Citas_oculto'); }
			...
			{if(confirm('Asignar este operario a la entrega del veh&iacute;culo?'))
			...
			{if(confirm('Asignar este operario a la devoluci&oacute;n del veh&iacute;culo?'))
	...
	" Aseguradora: ".menu1('ASEG',"Select id,nombre from aseguradora where id not in (6) order by orden_monitor",$ASEG,1)."
		Ver: <select name='Modo'><option value='1' ".($Modo==1?"selected":"").">Solo Pendientes</option>
		 ...
		 <font style='font-size:8px'>Versi&oacute;n Mayo 2 de 2013</font>";
	...
	echo "<button class='buttonfunza' onclick=\"window.open('https://emtra.com.co/tarifas-transito-funza/')\" href='https://emtra.com.co/tarifas-transito-funza/'>Consulta tarifas Funza</button>";
	echo "<button style='float: right;' onclick=\"window.open('https://www.ventanillamovilidad.com.co/tarifas')\" class='buttonfunza' href='https://www.ventanillamovilidad.com.co/tarifas'>Consulta tarifas Bogota</button>";
	// /////////////////////  *******************     ENTREGAS   *****************************************//////////////////////////////////////////////////////
	...
	if (mysql_num_rows($Citas))
	{
		echo "<table border cellspacing=0 style='empty-cells:show' width='100%'><tr>
						...
						<th>Conductor</th>
						<th>Observaciones</th>
						<th>Veh&iacute;culo</th>".($USUARIO!=32?"<th>Estado</th>":"")."
		...
		while ($C = mysql_fetch_object($Citas))
		{
					...
					if ($Sin_conc == '' && $Futuro == '')/* si existen servicios sin concluir no permite cambiar ningun estado de la cita */
					{
						...
						{ 
							...
							if($Autorizaciones[$C->siniestro])
							{
								...
								// boton para la impresi&oacute;n del acta de entrega/devolucion
			...
			$_SESSION['excel_citas_entregas'][$countN]=array(
						...
						'placa_siniestro'=>$C->siniestroplaca,
						'fecha'=>$C->fecha,
...
*/
	...
	if(mysql_num_rows($Devoluciones))
	{
		...
		while ($C = mysql_fetch_object($Devoluciones))
		{
			...
			if ($C->estadod == 'C') citas_devolucion_cumplido($C,$T); // presenta los estados de devoluci&oacute;n cumplida
			echo "</td></tr>";
			$_SESSION['excel_citas_devoluciones'][$conuntNoDev]=array(
						...
						'cliente'=>$Aseguradora->nombre,
						'siniestro'=>$C->nsiniestro,
						'placa_siniestro'=>$C->placasiniestro,
...
function citas_inicial_modo_entregas($Modo,$OFICINA,$ASEG,$Dia) // construcci n del query de citas de acuerdo al modo que quier ver el usuario
{
	...
	switch($Modo)
	{
		...
		case 1:
			...
			s.*,es.nombre as nestado,c.estado as cestado,s.estado as sestado,s.placa as siniestroplaca
...
function citas_devolucion_modo($Modo,$OFICINA,$ASEG,$Dia)
{
	...
	switch($Modo)
	{
		...
		case 1: $sql = "select c.*,o.nombre as noficina,s.numero as nsiniestro,a.nombre as nflota,c.observaciones as obscita,c.id as cid,s.id as sid,c.placa as cplaca,
									s.*,es.nombre as nestado,c.estado as cestado,s.estado as sestado,s.placa as placasiniestro
									from cita_servicio c,siniestro s,vehiculo v,oficina o,aseguradora a,estado_siniestro es
									where c.flota=a.id and c.oficina=o.id and c.siniestro=s.id and s.estado=es.id
...
function citas_inicial_busqueda_futuros($LINK,$C) // busca ultimos estados, futuros o servicios sin concluir para mostrar un alerta
{
	...
	$Futuro='';
	if($Ultimos[$C->cplaca])  // busca el &uacute;ltimo estado hasta el dia de hoy, si es distinto a 
	...
	// si hay eventos futuros o &uacute;ltimos eventos distintos de parqueadero, presenta un alerta
...
function citas_inicial_estado_cumplido($LINK,$C,$T) // presenta las opciones de impresion y actualizaciones de una cita cumplida.
{
	...
	if($T)
	{// bot n para modificaci&oacute;n del siniestro. para subir imagenes
		echo "<br />Hora: <a onclick=\"modal('marcoindex.php?Acc=mod_reg&Num_Tabla=$T&id=$C->siniestro',0,0,600,1000,'eds');\"
	...
	if($C->img_cedula_f) echo $okimg1.$C->img_cedula_f.$okimg2." alt='C&eacute;dula' title='C&eacute;dula'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='C&eacute;dula' title='C&eacute;dula'>";
	if($C->img_pase_f) echo $okimg1.$C->img_pase_f.$okimg2." alt='Reverso C&eacute;dula' title='Reverso C&eacute;dula'>"; else echo "<img src='gifs/standar/Warning.png' border='0' alt='Reverso C&eacute;dula' title='Reverso C&eacute;dula'>";
...
function citas_inicial_entrega_hora($C,$Aseguradora) // Presenta la hora de la cita con ayuda de tooltip
{
	...
	echo "<td align='center'><a class='info'>".
				...
				<h3>Control de Seguimiento de Citas <i style='color:00000'>$Aseguradora->nombre  P&oacute;liza N&uacute;mero: $C->poliza Siniestro No. $C->numero </i></h3>
				<br />Fecha de ingreso del siniestro: <b>$C->ingreso</b> FECHA DE AUTORIZACION: <b style='color:ff0000'>$C->fec_autorizacion</b>
				<hr color='eeeeee'><b><u>ASEGURADO:</u></b> Nombre: <b>$C->asegurado_nombre</b> Identificaci&oacute;n: <b>$C->asegurado_id</b>
				<hr color='eeeeee'><b><u>Declarante:</u></b> Nombre: <b>$C->declarante_nombre</b> Identificaci&oacute;n: <b>$C->declarante_id</b>
...
function citas_inicial_entrega_fecha($C) // presenta la fecha y el n&uacute;mero de dias de servicio
{
	global $USUARIO;
	echo "<td nowrap='yes'>$C->fecha<br>D&iacute;as de <br>Servicio: "; // si el usuario es coordinador de call center puede cambiar los dias de servicio
...
function citas_inicial_entrega_siniestro($con_of,$C,$Aseguradora,$T) // presenta el numero de siniestro
{
	...
	echo " <a class='info'><img src='img/nogarantia.png' border='0' height='20px' alt='Servicio Sin Garantia' title='Servicio Sin Garantia' align='middle'>
				<span><img src='img/nogarantia.png'><h3>Servicio Sin Garant&iacute;a</h3></span></a> ";
	...
	if($C->cestado == 'P' && inlist($USUARIO,'1,2,5,7,10')) // SI ES USUARIO GERENCIA, AUTORIZACIONES, OPERATIVO, OFICINA
		echo "<a class='info' href=\"javascript:modal('zautorizaciones.php?sini=$C->numero',0,0,600,600,'call');\" alt='Solicitar Autorizacion' title='Solicitar Autorizacion'>
							<img src='img/solicita_autorizacion.png' border='0' height='20' align='middle'><span style='width:100px'>Solicitar Autorizaci&oacute;n</span></a>&nbsp;";
	if(inlist($USUARIO,'1,6')) // SI EL USUARIO ES FACTURACION
	{ // presenta el  cono de inicio de proceso de facturaci&oacute;n
		echo "<a href=\"javascript:inicia_facturacion($C->cid);void(null);\" class='info'><img src='img/facturar.png' height='20' border='0' align='middle'><span style='width:200px'>Iniciar proceso de facturaci&oacute;n</span></a>&nbsp;";
	}
	if(inlist($USUARIO,'1,10,33'))  // SI EL USUARIO ES CAJERO
	{ // presenta el  cono de captura de nuevo recibo de garant&iacute;a
	...
	{ //  muestra el icono de elaboracion de recibos de caja por garantia
		echo "<a href=\"javascript:nuevo_recibo_garantia($C->cid);void(null);\" class='info'><img src='img/caja_registradora.png' height='20' border='0' align='middle'><span style='width:200px'>PRE-Autorizaci&oacute;n Efectivo o TD</span></a>&nbsp;";
	}
	...
	if(inlist($USUARIO,'1,2,5,6,13,33') && $C->cestado=='C') // muestra el bot n de solicitud de visualizaci&oacute;n de la garant&iacute;a
		echo "	<a onclick=\"modal('zautorizaciones.php?Acc=datos_autorizacion&id=$C->siniestro',0,0,600,1000,'eds');\"	style='cursor:pointer'><u>Visualizaci&oacute;n de Garant&iacute;a</u></a>";
	echo "</td>";
}
...
function citas_inicial_tiempos($C,$LINK) // muestra el registro de tiempos de cada servicio desde el ingreso hasta la entrega del vehiculo
{
	...
	if($C->cestado=='P') // CONTROL DE TIEMPOS
	{
		...
		{
			if($Autorizacion = qom("select id,fecha_solicitud,fecha_proceso from sin_autor where siniestro='$C->siniestro' and estado!='R' ",$LINK)) // trae los datos de la autorizaci&oacute;n
			{
				// calcula el tiempo entre el arribo y la autorizaci&oacute;n
				...
				echo "<br>Tiempo entre Arribo y Solicitud Autorizaci&oacute;n: <b style='font-size:12px;color:4444ff'>$Tiempo1_solicitud_autorizacion</b>";
				if($Autorizacion->fecha_proceso!='0000-00-00 00:00:00')
				{
					// calcula el tiempo en tre la solicitud de autorizaci&oacute;n y el tiempo de solicitud
					$Tiempo2_proceso_autorizacon=diferencia_tiempo($Autorizacion->fecha_solicitud,$Autorizacion->fecha_proceso);
					echo "<br>Tiempo entre Solicitud y Proceso Autorizaci&oacute;n: <b style='font-size:12px;color:4444ff'>$Tiempo2_proceso_autorizacon</b>";
					if($C->momento_entrega!='0000-00-00 00:00:00')
					{
						//calcula el tiempo entre la aprobaci n de la autorizaci&oacute;n y la entrega del vehiculo
						$Tiempo3_entrega=diferencia_tiempo($Autorizacion->fecha_proceso,$C->momento_entrega);
						echo "<br>Tiempo entre Proceso Autorizaci&oacute;n y entrega del Veh&iacute;culo: <b style='font-size:12px;color:4444ff'>$Tiempo3_entrega</b>";
					}
					else
					{
						if(inlist($USUARIO,'1,13,23,10'))
						{// para los operarios y auxiliares de informaci&oacute;n y directores de oficina: pinta el icono de entrega de vehiculo
							echo "<br><a class='info' style='cursor:pointer' onclick='marcar_entrega($C->cid);'><img src='img/vehiculo.png' border='0' height='30'> Marcar Entrega del Veh&iacute;culo <span  style='width:200px'>Marcar Entrega del Vehiculo</span></a>";
						}
					}
				}
				else
				{// en espera del proceso de autorizaci&oacute;n
					$Tiempo=diferencia_tiempo($Autorizacion->fecha_solicitud,date('Y-m-d H:i:s'));
					echo "<br><b style='font-size:14px;text-decoration:blink;color:cc0000;background-color:ffff55;'>Esperando proceso de Autorizaci&oacute;n: $Tiempo EN ESPERA</b>";
				}
			}
			else
	...
	{// cuando esta cumplida pinta todos los tiempos grabados.
		...
		echo "<br /><table cellspacing='1'><tr ><td bgcolor='dedede'>Arribo:</td><td colspan=4 bgcolor='dedede'>".date('H:i',strtotime($C->arribo))."</td></tr>
					<tr ><td bgcolor='dedede'>Autorizaci&oacute;n:</td><td bgcolor='dedede'>".date('H:i',strtotime($Autorizacion->fecha_solicitud))."</td><td bgcolor='dedede'>Dif:</td><td bgcolor='dedede'>$Tiempo1_solicitud_autorizacion</td></tr>
					 <tr ><td bgcolor='dedede'>Proceso Autorizaci&oacute;n:</td><td bgcolor='dedede'>".date('H:i',strtotime($Autorizacion->fecha_proceso))."</td><td bgcolor='dedede'>Dif:</td><td bgcolor='dedede'>$Tiempo2_proceso_autorizacion</td></tr>
				    <tr ><td bgcolor='dedede'>Entrega Veh&iacute;culo:</td><td bgcolor='dedede'>".date('H:i',strtotime($C->momento_entrega))."</td><td bgcolor='dedede'>Dif:</td><td bgcolor='dedede'>$Tiempo3_entrega</td></tr>
...
function citas_devolucion_conductor($C,$LINK) // pinta la zona de devoluciones donde puede pedir facturas y asignar domicilios en la devolucion y asignacion del oprerario
{
	...
	{
		echo "&nbsp;<a class='info' style='cursor:pointer' onclick='insertar_domiciliod($C->cid);'><img src='gifs/standar/seguir_amarillo.png' border='0'><span style='width:200px'>Crear Domicilio Devoluci&oacute;n</span></a>&nbsp; ";
	}
	if(inlist($USUARIO,'1,2,10,13,27'))
	{	
...
function citas_devolucion_siniestro($C,$Aseguradora,$T) // Pinta el siniestro en la devolucion
{
	...
	if(inlist($USUARIO,'1,6')) // para el perfil de facturacion pinta el bot n de inicio de proceso de facturacion
		echo "<a href=\"javascript:inicia_facturacion($C->cid);void(null);\" class='info'><img src='img/facturar.png' height='24' border='0' align='middle'><span style='width:200px'>Iniciar proceso de facturaci&oacute;n</span></a>&nbsp;";
	if(inlist($USUARIO,'1,6,10')) // para facturaci&oacute;n y oficinas, pinta solicitud de autorizacion para pago de facturas con tarjeta de credito.
		echo "&nbsp;<a class='info' href=\"javascript:modal('zautorizaciones.php?sini=$C->numero',0,0,600,600,'call');\" alt='Solicitar Autorizacion' title='Solicitar Autorizacion'><img src='img/solicita_autorizacion.png' border='0' height='30' align='middle'><span style='width:100px'>Solicitar Autorizaci&oacute;n</span></a> ";
...
function citas_devolucion_fechahora($C,$Aseguradora) // pinta la hora con ayuda tooltip
{
	...
	echo "<td nowrap='yes'>$C->fec_devolucion</td><td align='center'>".
		...
		<h3>Control de Seguimiento de Citas <i style='color:00000'>$Aseguradora->nombre  P&oacute;liza N&uacute;mero: $C->poliza Siniestro No. $C->numero </i></h3>
		<br />Fecha del siniestro: <b>$C->fec_siniestro</b> Fecha de declaraci&oacute;n del siniestro: <b>$C->fec_declaracion</b>
		FECHA DE AUTORIZACION: <b style='color:ff0000'>$C->fec_autorizacion</b>
		<br />Vigencia de la p&oacute;liza:  Desde <b>$C->vigencia_desde</b> hasta <b>$C->vigencia_hasta</b>
		<hr color='eeeeee'><b><u>ASEGURADO:</u></b> Nombre: <b>$C->asegurado_nombre</b> Identificaci&oacute;n: <b>$C->asegurado_id</b>
		<hr color='eeeeee'><b><u>Declarante:</u></b> Nombre: <b>$C->declarante_nombre</b> Identificaci&oacute;n: <b>$C->declarante_id</b>
...
function citas_devolucion_cumplido($C,$T) // pinta el formulario de cierre de devoluci&oacute;n
{
	if($T) // pinta el lapiz de modificaci&oacute;n del siniestro para cargar las imagenes.
	...
	else echo "<br />Hora: $C->hora_devol_real <br />"; // pinta la hora de devoluci&oacute;n real
	...
	echo "<script language='javascript'>
			...
			function validar_datos()
			{
				with(document.forma)
				{
					...
					{
						alert('Debe escribir una direcci&oacute;n v&aacute;lida del cliente');
						direccion.style.backgroundColor='ffffdd';
						direccion.focus();
						return false;
					}
					if(confirm('Desea grabar la informaci&oacute;n e imprimir el recibo de caja provisional?'))
					{
						submit();
					}
				}
		...
		<form action='zcitas.php' method='post' target='_self' name='forma' id='forma'>
			...
			<tr><td align='right'>Cliente:</td><td><b>$R->nombre</b></td><td align='right'>Identificaci&oacute;n:</td><td><b>".coma_format($R->identificacion)."</b></td></tr>
			<tr><td align='right'>Oficina:</td><td><b>$Oficina->nombre</b></td><td align='right'>Aseguradora:</td><td><b>$Aseguradora->nombre</b></td></tr>
			<tr><td align='right'>Fecha Autorizaci&oacute;n Siniestro:</td><td><b>$Sin->fec_autorizacion</b></td><td align='right'>Valor:</td><td><b>$ ".coma_format($R->valor)."</b></td></tr>
			<tr><td align='right'>Direcci&oacute;n del Cliente:</td><td colspan=3><input type='text' name='direccion' size='50' maxlength='100'></td></tr>
...
function imprimir_rcp() // funcion que imprime recibo de caja provisional. Esta funcion ya no se usa
{
	...
	$P->Cell(90,5,'ADMISTRACI&Oacute;N OPERATIVA AUTOMOTRIZ S.A.',0,0,'C');
	$P->setxy(100,9);
	...
	$P->setxy(20,25);
	$P->cell(80,4,"www.aoacolombia.com - Bogot&aacute;, D.C. - Colombia",0,0,'C');
	$P->rect(100,14,90,14);
	$P->setfont('Arial','',10);
	$P->setxy(20,30);
	...
	$P->cell(100,5,$Sin->numero.' F.Autorizaci&oacute;n: '.$Sin->fec_autorizacion,1,0,'L');
	$P->setxy(20,$P->y+5);
	...
	$P->setxy(20,$P->y+5);
	$P->Cell(22,5,'Direcci&oacute;n:',1,0,'L');
	...
	$P->setxy(20,$P->y);
	$P->multicell(180,5,'Por concepto de: Dep&oacute;sito en efectivo en garant&iacute;a para el servicio de veh&iacute;culo de reemplazo correspondiente al siniestro n&uacute;mero: '.
	$Sin->numero.'. Veh&iacute;culo que se entrega: '.$Cita->placa,1,'J');
	$P->setxy(20,$P->y+2);
	$P->setfont('Arial','',8);
	$ny=$P->y+5;
	$ny=$P->y+4;
...
class d_tarjeta // clase que se usa para pintar datos del tarjeta habiente en la cabecera del acta de entrega/devolucion para garantias de tarjeta credito
{
	...
	var $Identificacion=0; // identificaci&oacute;n del tarjeta habiente quien autoriza el congelamiento de cupo
	...
	var $Valor_garantia=''; // monto de la garant&iacute;a a ser congelado
...
class d_efectivo // clase que se usa para pintar datos del tarjeta habiente en la cabecera del acta de entrega/devolucion para garantias de efectivo
{
	var $Nombre_tarjeta_habiente='';  // nombre del due o de la tarjeta quien autoriza el congelamiento de cupo
	var $Identificacion=0; // identificaci&oacute;n del tarjeta habiente quien autoriza el congelamiento de cupo
	var $Numero_cuenta=''; // n&uacute;mero de cuenta
	var $Banco=''; // Banco
...
function imprimir_acta() // imprime el acta de entrega/devolucion
{
	...
	$Cita=qo("select * from cita_servicio where id=$idc"); // trae los datos de la cita
	$sql = "SELECT * FROM oficina WHERE id = $Cita->oficina";
	$Oficina=qo($sql); 
	$Fec_entrega = date('Y-m-d',strtotime(aumentadias($Cita->fecha, $Cita->dias_servicio))).' '.$Cita->hora; // calcula la fecha de devolucion
 	
	if(!$Fdevol)
	{ // formulario en el que solicita la fecha de devoluci&oacute;n para tener encuenta si es domingo, festivo o pico y placa y correr la fecha y hora.
		html('IMPRESION ACTA DE ENTREGA Y DEVOLUCION');
		$varDia = get_nombre_dia($Fec_entrega);
		$festivos = new Festivos();
		$myDate = strtotime($Fec_entrega);
		$va =  $festivos->esFestivo(date('d', $myDate), date('m',$myDate));
...
.closebtn:hover {
  color: black;
		...
		<div class="alert alert-success">
				  <strong>Pipo y placa '.$Oficina->nombre.' !</strong>  <h5 style="font-size: initial;"> '.$Oficina->picoyplaca.'<h5>
				</div>';   
		
        
		
		if($Cita->aplazar_entrega){
			$sqlSeguimiento = "select *  from seguimiento where siniestro = $Cita->siniestro and tipo = 32 order by id desc limit 1";
			$seguimientoText = qo($sqlSeguimiento);
			echo "<h2 style='color:blue;'>Esta cita cuenta con un aplazo en la fecha de devoluci&oacute;n: <br> $seguimientoText->descripcion<h2>";
		}		
				
		$arrayValidation =  validationDays($Fec_entrega,$Cita,$Oficina);
		
		if($arrayValidation['date'] == date('Y-m-d', $myDate)){
		  $isPostpone = "<h2 style='color: blue;'>La devoluci&oacute;n no fue aplazada segun el horario de la oficina $Oficina->nombre</h2>";
		$dateDelivery = date('Y-m-d', $myDate);
		$hour = date('h:i:s',$myDate);
		
		}else{
			$isPostpone = "<h2 style='color: red;'>Por horarios de la oficina $Oficina->nombre, esta devolucion se plaza para el dia ". $arrayValidation['day']." fecha ".$arrayValidation['date']." el vehiculo se debera entrega a las ".$arrayValidation['hora']." y punto.</h2>";
		...
		$calculationDays = ($Cita->dias_servicio + $diff->days);
		
		$observations = "Se a&ntilde;aden $diff->days dias por horarios de la oficina, fecha anterior de devoluci&oacute;n: $Fec_entrega, fecha nueva: $dateDelivery $hour!";
		
		$sqlInsert = "INSERT INTO temp_date_appointment 
		             (dias_servicio,observaciones,hora,aplazar_entrega,id_cita)
					  VALUES ($calculationDays,'$observations','$hour',1,$idc)";
		
		$sqlDateAppointment = "select * from temp_date_appointment where id_cita = $idc";
		...
		if($va){
			...
			<h2>El dia de devolucion  es el: <div class='colorDia'>".$varDia."</div> ".$Fec_entrega."<div class='colorDiaFestivo'> Es un dia festivo. <br><h2> $isPostpone</h2></div><h2>";
		}else{
			$diaD = "$style <h2>El dia de devolucion es el: <div class='colorDia'>".$varDia."</div> ".date("Y-m-d", $myDate)." No es un dia festivo </h2></div></h2>";
		}
		...
		<form action='zcitas2.php' target='_self' method='POST' name='forma' id='forma'>
			Fecha de Devoluci&oacute;n: ".pinta_fc('forma','Fdevol',$dateDelivery)."
			Hora de Devoluci&oacute;n: ".pinta_hora('forma','Hdevol',$hour)." $diaD 
			...
			<b style='font-size:16px'>ESTIMADO (A) USUARIO (A) ".$_SESSION['Nombre']." Tenga en cuenta que si en el momento de la devoluci&oacute;n del automovil hay restricci&oacute;n
			vehicular, debe modificar la entrega a la siguiente hora h&aacute;bil la cual no tenga restricci&oacute;n seg&uacute;n las leyes y normas vigentes en su ciudad. De lo contrario
			contin&uacute;e con la impresi&oacute;n del acta sin hacer ninguna modificaci&oacute;n. <br><br>Gracias.</b>
		</form></body>";
		die();
	}
	//echo "try";
...
function insertar_domicilio() // formulario para insertar la informaci&oacute;n de un domicilio a la cita
{
	global $id;
	...
	echo "<script language='javascript'>
			function carga()
			{
				alert('Generaci&oacute;n de Solicitud hecha satisfactoriamente');
			}
		</script>
		<body><script language='javascript'>centrar(500,300);</script>
		<form action='zcitas.php' method='post' target='_self' name='forma' id='forma'>
			<h3>Creaci&oacute;n de Domicilio</h3>
			<table align='center'>
			<tr><td align='ritght'>Direcci&oacute;n Domicilio</td><td><input type='text' name='dir_domicilio' size='70' maxlength='200'></td></tr>
			<tr><td align='ritght'>Tel&eacute;fono Domicilio</td><td><input type='text' name='tel_domicilio' size='50' maxlength='50'></td></tr>
...
function insertar_domicilio_ok() // graba la informaci&oacute;n del domicilio en la cita
{
...
{
	global $idcita,$estado,$USUARIO,$selectsubcitas;
	...
	$ultimokm=qo1("select kilometraje($Veh->id)"); // obtiene el ultimo kilometraje del veh&iacute;culo
	if($estado=='C')
	{
	...
	if($estado=='C') // CUMPLIDA
	{
		...
		if($C->dir_domicilio) // cuando es domicilio, solicita un kilometraje adicional para determinar cuanto se gasto en el domicilio
			echo "
						...
						if(Dato<=$ultimokm) {alert('Debe escribir un kilometraje inicial v&aacute;lido mayor o igual que el registrado antes del desplazamiento del domicilio'); kmi.style.backgroundColor='ffff00';kmi.focus();continuar.style.visibility='hidden';return false;}
						...
						if(Desplazamiento_domicilio>$Oficina->km_domicilio)
						{
							...
							alert('Al grabar este cumplimiento de servicio, se enviar&aacute; un correo electr&oacute;nico alertando al Director de Operaciones sobre un desplazamiento excesivo para efectos de auditor&iacute;a y control');
						}
						else
			...
			echo "
						...
						if(Dato<$ultimokm) {alert('Debe escribir un kilometraje inicial v&aacute;lido igual que el &uacute;ltimo registrado ".($USUARIO==1?$ultimokm:'')."'); kmi.style.backgroundColor='ffff00';kmi.focus();continuar.style.visibility='hidden';return false;}
						if(Dato>$ultimokm)
						{
							...
							{
								alert('Debe escribir un kilometraje inicial v&aacute;lido igual que el &uacute;ltimo registrado.  No puede ser menor ni mayor que 2 kilometros mas del &uacute;ltimo registrado en la tabla de control.');
		...
		echo "
				...
				function validar_kmd()
				{
					with(document.forma)
					{
						...
						if(Dato<0) {alert('Debe escribir un kilometraje inicial v&aacute;lido igual o mayor que el ultimo registrado'); kmd.style.backgroundColor='ffff00';kmd.focus();continuar.style.visibility='hidden';return false;}
						if(Dato>$ultimokm)
						{
							var Diferencia=Dato-$ultimokm;
							if(Diferencia>0 && Diferencia<3)
							...
							{
								alert('Debe escribir un kilometraje inicial v&aacute;lido igual o mayor que el &uacute;ltimo registrado. No puede ser menor ni mayor que 2 kilometros mas del &uacute;ltimo registrado en la tabla de control.');
								kmd.style.backgroundColor='ffff00';kmd.focus();continuar.style.visibility='hidden';return false;
							}
						}
						if(Dato<$ultimokm) {alert('Debe escribir un kilometraje inicial v&aacute;lido mayor o igual que el &uacute;ltimo registrado. No puede ser menor al &uacute;ltimo estado en la tabla de control.');
										            	    kmd.style.backgroundColor='ffff00';kmd.focus();continuar.style.visibility='hidden';return false;}
						if(alltrim(observaciones.value) && Dato==$ultimokm)	{continuar.style.visibility='visible';}
				...
				function validarobs()
				{
					with(document.forma)
					{
						if(!alltrim(observaciones.value)) {alert('Debe digitar alguna observaci&oacute;n');observaciones.style.backgroundColor='ffff00';observaciones.focus();continuar.style.visibility='hidden';return false;}
	...
	*/	
	...
	if($estado=='S') // cumplida y no toma el servicio  CANCELACION POST ADJUDICACION
	{
		...
		echo capa('cp_sc',1,'Relative','');  // cuando se le da cumplida y no toma el servicio, el programa pide seleccionar una sub causal para la estadistica de la aseguradora.
			echo "<br /><br />Sub Causal: ".menu1("subcausal","select id,nombre from subcausal where activo=1 and vista=1",0,1,''," onchange='valida_subcausal();'");
		echo fincapa();
		// causalSubcausal();
		echo "<br /><br /><input type='submit' id='continuar' value='CONTINUAR' style='visibility:hidden' onclick='enviar_estado();'>
				...
				<input type='hidden' name='id' value='$idcita'>
				<input type='hidden' name='estado' value='$estado'>
				<input type='hidden' name='selectsubcitas' id='changeval' value='$selectsubcitas'>
			</form>
			</body>";
	}
	if($estado=='X') // CANCELADA POR SOLICITUD DEL ASEGURADO  // SOLO LA CAUSAL SIN SUB-CAUSAL [Requerimiento de Oscar Gomez]
	{
		echo "<script language='javascript'>
				...
				function changeFunc() {
					...
					document.getElementById('changeval').value =selectedValue;
					with(document.forma)
					{
						if(Number(estado_siniestro.value)==1) {document.getElementById('continuar').style.visibility='visible';}
					}
	...
	if($estado=='T') // NO CUMPLIDA
	{
		...
		echo "<script language='javascript'>
				...
				function enviar_estado()
				{
					if(document.getElementById('selectBox').val != ''){
						document.forma.submit();
					}
				}
				function changeFunc() {
...
function causalSubcausal()
{
	...
	$Causales=q("select s.id,c.nombre as causal,s.nombre from causal c,subcausal s where s.causal in (1,10,19,25,26,24,2,18,21,22,23,27) and s.activo=1 and s.vista=1 and s.causal=c.id order by c.nombre,s.nombre");
	echo "<select name='subcausal' onchange='changeFunc();' id='selectBox'> 
	<option value=''>Seleccione una subcausal</option
	";
	$Causal='';
...
{
	global $id, $estado, $kmi, $observaciones, $estado_siniestro,$subcausal,$kmd,$tpq,$Nusuario,$selectsubcitas;
	...
	if ($D = qom("select * from cita_servicio where id=$id",$LINK)) // trae los datos de la cita
	{
		...
		$Fec_entrega = aumentadias($D->fecha, $D->dias_servicio); // calcula la fecha de devoluci&oacute;n del veh&iacute;culo
		if($estado=='C') $estadod='P'; else $estadod='';  // REPROGRAMA EL ESTADO DE DEVOLUCION
		$selectsubcitas=$_POST['selectsubcitas'];
		// actualiza la cita
		if(!mysql_query("update cita_servicio set estado='$estado',estadod='$estadod',hora_llegada='$Hora',hora_devol=hora,fec_devolucion='$Fec_entrega',subcausal='$selectsubcitas' where id=$id",$LINK))
			die(mysql_error());
		...
		if ($estado == 'C') // CUMPLIDO
		{
			...
			if($tpq) // si hay recorido en el parqueadero
			{
				...
				graba_bitacora('ubicacion','A',$IDU1,'Adiciona autom&aacute;ticamente en cumplimiento de entrega.',$LINK);
			}
			if($kmd) // si hay recorrido de domicilio
			{
				...
				// inserta la bitacora de la ubicacion
				graba_bitacora('ubicacion','A',$IDU2,'Adiciona domicilio autom&aacute;ticamente',$LINK);
			}
...
function solicitar_factura() // formulario de solicitud de facturacion
{
	...
	echo "<script language='javascript'>
				function validar_formulario()
				{
					with(document.forma)
					{
						if(!alltrim(descripcion.value)) {alert('Debe especificar una descripci&oacute;n');descripcion.style.backgroundColor='ffffdd';descripcion.focus();return false;}
					...
					document.forma.submit();
                                ...
                                function pedirdatos()
                                {
                                    var con = document.getElementById('concepto').value;
                                    if(con==88 || con==85){
                                       document.getElementById('iva').readOnly = false;
                                       document.getElementById('seguro').readOnly = false;
                                       document.getElementById('nom').innerHTML = 'Valor Canon:';
                                       
                                    }else{
                                        document.getElementById('iva').readOnly = true;
                                        document.getElementById('seguro').readOnly = true;
                                        document.getElementById('nom').innerHTML = 'Valor sin  Iva:';
		...
		<body><script language='javascript'>centrar(600,400);</script><h3>Solicitud de Facturaci&oacute;n</h3>
		<form action='zcitas.php' method='post' target='_self' name='forma' id='forma'>
			Concepto a facturar: ".menu1("concepto","SELECT a.concepto, b.nombre FROM tarifa a,concepto_fac b  WHERE a.concepto=b.id AND a.aseguradora=$Sin->aseguradora AND a.activa=1",0,0,"","onchange='pedirdatos()'")."  
			<br><br>Cantidad: <input type='text' name='cantidad' id='cantidad' size=5 maxlength=10 value='1' class='numero'>
			<br /><br />Escriba la descripci&oacute;n de lo que se necesita facturar:<br />
			...
			<span id='nom'>Valor sin  Iva:</span><input type='text' class='numero' name='valor'><br /><br />
                        Valor Iva:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type='text' class='numero' id='iva' name='iva' readonly value='0'><br /><br />
                        Valor Seguro:<input type='text' class='numero'  name='seguro' id='seguro'  readonly value='0'><br /><br />
		...
		</body>"; // consulta reemplazada por james Select id,nombre from concepto_fac where activo_solicitud=1
}

function solicitar_factura_ok() // guarda la solicitud de facturacion hecha desde la tabla de citas
{
	global $siniestro,$concepto,$cita,$descripcion,$valor,$Nusuario,$Hoyl,$forma_pago,$cantidad,$iva,$seguro;
	// inserta el registro en la tabla de solicitudes
	q("insert into solicitud_factura (siniestro,cita,concepto,solicitado_por,fecha_solicitud,descripcion,valor,forma_pago,cantidad,canon,iva,seguro)
			values ('$siniestro','$cita','$concepto','$Nusuario','$Hoyl',\"$descripcion\",'$valor','$forma_pago','$cantidad','$valor','$iva','$seguro')");
...
function generar_xml() // programa que genera un xml para graficos. no se est  usando en el momento
{
	...
	echo "<citas>";
	// pinta la informaci&oacute;n de las citas con su estado
	while($C=mysql_fetch_object($Citas))
	{
...
function insertar_domiciliod() // formulario para insertar un domicilio de devoluci&oacute;n
{
	global $id;
	...
	echo "<script language='javascript'>
		...
		<body><script language='javascript'>centrar(500,300);</script>
		<form action='zcitas.php' method='post' target='_self' name='forma' id='forma'>
			<h3>Creaci&oacute;n de Domicilio de Devoluci&oacute;n</h3>
			<table align='center'>
			<tr><td align='ritght'>Direcci&oacute;n Domicilio</td><td><input type='text' name='dir_domiciliod' size='70' maxlength='200'></td></tr>
			<tr><td align='ritght'>Tel&eacute;fono Domicilio</td><td><input type='text' name='tel_domiciliod' size='50' maxlength='50'></td></tr>
			<tr><td colspan='2' align='center'><input type='submit' value='Continuar'></td></tr></table>
...
function insertar_domiciliod_ok() // graba los datos de un domicilio de devoluci&oacute;n
{
	...
	graba_bitacora('cita_servicio','M',$id,'Asigna Domicilio Devoluci&oacute;n'); // graba la bitacora
	echo "<body><script language='javascript'>window.close();void(null);opener.location.reload();</script></body>";
}
...
function asigna_operario_devolucion() // asigna el operario de devolucion segun lo haya seleccionado el funcionario
{
	...
	graba_bitacora('cita_servicio','M',$cita,'Asigna operario para devoluci&oacute;n.'); // graba la bitacora de la cita
	echo "<body><script language='javascript'>alert('Operario Asignado Satisfactoriamente');</script>";
}
...
function estado_operarios() // muestra diariamente, semanalmente y mensualmente la cantidad de entregas y solicitudes por oficina y por operario.
{
	...
	$Devoluciones=q("select concat(op.nombre,' ',op.apellido) as noperario, count(cs.id) as cantidad
							...
							GROUP BY noperario ORDER BY noperario");
	// inicia la creaci&oacute;n de los xml para mostrar las gr ficas
...
function formulario_devolucion() // formulario para capturar los datos de una devolucion
{
	...
	$C=qo("select * from cita_servicio where id=$idcita");  // se halla la informaci&oacute;n de la cita
	
	if(!$C->operario_domiciliod)
	{ // verifica que tenga asignado ya un operario en la devoluci&oacute;n del vehiculo
			...
			function valida_km_final2()
			{
				...
				{ 	
					...
					{	currentProcess = true; 
						...
						alert('Debe escribir un odometro mayor que el od&oacute;metro inicial');					
					    return false;
					}
				...
				if(consumo > lkm){
			       if(lkm !=0){
					...
					alert('Se informar&aacute; via correo electr&oacute;nico al Director de Operaciones sobre este exceso en el kilometraje de este servicio');   
					   }
				   } 
			    }else
	...
	echo "
			...
			function valida_obs()
			{
				...
				if(!alltrim(dato)) {
					...
					{	
						currentProcess = true; 
						alert('Debe escribir la observaci&oacute;n');
			...
			function prevalida_ords()
			{
				if(document.forma.Nuevo_estado.value==0) {
					...
					{	
						currentProcess = true;					
						alert('Debe primero seleccionar el nuevo estado en que quedara el veh&iacute;culo');
			...
			function valida_ords()
			{
				...
				if(!alltrim(dato)) {alert('Debe digitar la observaci&oacute;n para el nuevo estado');document.forma.ords.style.backgroundColor='ffffdd';document.forma.ords.focus();return false;}
			...
			function valida_km_final3()
			{
				with(document.forma)
				{
					if(!Number(kmd.value)) {alert('Debe escribir un kilometraje v&aacute;lido mayor o igual que el &uacute;ltimo registrado, sin comas ni puntos');kmd.style.backgroundColor='ffffdd';kmd.focus();return false;}
					if(Number(kmd.value)<Number(kmf.value)) {alert('Debe escribir un kilometraje v&aacute;lido mayor o igual que el &uacute;ltimo registrtado.');kmd.style.backgroundColor='ffffdd';kmd.focus();return false;}
					...
					if(Desplazamiento_domicilio>$Oficina->km_domicilio)
					{
						...
						alert('Al grabar este cumplimiento de devoluci&oacute;n, se enviar&aacute; un correo electr&oacute;nico alertando al Director de Operaciones sobre un desplazamiento excesivo para efectos de auditor&iacute;a y control');
					}
					else
	...
	echo "
				...
				Estado en que quedara el Veh&iacute;culo:".
				...
				<input type='button' id='gr' name='gr' value='Grabar Devoluci&oacute;n' style='visibility:hidden;width:300px;height:60px;' onclick='enviar_frm();'>
...
function cambia_estadod() // cambia el estado de devoluci&oacute;n de acuerdo a los datos capturados en el formulario de devoluci&oacute;n
{
	...
	if ($D = qom("select * from cita_servicio where id=$id",$LINK)) // trae los datos de la cita
	{
		...
		$Oficina=qom("select * from oficina where id=$D->oficina",$LINK); // trae los datos de la oficina
		// actualiza la cita del servicio en el estado,  fecha y hora de devoluci&oacute;n
		...
		if($kmd)   /// si hubo consumo de kilometraje por domicilio
		{
			...
			mysql_query("insert into ubicacion (oficina,vehiculo,estado,fecha_inicial,fecha_final,odometro_inicial,odometro_final,odometro_diferencia,observaciones,flota) values
				('$D->oficina','$idv','$Estado_domicilio','$Fecha','$Fecha','$kmf','$kmd','$Diferencia','Domicilio de devoluci&oacute;n $D->dir_domiciliod',$D->flota) ",$LINK);
...
function desencripta_data($id) // desencripta datos de una garant&iacute;a para mostrar en el acta de entrega/devolucion.
{
