...
class brow_columna
{
	...
	function brow_columna($C,$LINK,$Tabla)
	{
		...
		if(!$C->traet && $C->traex && strpos($C->traex,';'))
		{
			...
			if(!$Esta)
			{
				...
				$Comando="create function $Nombre_funcion(Dato_ $Tipo->Type) returns varchar(200) no sql
					begin Declare Resultado_ varchar(200) default '';
					select case Dato_ when '$Cases' end into Resultado_; return Resultado_; end";
				if(!mysql_query($Comando,$LINK)) die($Comando.'<br>'.$Tabla.'<br>'.$this->campo.'<br>'.mysql_error($LINK) );
			}
		}
	}
	...
	{
		echo "<th nowrap='yes' id='th_$this->campo' name='th_$this->campo'  bgcolor='ffffff' ".($Movil?"onclick":"oncontextmenu")."=\"m_c('$this->campo',$this->idcampo);return false;\" >$this->titulo</th>";
	}
}
...
class brow_tabla
{
	...
	var $Lineas_por_pagina=50; // lineas por página
...
class brow_tabla_detalle
{
	...
	function aparece()
	{
		...
		function no_modreg()
		{
			alert('No tiene acceso a la modificación de este registro');
		}
