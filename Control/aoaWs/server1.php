...
function ObtenerCitasSms($token)
{
	...
	if(qo1("select id from usuario_ws where token='$token' and id=3 "))
	{
		...
		if($Citas=q("select a.nombre as naseg,c.hora,s.numero,s.placa,s.declarante_celular,s.declarante_tel_resid,s.declarante_tel_ofic,s.declarante_telefono,s.declarate_tel_otro,
			...
			where c.siniestro=s.id and s.aseguradora=a.id and c.oficina=o.id and c.fecha='$Manana' and c.estado='P' and c.dir_domicilio = '' and c.siniestro not in (select siniestro from $tempo1) "))
		{
		...
		if($Devoluciones=q("select a.nombre as naseg,c.hora_devol,s.numero,s.placa,s.declarante_celular,s.declarante_tel_resid,s.declarante_tel_ofic,s.declarante_telefono,s.declarate_tel_otro,
			...
			from cita_servicio c,siniestro s,aseguradora a , oficina o
			where c.siniestro=s.id and s.aseguradora=a.id and c.oficina=o.id and c.fec_devolucion='$Manana' and c.estadod='P' AND c.dir_domiciliod = '' and c.siniestro not in (select siniestro from $tempo2) "))
		{
...
$Servidor_ws->register('ObtenerSiniestrosProcesados',
array(
	'token'=>'xsd:string', // token de validacion
	'aseg'=>'xsd:string' // id de la aseguradora que se desea obtener
)
...
$Servidor_ws->wsdl->addComplexType(
'registro_procesado','complexType','struct','all','',
array(
		'id' => array('name' => 'id', 'type' => 'xsd:int') // id del siniestro
	)
);
...
$Servidor_ws->wsdl->addComplexType(
'registros_procesados','complexType','array','','SOAP-ENC:Array',
array(),
array(
	array('ref' =>'SOAP-ENC:arrayType','wsdl:arrayType'=>'tns:registro_procesado[]' )
),
'tns:registro_procesado'
);

function ObtenerSiniestrosProcesados($token)
{
	// valida el token
	$a=fopen('r/teleone_obtener_adjudicados.txt','w');fwrite($a,"token: $token aseguradora: $aseg");fclose($a);
	if(qo1("select id from usuario_ws where token='$token' and id=3 "))
	{
		$Hoy=date('Y-m-d'); // obtengo la fecha de hoy
		$Ayer=date('Y-m-d',strtotime(aumentadias($Hoy,-1)));
		if($Siniestros=q("select distinct s.siniestro FROM seguimiento s,tipo_seguimiento t WHERE s.fecha between '$Ayer' and '$Hoy' and s.tipo=t.id and t.procesado=1 "))
		{
			$respuesta=array();
			//$idcompromisos='0';
			while($S=mysql_fetch_object($Siniestros))
			...
			return new soapval('return','tns:registros_procesados',$respuesta);
		}
		else return new soapval('return','xsd:string',"No hay registros");
	}
	else return soapval('return','xsd:string',"Registro de Token Invalido");
