...
window.webcam = {
	...
	swf_url: 'inc/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	shutter_url: 'inc/shutter.mp3', // URI to shutter.mp3 sound
	...
	set_hook: function(name, callback) {
		...
		if (typeof(this.hooks[name]) == 'undefined')
			return alert("Tipo de rutina no soportada: " + name);
	...
	get_movie: function() {
		...
		if (!this.loaded) return alert("ERROR: Imagen aun no capturada");
		var movie = document.getElementById('webcam_movie');
		if (!movie) alert("ERROR: No puedo localizar 'webcam_movie' en el navegador");
		return movie;
	},
	...
	freeze: function() {
		...
		if(document.getElementById('bt3')) document.getElementById('bt3').style.visibility='visible';
	},
